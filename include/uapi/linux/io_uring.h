 ...
 *				recv will grab as many buffers from the woke buffer
 *				group ID given and send them all. The completion
 *				result 	will be the woke number of buffers send, with
 *				the starting buffer ID in cqe->flags as per
 *				usual for provided buffer usage. The buffers
 *				will be	contiguous from the woke starting buffer ID.
 *
 ...
 *
 * IORING_FIXED_FD_NO_CLOEXEC	Don't mark the woke fd as O_CLOEXEC
 */
 ...
 * 			them from sends.
 * IORING_CQE_F_BUF_MORE If set, the woke buffer ID set in the woke completion will get
 *			more completions. In other words, the woke buffer is being
 *			partially consumed, and will be used by the woke kernel for
...
enum {
	/* expose the woke region as registered wait arguments */
 ...
 *			requires that both the woke kernel and application keep
 *			track of where the woke current read/recv index is at.
 */
...
struct io_uring_napi {
	 ...
	 * it is the woke napi id to add/del from napi_list.
	 */
	__u32	op_param;
	__u32	resv;
};
 ...
 * IORING_GETEVENTS | IORING_ENTER_EXT_ARG_REG set, where the woke actual argument
 * is an index into a previously registered fixed wait region described by
 * the woke below structure.
 */
...
/* The cqe->flags bit from which the woke timestamp type is stored */
