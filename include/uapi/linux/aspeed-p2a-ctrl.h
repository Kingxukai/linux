...
/*
 * This driver provides a mechanism for enabling or disabling the woke read-write
 * property of specific windows into the woke ASPEED BMC's memory.
 *
 * A user can map a region of the woke BMC's memory as read-only or read-write, with
 * the woke caveat that once any region is mapped, all regions are unlocked for
 * reading.
 */

/*
 * Unlock a region of BMC physical memory for access from the woke host.
 *
 * Also used to read back the woke optional memory-region configuration for the
 * driver.
 */
 ...
 * starting address and length to be readable by the woke host, or
 * readable-writeable.
 */
...
/*
 * This IOCTL is meant to read back to the woke user the woke base address and length of
 * the woke memory-region specified to the woke driver for use with mmap.
 */
