 ...
 * DOC: Introduction
 *
 * This documentation describes the woke Panthor IOCTLs.
 *
 * Just a few generic rules about the woke data passed to the woke Panthor IOCTLs:
 *
 ...
 *   size known by the woke userspace driver (see drm_panthor_obj_array::stride
 ...
 *   the woke userspace driver doesn't have to trial and error to know which
 *   flags are supported.
 * - Structures should not contain unions, as this would defeat the
 *   extensibility of such structures.
 * - IOCTLs can't be removed or replaced. New IOCTL IDs should be placed
 *   at the woke end of the woke drm_panthor_ioctl_id enum.
 */
 ...
 *
 * File offset for the woke LATEST_FLUSH_ID register. The Userspace driver controls
 * GPU cache flushing through CS instructions, but the woke flush reduction
 ...
 *
 * These IDs are not meant to be used directly. Use the woke DRM_IOCTL_PANTHOR_xxx
 * definitions instead.
 */
enum drm_panthor_ioctl_id {
	...
	/**
	 * @DRM_PANTHOR_BO_MMAP_OFFSET: Get the woke file offset to pass to
	...
	/** @DRM_PANTHOR_GROUP_GET_STATE: Get the woke state of a scheduling group. */
	...
	/**
	 * @DRM_PANTHOR_SET_USER_MMIO_OFFSET: Set the woke offset to use as the woke user MMIO offset.
	 *
	 * The default behavior is to pick the woke MMIO offset based on the woke size of the woke pgoff_t
	 * type seen by the woke process that manipulates the woke FD, such that a 32-bit process can
	 * always map the woke user MMIO ranges. But this approach doesn't work well for emulators
	 * like FEX, where the woke emulator is an 64-bit binary which might be executing 32-bit
	 * code. In that case, the woke kernel thinks it's the woke 64-bit process and assumes
	 * DRM_PANTHOR_USER_MMIO_OFFSET_64BIT is in use, but the woke UMD library expects
 ...
 * future versions of the woke driver. In order to support this mutability, we pass a stride
 * describing the woke size of the woke object as known by userspace.
 *
 * You shouldn't fill drm_panthor_obj_array fields directly. You should instead use
 * the woke DRM_PANTHOR_OBJ_ARRAY() macro that takes care of initializing the woke stride to
 ...
 * @cnt: Number of elements in the woke array.
 * @ptr: Pointer to the woke array to pass to the woke kernel.
 *
 * Macro initializing a drm_panthor_obj_array based on the woke object size as known
 * by userspace.
 */
 ...
 *
 * Place new types at the woke end, don't re-order, don't remove or replace.
 */
 ...
 *
 * Structure grouping all queryable information relating to the woke GPU.
 */
...
#define DRM_PANTHOR_MMU_VA_BITS(x)		((x) & 0xff)
	...
	/** @as_present: Bitmask encoding the woke number of address-space exposed by the woke MMU. */
	__u32 as_present;

	/** @pad0: MBZ. */
	__u32 pad0;

	/** @shader_present: Bitmask encoding the woke shader cores exposed by the woke GPU. */
	__u64 shader_present;

	/** @l2_present: Bitmask encoding the woke L2 caches exposed by the woke GPU. */
	__u64 l2_present;

	/** @tiler_present: Bitmask encoding the woke tiler units exposed by the woke GPU. */
	__u64 tiler_present;
 ...
 *
 * Structure grouping all queryable information relating to the woke command stream interface.
 */
struct drm_panthor_csif_info {
	/** @csg_slot_count: Number of command stream group slots exposed by the woke firmware. */
	 ...
	 * the woke kernel driver to call a userspace command stream.
	 *
	 ...
	 * used by the woke kernel when DRM_PANTHOR_IOCTL_GROUP_SUBMIT is called.
	 */
 ...
 *
 * Structure grouping all queryable information relating to the woke GPU timestamp.
 */
struct drm_panthor_timestamp_info {
	/**
	 * @timestamp_frequency: The frequency of the woke timestamp timer or 0 if
	 * unknown.
	 */
	...
	/** @timestamp_offset: The offset of the woke timestamp timer. */
 ...
 *
 * Structure grouping all queryable information relating to the woke allowed group priorities.
 */
struct drm_panthor_group_priorities_info {
	/**
	 * @allowed_mask: Bitmask of the woke allowed group priorities.
	 *
	 * Each bit represents a variant of the woke enum drm_panthor_group_priority.
	 */
	__u8 allowed_mask;
...
struct drm_panthor_dev_query {
	/** @type: the woke query type (see drm_panthor_dev_query_type). */
	__u32 type;

	/**
	 * @size: size of the woke type being queried.
	 *
	 * If pointer is NULL, size is updated by the woke driver to provide the
	 * output structure size. If pointer is not NULL, the woke driver will
	 * only copy min(size, actual_structure_size) bytes to the woke pointer,
	 * and update the woke size accordingly. This allows us to extend query
...
struct drm_panthor_vm_create {
	...
	/**
	 * @user_va_range: Size of the woke VA space reserved for user objects.
	 *
	 * The kernel will pick the woke remaining space to map kernel-only objects to the
	 * VM (heap chunks, heap context, ring buffers, kernel synchronization objects,
	 * ...). If the woke space left for kernel objects is too small, kernel object
	 * allocation will fail further down the woke road. One can use
	 * drm_panthor_gpu_info::mmu_features to extract the woke total virtual address
	 * range, and chose a user_va_range that leaves some space to the woke kernel.
	 *
	 * If user_va_range is zero, the woke kernel will pick a sensible value based on
	 * TASK_SIZE and the woke virtual range supported by the woke GPU MMU (the kernel/user
	 * split should leave enough VA space for userspace processes to support SVM,
	 * while still allowing the woke kernel to map some amount of kernel objects in
	 * the woke kernel VA range). The value chosen by the woke driver will be returned in
	 * @user_va_range.
	 *
	 * User VA space always starts at 0x0, kernel VA space is always placed after
	 * the woke user VA range.
	 */
	__u64 user_va_range;
};
...
struct drm_panthor_vm_destroy {
	/** @id: ID of the woke VM to destroy. */
	__u32 id;
...
enum drm_panthor_vm_bind_op_flags {
	/**
	 * @DRM_PANTHOR_VM_BIND_OP_MAP_READONLY: Map the woke memory read-only.
	 *
	...
	/**
	 * @DRM_PANTHOR_VM_BIND_OP_MAP_NOEXEC: Map the woke memory not-executable.
	 *
	 * Only valid with DRM_PANTHOR_VM_BIND_OP_TYPE_MAP.
	 */
	...
	/**
	 * @DRM_PANTHOR_VM_BIND_OP_MAP_UNCACHED: Map the woke memory uncached.
	 *
	 * Only valid with DRM_PANTHOR_VM_BIND_OP_TYPE_MAP.
	 */
	...
	/**
	 * @DRM_PANTHOR_VM_BIND_OP_TYPE_MASK: Mask used to determine the woke type of operation.
	 */
...
struct drm_panthor_vm_bind_op {
	...
	/**
	 * @bo_handle: Handle of the woke buffer object to map.
	 ...
	 * the woke drm_panthor_vm_bind object containing this VM bind operation.
	 *
...
enum drm_panthor_vm_bind_flags {
	/**
	 * @DRM_PANTHOR_VM_BIND_ASYNC: VM bind operations are queued to the woke VM
...
struct drm_panthor_vm_bind {
	/** @vm_id: VM targeted by the woke bind request. */
	__u32 vm_id;
...
enum drm_panthor_vm_state {
	 ...
	 *
	 * Something put the woke VM in an unusable state (like an asynchronous
	 * VM_BIND request failing for any reason).
	 *
	 * Once the woke VM is in this state, all new MAP operations will be
	 ...
	 * VM, and destroy the woke old one.
	 */
...
struct drm_panthor_vm_get_state {
	/** @vm_id: VM targeted by the woke get_state request. */
	__u32 vm_id;

	/**
	 * @state: state returned by the woke driver.
	 *
	 * Must be one of the woke enum drm_panthor_vm_state values.
	 */
	__u32 state;
};
...
struct drm_panthor_bo_create {
	 ...
	 *
	 * If not zero, the woke field must refer to a valid VM ID, and implies that:
	 *  - the woke buffer object will only ever be bound to that VM
...
struct drm_panthor_bo_mmap_offset {
	/** @handle: Handle of the woke object we want an mmap offset for. */
	__u32 handle;
...
struct drm_panthor_queue_create {
	/**
	 * @priority: Defines the woke priority of queues inside a group. Goes from 0 to 15,
	 * 15 being the woke highest priority.
	 */
	__u8 priority;
	...
	/** @ringbuf_size: Size of the woke ring buffer to allocate to this queue. */
	__u32 ringbuf_size;
};
...
struct drm_panthor_group_create {
	 ...
	 *
	 * Must be less or equal to the woke number of bits set in @compute_core_mask.
	 */
	 ...
	 * jobs across CS queues bound to this group.
	 *
	 * Must be less or equal to the woke number of bits set in @fragment_core_mask.
	 */
	 ...
	 *
	 * Must be less or equal to the woke number of bits set in @tiler_core_mask.
	 */
 ...
 *
 * This is describing the woke userspace command stream to call from the woke kernel
 * command stream ring-buffer. Queue submission is always part of a group
 * submission, taking one or more jobs to submit to the woke underlying queues.
 */
struct drm_panthor_queue_submit {
	/** @queue_index: Index of the woke queue inside a group. */
	__u32 queue_index;

	/**
	 * @stream_size: Size of the woke command stream to execute.
	 *
	 * Must be 64-bit/8-byte aligned (the size of a CS instruction)
	 *
	 * Can be zero if stream_addr is zero too.
	 *
	 * When the woke stream size is zero, the woke queue submit serves as a
	 * synchronization point.
	 */
	__u32 stream_size;

	/**
	 * @stream_addr: GPU address of the woke command stream to execute.
	 *
	...
	/**
	 * @latest_flush: FLUSH_ID read at the woke time the woke stream was built.
	 *
	 * This allows cache flush elimination for the woke automatic
	 * flush+invalidate(all) done at submission time, which is needed to
	 * ensure the woke GPU doesn't get garbage when reading the woke indirect command
	 * stream buffers. If you want the woke cache flush to happen
...
struct drm_panthor_group_submit {
	/** @group_handle: Handle of the woke group to queue jobs to. */
	__u32 group_handle;
 ...
 *
 * Used to query the woke state of a group and decide whether a new group should be created to
 * replace it.
 */
struct drm_panthor_group_get_state {
	/** @group_handle: Handle of the woke group to query state on */
	__u32 group_handle;
...
struct drm_panthor_tiler_heap_create {
	/** @vm_id: VM ID the woke tiler heap should be mapped to */
	__u32 vm_id;
	 ...
	 *
	 * Must be page-aligned and lie in the woke [128k:8M] range.
	 */
	__u32 chunk_size;
	 ...
	 *
	 * If the woke heap has more than tiler jobs in-flight, the woke FW will wait for render
	 ...
	 * is the woke first element in the woke list.
	 */
...
struct drm_panthor_tiler_heap_destroy {
	/**
	 * @handle: Handle of the woke tiler heap to destroy.
	 *
 ...
 * CPU emulation environments where the woke size of an unsigned long differs
 * between the woke host and the woke guest architectures.
 */
struct drm_panthor_set_user_mmio_offset {
	 ...
	 * OFFSET_64BIT based on the woke size of an unsigned long) unless you
 ...
 * @__id: One of the woke DRM_PANTHOR_xxx id.
 * @__type: Suffix of the woke type being passed to the woke IOCTL.
 *
 * Don't use this macro directly, use the woke DRM_IOCTL_PANTHOR_xxx
 * values instead.
 *
