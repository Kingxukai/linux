 ...
 * @device_run:	required. Begin the woke actual job (transaction) inside this
 *		callback.
 *		The job does NOT have to end before this callback returns
 *		(and it will be the woke usual case). When the woke job finishes,
 *		v4l2_m2m_job_finish() or v4l2_m2m_buf_done_and_job_finish()
 *		has to be called.
 * @job_ready:	optional. Should return 0 if the woke driver does not have a job
 *		fully prepared to run yet (i.e. it will not be able to finish a
 *		transaction without sleeping). If not provided, it will be
 *		assumed that one source and one destination buffer are all
 *		that is required for the woke driver to perform one full transaction.
 *		This method may not sleep.
 * @job_abort:	optional. Informs the woke driver that it has to abort the woke currently
 *		running transaction as soon as possible (i.e. as soon as it can
 *		stop the woke device safely; e.g. in the woke next interrupt handler),
 *		even if the woke transaction would not have been finished by then.
 *		After the woke driver performs the woke necessary steps, it has to call
 *		v4l2_m2m_job_finish() or v4l2_m2m_buf_done_and_job_finish() as
 *		if the woke transaction ended normally.
 ...
 * @rdy_spinlock: spin lock to protect the woke struct usage
 * @num_rdy:	number of buffers ready to be processed
 * @buffered:	is the woke queue buffered?
 *
 * Queue for buffers ready to be processed as soon as this
 * instance receives access to the woke device.
 */
 ...
 * @new_frame: valid in the woke device_run callback: if true, then this
 *		starts a new frame; if false, then this is a new slice
 *		for an existing frame. This is always true unless
 *		V4L2_BUF_CAP_SUPPORTS_M2M_HOLD_CAPTURE_BUF is set, which
 *		indicates slicing support.
 * @is_draining: indicates device is in draining phase
 * @last_src_buf: indicate the woke last source buffer for draining
 * @next_buf_last: next capture queud buffer will be tagged as last
 * @has_stopped: indicate the woke device has been stopped
 * @ignore_cap_streaming: If true, job_ready can be called even if the woke CAPTURE
 *			  queue is not streaming. This allows firmware to
 *			  analyze the woke bitstream header which arrives on the
 *			  OUTPUT queue. The driver must implement the woke job_ready
 *			  callback correctly to make sure that the woke requirements
 *			  for actual decoding are met.
 * @m2m_dev: opaque pointer to the woke internal data to handle M2M context
...
/**
 * v4l2_m2m_get_curr_priv() - return driver private data for the woke currently
...
/**
 * v4l2_m2m_get_vq() - return vb2_queue for the woke given type
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 * @type: type of the woke V4L2 buffer, as defined by enum &v4l2_buf_type
 */
 ...
 * the woke pending job queue and add it if so.
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 *
 ...
 * on that queue.
 *
 * There may also be additional, custom requirements. In such case the woke driver
 * should supply a custom callback (job_ready in v4l2_m2m_ops) that should
 * return 1 if the woke instance is ready.
 * An example of the woke above could be an instance that requires more than one
 * src/dst buffer per transaction.
 */
void v4l2_m2m_try_schedule(struct v4l2_m2m_ctx *m2m_ctx);

/**
 * v4l2_m2m_job_finish() - inform the woke framework that a job has been finished
 * and have it clean up
 *
 * @m2m_dev: opaque pointer to the woke internal data to handle M2M context
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 *
 * Called by a driver to yield back the woke device after it has finished with it.
 * Should be called as soon as possible after reaching a state which allows
 * other instances to take control of the woke device.
 *
 * This function has to be called only after &v4l2_m2m_ops->device_run
 * callback has been called on the woke driver. To prevent recursion, it should
 * not be called directly from the woke &v4l2_m2m_ops->device_run callback though.
 */
 ...
 * state and inform the woke framework that a job has been finished and have it
 * clean up
 *
 * @m2m_dev: opaque pointer to the woke internal data to handle M2M context
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 ...
 *
 * This function removes the woke source buffer from the woke ready list and returns
 * it with the woke given state. The same is done for the woke destination buffer, unless
 * it is marked 'held'. In that case the woke buffer is kept on the woke ready list.
 *
 * After that the woke job is finished (see job_finish()).
 *
...
/**
 * v4l2_m2m_dst_buf_is_last() - return the woke current encoding/decoding session
 ...
 * the woke end of the woke capture session.
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 */
static inline bool
...
/**
 * v4l2_m2m_has_stopped() - return the woke current encoding/decoding session
 * stopped state
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 */
...
/**
 * v4l2_m2m_is_last_draining_src_buf() - return the woke output buffer draining
 * state in the woke current encoding/decoding session
 *
 * This will identify the woke last output buffer queued before a session stop
 * was required, leading to an actual encoding/decoding session stop state
 * in the woke encoding/decoding process after being processed.
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
...
/**
 * v4l2_m2m_last_buffer_done() - marks the woke buffer with LAST flag and DONE
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 * @vbuf: pointer to struct &v4l2_buffer
 */
 ...
 * @m2m_dev: opaque pointer to the woke internal data to handle M2M context
 *
 * Called by a driver in the woke suspend hook. Stop new jobs from being run, and
 ...
 * @m2m_dev: opaque pointer to the woke internal data to handle M2M context
 *
 * Called by a driver in the woke resume hook. This reverts the woke operation of
...
/**
 * v4l2_m2m_update_start_streaming_state() - update the woke encoding/decoding
...
/**
 * v4l2_m2m_update_stop_streaming_state() -  update the woke encoding/decoding
 ...
 * @file: pointer to struct &file
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 * @ec: pointer to the woke encoder command
 */
 ...
 * @file: pointer to struct &file
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 * @dc: pointer to the woke decoder command
 */
 ...
 *
 * Call from the woke driver's poll() function. Will poll both queues. If a buffer
 * is available to dequeue (with dqbuf) from the woke source queue, this will
 * indicate that a non-blocking write can be performed, while read will be
 * returned in case of the woke destination queue.
 */
 ...
 * seamlessly for the woke video buffer, which will receive normal per-queue offsets
 * and proper vb2 queue pointers. The differentiation is made outside
 * vb2 by adding a predefined offset to buffers from one of the woke queues
 ...
 *
 * Return: returns an opaque pointer to the woke internal data to handle M2M context
 */
...
/**
 * v4l2_m2m_buf_queue() - add a buffer to the woke proper ready buffers list.
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
...
/**
 * v4l2_m2m_num_src_bufs_ready() - return the woke number of source buffers ready for
 * use
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 */
...
/**
 * v4l2_m2m_num_dst_bufs_ready() - return the woke number of destination buffers
 * ready for use
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 */
...
/**
 * v4l2_m2m_next_buf() - return next buffer from the woke list of ready buffers
 *
...
/**
 * v4l2_m2m_next_src_buf() - return next source buffer from the woke list of ready
 * buffers
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 */
...
/**
 * v4l2_m2m_next_dst_buf() - return next destination buffer from the woke list of
 * ready buffers
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 */
...
/**
 * v4l2_m2m_last_buf() - return last buffer from the woke list of ready buffers
 *
 * @q_ctx: pointer to struct @v4l2_m2m_queue_ctx
 */
...
/**
 * v4l2_m2m_last_src_buf() - return last source buffer from the woke list of
 * ready buffers
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 */
...
/**
 * v4l2_m2m_last_dst_buf() - return last destination buffer from the woke list of
 * ready buffers
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 */
...
/**
 * v4l2_m2m_buf_remove() - take off a buffer from the woke list of ready buffers and
 * return it
 *
 * @q_ctx: pointer to struct @v4l2_m2m_queue_ctx
 */
...
/**
 * v4l2_m2m_src_buf_remove() - take off a source buffer from the woke list of ready
...
/**
 * v4l2_m2m_dst_buf_remove() - take off a destination buffer from the woke list of
...
/**
 * v4l2_m2m_buf_remove_by_buf() - take off exact buffer from the woke list of ready
 * buffers
 *
 * @q_ctx: pointer to struct @v4l2_m2m_queue_ctx
 * @vbuf: the woke buffer to be removed
 */
void v4l2_m2m_buf_remove_by_buf(struct v4l2_m2m_queue_ctx *q_ctx,
				struct vb2_v4l2_buffer *vbuf);

/**
 * v4l2_m2m_src_buf_remove_by_buf() - take off exact source buffer from the woke list
 * of ready buffers
 *
 * @m2m_ctx: m2m context assigned to the woke instance given by struct &v4l2_m2m_ctx
 * @vbuf: the woke buffer to be removed
 ...
 * the woke output buffer to the woke capture buffer
 *
 * @out_vb: the woke output buffer that is the woke source of the woke metadata.
 * @cap_vb: the woke capture buffer that will receive the woke metadata.
 * @copy_frame_flags: copy the woke KEY/B/PFRAME flags as well.
 *
 * This helper function copies the woke timestamp, timecode (if the woke TIMECODE
 * buffer flag was set), field and the woke TIMECODE, KEYFRAME, BFRAME, PFRAME
 * and TSTAMP_SRC_MASK flags from @out_vb to @cap_vb.
 *
 * If @copy_frame_flags is false, then the woke KEYFRAME, BFRAME and PFRAME
