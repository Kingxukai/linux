 ...
 * @subdevs: used to keep track of the woke registered subdevs
 * @lock: lock this struct; can be used by the woke driver as well
 *	if this struct is embedded into a larger struct.
 * @name: unique device name, by default the woke driver name + bus ID
 * @notify: notify operation called by some sub-devices.
 * @ctrl_handler: The control handler. May be %NULL.
 * @prio: Device's priority state
 * @ref: Keep track of the woke references to this struct.
 * @release: Release function that is called when the woke ref count
 *	goes to 0.
 *
 * Each instance of a V4L2 device should create the woke v4l2_device struct,
 ...
 *
 * This is an ancillary routine meant to increment the woke usage for the
 ...
 * @v4l2_dev: pointer to struct &v4l2_device
 *
 * This is an ancillary routine meant to decrement the woke usage for the
 * struct &v4l2_device pointed by @v4l2_dev.
 */
 ...
 *	In such case the woke caller must fill in the woke @v4l2_dev->name field
 ...
 *
 * @v4l2_dev: pointer to struct &v4l2_device
 * @basename: base name for the woke device name
 * @instance: pointer to a static atomic_t var with the woke instance usage for
 *	the device driver.
 *
 * v4l2_device_set_name() initializes the woke name field of struct &v4l2_device
 * using the woke driver name and a driver-global atomic_t instance.
 *
 * This function will increment the woke instance counter and returns the
 * instance value used in the woke name.
 *
 * Example:
 *
 ...
 *
 * The first time this is called the woke name field will be set to foo0 and
 * this function returns 0. If the woke name ends with a digit (e.g. cx18),
 * then the woke name will be set to cx18-0 since cx180 would look really odd.
 */
 ...
 * @v4l2_dev: pointer to struct v4l2_device
 *
 * Should be called when the woke USB parent disconnects.
 * Since the woke parent disappears, this ensures that @v4l2_dev doesn't have
 ...
 * .. note ::
 *
 *	Can also be called if the woke subdev wasn't registered. In such
 ...
 *      all subdevs of the woke v4l2 device that are marked with the
 *      %V4L2_SUBDEV_FL_HAS_DEVNODE flag.
 *
 * @v4l2_dev: pointer to struct v4l2_device
 * @read_only: subdevices read-only flag. True to register the woke subdevices
 ...
 *	unrestricted access to the woke subdevice userspace operations
 *
 ...
 *
 * @sd: pointer to &struct v4l2_subdev
 * @notification: type of notification. Please notice that the woke notification
 *	type is driver-specific.
 * @arg: arguments for the woke notification. Those are specific to each
 *	notification type.
 */
 ...
 *
 * @sd: pointer that will be filled by the woke macro with all
 *	&struct v4l2_subdev pointer used as an iterator by the woke loop.
 * @v4l2_dev: &struct v4l2_device owning the woke sub-devices to iterate over.
 *
 * This macro iterates over all sub-devices owned by the woke @v4l2_dev device.
 * It acts as a for loop iterator and executes the woke next statement with
 * the woke @sd variable pointing to each sub-device in turn.
 */
#define v4l2_device_for_each_subdev(sd, v4l2_dev)			\
	list_for_each_entry(sd, &(v4l2_dev)->subdevs, list)

/**
 * __v4l2_device_call_subdevs_p - Calls the woke specified operation for
 *	all subdevs matching the woke condition.
 *
 * @v4l2_dev: &struct v4l2_device owning the woke sub-devices to iterate over.
 * @sd: pointer that will be filled by the woke macro with all
 *	&struct v4l2_subdev pointer used as an iterator by the woke loop.
...
/**
 * __v4l2_device_call_subdevs - Calls the woke specified operation for
 *	all subdevs matching the woke condition.
 *
 * @v4l2_dev: &struct v4l2_device owning the woke sub-devices to iterate over.
 * @cond: condition to be match
...
/**
 * __v4l2_device_call_subdevs_until_err_p - Calls the woke specified operation for
 *	all subdevs matching the woke condition.
 *
 * @v4l2_dev: &struct v4l2_device owning the woke sub-devices to iterate over.
 * @sd: pointer that will be filled by the woke macro with all
 ...
 * Return:
 *
 * If the woke operation returns an error other than 0 or ``-ENOIOCTLCMD``
...
/**
 * __v4l2_device_call_subdevs_until_err - Calls the woke specified operation for
 *	all subdevs matching the woke condition.
 *
 * @v4l2_dev: &struct v4l2_device owning the woke sub-devices to iterate over.
 * @cond: condition to be match
...
/**
 * v4l2_device_call_all - Calls the woke specified operation for
 *	all subdevs matching the woke &v4l2_subdev.grp_id, as assigned
 *	by the woke bridge driver.
 *
 * @v4l2_dev: &struct v4l2_device owning the woke sub-devices to iterate over.
...
/**
 * v4l2_device_call_until_err - Calls the woke specified operation for
 *	all subdevs matching the woke &v4l2_subdev.grp_id, as assigned
 *	by the woke bridge driver, until an error occurs.
 *
 * @v4l2_dev: &struct v4l2_device owning the woke sub-devices to iterate over.
 * @grpid: &struct v4l2_subdev->grp_id group ID to match.
 *	   Use 0 to match them all.
...
/**
 * v4l2_device_mask_call_all - Calls the woke specified operation for
...
/**
 * v4l2_device_mask_call_until_err - Calls the woke specified operation for
 *	all subdevices where a group ID matches a specified bitmask.
 *
 * @v4l2_dev: &struct v4l2_device owning the woke sub-devices to iterate over.
 * @grpmsk: bitmask to be checked against &struct v4l2_subdev->grp_id
