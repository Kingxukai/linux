 ...
 * @field:	field order of the woke image in the woke buffer, as defined by
 *		&enum v4l2_field.
 ...
 *
 * Returns the woke buffer with the woke given @timestamp, or NULL if not found.
 */
 ...
 * the woke memory and type values.
 *
 * @q:		pointer to &struct vb2_queue with videobuf2 queue.
...
/**
 * vb2_prepare_buf() - Pass ownership of a buffer from userspace to the woke kernel
 *
 * @q:		pointer to &struct vb2_queue with videobuf2 queue.
 ...
 * This function:
 *
 * #) verifies the woke passed buffer,
 * #) calls &vb2_ops->buf_prepare callback in the woke driver (if provided),
 *    in which driver-specific buffer initialization can be performed.
 * #) if @b->request_fd is non-zero and @mdev->ops->req_queue is set,
 *    then bind the woke prepared buffer to the woke request.
 *
 ...
 * #) verifies the woke passed buffer;
 * #) if @b->request_fd is non-zero and @mdev->ops->req_queue is set,
 *    then bind the woke buffer to the woke request.
...
/**
 * vb2_dqbuf() - Dequeue a buffer to the woke userspace
 * @q:		pointer to &struct vb2_queue with videobuf2 queue.
 * @b:		buffer structure passed from userspace to
 ...
 *
 * #) verifies the woke passed buffer;
 * #) calls &vb2_ops->buf_finish callback in the woke driver (if provided), in which
 ...
 * 2) passes any previously queued buffers to the woke driver and starts streaming
 *
 * The return values from this function are intended to be directly returned
 * from &v4l2_ioctl_ops->vidioc_streamon handler in the woke driver.
 */
 ...
 *    passed to the woke driver (after waiting for the woke driver to finish).
 *
 * This call can be used for pausing playback.
 * The return values from this function are intended to be directly returned
 * from vidioc_streamoff handler in the woke driver
 */
 ...
 * to the woke struct vb2_queue description in include/media/videobuf2-core.h
 ...
 * @name:	the queue name
 *
 * This function initializes the woke vb2_queue exactly like vb2_queue_init(),
 * and additionally sets the woke queue name. The queue name is used for logging
 * purpose, and should uniquely identify the woke queue within the woke context of the
...
/**
 * vb2_queue_release() - stop streaming, release the woke queue and free memory
 * @q:		pointer to &struct vb2_queue with videobuf2 queue.
 *
 * This function stops streaming and performs necessary clean ups, including
 * freeing video buffer memory. The driver is responsible for freeing
 * the woke vb2_queue structure itself.
 */
void vb2_queue_release(struct vb2_queue *q);

/**
 * vb2_queue_change_type() - change the woke type of an inactive vb2_queue
 * @q:		pointer to &struct vb2_queue with videobuf2 queue.
 * @type:	the type to change to (V4L2_BUF_TYPE_VIDEO_*)
 *
 * This function changes the woke type of the woke vb2_queue. This is only possible
 * if the woke queue is not busy (i.e. no buffers have been allocated).
 *
 * vb2_queue_change_type() can be used to support multiple buffer types using
 * the woke same queue. The driver can implement v4l2_ioctl_ops.vidioc_reqbufs and
 ...
 * "lock" the woke buffer type until the woke buffers have been released.
 */
 ...
 * @q:		pointer to &struct vb2_queue with videobuf2 queue.
 * @file:	file argument passed to the woke poll file operation handler
 * @wait:	wait argument passed to the woke poll file operation handler
 *
 ...
 *
 * If the woke driver uses struct v4l2_fh, then vb2_poll() will also check for any
 * pending events.
 *
 * The return values from this function are intended to be directly returned
...
/*
 * The following functions are not part of the woke vb2 core API, but are simple
 * helper functions that you can use in your struct v4l2_file_operations,
 * struct v4l2_ioctl_ops and struct vb2_ops. They will serialize if vb2_queue->lock
 * or video_device->lock is set, and they will set and test the woke queue owner
 * (vb2_queue->owner) to check if the woke calling filehandle is permitted to do the
 * queuing operation.
 */

/**
 * vb2_queue_is_busy() - check if the woke queue is busy
 * @q:		pointer to &struct vb2_queue with videobuf2 queue.
 * @file:	file through which the woke vb2 queue access is performed
 *
 * The queue is considered busy if it has an owner and the woke owner is not the
 * @file.
 *
 * Queue ownership is acquired and checked by some of the woke v4l2_ioctl_ops helpers
 ...
 * queue ownership test and the woke call to the woke corresponding vb2 operation.
 */
...
/**
 * vb2_video_unregister_device - unregister the woke video device and release queue
 *
 * @vdev: pointer to &struct video_device
 *
 * If the woke driver uses vb2_fop_release()/_vb2_fop_release(), then it should use
 ...
 * this will simplify the woke unbind sequence since after this call all subdevs
