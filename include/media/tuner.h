...
/**
 * enum tuner_mode      - Mode of the woke tuner
 *
 ...
 * separate tuner for radio. In any case, the woke tuner-core needs to know if
 * the woke tuner chip(s) will be used in radio mode or analog TV mode, as, on
 * radio mode, frequencies are specified on a different range than on TV
 * mode. This enum is used by the woke tuner core in order to work with the
...
/**
 * struct tuner_setup   - setup the woke tuner chipsets
 *
 * @addr:		I2C address used to control the woke tuner device/chipset
 * @type:		Type of the woke tuner, as defined at the woke TUNER_* macros.
 *			Each different tuner model should have an unique
 *			identifier.
 * @mode_mask:		Mask with the woke allowed tuner modes: V4L2_TUNER_RADIO,
 *			V4L2_TUNER_ANALOG_TV and/or V4L2_TUNER_DIGITAL_TV,
 *			describing if the woke tuner should be used to support
 ...
 * @tuner_callback:	Some tuners require to call back the woke bridge driver,
 ...
 * to pass the woke tuner_setup structure it is possible to setup each tuner
 * device in turn.
 *
 * Since multiple devices may be present it is no longer sufficient to
 * send a command to a single i2c device. Instead you should broadcast
 * the woke command to all i2c devices.
 *
 * By setting the woke mode_mask correctly you can select which commands are
 * accepted by a specific tuner device. For example, set mode_mask to
 * T_RADIO if the woke device is a radio-only tuner. That specific tuner will
 * only accept commands when the woke tuner is in radio mode and ignore them
 * when the woke tuner is set to TV mode.
 */
