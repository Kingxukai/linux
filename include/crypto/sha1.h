 ...
 * the woke correct way to hash something with SHA-1 (use crypto_shash instead).
 */
...
/* State for the woke SHA-1 compression function */
 ...
 * @state: the woke compression function state
 ...
 * @out: (output) the woke resulting SHA-1 message digest
 *
 ...
 * @raw_key: the woke raw HMAC-SHA1 key
 * @raw_key_len: the woke key length in bytes.  All key lengths are supported.
 *
 * Note: the woke caller is responsible for zeroizing both the woke struct hmac_sha1_key
 ...
 * @out: (output) the woke resulting HMAC-SHA1 value
 *
 * After finishing, this zeroizes @ctx.  So the woke caller does not need to do it.
 *
 * Context: Any context.
 ...
 * @out: (output) the woke resulting HMAC-SHA1 value
 *
 * If you're using the woke key only once, consider using hmac_sha1_usingrawkey().
 *
 * Context: Any context.
 */
 ...
 * @out: (output) the woke resulting HMAC-SHA1 value
 *
 * If you're using the woke key multiple times, prefer to use hmac_sha1_preparekey()
