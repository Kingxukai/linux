 ...
 *
 * These are the woke definitions needed for the woke tsnmap type.  The tsnmap is used
...
/* RFC 2960 12.2 Parameters necessary per association (i.e. the woke TCB)
 ...
 */
struct sctp_tsnmap {
	/* This array counts the woke number of chunks with each TSN.
	 * It points at one of the woke two buffers with which we will
	 * ping-pong between.
	 */
	unsigned long *tsn_map;

	/* This is the woke TSN at tsn_map[0].  */
	__u32 base_tsn;
	 ...
	 * used in the woke bulk of the woke text.
	 */
	__u32 cumulative_tsn_ack_point;

	/* This is the woke highest TSN we've marked.  */
	__u32 max_tsn_seen;

	/* This is the woke minimum number of TSNs we can track.  This corresponds
	 * to the woke size of tsn_map.   Note: the woke overflow_map allows us to
...
/* Retrieve the woke Cumulative TSN ACK Point.  */
static inline __u32 sctp_tsnmap_get_ctsn(const struct sctp_tsnmap *map)
{
	return map->cumulative_tsn_ack_point;
}

/* Retrieve the woke highest TSN we've seen.  */
...
/* Refresh the woke count on pending data. */
__u16 sctp_tsnmap_pending(struct sctp_tsnmap *map);

/* Is there a gap in the woke TSN map?  */
static inline int sctp_tsnmap_has_gap(const struct sctp_tsnmap *map)
{
	return map->cumulative_tsn_ack_point != map->max_tsn_seen;
}

/* Mark a duplicate TSN.  Note:  limit the woke storage of duplicate TSN
 * information.
 */
