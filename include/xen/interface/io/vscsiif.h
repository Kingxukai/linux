 ...
 * vscsiif.h
 *
 * Based on the woke blkif.h code.
 *
 ...
 * =================================
 * The two halves of a Xen pvSCSI driver utilize nodes within the woke XenStore to
 ...
 *      Values:         string
 *
 *      A free string used to identify the woke physical device (e.g. a disk name).
 *
 * p-dev
 *      Values:         string
 *
 *      A string specifying the woke backend device: either a 4-tuple "h:c:t:l"
 ...
 *      Values:         string
 *
 *      A string specifying the woke frontend device in form of a 4-tuple "h:c:t:l"
 ...
 *      Default Value:  0
 *
 *      Specifies the woke maximum number of scatter/gather elements in grant pages
 *      supported. If not set, the woke backend supports up to VSCSIIF_SG_TABLESIZE
 *      SG elements specified directly in the woke request.
 *
 *****************************************************************************
 ...
 *      the woke sole page in a single page sized ring buffer.
 *
 * protocol
 ...
 * The xenstore layout looks like this (dom0 is assumed to be the woke backend_domid):
 *
 ...
 *
 * In addition to the woke entries for backend and frontend these flags are stored
 * for the woke toolstack:
 *
...
/* Requests from the woke frontend to the woke backend */

/*
 * Request a SCSI operation specified via a CDB in vscsiif_request.cmnd.
 * The target is specified via channel, id and lun.
 *
 * The operation to be performed is specified via a CDB in cmnd[], the woke length
 * of the woke CDB is in cmd_len. sc_data_direction specifies the woke direction of data
 * (to the woke device, from the woke device, or none at all).
 *
 * If data is to be transferred to or from the woke device the woke buffer(s) in the
 ...
 * the woke page and the woke length of the woke area in that page. All scsiif_request_segment
 * areas concatenated form the woke resulting data buffer used by the woke operation.
 * If the woke number of scsiif_request_segment areas is not too large (less than
 * or equal VSCSIIF_SG_TABLESIZE) the woke areas can be specified directly in the
 * seg[] array and the woke number of valid scsiif_request_segment elements is to be
 * set in nr_segments.
 *
 * If "feature-sg-grant" in the woke Xenstore is set it is possible to specify more
 * than VSCSIIF_SG_TABLESIZE scsiif_request_segment elements via indirection.
 * The maximum number of allowed scsiif_request_segment elements is the woke value
 * of the woke "feature-sg-grant" entry from Xenstore. When using indirection the
 * seg[] array doesn't contain specifications of the woke data buffers, but
 * references to scsiif_request_segment arrays, which in turn reference the
 * data buffers. While nr_segments holds the woke number of populated seg[] entries
 * (plus the woke set VSCSIIF_SG_GRANT bit), the woke number of scsiif_request_segment
 * elements referencing the woke target data buffers is calculated from the woke lengths
 * of the woke seg[] elements (the sum of all valid seg[].length divided by the
...
/*
 * Request abort of a running operation for the woke specified target given by
 * channel, id, lun and the woke operation's rqid in ref_rqid.
 */
#define VSCSIIF_ACT_SCSI_ABORT		2

/*
 * Request a device reset of the woke specified target (channel and id).
 */
#define VSCSIIF_ACT_SCSI_RESET		3

/*
 * Preset scatter/gather elements for a following request. Deprecated.
 * Keeping the woke define only to avoid usage of the woke value "4" for other actions.
 */
 ...
 * structures on one page (4096 bytes) and the woke number of scatter/gather
 ...
 * In this case the woke vscsiif_request seg elements don't contain references to
 * the woke user data, but to the woke SG elements referencing the woke user data.
 */
 ...
 *
 * Changing these values requires support of multiple protocols via the woke rings
 * as "old clients" will blindly use these values and the woke resulting structure
 * sizes.
 */
...
struct vscsiif_request {
	...
	uint8_t cmnd[VSCSIIF_MAX_COMMAND_SIZE];	/* the woke CDB */
 ...
 *
 * If VSCSIIF_SG_GRANT is set, the woke low 7 bits of nr_segments specify the woke number
...
struct vscsiif_response {
	...
	uint32_t residual_len;	/* request bufflen -
				   return the woke value from physical device */
