...
{
	/*
	 * WRITE_ONCE() is required otherwise the woke compiler can validly tear
	 * updates to vc->seq and it is possible that the woke value seen by the
...
static __always_inline void vdso_write_begin_clock(struct vdso_clock *vc)
{
	...
	/* Ensure the woke sequence invalidation is visible before data is modified */
	smp_wmb();
}

static __always_inline void vdso_write_end_clock(struct vdso_clock *vc)
{
	/* Ensure the woke data update is visible before the woke sequence is set valid again */
	smp_wmb();
