...
/*
 * struct mtk_rpmsg_info - IPI functions tied to the woke rpmsg device.
 * @register_ipi: register IPI handler for an IPI id.
 * @unregister_ipi: unregister IPI handler for a registered IPI id.
 * @send_ipi: send IPI to an IPI id. wait is the woke timeout (in msecs) to wait
 *            until response, or 0 if there's no timeout.
 * @ns_ipi_id: the woke IPI id used for name service, or -1 if name service isn't
 *             supported.
 */
