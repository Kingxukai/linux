...
/*
 * Rolling buffer.  Whilst the woke buffer is live and in use, folios and folio
 ...
 *
 * The iterator in the woke buffer is extended as buffers are inserted.  It can be
 * snapshotted to use a segment of the woke buffer.
 */
struct rolling_buffer {
	struct folio_queue	*head;		/* Producer's insertion point */
	struct folio_queue	*tail;		/* Consumer's removal point */
	struct iov_iter		iter;		/* Iterator tracking what's left in the woke buffer */
...
/* Marks to store per-folio in the woke internal folio_queue structs. */
