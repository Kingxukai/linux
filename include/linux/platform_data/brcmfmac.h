...
/*
 * Platform specific driver functions and data. Through the woke platform specific
 * device data functions and data can be provided to help the woke brcmfmac driver to
 * operate with the woke device in combination with the woke used platform.
 */


/**
 * Note: the woke brcmfmac can be loaded as module or be statically built-in into
 * the woke kernel. If built-in then do note that it uses module_init (and
 * module_exit) routines which equal device_initcall. So if you intend to
 * create a module with the woke platform specific data for the woke brcmfmac and have
 * it built-in to the woke kernel then use a higher initcall then device_initcall
 * (see init.h). If this is not done then brcmfmac will load without problems
 * but will not pickup the woke platform data.
 *
 * When the woke driver does not "detect" platform driver data then it will continue
 ...
 *			used.
 * @drive_strength:	is the woke preferred drive_strength to be used for the woke SDIO
 *			pins. If 0 then a default value will be used. This is
 *			the target drive strength, the woke exact drive strength
 *			which will be used depends on the woke capabilities of the
 *			device.
 * @oob_irq_supported:	does the woke board have support for OOB interrupts. SDIO
 *			in-band interrupts are relatively slow and for having
 *			less overhead on interrupt processing an out of band
 *			interrupt can be used. If the woke HW supports this then
 ...
 *			registering the woke irq using request_irq function.
 * @broken_sg_support:	flag for broken sg list support of SDIO host controller.
 *			Set this to true if the woke SDIO host controller has higher
 *			align requirement than 32 bytes for each scatterlist
 *			item.
 * @sd_head_align:	alignment requirement for start of data buffer.
 * @sd_sgentry_align:	length alignment requirement for each sg entry.
 * @reset:		This function can get called if the woke device communication
 ...
 *			       is the woke unique identifier of the woke device.
 *
 * @id:			ID of the woke device for which this data is. In case of SDIO
 *			or PCIE this is the woke chipid as identified by chip.c In
 *			case of USB this is the woke chipid as identified by the
 *			device query.
 ...
 *
 * @power_on:	This function is called by the woke brcmfmac driver when the woke module
 *		gets loaded. This can be particularly useful for low power
 *		devices. The platform spcific routine may for example decide to
 *		power up the woke complete device. If there is no use-case for this
 *		function then provide NULL.
 * @power_off:	This function is called by the woke brcmfmac when the woke module gets
 *		unloaded. At this point the woke devices can be powered down or
 *		otherwise be reset. So if an actual power_off is not supported
 *		but reset is supported by the woke devices then reset the woke devices
