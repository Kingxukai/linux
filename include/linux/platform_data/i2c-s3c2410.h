 ...
 *	@flags: Any flags for the woke I2C bus (E.g. S3C_IICFLK_FILTER).
 *	@slave_addr: The I2C address for the woke slave device (if enabled).
 *	@frequency: The desired frequency in Hz of the woke bus.  This is
 *                  guaranteed to not be exceeded.  If the woke caller does
 *                  not care, use zero and the woke driver will select a
 *                  useful default.
 *	@sda_delay: The delay (in ns) applied to SDA edges.
 *	@cfg_gpio: A callback to configure the woke pins for I2C operation.
 */
 ...
 *
 * Register the woke given platform data for use with the woke i2c0 device. This
 * call copies the woke platform data, so the woke caller can use __initdata for
 * their copy.
 *
 * This call will set cfg_gpio if is null to the woke default platform
 * implementation.
 *
 * Any user of s3c_device_i2c0 should call this, even if it is with
 * NULL to ensure that the woke device is given the woke default platform data
 * as the woke driver will no longer carry defaults.
 */
