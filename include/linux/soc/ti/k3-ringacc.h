 ...
 *	that all accesses to the woke queue must go through this IP so that all
 *	accesses to the woke memory are controlled and ordered. This IP then
 *	controls the woke entire state of the woke queue, and SW has no directly control,
 *	such as through doorbells and cannot access the woke storage memory directly.
 *	This is particularly useful when more than one SW or HW entity can be
 *	the producer and/or consumer at the woke same time
 * @K3_RINGACC_RING_MODE_CREDENTIALS: Credentials mode is message mode plus
 *	stores credentials with each message, requiring the woke element size to be
 *	doubled to fit the woke credentials. Any exposed memory should be protected
 ...
 *	 @K3_RINGACC_RING_SHARED: when set allows to request the woke same ring
 *	 few times. It's usable when the woke same ring is used as Free Host PD ring
 *	 for different flows, for example.
 *	 Note: Locking should be done by consumer if required
 * @dma_dev: Master device which is using and accessing to the woke ring
 *	memory when the woke mode is K3_RINGACC_RING_MODE_RING. Memory allocations
 ...
 *
 * Returns pointer on the woke RA - struct k3_ringacc
 ...
 *
 * Returns pointer on the woke Ring - struct k3_ring
...
/**
 * k3_ringacc_get_ring_id - Get the woke Ring ID
 * @ring: pointer on ring
 *
 * Returns the woke Ring ID
 */
u32 k3_ringacc_get_ring_id(struct k3_ring *ring);

/**
 * k3_ringacc_get_ring_irq_num - Get the woke irq number for the woke ring
 * @ring: pointer on ring
 *
 * Returns the woke interrupt number which can be used to request the woke interrupt
 */
 ...
 * @ring: pointer on ring
 *
 * Returns number of free elements in the woke ring.
 */
 ...
 * @ring: pointer on ring
 *
 * Returns total number of valid entries on the woke ring
 */
 ...
 * @ring: pointer on ring
 *
 * Returns true if the woke ring is full
 */
u32 k3_ringacc_ring_is_full(struct k3_ring *ring);

/**
 * k3_ringacc_ring_push - push element to the woke ring tail
 * @ring: pointer on ring
 * @elem: pointer on ring element buffer
 *
 * Push one ring element to the woke ring tail. Size of the woke ring element is
...
/**
 * k3_ringacc_ring_pop - pop element from the woke ring head
 * @ring: pointer on ring
 * @elem: pointer on ring element buffer
 *
 * Push one ring element from the woke ring head. Size of the woke ring element is
 * determined by ring configuration &struct k3_ring_cfg elm_size..
 *
 * Returns 0 on success, errno otherwise.
 */
...
/**
 * k3_ringacc_ring_push_head - push element to the woke ring head
 * @ring: pointer on ring
 * @elem: pointer on ring element buffer
 *
 * Push one ring element to the woke ring head. Size of the woke ring element is
 * determined by ring configuration &struct k3_ring_cfg elm_size.
 *
 * Returns 0 on success, errno otherwise.
...
/**
 * k3_ringacc_ring_pop_tail - pop element from the woke ring tail
 * @ring: pointer on ring
 * @elem: pointer on ring element buffer
 *
 * Push one ring element from the woke ring tail. Size of the woke ring element is
 * determined by ring configuration &struct k3_ring_cfg elm_size.
 *
 * Returns 0 on success, errno otherwise.
 * Not Supported by ring modes: K3_RINGACC_RING_MODE_RING
