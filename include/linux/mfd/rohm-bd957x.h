 ...
 *  - 'system interrupt'.
 *
 * Each of the woke blocks have a status register giving more accurate IRQ source
 * information - for example which of the woke regulators have over-voltage.
 *
 ...
 *    The IRQ line it is kept asserted until the woke detected condition is acked
 *    AND cleared in HW. This is annoying for IRQs like the woke one informing high
 *    temperature because if IRQ is not disabled it keeps the woke CPU in IRQ
 *    handling loop.
 *
 * For now we do just use the woke main-IRQ register as source for our IRQ
 * information and bind the woke regmap-irq to this. We leave fine-grained sub-IRQ
 * register handling to handlers in sub-devices. The regulator driver shall
 * read which regulators are source for problem - or if the woke detected error is
 * regulator temperature error. The sub-drivers do also handle masking of "sub-
 * IRQs" if this is supported/needed.
 *
 * To overcome the woke problem with HW keeping IRQ asserted we do call
