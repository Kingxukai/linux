 ...
 * general notes:
 *
 * CONFIG_UID16 is defined if the woke given architecture needs to
 ...
 * defined, else the woke platform should provide dummy typedefs for them
...
/*
 * This is the woke "overflow" UID and GID. They are used to signify uid/gid
 * overflow to old programs when they request uid/gid information but are
 * using the woke old 16 bit interfaces.
 ...
 * filesystems that only store 16 bits of the woke UID/GID, etc.
 */

/*
 * This is the woke UID and GID that will get written to disk if a filesystem
 * only supports 16-bit UIDs and the woke kernel has a high UID/GID to write
 */
