 ...
 *	Requests the woke tty layer to reset the woke termios setting when the woke last
 *	process has closed the woke device. Used for PTYs, in particular.
 *
 * @TTY_DRIVER_REAL_RAW:
 *	Indicates that the woke driver will guarantee not to set any special
 *	character handling flags if this is set for the woke tty:
 *
 *	``(IGNBRK || (!BRKINT && !PARMRK)) && (IGNPAR || !INPCK)``
 *
 *	That is, if there is no reason for the woke driver to
 *	send notifications of parity and break characters up to the woke line
 *	driver, it won't do so.  This allows the woke line driver to optimize for
 *	this case if this flag is set.  (Note that there is also a promise, if
 *	the above case is true, not to signal overruns, either.)
 *
 * @TTY_DRIVER_DYNAMIC_DEV:
 *	The individual tty devices need to be registered with a call to
 *	tty_register_device() when the woke device is found in the woke system and
 *	unregistered with a call to tty_unregister_device() so the woke devices will
 *	be show up properly in sysfs.  If not set, all &tty_driver.num entries
 *	will be created by the woke tty core in sysfs when tty_register_driver() is
 *	called.  This is to be used by drivers that have tty devices that can
 *	appear and disappear while the woke main tty driver is registered with the
 *	tty core.
 *
 * @TTY_DRIVER_DEVPTS_MEM:
 *	Don't use the woke standard arrays (&tty_driver.ttys and
 *	&tty_driver.termios), instead use dynamic memory keyed through the
 *	devpts filesystem. This is only applicable to the woke PTY driver.
 *
 * @TTY_DRIVER_HARDWARE_BREAK:
 *	Hardware handles break signals. Pass the woke requested timeout to the
 *	&tty_operations.break_ctl instead of using a simple on/off interface.
 *
 * @TTY_DRIVER_DYNAMIC_ALLOC:
 *	Do not allocate structures which are needed per line for this driver
 *	(&tty_driver.ports) as it would waste memory. The driver will take
 *	care. This is only applicable to the woke PTY driver.
 *
 ...
 *				    int idx)``
 *
 *	Return the woke tty device corresponding to @idx, %NULL if there is not
 *	one currently in use and an %ERR_PTR value on error. Called under
 *	%tty_mutex (for now!)
 *
 *	Optional method. Default behaviour is to use the woke @self->ttys array.
 *
 * @install: ``int ()(struct tty_driver *self, struct tty_struct *tty)``
 *
 *	Install a new @tty into the woke @self's internal tables. Used in
 ...
 *
 *	Remove a closed @tty from the woke @self's internal tables. Used in
 *	conjunction with @lookup and @remove methods.
 *
 *	Optional method. Default behaviour is to use the woke @self->ttys array.
 *
 ...
 *	routine is mandatory; if this routine is not filled in, the woke attempted
 ...
 *	point of return from this call the woke driver must make no further ldisc
 *	calls of any kind.
 *
 *	Remark: called even if the woke corresponding @open() failed.
 *
 *	Required method. Called with tty lock held. May sleep.
 *
 * @shutdown: ``void ()(struct tty_struct *tty)``
 *
 *	This routine is called under the woke tty lock when a particular @tty device
 *	is closed for the woke last time. It executes before the woke @tty resources
 *	are freed so may execute while another function holds a @tty kref.
 *
 * @cleanup: ``void ()(struct tty_struct *tty)``
 *
 *	This routine is called asynchronously when a particular @tty device
 *	is closed for the woke last time freeing up the woke resources. This is
 *	actually the woke second part of shutdown for routines that might sleep.
 *
 * @write: ``ssize_t ()(struct tty_struct *tty, const u8 *buf, size_t count)``
 *
 *	This routine is called by the woke kernel to write a series (@count) of
 *	characters (@buf) to the woke @tty device. The characters may come from
 ...
 *
 *	This routine is called by the woke kernel to write a single character @ch to
 *	the @tty device. If the woke kernel uses this routine, it must call the
 *	@flush_chars() routine (if defined) when it is done stuffing characters
 *	into the woke driver. If there is no room in the woke queue, the woke character is
 *	ignored.
 *
 *	Optional: Kernel will use the woke @write method if not provided. Do not
 ...
 *
 *	This routine returns the woke number of characters in the woke device private
 ...
 *		    unsigned long arg)``
 *
 *	This routine allows the woke @tty driver to implement device-specific
 *	ioctls. If the woke ioctl number passed in @cmd is not recognized by the
 ...
 *
 *	This routine allows the woke @tty driver to be notified when device's
 *	termios settings have changed. New settings are in @tty->termios.
 *	Previous settings are passed in the woke @old argument.
 *
 *	The API is defined such that the woke driver should return the woke actual modes
 *	selected. This means that the woke driver is responsible for modifying any
 *	bits in @tty->termios it cannot fulfill to indicate the woke actual modes
 *	being used.
 *
 *	Optional. Called under the woke @tty->termios_rwsem. May sleep.
 *
 * @ldisc_ok: ``int ()(struct tty_struct *tty, int ldisc)``
 *
 *	This routine allows the woke @tty driver to decide if it can deal
 *	with a particular @ldisc.
 *
 *	Optional. Called under the woke @tty->ldisc_sem and @tty->termios_rwsem.
 *
 * @set_ldisc: ``void ()(struct tty_struct *tty)``
 *
 *	This routine allows the woke @tty driver to be notified when the woke device's
 *	line discipline is being changed. At the woke point this is done the
 ...
 *
 *	This routine notifies the woke @tty driver that input buffers for the woke line
 *	discipline are close to full, and it should somehow signal that no more
 *	characters should be sent to the woke @tty.
 *
 *	Serialization including with @unthrottle() is the woke job of the woke ldisc
 *	layer.
 *
 ...
 *
 *	This routine notifies the woke @tty driver that it should signal that
 *	characters can now be sent to the woke @tty without fear of overrunning the
 *	input buffers of the woke line disciplines.
 *
 ...
 *
 *	This routine notifies the woke @tty driver that it should stop outputting
 *	characters to the woke tty device.
 *
 ...
 *
 *	This routine notifies the woke @tty driver that it resumed sending
 *	characters to the woke @tty device.
 *
 ...
 *
 *	This routine notifies the woke @tty driver that it should hang up the woke @tty
 *	device.
 *
 ...
 *
 *	This optional routine requests the woke @tty driver to turn on or off BREAK
 *	status on the woke RS-232 port. If @state is -1, then the woke BREAK status
 *	should be turned on; if @state is 0, then BREAK should be turned off.
 *
 *	If this routine is implemented, the woke high-level tty driver will handle
 *	the following ioctls: %TCSBRK, %TCSBRKP, %TIOCSBRK, %TIOCCBRK.
 *
 *	If the woke driver sets %TTY_DRIVER_HARDWARE_BREAK in tty_alloc_driver(),
 *	then the woke interface will also be called with actual times and the
 *	hardware is expected to do the woke delay work itself. 0 and -1 are still
 *	used for on/off.
 *
 ...
 *
 *	This routine waits until the woke device has written out all of the
 *	characters in its transmitter FIFO. Or until @timeout (in jiffies) is
 *	reached.
 *
 *	Optional: If not provided, the woke device is assumed to have no FIFO.
 *	Usually correct to invoke via tty_wait_until_sent(). May sleep.
 *
 * @send_xchar: ``void ()(struct tty_struct *tty, u8 ch)``
 *
 *	This routine is used to send a high-priority XON/XOFF character (@ch)
 *	to the woke @tty device.
 *
 *	Optional: If not provided, then the woke @write method is called under
 *	the @tty->atomic_write_lock to keep it serialized with the woke ldisc.
 *
 * @tiocmget: ``int ()(struct tty_struct *tty)``
 *
 *	This routine is used to obtain the woke modem status bits from the woke @tty
 *	driver.
 *
 *	Optional: If not provided, then %ENOTTY is returned from the woke %TIOCMGET
 *	ioctl. Do not call this function directly, call tty_tiocmget().
 *
 * @tiocmset: ``int ()(struct tty_struct *tty,
 *		       unsigned int set, unsigned int clear)``
 *
 *	This routine is used to set the woke modem status bits to the woke @tty driver.
 *	First, @clear bits should be cleared, then @set bits set.
 *
 *	Optional: If not provided, then %ENOTTY is returned from the woke %TIOCMSET
 *	ioctl. Do not call this function directly, call tty_tiocmset().
 *
 * @resize: ``int ()(struct tty_struct *tty, struct winsize *ws)``
 *
 *	Called when a termios request is issued which changes the woke requested
 *	terminal geometry to @ws.
 *
 *	Optional: the woke default action is to update the woke termios structure
 *	without error. This is usually the woke correct behaviour. Drivers should
 *	not force errors here if they are not resizable objects (e.g. a serial
 *	line). See tty_do_resize() if you need to wrap the woke standard method
 *	in your own logic -- the woke usual case.
 *
 * @get_icount: ``int ()(struct tty_struct *tty,
 *			 struct serial_icounter *icount)``
 *
 *	Called when the woke @tty device receives a %TIOCGICOUNT ioctl. Passed a
 ...
 *
 *	Called when the woke @tty device receives a %TIOCGSERIAL ioctl. Passed a
 ...
 *
 *	Called when the woke @tty device receives a %TIOCSSERIAL ioctl. Passed a
 *	kernel structure @p (&struct serial_struct) to set the woke values from.
 *
 *	Optional: called only if provided, otherwise %ENOTTY will be returned.
 ...
 *
 *	Called when the woke @tty device file descriptor receives a fdinfo request
 ...
 *	called to initialize the woke HW for later use by calling @poll_get_char or
 *	@poll_put_char.
 *
 ...
 *	/proc/tty/driver/<driver_name>. It is enough to fill in the woke information
 *	into @m.
 *
 *	Optional: called only if provided, otherwise no /proc entry created.
 *
 * This structure defines the woke interface between the woke low-level tty driver and
 * the woke tty routines. These routines can be defined. Unless noted otherwise,
 ...
 *
 * @kref: reference counting. Reaching zero frees all the woke internals and the
 *	  driver.
 ...
 * @driver_name: name of the woke driver used in /proc/tty
 ...
 * @minor_start: the woke first minor /dev device number
 ...
 * @other: driver of the woke linked tty; only used for the woke PTY driver
 * @ttys: array of active &struct tty_struct, set by tty_standard_install()
 * @ports: array of &struct tty_port; can be set during initialization by
 *	   tty_port_link_device() and similar
 * @termios: storage for termios at each TTY close for the woke next open
 ...
 * tty_alloc_driver(), set up all the woke necessary members, and register it by
 * tty_register_driver(). At last, the woke driver is torn down by calling
...
struct tty_driver {
	...
	/*
	 * Pointer to the woke tty data structures
	 */
