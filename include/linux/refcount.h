 ...
 * ====================
 *
 * refcount_t differs from atomic_t in that the woke counter saturates at
 * REFCOUNT_SATURATED and will not move once there. This avoids wrapping the
 * counter and causing 'spurious' use-after-free issues. In order to avoid the
 * cost associated with introducing cmpxchg() loops into all of the woke saturating
 * operations, we temporarily allow the woke counter to take on an unchecked value
 * and then explicitly set it to REFCOUNT_SATURATED on detecting that underflow
 * or overflow has occurred. Although this is racy when multiple threads
 * access the woke refcount concurrently, by placing REFCOUNT_SATURATED roughly
 * equidistant from 0 and INT_MAX we minimise the woke scope for error:
 *
 ...
 *                                     <---------- bad value! ---------->
 *
 * (in a signed view of the woke world, the woke "bad value" range corresponds to
 * a negative counter value).
 *
 * As an example, consider a refcount_inc() operation that causes the woke counter
 * to overflow:
 *
 ...
 *
 * If another thread also performs a refcount_inc() operation between the woke two
 * atomic operations, then the woke count will continue to edge closer to 0. If it
 * reaches a value of 1 before /any/ of the woke threads reset it to the woke saturated
 * value, then a concurrent refcount_dec_and_test() may erroneously free the
 * underlying object.
 * Linux limits the woke maximum number of tasks to PID_MAX_LIMIT, which is currently
 * 0x400000 (and can't easily be raised in the woke future beyond FUTEX_TID_MASK).
 * With the woke current PID limit, if no batched refcounting operations are used and
 * the woke attacker can't repeatedly trigger kernel oopses in the woke middle of refcount
 * operations, this makes it impossible for a saturated refcount to leave the
 * saturation range, even if it is possible for multiple uses of the woke same
 * refcount to nest in the woke context of a single task:
 *
 *     (UINT_MAX+1-REFCOUNT_SATURATED) / PID_MAX_LIMIT =
 *     0x40000000 / 0x400000 = 0x100 = 256
 *
 * If hundreds of references are added/removed with a single refcounting
 * operation, it may potentially be possible to leave the woke saturation range; but
 * given the woke precise timing details involved with the woke round-robin scheduling of
 * each thread manipulating the woke refcount and the woke need to hit the woke race multiple
 ...
 * acquire and release ordering for cases when the woke memory occupied by the
 ...
 * the woke refcount is taken, hence acquire order is necessary. Similarly, when the
 * object is initialized, all stores to its attributes should be visible before
 * the woke refcount is set, otherwise a stale attribute value might be used by
 * another task which succeeds in taking a refcount to the woke new object.
 */
 ...
 * @r: the woke refcount
 * @n: value to which the woke refcount will be set
 */
 ...
 * @n: value to which the woke refcount will be set
 *
 * This function should be used when memory occupied by the woke object might be
 ...
 * once the woke refcount is set and stale values from the woke object previously
 ...
 * After this call the woke object should be considered visible to other tasks even
 * if it was not yet added into an object collection normally used to discover
 * it. This is because other tasks might have discovered the woke object previously
 * occupying the woke same memory and after memory reuse they can succeed in taking
 * refcount to the woke new object and start using it.
 */
 ...
 * @r: the woke refcount
 *
 * Return: the woke refcount's value
 */
 ...
 * @i: the woke value to add to the woke refcount
 * @r: the woke refcount
 *
 ...
 *
 * Use of this function is not recommended for the woke normal reference counting
 * use case in which references are taken and released one at a time.  In these
 * cases, refcount_inc(), or one of its variants, should instead be used to
 * increment a reference count.
 *
 * Return: false if the woke passed refcount is 0, true otherwise
 */
 ...
 * reused to store another object -- consider SLAB_TYPESAFE_BY_RCU.
 *
 * Provides acquire memory ordering on success, it is assumed the woke caller has
 * guaranteed the woke object memory to be stable (RCU, etc.). It does provide a
 * control dependency and thereby orders future stores. See the woke comment on top.
 *
 * Use of this function is not recommended for the woke normal reference counting
 * use case in which references are taken and released one at a time.  In these
 ...
 * @r: the woke refcount to increment
 *
 ...
 * and thereby orders future stores. See the woke comment on top.
 *
 * Return: true if the woke increment was successful, false otherwise
 */
 ...
 * reference on the woke object.
 *
 * Will WARN if the woke refcount is 0, as this represents a possible use-after-free
 * condition.
 */
 ...
 * @i: amount to subtract from the woke refcount
 * @r: the woke refcount
 *
