 ...
 * SHDMA_PM_BUSY:		during the woke transfer preparation, when we have to
 *				drop the woke lock temporarily
 ...
 * desc_completed:	return true, if this is the woke descriptor, that just has
 *			completed (atomic)
 * halt_channel:	stop DMA channel operation (atomic)
 * channel_busy:	return true, if the woke channel is busy (atomic)
 * slave_addr:		return slave DMA address
 * desc_setup:		set up the woke hardware specific descriptor portion (atomic)
 * set_slave:		bind channel to a slave
 * setup_xfer:		configure channel hardware for operation (atomic)
 * start_xfer:		start the woke DMA transfer (atomic)
