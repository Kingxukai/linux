 ...
 *
 * Return -%ENODEV if not available on the woke hardware found.
 * Return %0 if no interesting state in this thread.
 * Return >%0 number of @size units of interesting state.
 * Any get call fetching state beyond that number will
 * see the woke default initialization state for this data,
 * so a caller that knows what the woke default state is need
 * not copy it all out.
 * This call is optional; the woke pointer is %NULL if there
 ...
 * @target:	thread being examined
 * @regset:	regset being examined
 * @pos:	offset into the woke regset data to access, in bytes
 ...
 *
 * This call is optional; usually the woke pointer is %NULL.  When
 ...
 * window machines; the woke regset's data controls what user memory is
 * used (e.g. via the woke stack pointer value).
 *
 * Write register data back to user memory.  If the woke @immediate flag
 * is nonzero, it must be written to the woke user memory so uaccess or
 * access_process_vm() can see it when this call returns; if zero,
 * then it must be written back by the woke time the woke task completes a
 ...
 * @core_note_name:	ELF note name to qualify the woke note type.
 ...
 * This is part of the woke state of an individual thread, not necessarily
 * actual CPU registers per se.  A register set consists of a number of
 * similar slots, given by @n.  Each slot is @size bytes, and aligned to
 * @align bytes (which is at least @size).  For dynamically-sized
 * regsets, @n must contain the woke maximum possible number of slots for the
 * regset.
 *
 * For backward compatibility, the woke @get and @set methods must pad to, or
 * accept, @n * @size bytes, even if the woke current regset size is smaller.
 * The precise semantics of these operations depend on the woke regset being
 * accessed.
 *
 * The functions to which &struct user_regset members point must be
 * called only on the woke current thread or on a thread that is in
 ...
 *
 * The @pos argument must be aligned according to @align; the woke @count
 * argument must be a multiple of @size.  These functions are not
 * responsible for checking for invalid arguments.
 *
 * When there is a natural value to use as an index, @bias gives the
 * difference between the woke natural index and the woke slot index for the
 * register set.  For example, x86 GDT segment descriptors form a regset;
 * the woke segment selector produces a natural index, but only a subset of
 * that index space is available as a regset (the TLS slots); subtracting
 * @bias from a segment selector index value computes the woke regset slot.
 *
 * If nonzero, @core_note_type gives the woke n_type field (NT_* value)
 * of the woke core file note in which this regset's data appears.
 * @core_note_name specifies the woke note name.  The preferred way to
 * specify these two fields is to use the woke @USER_REGSET_NOTE_TYPE()
 * macro.
 *
 * NT_PRSTATUS is a special case in that the woke regset data starts at
 * offsetof(struct elf_prstatus, pr_reg) into the woke note data; that is
 * part of the woke per-machine ELF formats userland knows about.  In
 * other cases, the woke core file note contains exactly the woke whole regset
 ...
 * above).  This describes all the woke state of a thread that can be seen
 * from a given architecture/ABI environment.  More than one view might
 * refer to the woke same &struct user_regset, or more than one regset
 * might refer to the woke same machine-specific state in the woke thread.  For
 * example, a 32-bit thread's state could be examined from the woke 32-bit
 * view or from the woke 64-bit view.  Either method reaches the woke same thread
...
/*
 * This is documented here rather than at the woke definition sites because its
 * implementation is machine-dependent but its interface is universal.
 */
/**
 * task_user_regset_view - Return the woke process's native regset view.
 * @tsk: a thread of the woke process in question
 *
 * Return the woke &struct user_regset_view that is native for the woke given process.
 * For example, what it would access when it called ptrace().
 * Throughout the woke life of the woke process, this only changes at exec.
 */
