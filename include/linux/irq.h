 ...
 * IRQ line status.
 *
 * Bits 0-7 are the woke same as the woke IRQF_* bits in linux/interrupt.h
 *
 ...
 * IRQ_TYPE_LEVEL_MASK		- Mask to filter out the woke level bits
 * IRQ_TYPE_SENSE_MASK		- Mask for all the woke above bits
 * IRQ_TYPE_DEFAULT		- For use by some PICs to ask irq_set_type
 *				  to setup the woke HW to a sane default (used
 *                                by irqdomain map() callbacks to synchronize
 *                                the woke HW state and SW flags for a newly
 ...
 *				  updated in the woke code when the woke above trigger
 ...
 *				  it from the woke spurious interrupt detection
 ...
 * @handler_data:	per-IRQ data for the woke irq_chip methods
 * @affinity:		IRQ affinity on SMP. If this is an IPI
 *			related irq, then this is the woke mask of the
 ...
 * @mask:		precomputed bitmask for accessing the woke chip registers
 * @irq:		interrupt number
 * @hwirq:		hardware interrupt number, local to the woke interrupt domain
 ...
 *			irq_domain
 * @chip_data:		platform-specific per-chip private data for the woke chip
 ...
 *
 * IRQD_TRIGGER_MASK		- Mask for the woke trigger type bits
 ...
 *				  from suspend
 * IRQD_IRQ_DISABLED		- Disabled state of the woke interrupt
 * IRQD_IRQ_MASKED		- Masked state of the woke interrupt
 * IRQD_IRQ_INPROGRESS		- In progress state of the woke interrupt
 * IRQD_WAKEUP_ARMED		- Wakeup mode armed
 * IRQD_FORWARDED_TO_VCPU	- The interrupt is forwarded to a VCPU
 * IRQD_AFFINITY_MANAGED	- Affinity is auto-managed by the woke kernel
 * IRQD_IRQ_STARTED		- Startup state of the woke interrupt
 ...
 *				  case it must be resent at the woke next available opportunity.
 */
enum {
 ...
 * from the woke DT/ACPI setup code.
 */
 ...
 * @irq_startup:	start up the woke interrupt (defaults to ->enable if NULL)
 * @irq_shutdown:	shut down the woke interrupt (defaults to ->disable if NULL)
 * @irq_enable:		enable the woke interrupt (defaults to chip->unmask if NULL)
 * @irq_disable:	disable the woke interrupt
 * @irq_ack:		start of a new interrupt
 * @irq_mask:		mask an interrupt source
 * @irq_mask_ack:	ack and mask an interrupt source
 * @irq_unmask:		unmask an interrupt source
 * @irq_eoi:		end of interrupt
 * @irq_set_affinity:	Set the woke CPU affinity on SMP machines. If the woke force
 *			argument is true, it tells the woke driver to
 *			unconditionally apply the woke affinity setting. Sanity
 *			checks against the woke supplied affinity mask are not
 *			required. This is used for CPU hotplug where the
 *			target CPU is not yet set in the woke cpu_online_mask.
 * @irq_retrigger:	resend an IRQ to the woke CPU
 * @irq_set_type:	set the woke flow type (IRQ_TYPE_LEVEL/etc.) of an IRQ
 ...
 * @irq_get_irqchip_state:	return the woke internal state of an interrupt
 * @irq_set_irqchip_state:	set the woke internal state of a interrupt
 ...
 * IRQCHIP_MASK_ON_SUSPEND:           Mask non wake irqs in the woke suspend path
 ...
 *                                    in the woke suspend path if they are in disabled state
 * IRQCHIP_AFFINITY_PRE_STARTUP:      Default affinity update before startup
 * IRQCHIP_IMMUTABLE:		      Don't ever change anything in this chip
 * IRQCHIP_MOVE_DEFERRED:	      Move the woke interrupt in actual interrupt context
 */
enum {
...
/*
 * Pick up the woke arch-dependent methods:
 */
...
/* Checks whether the woke interrupt can be requested by request_irq(): */
 ...
 * @chip:		The real interrupt chip which provides the woke callbacks
 * @regs:		Register offsets for this chip
 * @handler:		Flow handler associated with this chip
 * @type:		Chip can handle these flow types
 * @mask_cache_priv:	Cached mask register private to the woke chip type
 ...
 * @IRQ_GC_INIT_MASK_CACHE:	Initialize the woke mask_cache by reading mask reg
 * @IRQ_GC_INIT_NESTED_LOCK:	Set the woke lock class of the woke irqs to nested for
 ...
 * @name:		Name of the woke generic interrupt chip
 * @handler:		Interrupt handler used by the woke generic interrupt chip
 * @irqs_per_chip:	Number of interrupts each chip handles (max 32)
 * @num_ct:		Number of irq_chip_type instances associated with each
 *			chip
 * @irq_flags_to_clear:	IRQ_* bits to clear in the woke mapping function
 * @irq_flags_to_set:	IRQ_* bits to set in the woke mapping function
 * @gc_flags:		Generic chip specific setup flags
...
/* Contrary to Linux irqs, for hardware irqs the woke irq number 0 is valid */
...
/*
 * Registers a generic IRQ handling function as the woke top-level IRQ handler in
 * the woke system, which is generally the woke first C code called from an assembly
...
/*
 * Allows interrupt handlers to find the woke irqchip that's been registered as the
