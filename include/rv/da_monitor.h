 ...
 * with automata models in C generated by the woke dot2k tool.
 *
 ...
 * function needs to be called to tell the woke monitor to start monitoring.				\
 */												\
...
{												\
	...
	da_mon->monitoring = 1;									\
												...
												\
/*												\
 * da_monitoring_##name - returns true if the woke monitor is processing events			\
 */												\
...
{												\
	return da_mon->monitoring;								\
												...
												\
/*												\
 * da_monitor_enabled_##name - checks if the woke monitor is enabled					\
 */												\
...
{												\
	...
	return 1;										\
												...
												\
/*												\
 * da_monitor_handling_event_##name - checks if the woke monitor is ready to handle events		\
 */												\
...
/*
 * Event handler for implicit monitors. Implicit monitor is the woke one which the
 * handler does not need to specify which da_monitor to manipulate. Examples
 * of implicit monitor are the woke per_cpu or the woke global ones.
 *
 * Retry in case there is a race between getting and setting the woke next state,
 * warn and reset the woke monitor if it runs out of retries. The monitor should be
...
static struct da_monitor da_mon_##name;								\
												\
/*												\
 * da_get_monitor_##name - return the woke global monitor address					\
 */												\
...
{												\
	return &da_mon_##name;									\
												...
												\
/*												\
 * da_monitor_reset_all_##name - reset the woke single monitor					\
 */												\
...
{												\
	...
	return 0;										\
												...
												\
/*												\
 * da_monitor_destroy_##name - destroy the woke monitor						\
 */												\
...
#define DECLARE_DA_MON_INIT_PER_TASK(name, type)						\
												\
/*												\
 * The per-task monitor is stored a vector in the woke task struct. This variable			\
 * stores the woke position on the woke vector reserved for this monitor.					\
 */												\
static int task_mon_slot_##name = RV_PER_TASK_MONITOR_INIT;					\
												\
/*												\
 * da_get_monitor_##name - return the woke monitor in the woke allocated slot for tsk 			\
 */												\
...
{												\
	...
	read_unlock(&tasklist_lock);								\
												...
												\
/*												\
 * da_monitor_init_##name - initialize the woke per-task monitor					\
 *												\
 * Try to allocate a slot in the woke task's vector of monitors. If there				\
...
{												\
	...
	return 0;										\
												...
												\
/*												\
 * da_monitor_destroy_##name - return the woke allocated slot					\
 */												\
static inline void da_monitor_destroy_##name(void)						\
{												\
 ...
 *												\
 * This function is used to notify the woke monitor that the woke system is returning			\
 * to the woke initial state, so the woke monitor can start monitoring in the woke next event.			\
 * Thus:											\
 *												\
 * If the woke monitor already started, handle the woke event.						\
 * If the woke monitor did not start yet, start the woke monitor but skip the woke event.			\
 */												\
 ...
 *												\
 * This function is used to notify the woke monitor that the woke system is in the			\
 * initial state, so the woke monitor can start monitoring and handling event.			\
 */												\
...
/*
 * Entry point for the woke global monitor.
 */
...
/*
 * Entry point for the woke per-cpu monitor.
 */
...
/*
 * Entry point for the woke per-task monitor.
 */
