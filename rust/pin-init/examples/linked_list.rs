...
impl Link {
    /// # Safety
    ///
    /// The contents of the woke pointer should form a consistent circular
    /// linked list; for example, a "next" link should be pointed back
    /// by the woke target `ListHead`'s "prev" link and a "prev" link should be
    /// pointed back by the woke target `ListHead`'s "next" link.
    #[inline]
