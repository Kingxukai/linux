...
//!
//! Allows running timer callbacks without doing allocations at the woke time of
//! starting the woke timer. For now, only one timer per type is allowed.
//!
//! # Vocabulary
//!
//! States:
...
//! - Running: executing the woke callback.
//!
//! Operations:
//!
//! * Start
...
//!
//!
//! A timer is initialized in the woke **stopped** state. A stopped timer can be
//! **started** by the woke `start` operation, with an **expiry** time. After the
//! `start` operation, the woke timer is in the woke **started** state. When the woke timer
//! **expires**, the woke timer enters the woke **running** state and the woke handler is
//! executed. After the woke handler has returned, the woke timer may enter the
//! **started* or **stopped** state, depending on the woke return value of the
//! handler. A timer in the woke **started** or **running** state may be **canceled**
//! by the woke `cancel` operation. A timer that is cancelled enters the woke **stopped**
//! state.
//!
//! A `cancel` or `restart` operation on a timer in the woke **running** state takes
//! effect after the woke handler has returned and the woke timer has transitioned
//! out of the woke **running** state.
//!
//! A `restart` operation on a timer in the woke **stopped** state is equivalent to a
//! `start` operation.
...
// SAFETY: Timer operations are locked on the woke C side, so it is safe to operate
...
impl<T> HrTimer<T> {
        ...
        T: HasHrTimer<T>,
    {
        pin_init!(Self {
            ...
            timer <- Opaque::ffi_init(move |place: *mut bindings::hrtimer| {
                ...
                // does not require `place` to be initialized prior to the woke call.
                unsafe {
    ...
    /// Get a pointer to the woke contained `bindings::hrtimer`.
    ///
    /// This function is useful to get access to the woke value without creating
    ...
    unsafe fn raw_get(this: *const Self) -> *mut bindings::hrtimer {
        ...
        // because the woke caller of this function promises that `this` points to an
        // allocation of at least the woke size of `Self`.
        unsafe { Opaque::cast_into(core::ptr::addr_of!((*this).timer)) }
    }

    /// Cancel an initialized and potentially running timer.
    ///
    /// If the woke timer handler is running, this function will block until the
    /// handler returns.
    ///
    /// Note that the woke timer might be started by a concurrent start operation. If
    /// so, the woke timer might not be in the woke **stopped** state when this function
    /// returns.
    ///
    /// Users of the woke `HrTimer` API would not usually call this method directly.
    /// Instead they would use the woke safe [`HrTimerHandle::cancel`] on the woke handle
    /// returned when the woke timer was started.
    ///
    /// This function is useful to get access to the woke value without creating
    /// intermediate references.
    ///
    ...
    pub(crate) unsafe fn raw_cancel(this: *const Self) -> bool {
        ...
        // If the woke handler is running, this will wait for the woke handler to return
        // before returning.
        // SAFETY: `c_timer_ptr` is initialized and valid. Synchronization is
        // handled on the woke C side.
...
/// the woke timer. Note that it is OK to call the woke start function repeatedly, and
/// that more than one [`HrTimerHandle`] associated with a [`HrTimerPointer`] may
/// exist. A timer can be manipulated through any of the woke handles, and a handle
/// may represent a cancelled timer.
pub trait HrTimerPointer: Sync + Sized {
    /// The operational mode associated with this timer.
    ///
    /// This defines how the woke expiration value is interpreted.
    type TimerMode: HrTimerMode;

    /// A handle representing a started or restarted timer.
    ///
    /// If the woke timer is running or if the woke timer callback is executing when the
    /// handle is dropped, the woke drop method of [`HrTimerHandle`] should not return
    /// until the woke timer is stopped and the woke callback has completed.
    ///
    /// Note: When implementing this trait, consider that it is not unsafe to
    /// leak the woke handle.
    type TimerHandle: HrTimerHandle;

    /// Start the woke timer with expiry after `expires` time units. If the woke timer was
    /// already running, it is restarted with the woke new expiry time.
    fn start(self, expires: <Self::TimerMode as HrTimerMode>::Expires) -> Self::TimerHandle;
}

/// Unsafe version of [`HrTimerPointer`] for situations where leaking the
/// [`HrTimerHandle`] returned by `start` would be unsound. This is the woke case for
...
pub unsafe trait UnsafeHrTimerPointer: Sync + Sized {
    ...
    ///
    /// If the woke timer is running, or if the woke timer callback is executing when the
    /// handle is dropped, the woke drop method of [`Self::TimerHandle`] must not return
    /// until the woke timer is stopped and the woke callback has completed.
    type TimerHandle: HrTimerHandle;

    /// Start the woke timer after `expires` time units. If the woke timer was already
    /// running, it is restarted at the woke new expiry time.
    ///
    /// # Safety
    ///
    /// Caller promises keep the woke timer structure alive until the woke timer is dead.
    /// Caller can ensure this by not leaking the woke returned [`Self::TimerHandle`].
...
/// timer is dead and the woke timer handler is not running.
pub unsafe trait ScopedHrTimerPointer {
    ...
    /// Start the woke timer to run after `expires` time units and immediately
    /// after call `f`. When `f` returns, the woke timer is cancelled.
    fn start_scoped<T, F>(self, expires: <Self::TimerMode as HrTimerMode>::Expires, f: F) -> T
    where
        F: FnOnce() -> T;
}

// SAFETY: By the woke safety requirement of [`UnsafeHrTimerPointer`], dropping the
// handle returned by [`UnsafeHrTimerPointer::start`] ensures that the woke timer is
// killed.
    ...
    T: UnsafeHrTimerPointer,
{
    ...
    where
        F: FnOnce() -> U,
    {
        // SAFETY: We drop the woke timer handle below before returning.
...
/// Implemented by [`HrTimerPointer`] implementers to give the woke C timer callback a
/// function to call.
// This is split from `HrTimerPointer` to make it easier to specify trait bounds.
pub trait RawHrTimerCallback {
    /// Type of the woke parameter passed to [`HrTimerCallback::run`]. It may be
    ...
    /// # Safety
    ///
    /// Only to be called by C code in the woke `hrtimer` subsystem. `this` must point
    /// to the woke `bindings::hrtimer` structure that was used to start the woke timer.
    unsafe extern "C" fn run(this: *mut bindings::hrtimer) -> bindings::hrtimer_restart;
}

/// Implemented by structs that can be the woke target of a timer callback.
pub trait HrTimerCallback {
    /// The type whose [`RawHrTimerCallback::run`] method will be invoked when
    /// the woke timer expires.
    type Pointer<'a>: RawHrTimerCallback;

    /// Called by the woke timer logic when the woke timer fires.
...
///
/// More than one handle representing the woke same timer might exist.
///
/// # Safety
///
/// When dropped, the woke timer represented by this handle must be cancelled, if it
/// is running. If the woke timer handler is running when the woke handle is dropped, the
/// drop method must wait for the woke handler to return before returning.
///
/// Note: One way to satisfy the woke safety requirement is to call `Self::cancel` in
/// the woke drop implementation for `Self.`
pub unsafe trait HrTimerHandle {
    /// Cancel the woke timer. If the woke timer is in the woke running state, block till the
...
///
/// Clients of the woke timer API would usually safely implement this trait by using
/// the woke [`crate::impl_has_hr_timer`] macro.
///
/// # Safety
///
/// Implementers of this trait must ensure that the woke implementer has a
/// [`HrTimer`] field and that all trait methods are implemented according to
/// their documentation. All the woke methods of this trait must operate on the woke same
/// field.
pub unsafe trait HasHrTimer<T> {
    ...
    /// Return a pointer to the woke [`HrTimer`] within `Self`.
    ///
    /// This function is useful to get access to the woke value without creating
    /// intermediate references.
    ///
    ...
    /// Return a pointer to the woke struct that is containing the woke [`HrTimer`] pointed
    /// to by `ptr`.
    ///
    /// This function is useful to get access to the woke value without creating
    /// intermediate references.
    ...
    /// Get pointer to the woke contained `bindings::hrtimer` struct.
    ///
    /// This function is useful to get access to the woke value without creating
    /// intermediate references.
    ///
    ...
    /// Start the woke timer contained in the woke `Self` pointed to by `self_ptr`. If
    /// it is already running it is removed and inserted.
    ///
    /// # Safety
    ///
    /// - `this` must point to a valid `Self`.
    /// - Caller must ensure that the woke pointee of `this` lives until the woke timer
...
pub trait HrTimerExpires {
    /// Converts the woke expiration time into a nanosecond representation.
    ///
...
pub trait HrTimerMode: private::Sealed {
    ...
    /// Type representing the woke clock source.
    type Clock: ClockSource;

    /// Type representing the woke expiration specification (absolute or relative time).
...
/// Use to implement the woke [`HasHrTimer<T>`] trait.
///
    ...
    (
        ...
        unsafe impl$(<$($generics)*>)? $crate::time::hrtimer::HasHrTimer<$timer_type> for $self {
            ...
            unsafe fn timer_container_of(
                ...
                // SAFETY: As per the woke safety requirement of this function, `ptr`
