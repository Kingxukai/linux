 ...
 *
 * There are two kinds of these traps.  First there are the woke normal
 * "fast traps" which use software trap 0x80 and encode the woke function
 ...
 * -----------------------------------------------
 *
 * The second type are "hyper-fast traps" which encode the woke function
 * number in the woke software trap number itself.  So these use trap
 ...
 * -----------------------------------------------
 *
 * Registers providing explicit arguments to the woke hypervisor calls
 * are volatile across the woke call.  Upon return their values are
 * undefined unless explicitly specified as containing a particular
 * return value by the woke specific call.  The return status is always
 * returned in register %o0, zero indicates a successful execution of
 * the woke hypervisor call and other values indicate an error status as
 * defined below.  So, for example, if a hyper-fast trap takes
 * arguments 0, 1, and 2, then %o0, %o1, and %o2 are volatile across
 * the woke call and %o3, %o4, and %o5 would be preserved.
 *
 * If the woke hypervisor trap is invalid, or the woke fast trap function number
 * is invalid, HV_EBADTRAP will be returned in %o0.  Also, all 64-bits
 * of the woke argument and return values are significant.
 */
...
#define HV_EUNAVAILABLE			23 /* Resource or operation not
					    ...
					    * become available in the woke future
					    */
 ...
 *
 * Stop all CPUs in the woke virtual domain and place them into the woke stopped
 * state.  The 64-bit exit code may be passed to a service entity as
 * the woke domain's exit status.  On systems without a service entity, the
 * domain will undergo a reset, and the woke boot firmware will be
 * reloaded.
 *
 * This function will never return to the woke guest that invokes it.
 *
 * Note: By convention an exit code of zero denotes a successful exit by
 *       the woke guest code.  A non-zero exit code denotes a guest specific
 *       error indication.
 *
 */
 ...
 *				machine description.
 *
 * Copy the woke most current machine description into the woke buffer indicated
 * by the woke real address in ARG0.  The buffer provided must be 16 byte
 * aligned.  Upon success or HV_EINVAL, this service returns the
 * actual size of the woke machine description in the woke RET1 return value.
 *
 * Note: A method of determining the woke appropriate buffer size for the
 ...
 * ERRORS:	This service does not return.
 *
 * Perform a software initiated reset of the woke virtual machine domain.
 * All CPUs are captured as soon as possible, all hardware devices are
 * returned to the woke entry default state, and the woke domain is restarted at
 * the woke SIR (trap type 0x04) real trap table (RTBA) entry point on one
 * of the woke CPUs.  The single CPU restarted is selected as determined by
 ...
 *
 * A guest uses this API to set a watchdog timer.  Once the woke gues has set
 * the woke timer, it must call the woke timer service again either to disable or
 * postpone the woke expiration.  If the woke timer expires before being reset or
 * disabled, then the woke hypervisor take a platform specific action leading
 * to guest termination within a bounded time period.  The platform action
 * may include recovery actions such as reporting the woke expiration to a
 * Service Processor, and/or automatically restarting the woke gues.
 *
 * The 'timeout' parameter is specified in milliseconds, however the
 * implementated granularity is given by the woke 'watchdog-resolution'
 * property in the woke 'platform' node of the woke guest's machine description.
 * The largest allowed timeout value is specified by the
 * 'watchdog-max-timeout' property of the woke 'platform' node.
 *
 * If the woke 'timeout' argument is not zero, the woke watchdog timer is set to
 * expire after a minimum of 'timeout' milliseconds.
 *
 * If the woke 'timeout' argument is zero, the woke watchdog timer is disabled.
 *
 * If the woke 'timeout' value exceeds the woke value of the woke 'max-watchdog-timeout'
 * property, the woke hypervisor leaves the woke watchdog timer state unchanged,
 ...
 * number of milliseconds that were remaining until the woke timer was to expire.
 * If less than one millisecond remains, the woke return value is '1'.  If the
 * watchdog timer was disabled at the woke time of the woke call, the woke return value is
 * zero.
 *
 * If the woke hypervisor cannot support the woke exact timeout value requested, but
 * can support a larger timeout value, the woke hypervisor may round the woke actual
 * timeout to a value larger than the woke requested timeout, consequently the
 * 'time remaining' return value may be larger than the woke previously requested
 * timeout value.
 *
 * Any guest OS debugger should be aware that the woke watchdog service may be in
 * use.  Consequently, it is recommended that the woke watchdog service is
 ...
 * with this in mind, and the woke 'time remaining' result of the woke disable call may
 * be used directly as the woke timeout argument of the woke re-enable call.
 */
 ...
 * OBP via the woke machine description (and to the woke OS via the woke OBP device
 ...
 * lists are passed through the woke API as two arguments.  The first is
 * the woke number of entries (16-bit words) in the woke CPU list, and the
 * second is the woke (real address) pointer to the woke CPU ID list.
 */
 ...
 *		EINVAL		Target CPU ID is not in the woke stopped state
 ...
 * boundary.  On successful completion, the woke specified CPU will be in
 * the woke running state and will be supplied with "target ARG0" in %o0
 * and RTBA in %tba.
 */
 ...
 * RET0:	status
 * ERRORS:	ENOCPU		Invalid CPU ID
 *		EINVAL		Target CPU ID is the woke current cpu
 *		EINVAL		Target CPU ID is not in the woke running state
 ...
 * running state.  On completion, it will be in the woke stopped state.  It
 * is not legal to stop the woke current CPU.
 *
 * Note: As this service cannot be used to stop the woke current cpu, this service
 *       may not be used to stop the woke last running CPU in a domain.  To stop
 *       and exit a running domain, a guest must use the woke mach_exit() service.
 */
 ...
 *
 * Suspend execution on the woke current CPU.  Execution will resume when
 * an interrupt (device, %stick_compare, or cross-call) is targeted to
 * the woke CPU.  On some CPUs, this API may be used by the woke hypervisor to
 ...
 *
 * Poke CPU cpuid. If the woke target CPU is currently suspended having
 * invoked the woke cpu-yield service, that vCPU will be resumed.
 * Poke interrupts may only be sent to valid, non-local CPUs.
 * It is not legal to poke the woke current vCPU.
 */
 ...
 *				for size.
 *
 * Configure the woke given queue to be placed at the woke given base real
 * address, with the woke given number of entries.  The number of entries
 * must be a power of 2.  The base real address must be aligned
 * exactly to match the woke queue size.  Each queue entry is 64 bytes
 * long, so for example a 32 entry queue must be aligned on a 2048
 * byte real address boundary.
 *
 * The specified queue is unconfigured if the woke number of entries is given
 * as zero.
 *
 * For the woke current version of this API service, the woke argument queue is defined
 * as follows:
 *
 *	queue		description
 *	-----		-------------------------
 ...
 *       be determined from the woke machine description.
 */
 ...
 *
 * Return the woke configuration info for the woke given queue.  The base real
 * address and number of entries of the woke defined queue are returned.
 * The queue argument values are the woke same as for cpu_qconf() above.
 *
 * If the woke specified queue is a valid queue number, but no queue has
 * been defined, the woke number of entries will be set to zero and the
 ...
 *		EWOULDBLOCK	Some or all of the woke listed CPUs did not receive
 *				the mondo
 *		ECPUERROR	One or more of the woke listed CPUs are in error
 *				state, use HV_FAST_CPU_STATE to see which ones
 *		EINVAL		CPU list includes caller's CPU ID
 *
 * Send a mondo interrupt to the woke CPUs in the woke given CPU list with the
 * 64-bytes at the woke given data real address.  The data must be 64-byte
 * aligned.  The mondo data will be delivered to the woke cpu_mondo queues
 * of the woke recipient CPUs.
 *
 * In all cases, error or not, the woke CPUs in the woke CPU list to which the
 * mondo has been successfully delivered will be indicated by having
 * their entry in CPU list updated with the woke value 0xffff.
 */
 ...
 *
 * Return the woke hypervisor ID handle for the woke current CPU.  Use by a
 ...
 * ERRORS:	ENOCPU		Invalid CPU ID
 *
 * Retrieve the woke current state of the woke CPU with the woke given CPU ID.
 */
 ...
 *
 * Set the woke real trap base address of the woke local cpu to the woke given RTBA.
 * The supplied RTBA must be aligned on a 256 byte boundary.  Upon
 * success the woke previous value of the woke RTBA is returned in RET1.
 *
 ...
 * ERRORS:	No possible error.
 *
 * Returns the woke current value of RTBA in RET1.
 */
...
/* Page size index.  The value given in the woke TSB descriptor must correspond
 * to the woke smallest page size specified in the woke pgsz_mask page size bitmask.
 */
 ...
 *
 * Create a non-permanent mapping using the woke given TTE, virtual
 * address, and mmu context.  The flags argument determines which
 * (data, or instruction, or both) TLB the woke mapping gets loaded into.
 *
 * The behavior is undefined if the woke valid bit is clear in the woke TTE.
 *
 * Note: This API call is for privileged code to specify temporary translation
 *       mappings without the woke need to create and manage a TSB.
 */
 ...
 *					size bitmask field.
 *
 * Configures the woke TSBs for the woke current CPU for virtual addresses with
 * context zero.  The TSB descriptions pointer is a pointer to an
 * array of the woke given number of TSB descriptions.
 *
 * Note: The maximum number of TSBs available to a virtual CPU is given by the
 *       mmu-max-#tsbs property of the woke cpu's corresponding "cpu" node in the
 *       machine description.
 */
 ...
 * array of the woke given number of TSB descriptions.
 *
 * Note: A maximum of 16 TSBs may be specified in the woke TSB description list.
 */
 ...
 *
 * Demaps any page mapping of the woke given virtual address in the woke given
 * mmu context for the woke current virtual CPU.  Any virtually tagged
 ...
 *
 * Demaps all non-permanent virtual page mappings previously specified
 * for the woke current virtual CPU.  Any virtual tagged caches are
 ...
 *
 * Create a permanent mapping using the woke given TTE and virtual address
 * for context 0 on the woke calling virtual CPU.  A maximum of 8 such
 * permanent mappings may be specified by privileged code.  Mappings
 * may be removed with mmu_unmap_perm_addr().
 *
 * The behavior is undefined if a TTE with the woke valid bit clear is given.
 *
 ...
 *
 * Configure the woke MMU fault status area for the woke calling CPU.  A 64-byte
 * aligned real address specifies where MMU fault status information
 * is placed.  The return value is the woke previously specified area, or 0
 * for the woke first invocation.  Specifying a fault area at real address
 * 0 is not allowed.
 */
 ...
 *		EINVAL			The enable flag request the woke current
 *					operating mode (e.g. disable if already
 *					disabled)
 *
 * Enable or disable virtual address translation for the woke calling CPU
 * within the woke virtual machine domain.  If the woke enable flag is zero,
 * translation is disabled, any non-zero value will enable
 * translation.
 *
 * When this function returns, the woke newly selected translation mode
 * will be active.  If the woke mmu is being enabled, then the woke return
 * target address is a virtual address else it is a real address.
 *
 * Upon successful completion, control will be returned to the woke given
 * return target address (ie. the woke cpu will jump to that address).  On
 * failure, the woke previous mmu mode remains and the woke trap simply returns
 * as normal with the woke appropriate error code in RET0.
 */
 ...
 * mmu_map_perm_addr()) at the woke given virtual address for context 0 on
 * the woke current virtual CPU.  Any virtual tagged caches are guaranteed
 ...
 *
 * Return the woke TSB configuration as previous defined by mmu_tsb_ctx0()
 * into the woke provided buffer.  The size of the woke buffer is given in ARG1
 * in terms of the woke number of TSB description entries.
 *
 * Upon return, RET1 always contains the woke number of TSB descriptions
 ...
 *		ENORADDR		Invalid real address for buffer pointer
 *
 * Return the woke TSB configuration as previous defined by
 * mmu_tsb_ctxnon0() into the woke provided buffer.  The size of the woke buffer
 * is given in ARG1 in terms of the woke number of TSB description entries.
 *
 * Upon return, RET1 always contains the woke number of TSB descriptions
 * previously configured.  If zero TSBs were configured, EOK is
 * returned with RET1 containing 0.
 ...
 * ERRORS:	No errors defined.
 *
 * Return the woke currently defined MMU fault status area for the woke current
 * CPU.  The real address of the woke fault status area is returned in
 * RET1, or 0 is returned in RET1 if no fault status area is defined.
 *
 * Note: mmu_fault_area_conf() may be called with the woke return value (RET1)
 *       from this service if there is a need to save and restore the woke fault
 *	 area for a cpu.
 */
 ...
 *
 * Zero the woke memory contents in the woke range real address to real address
 * plus length minus 1.  Also, valid ECC will be generated for that
 * memory address range.  Scrubbing is started at the woke given real
 * address, but may not scrub the woke entire given length.  The actual
 * length scrubbed will be returned in RET1.
 *
 * The real address and length must be aligned on an 8K boundary, or
 * contain the woke start address and length from a sun4v error report.
 *
 * Note: There are two uses for this function.  The first use is to block clear
 *       and initialize memory and the woke second is to scrub an u ncorrectable
 *       error reported via a resumable or non-resumable trap.  The second
 *       use requires the woke arguments to be equal to the woke real address and length
 ...
 *		EINVAL		Length is zero
 *
 * Force the woke next access within the woke real address to real address plus
 * length minus 1 to be fetches from main system memory.  Less than
 * the woke given length may be synced, the woke actual amount synced is
 ...
 *		than arg1)
 * RET2:	status data
 *		if status == ENOMAP or ENOACCESS, identifies the woke VA in question
 ...
 *
 * The hypervisor maintains the woke time of day on a per-domain basis.
 * Changing the woke time of day in one domain does not affect the woke time of
 * day on any other domain.
 *
 * Time is described by a single unsigned 64-bit word which is the
 * number of seconds since the woke UNIX Epoch (00:00:00 UTC, January 1,
 * 1970).
 */
 ...
 *
 * Return the woke current time of day.  May block if TOD access is
 ...
 *		ENOTSUPPORTED	If TOD not supported on this platform
 *
 * The current time of day is set to the woke value specified in ARG0.  May
 ...
 *
 * Returns a character from the woke console device.  If no character is
 * available then an EWOULDBLOCK error is returned.  If a character is
 * available, then the woke returned status is EOK and the woke character value
 * is in RET1.
 *
 * A virtual BREAK is represented by the woke 64-bit value -1.
 *
 * A virtual HUP signal is represented by the woke 64-bit value -2.
 */
 ...
 *
 * Send a character to the woke console device.  Only character values
 * between 0 and 255 may be used.  Values outside this range are
 * invalid except for the woke 64-bit value -1 which is used to send a
 * virtual BREAK.
 */
 ...
 * ERRORS:	EWOULDBLOCK	No character available.
 *
 * Reads characters into a buffer from the woke console device.  If no
 * character is available then an EWOULDBLOCK error is returned.
 * If a character is available, then the woke returned status is EOK
 * and the woke number of bytes read into the woke given buffer is provided
 * in RET1.
 *
 * A virtual BREAK is represented by the woke 64-bit RET1 value -1.
 *
 * A virtual HUP signal is represented by the woke 64-bit RET1 value -2.
 *
 ...
 *
 * Send a characters in buffer to the woke console device.  Breaks must be
 ...
 *				aligned
 *
 * This allows the woke guest to report its soft state to the woke hypervisor.  There
 * are two primary components to this state.  The first part states whether
 * the woke guest software is running or not.  The second containts optional
 * details specific to the woke software.
 *
 * The software state argument is defined below in HV_SOFT_STATE_*, and
 * indicates whether the woke guest is operating normally or in a transitional
 * state.
 *
 ...
 *				aligned
 *
 * Retrieve the woke current value of the woke guest's software state.  The rules
 * for the woke software state pointer are the woke same as for mach_set_soft_state()
 * above.
 */
 ...
 * round-robin trap trace queue within which the woke hypervisor writes
 ...
 * start (offset 0) of the woke trap trace buffer, and is described as
 * follows:
 */
#ifndef __ASSEMBLY__
...
/* The head offset is the woke offset of the woke most recently completed entry
 * in the woke trap-trace buffer.  The tail offset is the woke offset of the
 * next entry to be written.  The control structure is owned and
 * modified by the woke hypervisor.  A guest may not modify the woke control
 * structure contents.  Attempts to do so will result in undefined
 * behavior for the woke guest.
 *
 ...
 * trace buffer to the woke hypervisor.  The real address supplies the woke real
 * base address of the woke trap trace queue and must be 64-byte aligned.
 * Specifying a value of 0 for the woke number of entries disables trap
 * tracing for the woke calling virtual CPU.  The buffer allocated must be
 ...
 *
 * If the woke real address is illegal or badly aligned, then trap tracing
 ...
 * ERRORS:	None defined.
 *
 * Returns the woke size and location of the woke previously declared trap-trace
 * buffer.  In the woke event that no buffer was previously defined, or the
 ...
 *
 * Enable or disable trap tracing, and return the woke previous enabled
 * state in RET1.  Future systems may define various flags for the
 * enable argument (ARG0), for the woke moment a guest should pass
 ...
 * ERRORS:	EINVAL		No trap trace buffer currently defined
 *
 * Freeze or unfreeze trap tracing, returning the woke previous freeze
 ...
 * ERRORS:	EINVAL		No trap trace buffer currently defined
 *
 * Add an entry to the woke trap trace buffer.  Upon return only ARG0/RET0
 * is modified - none of the woke other registers holding arguments are
 * volatile across this hypervisor service.
 */

/* Core dump services.
 *
 * Since the woke hypervisor viraulizes and thus obscures a lot of the
 * physical machine layout and state, traditional OS crash dumps can
 * be difficult to diagnose especially when the woke problem is a
 ...
 * a core dump, requests that the woke hypervisor update any information in
 * the woke dump buffer in preparation to being dumped as part of the
 ...
 *				domain
 *
 * Declare a domain dump buffer to the woke hypervisor.  The real address
 * provided for the woke domain dump buffer must be 64-byte aligned.  The
 * size specifies the woke size of the woke dump buffer and may be larger than
 * the woke minimum size specified in the woke machine description.  The
 * hypervisor will fill the woke dump buffer with opaque data.
 *
 ...
 *       dump_buf_update() atomically declares the woke new dump buffer to the
 *       hypervisor.
 *
 * A specified size of 0 unconfigures the woke dump buffer.  If the woke real
 * address is illegal or badly aligned, then any currently active dump
 * buffer is disabled and an error is returned.
 *
 * In the woke event that the woke call fails with EINVAL, RET1 contains the
 * minimum size requires by the woke hypervisor for a valid dump buffer.
 */
 ...
 * ERRORS:	No errors defined.
 *
 * Return the woke currently configures dump buffer description.  A
 * returned size of 0 bytes indicates an undefined dump buffer.  In
 * this case the woke return address in RET1 is undefined.
 */
#define HV_FAST_DUMP_BUF_INFO		0x95

/* Device interrupt services.
 *
 * Device interrupts are allocated to system bus bridges by the woke hypervisor,
 * and described to OBP in the woke machine description.  OBP then describes
 * these interrupts to the woke OS via properties in the woke device tree.
 *
 * Terminology:
 *
 ...
 *			consistes of the woke lower 28-bits of the woke hi-cell of the
 *			first entry of the woke device's "reg" property in the
 *			OBP device tree.
 *
 *	devino		Device interrupt number.  Specifies the woke relative
 *			interrupt number within the woke device.  The unique
 *			combination of devhandle and devino are used to
 *			identify a specific device interrupt.
 *
 *			Note: The devino value is the woke same as the woke values in the
 *			      "interrupts" property or "interrupt-map" property
 *			      in the woke OBP device tree for that device.
 *
 *	sysino		System interrupt number.  A 64-bit unsigned interger
 *			representing a unique interrupt within a virtual
 *			machine.
 *
 *	intr_state	A flag representing the woke interrupt state for a given
 *			sysino.  The state values are defined below.
 *
 *	intr_enabled	A flag representing the woke 'enabled' state for a given
 ...
 *
 * Converts a device specific interrupt number of the woke given
 ...
 *
 * Returns interrupt enabled state in RET1 for the woke interrupt defined
 * by the woke given sysino.
 */
 ...
 *
 * Set the woke 'enabled' state of the woke interrupt sysino.
 */
 ...
 * ERRORS:	EINVAL		Invalid sysino
 *
 * Returns current state of the woke interrupt defined by the woke given sysino.
 */
 ...
 *
 * Sets the woke current state of the woke interrupt described by the woke given sysino
 * value.
 *
 * Note: Setting the woke state to HV_INTR_STATE_IDLE clears any pending
 ...
 * ERRORS:	EINVAL		Invalid sysino
 *
 * Returns CPU that is the woke current target of the woke interrupt defined by
 * the woke given sysino.  The CPU value returned is undefined if the woke target
 ...
 *
 * Set the woke target CPU for the woke interrupt defined by the woke given sysino.
 */
...
/* PCI IO services.
 *
 * See the woke terminology descriptions in the woke device interrupt services
 ...
 *			For this version of the woke specification, tsbnum
 *			must be zero.
 *
 *	tsbindex	TSB index.  Identifies which entry in the woke TSB
 ...
 *			tsbnum and bits 31:00 contain the woke tsbindex.
 *
 *			Use the woke HV_PCI_TSBID() macro to construct such
 * 			values.
 *
 *	io_attributes	IO attributes for IOMMU mappings.  One of more
 *			of the woke attritbute bits are stores in a 64-bit
 ...
 *			with the woke following format:
 *
 *				00000000.bbbbbbbb.dddddfff.00000000
 *
 *			Use the woke HV_PCI_DEVICE_BUILD() macro to construct
 *			such values.
 *
 *	pci_config_offset
 ...
 *
 *			Note: For PCI configuration space accesses, the woke offset
 *			      must be aligned to the woke access size.
 *
 *	error_flag	A return value which specifies if the woke action succeeded
 *			or failed.  0 means no error, non-0 means some error
 *			occurred while performing the woke service.
 *
 *	io_sync_direction
 ...
 *				 counters in native form when the woke processor
 ...
 *
 * Create IOMMU mappings in the woke sun4v device defined by the woke given
 * devhandle.  The mappings are created in the woke TSB defined by the
 * tsbnum component of the woke given tsbid.  The first mapping is created
 * in the woke TSB i ndex defined by the woke tsbindex component of the woke given tsbid.
 * The call creates up to #ttes mappings, the woke first one at tsbnum, tsbindex,
 * the woke second at tsbnum, tsbindex + 1, etc.
 *
 * All mappings are created with the woke attributes defined by the woke io_attributes
 * argument.  The page mapping addresses are described in the woke io_page_list
 * defined by the woke given io_page_list_p, which is a pointer to the woke io_page_list.
 * The first entry in the woke io_page_list is the woke address for the woke first iotte, the
 * 2nd for the woke 2nd iotte, and so on.
 *
 * Each io_page_address in the woke io_page_list must be appropriately aligned.
 * #ttes must be greater than zero.  For this version of the woke spec, the woke tsbnum
 * component of the woke given tsbid must be zero.
 *
 * Returns the woke actual number of mappings creates, which may be less than
 * or equal to the woke argument #ttes.  If the woke function returns a value which
 * is less than the woke #ttes, the woke caller may continus to call the woke function with
 ...
 *
 * Demap and flush IOMMU mappings in the woke device defined by the woke given
 * devhandle.  Demaps up to #ttes entries in the woke TSB defined by the woke tsbnum
 * component of the woke given tsbid, starting at the woke TSB index defined by the
 * tsbindex component of the woke given tsbid.
 *
 * For this version of the woke spec, the woke tsbnum of the woke given tsbid must be zero.
 * #ttes must be greater than zero.
 *
 * Returns the woke actual number of ttes demapped, which may be less than or equal
 * to the woke argument #ttes.  If #ttes demapped is less than #ttes, the woke caller
 ...
 *       unmapped page will flush the woke entry from the woke tte cache.
 */
 ...
 *
 * Read and return the woke mapping in the woke device described by the woke given devhandle
 * and tsbid.  If successful, the woke io_attributes shall be returned in RET1
 * and the woke page address of the woke mapping shall be returned in RET2.
 *
 * For this version of the woke spec, the woke tsbnum component of the woke given tsbid
 * must be zero.
 */
 ...
 *
 * Create a "special" mapping in the woke device described by the woke given devhandle,
 * for the woke given real address and attributes.  Return the woke IO address in RET1
 * if successful.
 */
 ...
 *
 * Read PCI configuration space for the woke adapter described by the woke given
 * devhandle.  Read size (1, 2, or 4) bytes of data from the woke given
 * pci_device, at pci_config_offset from the woke beginning of the woke device's
 * configuration space.  If there was no error, RET1 is set to zero and
 * RET2 is set to the woke data read.  Insignificant bits in RET2 are not
 * guaranteed to have any specific value and therefore must be ignored.
 *
 * The data returned in RET2 is size based byte swapped.
 *
 * If an error occurs during the woke read, set RET1 to a non-zero value.  The
 ...
 *		ENOACCESS	Access to this offset is not permitted
 *
 * Write PCI configuration space for the woke adapter described by the woke given
 * devhandle.  Write size (1, 2, or 4) bytes of data in a single operation,
 * at pci_config_offset from the woke beginning of the woke device's configuration
 * space.  The data argument contains the woke data to be written to configuration
 * space.  Prior to writing, the woke data is size based byte swapped.
 *
 * If an error occurs during the woke write access, do not generate an error
 * report, do set RET1 to a non-zero value.  Otherwise RET1 is zero.
 * The given pci_config_offset must be 'size' aligned.
 *
 * This function is permitted to read from offset zero in the woke configuration
 * space described by the woke given pci_device if necessary to ensure that the
 ...
 *
 * Attempt to read the woke IO address given by the woke given devhandle, real address,
 * and size.  Size must be 1, 2, 4, or 8.  The read is performed as a single
 * access operation using the woke given size.  If an error occurs when reading
 * from the woke given location, do not generate an error report, but return a
 * non-zero value in RET1.  If the woke read was successful, return zero in RET1
 * and return the woke actual data read in RET2.  The data returned is size based
 * byte swapped.
 *
 * Non-significant bits in RET2 are not guaranteed to have any specific value
 * and therefore must be ignored.  If RET1 is returned as non-zero, the woke data
 * value is not guaranteed to have any specific value and should be ignored.
 *
 * The caller must have permission to read from the woke given devhandle, real
 ...
 * IO address that the woke guest does not have explicit permission to access.
 */
 ...
 *
 * Attempt to write data to the woke IO address given by the woke given devhandle,
 * real address, and size.  Size must be 1, 2, 4, or 8.  The write is
 * performed as a single access operation using the woke given size. Prior to
 * writing the woke data is size based swapped.
 *
 * If an error occurs when writing to the woke given location, do not generate an
 * error report, but return a non-zero value in RET1.  If the woke write was
 * successful, return zero in RET1.
 *
 * pci_device describes the woke configuration address of the woke device being
 * written to.  The implementation may safely read from offset 0 with
 * the woke configuration space of the woke device described by devhandle and
 * pci_device in order to guarantee that the woke write portion of the woke operation
 * completes
 *
 * Any error that occurs due to the woke read shall be reported using the woke normal
 * error reporting mechanisms .. the woke read error is not suppressed.
 *
 * The caller must have permission to write to the woke given devhandle, real
 * address, which must be an IO address.  The argument real address must be a
 * size aligned address.  The caller must have permission to read from
 * the woke given devhandle, pci_device cofiguration space offset 0.
 *
 * The hypervisor implementation of this function must block access to any
 * IO address that the woke guest does not have explicit permission to access.
 */
 ...
 *		ENORADDR	Bad real address
 *
 * Synchronize a memory region described by the woke given real address and size,
 * for the woke device defined by the woke given devhandle using the woke direction(s)
 * defined by the woke given io_sync_direction.  The argument size is the woke size of
 * the woke memory region in bytes.
 *
 * Return the woke actual number of bytes synchronized in the woke return value #synced,
 * which may be less than or equal to the woke argument size.  If the woke return
 * value #synced is less than size, the woke caller must continue to call this
 * function with updated real address and size arguments until the woke entire
 ...
 *		ENORADDR	Bad real address
 *
 * Configure the woke MSI queue given by the woke devhandle and msiqid arguments,
 * and to be placed at the woke given real address and be of the woke given
 * number of entries.  The real address must be aligned exactly to match
 * the woke queue size.  Each queue entry is 64-bytes long, so f.e. a 32 entry
 * queue must be aligned on a 2048 byte real address boundary.  The MSI-EQ
 * Head and Tail are initialized so that the woke MSI-EQ is 'empty'.
 *
 * Implementation Note: Certain implementations have fixed sized queues.  In
 *                      that case, number of entries must contain the woke correct
 *                      value.
 */
 ...
 *
 * Return the woke configuration information for the woke MSI queue described
 * by the woke given devhandle and msiqid.  The base address of the woke queue
 * is returned in ARG1 and the woke number of entries is returned in ARG2.
 * If the woke queue is unconfigured, the woke real address is undefined and the
 ...
 * ERRORS:	EINVAL		Invalid devhandle or msiqid
 *
 * Get the woke valid state of the woke MSI-EQ described by the woke given devhandle and
 * msiqid.
 */
 ...
 *
 * Set the woke valid state of the woke MSI-EQ described by the woke given devhandle and
 * msiqid to the woke given msiqvalid.
 */
 ...
 * ERRORS:	EINVAL		Invalid devhandle or msiqid
 *
 * Get the woke state of the woke MSI-EQ described by the woke given devhandle and
 * msiqid.
 */
 ...
 *				value or MSI EQ is uninitialized
 *
 * Set the woke state of the woke MSI-EQ described by the woke given devhandle and
 * msiqid to the woke given msiqvalid.
 */
 ...
 * ERRORS:	EINVAL		Invalid devhandle or msiqid
 *
 * Get the woke current MSI EQ queue head for the woke MSI-EQ described by the
 ...
 *
 * Set the woke current MSI EQ queue head for the woke MSI-EQ described by the
 * given devhandle and msiqid.
 */
 ...
 * ERRORS:	EINVAL		Invalid devhandle or msiqid
 *
 * Get the woke current MSI EQ queue tail for the woke MSI-EQ described by the
 * given devhandle and msiqid.
 */
 ...
 *
 * Get the woke current valid/enabled state for the woke MSI defined by the
 ...
 *
 * Set the woke current valid/enabled state for the woke MSI defined by the
 * given devhandle and msinum.
 */
 ...
 *
 * Get the woke MSI EQ that the woke MSI defined by the woke given devhandle and
 * msinum is bound to.
 */
 ...
 *
 * Set the woke MSI EQ that the woke MSI defined by the woke given devhandle and
 * msinum is bound to.
 */
 ...
 * ERRORS:	EINVAL		Invalid devhandle or msinum
 *
 * Get the woke state of the woke MSI defined by the woke given devhandle and msinum.
 ...
 *
 * Set the woke state of the woke MSI defined by the woke given devhandle and msinum.
 */
 ...
 *
 * Get the woke MSI EQ of the woke MSG defined by the woke given devhandle and msgtype.
 */
 ...
 *
 * Set the woke MSI EQ of the woke MSG defined by the woke given devhandle and msgtype.
 */
 ...
 * ERRORS:	EINVAL		Invalid devhandle or msgtype
 *
 * Get the woke valid/enabled state of the woke MSG defined by the woke given
 ...
 *
 * Set the woke valid/enabled state of the woke MSG defined by the woke given
 * devhandle and msgtype.
 */
 ...
 *
 * While the woke PCI IO definitions above is valid IOMMU v2 adds new PCI IO
 * definitions and services.
 *
 *	CTE		Clump Table Entry. First level table entry in the woke ATU.
 *
 *	pci_device_list
 ...
 *			to the woke pagesize of the woke given IOTSB.
 *
 ...
 *			a pagesize and table size supported by the woke un-derlying
 *			hardware implementation. The alignment requirements
 *			for an IOTSB depend on the woke pagesize used for that IOTSB.
 *			Each IOTTE in an IOTSB maps one pagesize-sized page.
 *			The size of the woke IOTSB dictates how large of a virtual
 *			address space the woke IOTSB is capable of mapping.
 *
 ...
 *
 * Create an IOTSB suitable for the woke PCI root complex identified by devhandle,
 * for the woke DMA virtual address defined by the woke argument iova.
 *
 * r_addr is the woke properly aligned base address of the woke IOTSB and size is the
 ...
 *
 * pagesize is the woke size of each page in the woke IOTSB. Note that the woke combination of
 * size (table size) and pagesize must be valid.
 *
 * virt is the woke DMA virtual address this IOTSB will map.
 *
 * If successful, the woke opaque 64-bit handle iotsb_handle is returned in ret1.
 * Once configured, privileged access to the woke IOTSB memory is prohibited and
 ...
 * legacy IOTSB that may or may not exist. If the woke service succeeds, the woke return
 * values describe the woke legacy IOTSB and I/O virtual addresses mapped by that
 * table. However, the woke table base address r_addr may contain the woke value -1 which
 * indicates a memory range that cannot be accessed or be reclaimed.
 *
 * The return value #bound contains the woke number of PCI devices that iotsb_handle
 ...
 *
 * This service unconfigures the woke IOTSB identified by the woke devhandle and
 * iotsb_handle arguments, previously created with pci_iotsb_conf.
 * The IOTSB must not be currently bound to any device or the woke service will fail
 *
 * If the woke call succeeds, iotsb_handle is no longer valid.
 */
 ...
 *		EBUSY	A PCI function is already bound to an IOTSB at the woke same
 *			address range as specified by devhandle, iotsb_handle.
 *
 * This service binds the woke PCI function specified by the woke argument pci_device to
 * the woke IOTSB specified by the woke arguments devhandle and iotsb_handle.
 *
 * The PCI device function is bound to the woke specified IOTSB with the woke IOVA range
 * specified when the woke IOTSB was configured via pci_iotsb_conf. If the woke function
 ...
 * RET0:	status
 * ERRORS:	EINVAL	Invalid devhandle, iotsb_handle, or pci_device
 *		ENOMAP	The PCI function was not bound to the woke specified IOTSB
 *
 * This service unbinds the woke PCI device specified by the woke argument pci_device
 * from the woke IOTSB identified  * by the woke arguments devhandle and iotsb_handle.
 *
 * If the woke PCI device is not bound to the woke specified IOTSB then this service will
 ...
 *
 * This service returns the woke IOTSB binding, iotsb_handle, for a given pci_device
 * and DMA virtual address, iova.
 *
 * iova must be the woke base address of a DMA virtual address range as defined by
 * the woke iommu-address-ranges property in the woke root complex device node defined
 * by the woke argument devhandle.
 */
 ...
 *				address in the woke I/O page list.
 *		ENORADDR	Invalid io_page_list_p or I/O page address in
 *				the I/O page list.
 *
 * This service creates and flushes mappings in the woke IOTSB defined by the
 ...
 *
 * The first mapping is created in the woke IOTSB index specified by iotsb_index.
 * Subsequent mappings are  created at iotsb_index+1 and so on.
 *
 * The attributes of each mapping are defined by the woke argument iotte_attributes.
 *
 * The io_page_list_p specifies the woke real address of the woke 64-bit-aligned list of
 * #iottes I/O page addresses. Each page address must be a properly aligned
 * real address of a page to be mapped in the woke IOTSB. The first entry in the woke I/O
 * page list contains the woke real address of the woke first page, the woke 2nd entry for the
 * 2nd page, and so on.
 *
 * #iottes must be greater than zero.
 *
 * The return value #mapped is the woke actual number of mappings created, which may
 * be less than or equal to the woke argument #iottes. If the woke function returns
 * successfully with a #mapped value less than the woke requested #iottes then the
 * caller should continue to invoke the woke service with updated iotsb_index,
 ...
 * mappings must be valid and have  one or both of the woke RW attribute bits set.
 *
 * Note:
 ...
 *
 * This service creates and flushes a single mapping in the woke IOTSB defined by the
 * arguments devhandle, iotsb.
 *
 * The mapping for the woke page at r_addr is created at the woke IOTSB index specified by
 * iotsb_index with  the woke attributes iotte_attributes.
 *
 * This service must not be used to demap a mapping. In other words, the woke mapping
 * must be valid and have one or both of the woke RW attribute bits set.
 *
 * Note:
 * It is implementation-defined whether I/O page real address validity checking
 * is done at time mappings are established or deferred until they are
 ...
 * iotsb_index from the woke IOTSB defined by the woke arguments devhandle, iotsb.
 *
 * #iottes must be greater than zero.
 *
 * The actual number of IOTTEs unmapped is returned in #unmapped and may be less
 * than or equal to the woke requested number of IOTTEs, #iottes.
 *
 * If #unmapped is less than #iottes, the woke caller should continue to invoke this
 ...
 *
 * This service returns the woke mapping specified by index iotsb_index from the
 * IOTSB defined by the woke arguments devhandle, iotsb.
 *
 * Upon success, the woke real address of the woke mapping shall be returned in
 ...
 * the woke call to create the woke  mapping.
 */
 ...
 * the woke IOTSB defined by the woke arguments devhandle, iotsb.
 *
 * #iottes must be greater than zero.
 *
 * The actual number of IOTTEs synchronized is returned in #synced, which may
 * be less than or equal to the woke requested number, #iottes.
 *
 * Upon a successful return, #synced is less than #iottes, the woke caller should
 ...
 * RET0:	status
 *
 * Configure transmit queue for the woke LDC endpoint specified by the
 * given channel ID, to be placed at the woke given real address, and
 * be of the woke given num entries.  Num entries must be a power of two.
 * The real address base of the woke queue must be aligned on the woke queue
 * size.  Each queue entry is 64-bytes, so for example, a 32 entry
 * queue must be aligned on a 2048 byte real address boundary.
 *
 * Upon configuration of a valid transmit queue the woke head and tail
 * pointers are set to a hypervisor specific identical value indicating
 * that the woke queue initially is empty.
 *
 ...
 * determined from the woke machine description.  A transmit queue may be
 * specified even in the woke event that the woke LDC is down (peer endpoint has no
 * receive queue specified).  Transmission will begin as soon as the woke peer
 ...
 * is undefined as to how many of the woke pending entries in the woke original queue
 * will be delivered prior to the woke re-configuration taking effect.
 * Furthermore, as the woke queue configuration causes a reset of the woke head and
 * tail pointers there is no way for a guest to determine how many entries
 * have been sent after the woke configuration operation.
 */
 ...
 *
 * Return the woke configuration info for the woke transmit queue of LDC endpoint
 * defined by the woke given channel ID.  The real address is the woke currently
 * defined real address base of the woke defined queue, and num entries is the
 * size of the woke queue in terms of number of entries.
 *
 * If the woke specified channel ID is a valid endpoint number, but no transmit
 * queue has been defined this service will return success, but with num
 * entries set to zero and the woke real address will have an undefined value.
 */
 ...
 * RET3:	channel state
 *
 * Return the woke transmit state, and the woke head and tail queue pointers, for
 * the woke transmit queue of the woke LDC endpoint defined by the woke given channel ID.
 * The head and tail values are the woke byte offset of the woke head and tail
 * positions of the woke transmit queue for the woke specified endpoint.
 */
 ...
 * RET0:	status
 *
 * Update the woke tail pointer for the woke transmit queue associated with the woke LDC
 * endpoint defined by the woke given channel ID.  The tail offset specified
 * must be aligned on a 64 byte boundary, and calculated so as to increase
 * the woke number of pending entries on the woke transmit queue.  Any attempt to
 * decrease the woke number of pending transmit queue entires is considered
 * an invalid tail offset and will result in an EINVAL error.
 *
 * Since the woke tail of the woke transmit queue may not be moved backwards, the
 * transmit queue may be flushed by configuring a new transmit queue,
 * whereupon the woke hypervisor will configure the woke initial transmit head and
 ...
 * RET0:	status
 *
 * Configure receive queue for the woke LDC endpoint specified by the
 * given channel ID, to be placed at the woke given real address, and
 * be of the woke given num entries.  Num entries must be a power of two.
 * The real address base of the woke queue must be aligned on the woke queue
 * size.  Each queue entry is 64-bytes, so for example, a 32 entry
 ...
 * The endpoint's transmit queue is un-configured if num entries is zero.
 *
 * If a valid receive queue is specified for a local endpoint the woke LDC is
 * in the woke up state for the woke purpose of transmission to this endpoint.
 *
 * The maximum number of entries for each queue for a specific cpu may be
 * determined from the woke machine description.
 *
 * As receive queue configuration causes a reset of the woke queue's head and
 * tail pointers there is no way for a gues to determine how many entries
 * have been received between a preceding ldc_get_rx_state() API call
 * and the woke completion of the woke configuration operation.  It should be noted
 ...
 * on the woke peer endpoint's transmit queue prior to the woke configuration process.
 */
 ...
 * RET2:	num entries in queue
 *
 * Return the woke configuration info for the woke receive queue of LDC endpoint
 * defined by the woke given channel ID.  The real address is the woke currently
 * defined real address base of the woke defined queue, and num entries is the
 * size of the woke queue in terms of number of entries.
 *
 * If the woke specified channel ID is a valid endpoint number, but no receive
 * queue has been defined this service will return success, but with num
 * entries set to zero and the woke real address will have an undefined value.
 */
 ...
 * RET3:	channel state
 *
 * Return the woke receive state, and the woke head and tail queue pointers, for
 * the woke receive queue of the woke LDC endpoint defined by the woke given channel ID.
 * The head and tail values are the woke byte offset of the woke head and tail
 * positions of the woke receive queue for the woke specified endpoint.
 */
 ...
 * RET0:	status
 *
 * Update the woke head pointer for the woke receive queue associated with the woke LDC
 * endpoint defined by the woke given channel ID.  The head offset specified
 * must be aligned on a 64 byte boundary, and calculated so as to decrease
 * the woke number of pending entries on the woke receive queue.  Any attempt to
 * increase the woke number of pending receive queue entires is considered
 * an invalid head offset and will result in an EINVAL error.
 *
 * The receive queue may be flushed by setting the woke head offset equal
 * to the woke current tail offset.
 */
#define HV_FAST_LDC_RX_SET_QHEAD	0xe7

/* LDC Map Table Entry.  Each slot is defined by a translation table
 * entry, as specified by the woke LDC_MTE_* bits below, and a 64-bit
 ...
 * RET0:	status
 *
 * Register the woke MTE table at the woke given table real address, with the
 * specified num entries, for the woke LDC indicated by the woke given channel
 * ID.
 */
 ...
 * RET2:	num entries
 *
 * Return the woke configuration of the woke current mapping table registered
 * for the woke given channel ID.
 */
 ...
 *
 * Read the woke value of the woke given DRAM/JBUS performance counter/control register.
 */
 ...
 *		ENOACCESS	No access allowed to performance counters
 *
 * Write the woke given performance reg value to the woke given DRAM/JBUS
 ...
 *
 * Enable MMU statistic gathering using the woke buffer at the woke given real
 * address on the woke current virtual CPU.  The new buffer real address
 * is given in ARG1, and the woke previously specified buffer real address
 * is returned in RET1, or is returned as zero for the woke first invocation.
 *
 * If the woke passed in real address argument is zero, this will disable
 * MMU statistic collection on the woke current virtual CPU.  If an error is
 * returned then no statistics are collected.
 *
 * The buffer contents should be initialized to all zeros before being
 * given to the woke hypervisor or else the woke statistics will be meaningless.
 */
 ...
 *
 * Return the woke current state and real address of the woke currently configured
 * MMU statistics buffer on the woke current virtual CPU.
 */
 ...
 * RET0:	status
 *
 * The MAU chip of the woke Niagara processor is not directly accessible
 ...
 * Privileged code registers a queue with the woke hypervisor by invoking
 * this HVAPI with the woke HV_NCS_QCONF sub-function, which defines the
 * base, end, and number of entries of the woke queue.  Each queue entry
 * contains a MAU register struct block.
 *
 * The privileged code then proceeds to add entries to the woke queue and
 * then invoke the woke HV_NCS_QTAIL_UPDATE sub-function.  Since only
 * synchronous operations are supported by the woke current hypervisor,
 * HV_NCS_QTAIL_UPDATE will run all the woke pending queue entries to
 * completion and return HV_EOK, or return an error code.
 *
 * The real address of the woke sub-function argument must be aligned on at
 * least an 8-byte boundary.
 *
 * The tail argument of HV_NCS_QTAIL_UPDATE is an index, not a byte
 * offset, into the woke queue and must be less than or equal the woke 'num_ents'
 * argument given in the woke HV_NCS_QCONF call.
 */
