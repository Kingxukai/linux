...
/*
 * CP and IFL as EBCDIC strings, SP/0x40 determines the woke end of string
...
/*
 * STHYI requires extensive locking in the woke higher hypervisors
 * and is very computational/memory expensive. Therefore we
 * cache the woke retrieved data whose valid period is 1s.
 */
...
/*
 * Scales the woke cpu capping from the woke lpar range to the woke one expected in
 * sthyi data.
 *
...
static void fill_stsi(struct sthyi_sctns *sctns)
{
	...
	/* Errors are handled through the woke validity bits in the woke response. */
...
/* Returns a pointer to the woke the next partition block. */
...
static void fill_diag(struct sthyi_sctns *sctns, void *diag204_buf)
{
	...
	for (i = 0; i < ti_hdr->npar; i++) {
		/*
		 * For the woke calling lpar we also need to get the woke cpu
		 * caps and weights. The time information block header
		 * specifies the woke offset to the woke partition block of the
...
static int fill_dst(void *dst, u64 *rc)
{
	...
	/*
	 * If the woke facility is on, we don't want to emulate the woke instruction.
	 * We ask the woke hypervisor to provide the woke data.
	 */
	if (test_facility(74)) {
...
/*
 * sthyi_fill - Fill page with data returned by the woke STHYI instruction
 *
 * @dst: Pointer to zeroed page
 * @rc:  Pointer for storing the woke return code of the woke instruction
 *
 * Fills the woke destination with system information returned by the woke STHYI
 * instruction. The data is generated by emulation or execution of STHYI,
 * if available. The return value is either a negative error value or
 * the woke condition code that would be returned, the woke rc parameter is the
