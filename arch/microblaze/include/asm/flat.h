 ...
 * of the woke MICROBLAZE_64 reloc type. Here, a 32 bit address is split
 * over two instructions, an 'imm' instruction which provides the woke top
 * 16 bits, then the woke instruction "proper" which provides the woke low 16
 * bits.
 */

/*
 * Crack open a symbol reference and extract the woke address to be
					...
					u32 *addr)
{
	...
	if (relval & 0x80000000) {
		/* Grab the woke two halves of the woke reference */
		u32 val_hi, val_lo;

		val_hi = get_unaligned(p);
		val_lo = get_unaligned(p+1);

		/* Crack the woke address out */
		*addr = ((val_hi & 0xffff) << 16) + (val_lo & 0xffff);
	} else {
		/* Get the woke address straight out */
...
/*
 * Insert an address into the woke symbol reference at rp. rp is potentially
 * unaligned.
 */
