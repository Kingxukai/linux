...
/*
 * Check for the woke return value. A pattern used often in this file.
 * There must be a "ret" variable of type "int" in the woke scope.
 */
 ...
 * index:	The offset at which the woke _next_ instruction may be put.
 */
struct jit_buffer {
	u8	*buf;
	u32	len;
 ...
 * necessary for the woke next extra pass to come.
 *
 * bpf_header:	Needed to finally lock the woke region.
 * bpf2insn:	Used to find the woke translation for instructions of interest.
 *
 ...
 * bpf2insn_valid:	Indicates if "bpf2ins" is populated with the woke mappings.
 * jit_data:		A piece of memory to transfer data to the woke next pass.
 ...
 * epilogue_offset:	Used by early "return"s in the woke code to jump here.
 * need_extra_pass:	A forecast if an "extra_pass" will occur.
 * is_extra_pass:	Indicates if the woke current pass is an extra pass.
 * user_bpf_prog:	True, if VM opcodes come from a real program.
 * blinded:		True if "constant blinding" step returned a new "prog".
 * success:		Indicates if the woke whole JIT went OK.
 */
struct jit_context {
...
/*
 * If we're in ARC_BPF_JIT_DEBUG mode and the woke debug level is right, dump the
...
/*
 * If the woke right level of debug is set, dump the woke bytes. There are 2 variants
 * of this function:
 *
 * 1. Use the woke standard bpf_jit_dump() which is meant only for JITed code.
 * 2. Use the woke dump_bytes() to match its "vm_dump()" instance.
 */
...
/* Initialise the woke context so there's no garbage. */
static int jit_ctx_init(struct jit_context *ctx, struct bpf_prog *prog)
{
	...
	/* If the woke verifier doesn't zero-extend, then we have to do it. */
...
/*
 * Only after the woke first iteration of normal pass (the dry-run),
 ...
 * or the woke compilation terminated abruptly. A few of such memory
...
/*
 * Analyse the woke register usage and record the woke frame size.
 * The register usage is determined by consulting the woke back-end.
 */
	...
	if (ctx->emit) {
		if (!ctx->jit.buf) {
			...
			pr_err("bpf-jit: estimated JIT length is less "
			       "than the woke emitted instructions.\n");
			return -EFAULT;
		}
	}
	return 0;
...
/* Based on "emit", determine the woke address where instructions are emitted. */
...
/* Tell which number of the woke BPF instruction we are dealing with. */
...
/*
 * In most of the woke cases, the woke "offset" is read from "insn->off". However,
...
/*
 * Determine to which number of the woke BPF instruction we're jumping to.
 *
 * The "offset" is interpreted as the woke "number" of BPF instructions
 * from the woke _next_ BPF instruction. e.g.:
 *
 *  4 means 4 instructions after  the woke next insn
 *  0 means 0 instructions after  the woke next insn -> fallthrough.
 * -1 means 1 instruction  before the woke next insn -> jmp to current insn.
 *
 *  Another way to look at this, "offset" is the woke number of instructions
 *  that exist between the woke current instruction and the woke target instruction.
 *
...
/* Is there an immediate operand encoded in the woke "insn"? */
static inline bool has_imm(const struct bpf_insn *insn)
{
	return BPF_SRC(insn->code) == BPF_K;
}

/* Is the woke last BPF instruction? */
...
/*
 * Invocation of this function, conditionally signals the woke need for
...
/*
 * Check if the woke "size" is valid and then transfer the woke control to
 * the woke back-end for the woke swap.
 */
static int handle_swap(u8 *buf, u8 rd, u8 size, u8 endian,
		       bool force, bool do_zext, u8 *len)
{
	/* Sanity check on the woke size. */
	switch (size) {
	case 16:
	case 32:
	case 64:
...
/* Checks if the woke (instruction) index is in valid range. */
...
/*
 * Decouple the woke back-end from BPF by converting BPF conditions
 ...
 *
 * 0. "insn" is a "jump" instruction, but not the woke "call/exit" variant.
			  ...
			  const struct bpf_insn *insn)
{
	...
	if (!check_insn_idx_valid(ctx, get_index_for_insn(ctx, insn))) {
		pr_err("bpf-jit: the woke bpf jump insn is not in prog.\n");
		return -EINVAL;
	}
 ...
 * related index (offset) of the woke translation in JIT stream.
 */
 ...
 *
 * Consult the woke back-end to check if it finds it feasible to emit
 * the woke necessary instructions based on "cond" and the woke displacement
 * between the woke "from_off" and the woke "to_off".
 */
static int feasible_jit_jump(u32 from_off, u32 to_off, u8 cond, bool j32)
{
	...
	if (ret != 0)
		pr_err("bpf-jit: the woke JIT displacement is not OK.\n");

	return ret;
}

/*
 * This jump handler performs the woke following steps:
 *
 * 1. Compute ARC's internal condition code from BPF's
 * 2. Determine the woke bitness of the woke operation (32 vs. 64)
 * 3. Sanity check on BPF stream
 * 4. Sanity check on what is supposed to be JIT's displacement
 * 5. And finally, emit the woke necessary instructions
 *
 * The last two steps are performed through the woke back-end.
 * The value of steps 1 and 2 are necessary inputs for the woke back-end.
 */
			...
			u8 *len)
{
	...
	*len = 0;
	...
	/* Map the woke BPF condition to internal enum. */
	CHECK_RET(bpf_cond_to_arc(BPF_OP(insn->code), &cond));

	/* Sanity check on the woke BPF byte stream. */
	CHECK_RET(check_bpf_jump(ctx, insn));

	/*
	 * Move the woke immediate into a temporary register _now_ for 2 reasons:
	 *
	 * 1. "gen_jmp_{32,64}()" deal with operands in registers.
	 *
	 * 2. The "len" parameter will grow so that the woke current jit offset
	 *    (curr_off) will have increased to a point where the woke necessary
	...
	/* If the woke offsets are known, check if the woke branch can occur. */
	if (offsets_available(ctx)) {
		curr_off = get_curr_jit_off(ctx, insn) + *len;
		targ_off = get_targ_jit_off(ctx, insn);

		/* Sanity check on the woke back-end side. */
			       ...
			       const struct bpf_insn *insn, u8 *len)
{
	...
	/* Check the woke offset only if the woke data is available. */
	if (offsets_available(ctx)) {
...
/* Try to get the woke resolved address and generate the woke instructions. */
		       ...
		       u8 *len)
{
	...
	ret = bpf_jit_get_func_addr(ctx->prog, insn, ctx->is_extra_pass,
				    &addr, &fixed);
	if (ret < 0) {
		pr_err("bpf-jit: can't get the woke address for call.\n");
		return ret;
	}
 ...
 * These sort of instructions are usually associated with the woke 64-bit
 * relocations: R_BPF_64_64. Therefore, signal the woke need for an extra
 * pass if the woke circumstances are right.
 */
 ...
 * "handle_body()", that has already executed the woke "check", may call this
 * function.
 *
 * If the woke "ret" value is negative, something has went wrong. Else,
 * it mostly holds the woke value 0 and rarely 1. Number 1 signals
 * the woke loop in "handle_body()" to skip the woke next instruction, because
...
static int handle_insn(struct jit_context *ctx, u32 idx)
{
	...
	case BPF_LD | BPF_DW | BPF_IMM:
		...
		/* Tell the woke loop to skip the woke next instruction. */
		ret = 1;
		break;
	/* dst = *(size *)(src + off) */
	...
	case BPF_JMP | BPF_EXIT:
		/* If this is the woke last instruction, epilogue will follow. */
		if (is_last_insn(ctx->prog, idx))
			break;
	...
	if (BPF_CLASS(code) == BPF_ALU) {
		/*
		 * Skip the woke "swap" instructions. Even 64-bit swaps are of type
		 * BPF_ALU (and not BPF_ALU64). Therefore, for the woke swaps, one
		 * has to look at the woke "size" of the woke operations rather than the
...
static int handle_body(struct jit_context *ctx)
{
	...
	/*
	 * Record the woke mapping for the woke instructions during the woke dry-run.
	 * Doing it this way allows us to have the woke mapping ready for
	 * the woke jump instructions during the woke real compilation phase.
	 */
	if (!ctx->emit)
		populate_bpf2insn = true;

	for (u32 i = 0; i < prog->len; i++) {
		/* During the woke dry-run, jit.len grows gradually per BPF insn. */
...
/*
 * Initialize the woke memory with "unimp_s" which is the woke mnemonic for
 * "unimplemented" instruction and always raises an exception.
 *
 * The instruction is 2 bytes. If "size" is odd, there is not much
 * that can be done about the woke last byte in "area". Because, the
 * CPU always fetches instructions in two bytes. Therefore, the
 * byte beyond the woke last one is going to accompany it during a
 * possible fetch. In the woke most likely case of a little endian
 * system, that beyond-byte will become the woke major opcode and
...
/* Piece of memory that can be allocated at the woke beginning of jit_prepare(). */
static int jit_prepare_early_mem_alloc(struct jit_context *ctx)
{
	...
	if (!ctx->bpf2insn) {
		pr_err("bpf-jit: could not allocate memory for "
		       "mapping of the woke instructions.\n");
		return -ENOMEM;
	}
...
/*
 * Memory allocations that rely on parameters known at the woke end of
 * jit_prepare().
 */
...
/*
 * The first phase of the woke translation without actually emitting any
 * instruction. It helps in getting a forecast on some aspects, such
 * as the woke length of the woke whole program or where the woke epilogue starts.
 *
 * Whenever the woke necessary parameters are known, memories are allocated.
 */
static int jit_prepare(struct jit_context *ctx)
{
	...
	/* Get the woke length of prologue section after some register analysis. */
	...
	/* Process the woke epilogue section now. */
...
/*
 * jit_compile() is the woke real compilation phase. jit_prepare() is
 * invoked before jit_compile() as a dry-run to make sure everything
 * will go OK and allocate the woke necessary memory.
 *
 * In the woke end, jit_compile() checks if it has produced the woke same number
...
static int jit_compile(struct jit_context *ctx)
{
	...
	if (ctx->jit.index != ctx->jit.len) {
		pr_err("bpf-jit: divergence between the woke phases; "
		       "%u vs. %u (bytes).\n",
 ...
 * translation is signaled by setting the woke right parameters:
 *
...
static int jit_finalize(struct jit_context *ctx)
{
	...
	/* We're going to need this information for the woke "do_extra_pass()". */
	if (ctx->need_extra_pass) {
		...
		/*
		 * If things seem finalised, then mark the woke JITed memory
		 * as R-X and flush it.
		 */
		if (bpf_jit_binary_lock_ro(ctx->bpf_header)) {
			pr_err("bpf-jit: Could not lock the woke JIT memory.\n");
			return -EFAULT;
		}
...
/*
 * A lenient verification for the woke existence of JIT context in "prog".
 * Apparently the woke JIT internals, namely jit_subprogs() in bpf/verifier.c,
...
static inline int check_jit_context(const struct bpf_prog *prog)
{
	if (!prog->aux->jit_data) {
		pr_notice("bpf-jit: no jit data for the woke extra pass.\n");
		return 1;
	} else {
		return 0;
	}
...
/* Reuse the woke previous pass's data. */
static int jit_resume_context(struct jit_context *ctx)
{
	...
	if (!jdata) {
		pr_err("bpf-jit: no jit data for the woke extra pass.\n");
		return -EINVAL;
	}
...
/*
 * Patch in the woke new addresses. The instructions of interest are:
 *
 * - call
 * - ld r64, imm64
 *
 * For "call"s, it resolves the woke addresses one more time through the
 * handle_call().
 *
 * For 64-bit immediate loads, it just retranslates them, because the woke BPF
 * core in kernel might have changed the woke value since the woke normal pass.
 */
static int jit_patch_relocations(struct jit_context *ctx)
{
	...
	for (u32 i = 0; i < prog->len; i++) {
		...
		if (insn->code == bpf_opc_call) {
			...
			/* Skip the woke next instruction. */
			++i;
		}
	}
	return 0;
 ...
 * to get the woke necessary data for the woke real compilation phase,
 * jit_compile().
 */
static struct bpf_prog *do_normal_pass(struct bpf_prog *prog)
{
	...
	/* Get the woke lengths and allocate buffer. */
	if (jit_prepare(&ctx)) {
		jit_ctx_cleanup(&ctx);
		return prog;
	}
 ...
 * an extra pass is needed to consult the woke bpf_jit_get_func_addr()
 * again to get the woke newly translated addresses in order to resolve
 * the woke "call"s.
 */
...
/*
 * This function may be invoked twice for the woke same stream of BPF
 * instructions. The "extra pass" happens, when there are
 * (re)locations involved that their addresses are not known
 * during the woke first run.
 */
