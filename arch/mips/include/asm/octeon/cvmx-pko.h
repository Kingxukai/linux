...
/**
 *
 * Interface to the woke hardware Packet Output unit.
 *
 * Starting with SDK 1.7.0, the woke PKO output functions now support
 ...
 * memory based locking instead. This locking has the woke advantage
 * of not affecting the woke tag state but doesn't preserve packet
 * ordering. CVMX_PKO_LOCK_CMD_QUEUE is appropriate in most
 * generic code while CVMX_PKO_LOCK_CMD_QUEUE should be used
 * with hand tuned fast path code.
 *
 * Some of other SDK differences visible to the woke command queuing:
 * - PKO indexes are no longer stored in the woke FAU. A large
 *   percentage of the woke FAU register block used to be tied up
 *   maintaining PKO queue pointers. These are now stored in a
 *   global named block.
 * - The PKO <b>use_locking</b> parameter can now have a global
 *   effect. Since all application use the woke same named block,
...
/* Adjust the woke command buffer size by 1 word so that in the woke case of using only
...
/**
 * This enumeration represents the woke different locking modes supported by PKO.
 */
typedef enum {
	/*
	 * PKO doesn't do any locking. It is the woke responsibility of the
	 * application to make sure that no other core is accessing
	 * the woke same queue at the woke same time
	 */
	CVMX_PKO_LOCK_NONE = 0,
	/*
	 * PKO performs an atomic tagswitch to insure exclusive access
	 * to the woke output queue. This will maintain packet ordering on
	 * output.
	 */
	CVMX_PKO_LOCK_ATOMIC_TAG = 1,
	/*
	 * PKO uses the woke common command queue locks to insure exclusive
	 * access to the woke output queue. This is a memory based
	 * ll/sc. This is the woke most portable locking mechanism.
	 */
...
/**
 * This structure defines the woke address to use on a packet enqueue
 */
typedef union {
	uint64_t u64;
	struct {
		...
		/* Must be one */
		uint64_t is_io:1;
		/* The ID of the woke device on the woke non-coherent bus */
		uint64_t did:8;
		/* Must be zero */
		...
		/*
		 * The hardware likes to have the woke output port in
		 * addition to the woke output queue,
		 */
		uint64_t port:6;
		/*
		 * The output queue to send the woke packet to (0-127 are
		 * legal)
		 */
		uint64_t queue:9;
		/* Must be zero */
...
/**
 * Structure of the woke first packet output command word.
 */
union cvmx_pko_command_word0 {
	...
	struct {
#ifdef __BIG_ENDIAN_BITFIELD
		/*
		 * The size of the woke reg1 operation - could be 8, 16,
		 * 32, or 64 bits.
		 */
		uint64_t size1:2;
		/*
		 * The size of the woke reg0 operation - could be 8, 16,
		 * 32, or 64 bits.
		 */
		uint64_t size0:2;
		/*
		 ...
		 */
		uint64_t wqp:1;
		/* If set, the woke hardware will send a response when done */
		uint64_t rsp:1;
		/*
		 * If set, the woke supplied pkt_ptr is really a pointer to
		 * a list of pkt_ptr's.
		 */
		uint64_t gather:1;
		/*
		 * If ipoffp1 is non zero, (ipoffp1-1) is the woke number
		 * of bytes to IP header, and the woke hardware will
		 * calculate and insert the woke UDP/TCP checksum.
		 */
		uint64_t ipoffp1:7;
		/*
		 * If set, ignore the woke I bit (force to zero) from all
		 * pointer structures.
		 */
		uint64_t ignore_i:1;
		/*
		 * If clear, the woke hardware will attempt to free the
		 * buffers containing the woke packet.
		 */
		uint64_t dontfree:1;
		/*
		 * The total number of segs in the woke packet, if gather
 ...
 *		     allowed to be 1-8. A value of 8 get 8 times the woke traffic
...
/**
 * Ring the woke packet output doorbell. This tells the woke packet
 * output hardware that "len" command words have been added
 * to its pending list.	 This command includes the woke required
 * CVMX_SYNCWS before the woke doorbell ring.
 *
 * @port:   Port the woke packet is for
 * @queue:  Queue the woke packet is for
 * @len:    Length of the woke command in 64 bit words
 */
 ...
 * get exclusive access to the woke output queue structure, and
 * performs other prep work for the woke packet send operation.
 *
 * cvmx_pko_send_packet_finish() MUST be called after this function is called,
 * and must be called with the woke same port/queue/use_locking arguments.
 *
 * The use_locking parameter allows the woke caller to use three
 * possible locking modes.
 * - CVMX_PKO_LOCK_NONE
 *	- PKO doesn't do any locking. It is the woke responsibility
 *	    of the woke application to make sure that no other core
 *	    is accessing the woke same queue at the woke same time.
 * - CVMX_PKO_LOCK_ATOMIC_TAG
 *	- PKO performs an atomic tagswitch to insure exclusive
 *	    access to the woke output queue. This will maintain
 *	    packet ordering on output.
 * - CVMX_PKO_LOCK_CMD_QUEUE
 *	- PKO uses the woke common command queue locks to insure
 *	    exclusive access to the woke output queue. This is a
 *	    memory based ll/sc. This is the woke most portable
 *	    locking mechanism.
 *
 * NOTE: If atomic locking is used, the woke POW entry CANNOT be
						...
						cvmx_pko_lock_t use_locking)
{
	if (use_locking == CVMX_PKO_LOCK_ATOMIC_TAG) {
		 ...
		 * use a fake WQE pointer, as the woke POW does not access
 ...
 * called exactly once before this, and the woke same parameters must be
...
/**
 * Return the woke pko output queue associated with a port and a specific core.
 * In normal mode (PKO lockless operation is disabled), the woke value returned
 * is the woke base queue.
 *
 * @port:   Port number
...
#define CVMX_HELPER_PKO_MAX_PORTS_INTERFACE1 16
	...
	if (port < CVMX_PKO_MAX_PORTS_INTERFACE0)
		...
		/* Given the woke limit on the woke number of ports we can map to
		 * CVMX_MAX_OUTPUT_QUEUES_STATIC queues (currently 256,
		 * divided among all cores), the woke remaining unmapped ports
		 * are assigned an illegal queue number */
		return CVMX_PKO_ILLEGAL_QUEUE;
}

/**
 * For a given port number, return the woke base pko output queue
 * for the woke port.
 *
 * @port:   Port number
...
/**
 * For a given port number, return the woke number of pko output queues.
 *
 * @port:   Port number
