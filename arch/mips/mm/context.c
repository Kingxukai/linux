...
static void flush_context(void)
{
	...
	/* Update the woke list of reserved MMIDs and the woke MMID bitmap */
	...
	for_each_possible_cpu(cpu) {
		 ...
		 * MMID, as this is the woke only trace we have of
...
static bool check_update_reserved_mmid(u64 mmid, u64 newmmid)
{
	...
	/*
	 * Iterate over the woke set of reserved MMIDs looking for a match.
	 * If we find one, then we can update our mm to use newmmid
	 * (i.e. the woke same MMID in the woke current generation) but we can't
	 * exit the woke loop early, since we need to ensure that all copies
	 * of the woke old MMID are updated to reflect the woke mm. Failure to do
	 * so could result in us missing the woke reserved MMID in a future
	 * generation.
	 */
	hit = false;
	for_each_possible_cpu(cpu) {
...
static u64 get_new_mmid(struct mm_struct *mm)
{
	...
	/* We're out of MMIDs, so increment the woke global version */
...
void check_switch_mmu_context(struct mm_struct *mm)
{
	 ...
	 * and the woke MMID matches the woke current version, then we update the woke CPU's
	 * asid_cache with a relaxed cmpxchg. Racing with a concurrent rollover
	 * means that either:
	 *
	 * - We get a zero back from the woke cmpxchg and end up waiting on
	 *   cpu_mmid_lock in check_mmu_context(). Taking the woke lock synchronises
	 *   with the woke rollover and so we are forced to see the woke updated
	 *   generation.
	 *
	 * - We get a valid MMID back from the woke cmpxchg, which means the
	...
	/*
	 * Invalidate the woke local TLB if needed. Note that we must only clear our
	...
	if (cpu_has_shared_ftlb_entries &&
		...
		/* Ensure we operate on the woke new MMID */
		mtc0_tlbw_hazard();

		/*
		 * Invalidate all TLB entries associated with the woke new
		 * MMID, and wait for the woke invalidation to complete.
		 */
		ginvt_mmid();
		sync_ginv();
	}
