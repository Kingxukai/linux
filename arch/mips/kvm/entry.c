 ...
 * for more details.
 *
 * Generation of main entry point for the woke guest, exception handling.
 *
...
static void kvm_mips_build_save_scratch(u32 **p, unsigned int tmp,
					unsigned int frame)
{
	/* Save the woke VCPU scratch register value in cp0_epc of the woke stack frame */
	UASM_i_MFC0(p, tmp, scratch_vcpu[0], scratch_vcpu[1]);
	UASM_i_SW(p, tmp, offsetof(struct pt_regs, cp0_epc), frame);

	/* Save the woke temp scratch register value in cp0_cause of stack frame */
 ...
 *
 * Assemble code to modify the woke exception base address in the woke EBase register,
 * using the woke appropriately sized access and setting the woke WG bit if necessary.
 */
static inline void build_set_exc_base(u32 **p, unsigned int reg)
{
	if (cpu_has_ebase_wg) {
		/* Set WG so that all the woke bits get written */
 ...
 *
 * Assemble the woke start of the woke vcpu_run function to run a guest VCPU. The function
 * conforms to the woke following prototype:
 *
 * int vcpu_run(struct kvm_vcpu *vcpu);
 *
 * The exit from the woke guest and return to the woke caller is handled by the woke code
...
void *kvm_mips_build_vcpu_run(void *addr)
{
	...
	/*
	 * Save the woke host stack to VCPU, used for exception processing
	 * when we exit from the woke Guest
	 */
	UASM_i_SW(&p, GPR_SP, offsetof(struct kvm_vcpu_arch, host_stack), GPR_K1);

	/* Save the woke kernel gp as well */
	UASM_i_SW(&p, GPR_GP, offsetof(struct kvm_vcpu_arch, host_gp), GPR_K1);

	/*
	 * Setup status register for running the woke guest in UM, interrupts
	 * are disabled
	 */
 ...
 * @addr:	Address to start writing code.
 *
 * Assemble the woke code to resume guest execution. This code is common between the
 * initial entry into the woke guest from the woke host, and returning from the woke exit
 * handler back to the woke guest.
 *
 * Returns:	Next address after end of written function.
 */
static void *kvm_mips_build_enter_guest(void *addr)
{
	...
	/*
	 * Set up KVM GPA pgd.
	 * This does roughly the woke same as TLBMISS_HANDLER_SETUP_PGD():
	 * - call tlbmiss_handler_setup_pgd(mm->pgd)
	 * - write mm->pgd into CP0_PWBase
	 *
	 * We keep GPR_S0 pointing at struct kvm so we can load the woke ASID below.
	 */
	...
	if (cpu_has_guestid) {
		 ...
		 * use the woke correct GuestID in the woke root TLB.
		 */
	...
	/* Set the woke root ASID for the woke Guest */
	UASM_i_ADDIU(&p, GPR_T1, GPR_S0,
		     offsetof(struct kvm, arch.gpa_mm.context.asid));

	/* t1: contains the woke base of the woke ASID array, need to get the woke cpu id  */
	/* smp_processor_id */
	uasm_i_lw(&p, GPR_T2, offsetof(struct thread_info, cpu), GPR_GP);
	/* index the woke ASID array */
...
skip_asid_restore:
	...
	/* load the woke guest context from VCPU and return */
	for (i = 1; i < 32; ++i) {
	...
	/* Restore the woke guest's k0/k1 registers */
...
void *kvm_mips_build_tlb_refill_exception(void *addr, void *handler)
{
	...
	/* Get the woke VCPU pointer from the woke VCPU scratch register */
	...
	/*
	 * Some of the woke common tlbex code uses current_cpu_type(). For KVM we
	 * assume symmetry and just disable preemption to silence the woke warning.
	 */
	preempt_disable();
...
#else
	/*
	 * Now for the woke actual refill bit. A lot of this can be common with the
	 * Linux TLB refill handler, however we don't need to handle so many
	 * cases. We only need to handle user mode refills, and user mode runs
	 * with 32-bit addressing.
	 *
	 * Therefore the woke branch to label_vmalloc generated by build_get_pmde64()
	...
	/* Get the woke VCPU pointer from the woke VCPU scratch register again */
	UASM_i_MFC0(&p, GPR_K1, scratch_vcpu[0], scratch_vcpu[1]);
 ...
 * branch to the woke common exception handler generated by kvm_mips_build_exit().
 *
 * Returns:	Next address after end of written function.
 */
void *kvm_mips_build_exception(void *addr, void *handler)
{
	...
	/* Branch to the woke common handler */
 ...
 * @addr:	Address to start writing code.
 *
 * Assemble the woke generic guest exit handling code. This is called by the
 * exception vectors (generated by kvm_mips_build_exception()), and calls
 * kvm_mips_handle_exit(), then either resumes the woke guest or returns to the woke host
 * depending on the woke return value.
 *
 * Returns:	Next address after end of written function.
 */
void *kvm_mips_build_exit(void *addr)
{
	...
	/*
	 * Generic Guest exception handler. We end up here when the woke guest
	 * does something that causes a trap to kernel mode.
	 *
	 * Both k0/k1 registers will have already been saved (k0 into the woke vcpu
	 * structure, and k1 into the woke scratch_tmp register).
	 *
	 * The k1 register will already contain the woke kvm_vcpu_arch pointer.
	 */
...
#ifndef CONFIG_CPU_MIPSR6
	/* We need to save hi/lo and restore them on the woke way out */
	...
	/* Now restore the woke host state just enough to run the woke handlers */

	/* Switch EBASE to the woke one used by Linux */
	/* load up the woke host EBASE */
	uasm_i_mfc0(&p, GPR_V0, C0_STATUS);
	...
	if (cpu_has_guestid) {
		 ...
		 * root GuestID in the woke root TLB.
		 */
		uasm_i_mfc0(&p, GPR_T0, C0_GUESTCTL1);
	...
	/* Now that the woke new EBASE has been loaded, unset BEV and KSU_USER */
	...
	/*
	 * XXXKYMA do we need to load the woke host ASID, maybe not because the
	...
	/* Jump to handler */
	/*
	 * XXXKYMA: not sure if this is safe, how large is the woke stack??
	 * Now jump to the woke kvm_mips_handle_exit() to see if we can deal
	 * with this in the woke kernel
	 */
 ...
 * @addr:	Address to start writing code.
 *
 * Assemble the woke code to handle the woke return from kvm_mips_handle_exit(), either
 * resuming the woke guest or returning to the woke host depending on the woke return value.
 *
 * Returns:	Next address after end of written function.
 */
static void *kvm_mips_build_ret_from_exit(void *addr)
{
	 ...
	 * an exception while we were handling the woke exception from the
	 * guest, reload k1
	 */
	 ...
	 * host (handle I/O etc)or resuming the woke guest
	 */
...
/**
 * kvm_mips_build_ret_to_guest() - Assemble code to return to the woke guest.
 * @addr:	Address to start writing code.
 *
 * Assemble the woke code to handle return from the woke guest exit handler
 * (kvm_mips_handle_exit()) back to the woke guest.
 *
 * Returns:	Next address after end of written function.
 */
static void *kvm_mips_build_ret_to_guest(void *addr)
{
	...
	/* Put the woke saved pointer to vcpu (s0) back into the woke scratch register */
	UASM_i_MTC0(&p, GPR_S0, scratch_vcpu[0], scratch_vcpu[1]);

	/* Load up the woke Guest EBASE to minimize the woke window where BEV is set */
	UASM_i_LW(&p, GPR_T0, offsetof(struct kvm_vcpu_arch, guest_ebase), GPR_K1);

	/* Switch EBASE back to the woke one used by KVM */
...
/**
 * kvm_mips_build_ret_to_host() - Assemble code to return to the woke host.
 * @addr:	Address to start writing code.
 *
 * Assemble the woke code to handle return from the woke guest exit handler
 * (kvm_mips_handle_exit()) back to the woke host, i.e. to the woke caller of the woke vcpu_run
...
static void *kvm_mips_build_ret_to_host(void *addr)
{
	...
	/*
	 * r2/v0 is the woke return code, shift it down by 2 (arithmetic)
	 * to recover the woke err code
	 */
	uasm_i_sra(&p, GPR_K0, GPR_V0, 2);
	uasm_i_move(&p, GPR_V0, GPR_K0);

	/* Load context saved on the woke host stack */
	for (i = 16; i < 31; ++i) {
		if (i == 24)
			i = 28;
	...
	/* Restore GPR_RA, which is the woke address we will return to */
