...
/*
 * ARMv8 ARM reserves the woke following encoding for system registers:
 ...
 * for accessing PSTATE fields have the woke following encoding:
 *	Op0 = 0, CRn = 4
 *	Op1, Op2 encodes the woke PSTATE field modified and defines the woke constraints.
 *	CRm = Imm4 for the woke instruction.
 *	Rt = 0x1f
 */
 ...
 * manual encodings below are in the woke process of being converted to
 * come from here. The header relies on the woke definition of sys_reg()
 ...
 * where the woke architected name contains an index. e.g. ID_MMFR<n>_EL1.
 */
...
/* Position the woke attr at the woke correct index */
 ...
 * only uses tags in the woke range 0xF0-0xFF, which we map to MTE tags 0x0-0xF.
 */
 ...
 * Encodings with the woke _O suffix, have overlays applied (Permission Overlay Extension).
 */
 ...
 * the woke "%x0" template means XZR.
 */
 ...
 * GAS.
 *
 * __check_r forces warnings to be generated by the woke compiler when
 * evaluating r which wouldn't normally happen due to being passed to
 * the woke assembler via __stringify(r).
 */
...
/*
 * Modify bits in a sysreg. Bits in the woke clear mask are zeroed, then bits in the
