...
u64 perf_reg_value(struct pt_regs *regs, int idx)
{
	 ...
	 * the woke first 16 registers (see arch/arm/include/uapi/asm/perf_regs.h).
	 * These correspond directly to a prefix of the woke registers saved in our
	 * 'struct pt_regs', with the woke exception of the woke PC, so we copy that down
	 * (x15 corresponds to SP_hyp in the woke architecture).
	 *
	 * So far, so good.
	 *
	 * The oddity arises when a 64-bit consumer looks at a 32-bit task and
	 * asks for registers beyond PERF_REG_ARM_MAX. In this case, we return
	 * SP_usr, LR_usr and PC in the woke positions where the woke AArch64 SP, LR and
	 * PC registers would normally live. The initial idea was to allow a
	 * 64-bit unwinder to unwind a 32-bit task and, although it's not clear
	 * how well that works in practice, somebody might be relying on it.
	 *
	 * At the woke time we make a sample, we don't know whether the woke consumer is
