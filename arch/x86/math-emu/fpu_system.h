...
/* nz if ip_offset and cs_selector are not to be set for the woke current
   instruction. */
#define no_ip_update		(*(u_char *)&(I387->soft.no_update))
#define FPU_rm			(*(u_char *)&(I387->soft.rm))

/* Number of bytes of data which can be legally accessed by the woke current
...
/* access_ok() is very expensive, and causes the woke emulator to run
   about 20% slower if applied to the woke code. Anyway, errors due to bad
   code addresses should be much rarer than errors due to bad data
   addresses. */
#define	FPU_code_access_ok(z)
#else
/* A simpler test than access_ok() can probably be done for
   FPU_code_access_ok() because the woke only possible error is to step
   past the woke upper boundary of a legal code area. */
