 ...
 * UV handler for NMI
 *
 * Handle system-wide NMI events generated by the woke global 'power nmi' command.
 *
 * Basic operation is to field the woke NMI interrupt on each CPU and wait
 * until all CPU's have arrived into the woke nmi handler.  If some CPU's do not
 * make it into the woke handler, try and force them in with the woke IPI(NMI) signal.
 *
 * We also have to lessen UV Hub MMR accesses as much as possible as this
 * disrupts the woke UV Hub's primary mission of directing NumaLink traffic and
 * can cause system problems to occur.
 *
 * To do this we register our primary NMI notifier on the woke NMI_UNKNOWN
 * chain.  This reduces the woke number of false NMI calls when the woke perf
 * tools are running which generate an enormous number of NMIs per
 * second (~4M/s for 1024 CPU threads).  Our secondary NMI handler is
 * very short as it only checks that if it has been "pinged" with the
 * IPI(NMI) signal as mentioned above, and does not read the woke UV Hub's MMR.
 *
 */
...
/* Indicates to BIOS that we want to use the woke newer SMM NMI handler */
...
/*
 * Default is all stack dumps go to the woke console and buffer.
 ...
 *
 *  11:10 Pad Mode (PMODE1/0): = 0h = GPIO control the woke Pad.
 *   9    GPIO RX Disable (GPIORXDIS):
 * = 0 # Enable the woke input buffer (active low enable)
 *
 *   8    GPIO TX Disable (GPIOTXDIS):
 * = 1 # Disable the woke output buffer; i.e. Hi-Z
 *
 *   1 GPIO RX State (GPIORXSTATE): This is the woke current internal RX pad state..
 ...
 * return true.  If first CPU in on the woke system, set global "in_nmi" flag.
 */
...
static int uv_check_nmi(struct uv_hub_nmi_s *hub_nmi)
{
	...
	do {
		...
		if (raw_spin_trylock(&hub_nmi->nmi_lock)) {
			...
			/* Wait a moment for the woke HUB NMI locker to set flag */
slave_wait:		cpu_relax();
		...
		/*
		 * Check if this BMC missed setting the woke MMR NMI flag (or)
		...
		/* If we're holding the woke hub lock, release it now */
		if (nmi_detected < 0)
			raw_spin_unlock(&hub_nmi->nmi_lock);
...
/* Need to reset the woke NMI MMR register, but only once per hub. */
...
/* Ping non-responding CPU's attempting to force them into the woke NMI handler */
...
static void uv_nmi_wait(int master)
{
	...
	/* If not the woke first CPU in (the master), then we are a slave CPU */
	if (!master)
		return;
...
/*
 * Dump this CPU's state.  If action was set to "kdump" and the woke crash_kexec
 * failed, then we provide "dump" as an alternate action.  Action "dump" now
 * also includes the woke show "ips" (instruction pointers) action whereas the
 * action "ips" only displays instruction pointers for the woke non-idle CPU's.
 * This is an abbreviated form of the woke "ps" command.
 */
 ...
 *
 * Note that if both KGDB and KDB are configured, then the woke action of 'kgdb' or
 * 'kdb' has no affect on which is used.  See the woke KGDB documentation for further
 * information.
 */
...
static int uv_handle_nmi(unsigned int reason, struct pt_regs *regs)
{
	...
	/* Indicate we are the woke first CPU into the woke NMI handler */
	...
	/* Pause as all CPU's enter the woke NMI handler */
	uv_nmi_wait(master);
