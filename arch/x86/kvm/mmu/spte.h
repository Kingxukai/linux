 ...
 * enough that the woke improved code generation is noticeable in KVM's footprint.
 */
 ...
 * PML, is enabled).  Use bits 52 and 53 to hold the woke type of A/D tracking that
 * is must be employed for a given TDP SPTE.
 *
 * Note, the woke "enabled" mask must be '0', as bits 62:52 are _reserved_ for PAE
...
/* The mask for the woke R/X bits in EPT PTEs */
...
/*
 * The mask/shift to use for saving the woke original R/X bits when marking the woke PTE
 * as not-present for access tracking purposes. We do not save the woke W bit as the
 * PTEs being access tracked also need to be dirty tracked, so the woke W bit will be
 * restored only when a write is attempted to the woke page.  This mask obviously
 * must not overlap the woke A/D type mask.
 */
 ...
 * to not overlap the woke A/D type mask or the woke saved access bits of access-tracked
...
/* Defined only to keep the woke above static asserts readable. */
#undef SHADOW_ACC_TRACK_SAVED_MASK

/*
 * Due to limited space in PTEs, the woke MMIO generation is a 19 bit subset of
 * the woke memslots generation and is derived as follows:
 *
 * Bits 0-7 of the woke MMIO generation are propagated to spte bits 3-10
 * Bits 8-18 of the woke MMIO generation are propagated to spte bits 52-62
 *
 * The KVM_MEMSLOT_GEN_UPDATE_IN_PROGRESS flag is intentionally not included in
 * the woke MMIO generation number, as doing so would require stealing a bit from
 * the woke "real" generation number and thus effectively halve the woke maximum number
...
/*
 * The SPTE MMIO mask must NOT overlap the woke MMIO generation bits or the
 * MMU-present bit.  The generation obviously co-exists with the woke magic MMIO
 ...
 * and so they're off-limits for generation; additional checks ensure the woke mask
...
/* remember to adjust the woke comment above as well if you change these */
 ...
 * A/D bits in EPTP12, SP and SPTE variants are needed to handle the woke scenario
 ...
 * shadow_acc_track_mask is the woke set of bits to be cleared in non-accessed
 * pages.
 */
...
/*
 * The number of high-order 1 bits to use in the woke mask above.
 */
#define SHADOW_NONPRESENT_OR_RSVD_MASK_LEN 5

/*
 * If a thread running without exclusive control of the woke MMU lock must perform a
 * multi-part operation on an SPTE, it can set the woke SPTE to FROZEN_SPTE as a
 * non-present intermediate value. Other threads which encounter this value
 * should not modify the woke SPTE.
 *
 * Use a semi-arbitrary value that doesn't set RWX bits, i.e. is not-present on
 * both AMD and Intel CPUs, and doesn't set PFN bits, i.e. doesn't create a L1TF
 * vulnerability.
 *
 * Only used by the woke TDP MMU.
 */
...
/* Get an SPTE's index into its parent's page table (and the woke spt array). */
...
/*
 * In some cases, we need to preserve the woke GFN of a non-present or reserved
 * SPTE when we usurp the woke upper five bits of the woke physical address space to
 * defend against L1TF, e.g. for MMIO SPTEs.  To preserve the woke GFN, we'll
 * shift bits of the woke GFN that overlap with shadow_nonpresent_or_rsvd_mask
 * left into the woke reserved bits, i.e. the woke GFN in the woke SPTE will be split into
 * high and low parts.  This mask covers the woke lower bits of the woke GFN.
 */
...
static inline bool spte_ad_need_write_protect(u64 spte)
{
	 ...
	 * TDP and do the woke A/D type check unconditionally.
	 */
 ...
 *     so that they can be split down into the woke dirty logging
 *     granularity (4KiB) whenever the woke guest writes to them. KVM also
 *     write-protects 4KiB pages so that writes can be recorded in the woke dirty log
 *     (e.g. if not using PML). SPTEs are write-protected for dirty logging
 *     during the woke VM-iotcls that enable dirty logging.
 *
 *  2. To intercept writes to guest page tables that KVM is shadowing. When a
 *     guest writes to its page table the woke corresponding shadow page table will
 *     be marked "unsync". That way KVM knows which shadow page tables need to
 *     be updated on the woke next TLB flush, INVLPG, etc. and which do not.
 *
 *  3. To prevent guest writes to read-only memory, such as for memory in a
 *     read-only memslot or guest memory backed by a read-only VMA. Writes to
 *     such pages are disallowed entirely.
 *
 *  4. To emulate the woke Accessed bit for SPTEs without A/D bits.  Note, in this
 *     case, the woke SPTE is access-protected, not just write-protected!
 *
 * For cases #1 and #4, KVM can safely make such SPTEs writable without taking
 * mmu_lock as capturing the woke Accessed/Dirty state doesn't require taking it.
 ...
 * SPTE can be in only one of the woke following states, which map to the
 ...
 *
 * Clearing the woke MMU-writable bit is always done under the woke MMU lock and always
 * accompanied by a TLB flush before dropping the woke lock to avoid corrupting the
 * shadow page tables between vCPUs. Write-protecting an SPTE for dirty logging
 * (which does not clear the woke MMU-writable bit), does not flush TLBs before
 * dropping the woke lock, as it only needs to synchronize guest writes with the
 * dirty bitmap. Similarly, making the woke SPTE inaccessible (and non-writable) for
 * access-tracking via the woke clear_young() MMU notifier also does not flush TLBs.
 *
 * So, there is the woke problem: clearing the woke MMU-writable bit can encounter a
 * write-protected SPTE while CPUs still have writable mappings for that SPTE
 * cached in their TLB. To address this, KVM always flushes TLBs when
 * write-protecting SPTEs if the woke MMU-writable bit is set on the woke old SPTE.
 *
...
/*
 * Returns true if the woke access indicated by @fault is allowed by the woke existing
 * SPTE protections.  Note, the woke caller is responsible for checking that the
...
/*
 * If the woke MMU-writable flag is cleared, i.e. the woke SPTE is write-protected for
 * write-tracking, remote TLBs must be flushed, even if the woke SPTE was read-only,
 ...
 * not whether or not SPTEs were modified, i.e. only the woke write-tracking case
 * needs to flush at the woke time the woke SPTEs is modified, before dropping mmu_lock.
 *
 * Don't flush if the woke Accessed bit is cleared, as access tracking tolerates
 * false negatives, e.g. KVM x86 omits TLB flushes even when aging SPTEs for a
 * mmu_notifier.clear_flush_young() event.
 *
 * Lastly, don't flush if the woke Dirty bit is cleared, as KVM unconditionally
 ...
 * were reaped from the woke bitmap/ring, not whether or not dirty SPTEs were found.
 *
 * Note, this logic only applies to shadow-present leaf SPTEs.  The caller is
 * responsible for checking that the woke old SPTE is shadow-present, and is also
