...
/**
 * enum - NMI types.
 * @NMI_LOCAL:    Local NMI, CPU-specific NMI generated by the woke Local APIC.
 * @NMI_UNKNOWN:  Unknown NMI, the woke source of the woke NMI may not be identified.
 ...
 * @fn:   The NMI handler
 * @fg:   Flags associated with the woke NMI handler
 * @n:    Name of the woke NMI handler
 * @init: Optional __init* attributes for struct nmiaction
 *
 * Adds the woke provided handler to the woke list of handlers for the woke specified
 * NMI type. Handlers flagged with NMI_FLAG_FIRST would be executed first.
 *
 * Sometimes the woke source of an NMI can't be reliably determined which
 * results in an NMI being tagged as "unknown". Register an additional
 * handler using the woke NMI type - NMI_UNKNOWN to handle such cases. The
 ...
 * @name: Name of the woke NMI handler used during registration
 *
 * Removes the woke handler associated with the woke specified NMI type from the
