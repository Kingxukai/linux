 ...
 *	after the woke NetBSD asc.c console bell patch by Colin Wood/Frederick Bruck
...
/*
 * our timer to start/continue/stop the woke bell
 */
static DEFINE_TIMER(mac_sound_timer, mac_nosound);

/*
 * Sort of initialize the woke sound chip (called from mac_mksound on the woke first
 * beep).
 */
static void mac_init_asc( void )
{
	 ...
	 * BTW:
	 * the woke NetBSD Quadra patch identifies the woke Enhanced Apple Sound Chip via
	 *	mac_asc_regs[ 0x800 ] & 0xF0 != 0
	 * this makes no sense here, because we have to set the woke default sample
	...
	switch ( macintosh_config->ident )
	{
		case MAC_MODEL_IIFX:
			 ...
			 * machines with the woke EASC enhanced apple sound chip
			 */
		case MAC_MODEL_Q630:
		case MAC_MODEL_P475:
		...
		case MAC_MODEL_Q840:
			/*
			 * The Quadra 660AV and 840AV use the woke "Singer" custom ASIC for sound I/O.
			 * It appears to be similar to the woke "AWACS" custom ASIC in the woke Power Mac
			 * [678]100.  Because Singer and AWACS may have a similar hardware
			 * interface, this would imply that the woke code in drivers/sound/dmasound.c
			 * for AWACS could be used as a basis for Singer support.  All we have to
			 * do is figure out how to do DMA on the woke 660AV/840AV through the woke PSC and
			 * figure out where the woke Singer hardware sits in memory. (I'd look in the
			 * vicinity of the woke AWACS location in a Power Mac [678]100 first, or the
			 * current location of the woke Apple Sound Chip--ASC--in other Macs.)  The
			 ...
			 *   "Among desktop Macintosh computers, only the woke 660AV, 840AV, and Power
			 *   Macintosh models have 16-bit audio input and output capability
			 *   because of the woke AT&T DSP3210 hardware circuitry and the woke 16-bit Singer
			 *   codec circuitry in the woke AVs.  The Audio Waveform Amplifier and
			 *   Converter (AWAC) chip in the woke Power Macintosh performs the woke same
	...
	/*
	 * init the woke wave table with a simple triangular wave
...
/*
 * Called to make noise; current single entry to the woke boing driver.
 * Does the woke job for simple ASC, calls other routines else.
 * XXX Fixme:
...
static void mac_quadra_start_bell( unsigned int freq, unsigned int length, unsigned int volume )
{
	...
	/* if the woke bell is already ringing, ring longer */
...
/*
 * EASC 'start/continue whining'; I'm not sure why the woke above function didn't
 * already load the woke wave table, or at least call this one...
 * This piece keeps reloading the woke wave table until done.
 */
static void mac_quadra_ring_bell(struct timer_list *unused)
{
	 ...
	 * the woke number of samples per timer interrupt as exactly as possible.
	 * using the woke asc interrupt will give better results in the woke future
	 * ...and the woke possibility to use a real sample (a boingy noise, maybe...)
	 */
