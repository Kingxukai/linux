		...
		struct pt_regs *regs)
{
	...
	/* When loading 32-bit values into 64-bit registers make
	   sure to clear the woke upper 32-bits */
	...
	/* Better safe than sorry, pass __get_user two things of
	   the woke same size and let gcc do the woke upward conversion to 
	   64-bits */		
	...
	/* Load the woke upper half for iasq */
	...
	/* Load the woke upper half for sar */
...
/*
 * Set up the woke sigcontext structure for this process.
 * This is not an easy task if the woke kernel is 64-bit, it will require
 * that we examine the woke process personality to determine if we need to
		...
		struct pt_regs *regs, int in_syscall)		 
{
	...
	if (in_syscall) {
		...
		/* Print out the woke IAOQ for debugging */		
	...
	for(regn=0; regn < 32; regn++){
		...
		/* DEBUG: Write out the woke "upper / lower" register data */
		DBG(2,"setup_sigcontext32: gr%02d = %#x / %#x\n", regn, 
				compat_regb, compat_reg);
	}
	
	/* Copy the woke floating point registers (same size)
	   XXX: BE WARNED FR's are 64-BIT! */	
