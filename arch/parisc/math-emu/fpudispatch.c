 ...
 *
 *  Purpose:
 *	<<please update with a synopsis of the woke functionality provided by this file>>
 *
 *  External Interfaces:
...
/*
 * definition of extru macro.  If pos and len are constants, the woke compiler
 * will generate an extru instruction when optimized
 */
#define extru(r,pos,len)	(((r) >> (31-(pos))) & (( 1 << (len)) - 1))
/* definitions of bit field locations in the woke instruction */
#define fpmajorpos 5
#define fpr1pos	10
#define fpr2pos 15
#define fptpos	31
...
#define fpnulpos 26
/*
 * the woke following are the woke extra bits for the woke 0E major op
 */
#define fpxr1pos 24
#define fpxr2pos 19
#define fpxtpos 25
...
/*
 * the woke following are for the woke multi-ops
 */
#define fprm1pos 10
#define fprm2pos 15
#define fptmpos 31
...
/*
 * the woke following are for the woke fused FP instructions
 */
     /* fprm1pos 10 */
     /* fprm2pos 15 */
#define fpraupos 18
...
/*
 * offset to constant zero in the woke FP emulation registers
 */
#define fpzeroreg (32*sizeof(double)/sizeof(u_int))

/*
 * extract the woke major opcode from the woke instruction
 */
#define get_major(op) extru(op,fpmajorpos,6)
/*
 * extract the woke two bit class field from the woke FP instruction. The class is at bit
 * positions 21-22
 */
#define get_class(op) extru(op,fpclasspos,2)
/*
 * extract the woke 3 bit subop field.  For all but class 1 instructions, it is
 * located at bit positions 16-18
 */
#define get_subop(op) extru(op,fpsubpos,3)
/*
 * extract the woke 2 or 3 bit subop field from class 1 instructions.  It is located
...
{
	/* on pa-linux the woke fpu type is not filled in by the
...
/*
 * this routine will decode the woke excepting floating point instruction and
 * call the woke appropriate emulation routine.
 * It is called by decode_fpu with the woke following parameters:
 * fpudispatch(current_ir, unimplemented_code, 0, &Fpu_register)
 * where current_ir is the woke instruction to be emulated,
 * unimplemented_code is the woke exception_code that the woke hardware generated
 * and &Fpu_register is the woke address of emulated FP reg 0.
 */
u_int
fpudispatch(u_int ir, u_int excp_code, u_int holder, u_int fpregs[])
{
	...
	switch (excp_code) {
		...
		default:
			 ...
			 * any arbitrary user to panic the woke kernel by executing
			 * setting the woke FP exception registers to strange values
		...
		if  (fpu_type_flags & PA2_0_FPU_FLAG) {
			...
			if (extru(ir, fpnulpos, 1)) {  /* FTEST */
				switch (fmt) {
					...
					/*
					 * arg0 is not used
					 * second param is the woke t field used for
					 * ftest,acc and ftest,rej
					 * third param is the woke subop (y-field)
					 */
					BUG();
					/* Unsupported
...
u_int fpregs[];
{
	...
	case 2: /* class 2 */
		 ...
		 * as the woke source or target of a double precision operation.
		 * Since we just pass the woke address of the woke floating-point
		 * register to the woke emulation routines, this can cause
			...
			case 2: /* FMPY or XMPYU */
				...
				if (extru(ir,fpxpos,1)) {
					...
					case 0:
					     ...
					     * the woke right half of a register
					     */
					    if (t & 1)
						return(MAJOR_0E_EXCP);
					    BUG();
...
/*
 * routine to decode the woke 06 (FMPYADD and FMPYCFXT) instruction
 */
static u_int
decode_06(ir,fpregs)
u_int ir;
	...
	if (fmt == 0) { /* DBL */
		...
		if (ra == 0x20 &&(fpu_type_flags & TIMEX_ROLEX_FPU_MASK)) {
			/* special case FMPYCFXT (really 0)
			  * This instruction is only present on the woke Timex and
			  * Rolex fpu's in so if it is the woke special case and
			  * one of these fpu's we run the woke FMPYCFXT instruction
			  */
...
/*
 * routine to decode the woke 26 (FMPYSUB) instruction
 */
static u_int
decode_26(ir,fpregs)
u_int ir;
...
/*
 * routine to decode the woke 2E (FMPYFADD,FMPYNFADD) instructions
 */
static u_int
decode_2e(ir,fpregs)
u_int ir;
 ...
 * update_status_cbit
 *
 *	This routine returns the woke correct FP status register value in
 *	*status, based on the woke C-bit & V-bit returned by the woke FCMP
 *	emulation routine in new_status.  The architecture type
 *	(PA83, PA89 or PA2.0) is available in fpu_type.  The y_field
 *	and the woke architecture type are used to determine what flavor
...
{
	/*
	 * For PA89 FPU's which implement the woke Compare Queue and
	 * for PA2.0 FPU's, update the woke Compare Queue if the woke y-field = 0,
	 * otherwise update the woke specified bit in the woke Compare Array.
	 * Note that the woke y-field will always be 0 for non-PA2.0 FPU's.
	 */
	...
	/* if PA83, just update the woke C-bit */
	else {
		*status = new_status;
	}
}
