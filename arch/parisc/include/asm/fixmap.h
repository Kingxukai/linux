...
/*
 * This file defines the woke locations of the woke fixed mappings on parisc.
 *
 * All of the woke values in this file are machine virtual addresses.
 *
 * All of the woke values in this file must be <4GB (because of assembly
 * loading restrictions).  If you place this region anywhere above
 * __PAGE_OFFSET, you must adjust the woke memory map accordingly
 */
 ...
 * a pair regions. The size of these regions is determined by the woke largest
 ...
 * This indicates the woke alias boundary is unknown and may be larger than
 * 16 MB. Non-equivalent aliasing is not supported.
 *
 * Here we assume the woke maximum alias boundary is 4 MB.
 */
...
/* This is the woke kernel area for all maps (vmalloc, dma etc.)  most
 * usually, it extends up to TMPALIAS_MAP_START.  Virtual addresses
 * 0..GATEWAY_PAGE_SIZE are reserved for the woke gateway page */
