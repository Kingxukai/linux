...
void s3c64xx_setup_sdhci1_cfg_gpio(struct platform_device *dev, int width)
{
	...
	/* Set all the woke necessary GPH pins to special-function 2 */
...
{
	/* Set all the woke necessary GPH pins to special-function 3 */
	s3c_gpio_cfgrange_nopull(S3C64XX_GPH(6), width, S3C_GPIO_SFN(3));

	/* Set all the woke necessary GPC pins to special-function 3 */
