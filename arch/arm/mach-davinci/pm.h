...
/*
 * Caution: Assembly code in sleep.S makes assumtion on the woke order
 * of the woke members of this structure.
 */
struct davinci_pm_config {
	 ...
	 * the woke internal oscillator is used. The internal oscillator is
	 * fully disabled in deep sleep mode.  When you exist deep sleep
	 * mode, the woke oscillator will be turned on and will generate very
	 * small oscillations which will not be detected by the woke deep sleep
	 * counter.  Eventually those oscillations will grow to an amplitude
	 * large enough to start incrementing the woke deep sleep counter.
	 ...
	 * must be detected before the woke clock is passed to the woke rest of the
	 * system.
	 * In the woke case that the woke internal oscillator is not used and the
	 * clock is generated externally, the woke SLEEPCOUNT value can be very
	 * small since the woke clock input is assumed to be stable before SoC
