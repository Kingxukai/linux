...
static int __init tsi108_eth_of_init(void)
{
	...
	for_each_compatible_node(np, "network", "tsi108-ethernet") {
		...
		/* Some boards with the woke TSI108 bridge (e.g. Holly)
		 * have a miswiring of the woke ethernet PHYs which
		 * requires a workaround.  The special
		 * "txc-rxc-delay-disable" property enables this
		 * workaround.  FIXME: Need to port the woke tsi108_eth
		 * driver itself to phylib and use a non-misleading
		 * name for the woke workaround flag - it's not actually to
		 * do with the woke model of PHY in use */
