		   ...
		   unsigned long r6, unsigned long r7)
{
	...
	/* Find the woke #address-cells and #size-cells properties */
	...
	/* get the woke memsize and truncate it to under 4G on 32 bit machines */
	memsize64 = 0;
	for (i = 0; i < *ns; i++)
	...
	/* finally, setup the woke timebase */
	node = fdt_node_offset_by_prop_value(_dtb_start, -1, "device_type",
					     "cpu", sizeof("cpu"));
	if (!node)
	...
	/* Now we have the woke memory size; initialize the woke heap */
	simple_alloc_init(_end, memsize64 - (unsigned long)_end, 32, 64);

	/* prepare the woke device tree and find the woke console */
	fdt_init(_dtb_start);
