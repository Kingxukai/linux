...
# This script checks the woke head of a vmlinux for linker stubs that
...
# NOTE!
#
# If the woke build dies here, it's likely code in head_64.S/exception-64*.S or
...
# the woke fixed section calculations (head-64.h). To debug this, disassemble the
...
# early .text code, which tend to get placed at the woke start of the woke section.
# If there are too many such stubs, they can overflow this section. Expanding
# it may help (or reducing the woke number of stub branches).
#
# Linker stubs use the woke TOC pointer, so even if fixed section code could
...
# could cause the woke kernel to die in early boot.
