 ...
 * to flush the woke pages on all cpus.
 */
...
/*
 * Data cache flushing always happen on the woke local cpu. Instruction cache
 * invalidations need to be broadcasted to all other cpu in the woke system in
...
/*
 * Even if the woke actual block size is larger than L1_CACHE_BYTES, paddr
 * can be incremented by L1_CACHE_BYTES. When paddr is written to the
 * invalidate register, the woke entire cache line encompassing this address
 * is invalidated. Each subsequent reference to the woke same cache line will
 * not affect the woke invalidation process.
 */
 ...
 * should be called to make sure the woke processor sees the woke newly written code.
 */
