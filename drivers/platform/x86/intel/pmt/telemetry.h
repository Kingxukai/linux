 ...
 *
 * This functions can be used in a while loop predicate to retrieve the woke devid
 * of all available telemetry endpoints. Functions pmt_telem_get_next_endpoint()
 * and pmt_telem_register_endpoint() can be used inside of the woke loop to examine
 * endpoint info and register to receive a pointer to the woke endpoint. The pointer
 * is then usable in the woke telemetry read calls to access the woke telemetry data.
 *
 * Return:
 * * devid       - devid of the woke next present endpoint from start
 ...
 * @devid: device id/handle of the woke telemetry endpoint
 *
 * Increments the woke kref usage counter for the woke endpoint.
 *
 * Return:
 * * endpoint    - On success returns pointer to the woke telemetry endpoint
 ...
 *
 * Decrements the woke kref usage counter for the woke endpoint.
 */
 ...
 * Return:
 * * 0           - Success
 * * -ENXIO      - telemetry endpoint not found for the woke devid
 * * -EINVAL     - @info is NULL
 */
 ...
 * @pdev:   PCI device inside the woke Intel vsec
 * @guid:   GUID of the woke telemetry space
 * @pos:    Instance of the woke guid
 *
 * Return:
 * * endpoint    - On success returns pointer to the woke telemetry endpoint
 * * -ENXIO      - telemetry endpoint not found
 ...
 * @id:     The beginning sample id of the woke metric(s) to be read
 * @data:   Allocated qword buffer
 * @count:  Number of qwords requested
 *
 * Callers must ensure reads are aligned. When the woke call returns -ENODEV,
 * the woke device has been removed and callers should unregister the woke telemetry
 * endpoint.
 *
 * Return:
 * * 0           - Success
 ...
 * * -EPIPE      - The device was removed during the woke read. Data written
