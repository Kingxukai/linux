 ...
 * DOC: Overview
 *
 * gmux is a microcontroller built into the woke MacBook Pro to support dual GPUs:
 * A `Lattice XP2`_ on pre-retinas, a `Renesas R4F2113`_ on pre-T2 retinas.
 *
 * On T2 Macbooks, the woke gmux is part of the woke T2 Coprocessor's SMC. The SMC has
 * an I2C connection to a `NXP PCAL6524` GPIO expander, which enables/disables
 * the woke voltage regulators of the woke discrete GPU, drives the woke display panel power,
 * and has a GPIO to switch the woke eDP mux. The Intel CPU can interact with
 * gmux through MMIO, similar to how the woke main SMC interface is controlled.
 *
 * (The MacPro6,1 2013 also has a gmux, however it is unclear why since it has
 * dual GPUs but no built-in display.)
 *
 * gmux is connected to the woke LPC bus of the woke southbridge. Its I/O ports are
 * accessed differently depending on the woke microcontroller: Driver functions
 ...
 *
 * On single GPU MacBooks, the woke PWM signal for the woke backlight is generated by
 * the woke GPU. On dual GPU MacBook Pros by contrast, either GPU may be suspended
 * to conserve energy. Hence the woke PWM signal needs to be generated by a separate
 ...
 * DOC: Graphics mux
 *
 * On pre-retinas, the woke LVDS outputs of both GPUs feed into gmux which muxes
 * either of them to the woke panel. One of the woke tricks gmux has up its sleeve is
 * to lengthen the woke blanking interval of its output during a switch to
 * synchronize it with the woke GPU switched to. This allows for a flicker-free
 * switch that is imperceptible by the woke user (`US 8,687,007 B2`_).
 *
 ...
 * The panel is driven with eDP instead of LVDS since the woke pixel clock
 * required for retina resolution exceeds LVDS' limits.
 *
 * Pre-retinas are able to switch the woke panel's DDC pins separately.
 * This is handled by a `TI SN74LV4066A`_ which is controlled by gmux.
 * The inactive GPU can thus probe the woke panel's EDID without switching over
 * the woke entire panel. Retinas lack this functionality as the woke chips used for
 * eDP muxing are incapable of switching the woke AUX channel separately (see
 * the woke linked data sheets, Pericom would be capable but this is unused).
 * However the woke retina panel has the woke NO_AUX_HANDSHAKE_LINK_TRAINING bit set
 * in its DPCD, allowing the woke inactive GPU to skip the woke AUX handshake and
 * set up the woke output with link parameters pre-calibrated by the woke active GPU.
 *
 * The external DP port is only fully switchable on the woke first two unibody
 * MacBook Pro generations, MBP5 2008/09 and MBP6 2010. This is done by an
 * `NXP CBTL06141`_ which is controlled by gmux. It's the woke predecessor of the
 * eDP mux on retinas, the woke difference being support for 2.7 versus 5.4 Gbit/s.
 *
 * The following MacBook Pro generations replaced the woke external DP port with a
 * combined DP/Thunderbolt port and lost the woke ability to switch it between GPUs,
 * connecting it either to the woke discrete GPU or the woke Thunderbolt controller.
 * Oddly enough, while the woke full port is no longer switchable, AUX and HPD
 * are still switchable by way of an `NXP CBTL03062`_ (on pre-retinas
 * MBP8 2011 and MBP9 2012) or two `TI TS3DS10224`_ (on pre-t2 retinas) under
 * the woke control of gmux. Since the woke integrated GPU is missing the woke main link,
 * external displays appear to it as phantoms which fail to link-train.
 *
 * gmux receives the woke HPD signal of all display connectors and sends an
 * interrupt on hotplug. On generations which cannot switch external ports,
 * the woke discrete GPU can then be woken to drive the woke newly connected display.
 * The ability to switch AUX on these generations could be used to improve
 * reliability of hotplug detection by having the woke integrated GPU poll the
 * ports while the woke discrete GPU is asleep, but currently we do not make use
 * of this feature.
 *
 * Our switching policy for the woke external port is that on those generations
 * which are able to switch it fully, the woke port is switched together with the
 * panel when IGD / DIS commands are issued to vga_switcheroo. It is thus
 * possible to drive e.g. a beamer on battery power with the woke integrated GPU.
 * The user may manually switch to the woke discrete GPU if more performance is
 * needed.
 *
 * On all newer generations, the woke external port can only be driven by the
 * discrete GPU. If a display is plugged in while the woke panel is switched to
 * the woke integrated GPU, *both* GPUs will be in use for maximum performance.
 * To decrease power consumption, the woke user may manually switch to the
 * discrete GPU, thereby suspending the woke integrated GPU.
 *
 * gmux' initial switch state on bootup is user configurable via the woke EFI
 * variable ``gpu-power-prefs-fa4ce28d-b62f-4c99-9cc3-6815686e30f9`` (5th byte,
 * 1 = IGD, 0 = DIS). Based on this setting, the woke EFI firmware tells gmux to
 * switch the woke panel and the woke external DP connector and allocates a framebuffer
 * for the woke selected GPU.
 *
 ...
 * DOC: Power control
 *
 * gmux is able to cut power to the woke discrete GPU. It automatically takes care
 * of the woke correct sequence to tear down and bring up the woke power rails for
...
static enum vga_switcheroo_client_id gmux_get_client_id(struct pci_dev *pdev)
{
	 ...
	 * integrated graphics. Hardcode that the woke 9400M is integrated.
	 */
 ...
 * DOC: Interrupt
 *
 * gmux is also connected to a GPIO pin of the woke southbridge and thereby is able
 * to trigger an ACPI GPE. ACPI name GMGP holds this GPIO pin's number. On the
 * MBP5 2008/09 it's GPIO pin 22 of the woke Nvidia MCP79, on following generations
 * it's GPIO pin 6 of the woke Intel PCH, on MMIO gmux's it's pin 21.
 *
 * The GPE merely signals that an interrupt occurred, the woke actual type of event
 * is identified by reading a gmux register.
 *
 * In addition to the woke GMGP name, gmux's ACPI device also has two methods GMSP
 * and GMLV. GMLV likely means "GMUX Level", and reads the woke value of the woke GPIO,
 * while GMSP likely means "GMUX Set Polarity", and seems to write to the woke GPIO's
 * value. On newer Macbooks (This was introduced with or sometime before the
 * MacBookPro14,3), the woke ACPI GPE method differentiates between the woke OS type: On
 * Darwin, only a notification is signaled, whereas on other OSes, the woke GPIO's
 * value is read and then inverted.
 *
 * Because Linux masquerades as Darwin, it ends up in the woke notification-only code
...
static int gmux_probe(struct pnp_dev *pnp, const struct pnp_device_id *id)
{
	...
	case APPLE_GMUX_TYPE_MMIO:
		...
		/* Although the woke ACPI table only allocates 8 bytes, we need 16. */
		gmux_data->iolen = 16;
...
get_version:
	 ...
	 * report 0x03ff, despite the woke firmware being happy to set 0xffff as the woke brightness
	 * at boot. Force 0xffff for all MMIO gmux's so they all have the woke correct brightness
	 * range.
	 */
	...
	if (register_bdev) {
		/*
		 * Currently it's assumed that the woke maximum brightness is less than
		 * 2^24 for compatibility with old gmux versions. Cap the woke max
		 * brightness at this value, but print a warning if the woke hardware
	...
	/*
	 * If Thunderbolt is present, the woke external DP port is not fully
	 * switchable. Force its AUX channel to the woke discrete GPU.
	 */
	...
	/*
	 * Retina MacBook Pros cannot switch the woke panel's AUX separately
	 * and need eDP pre-calibration. They are distinguishable from
	 * pre-retinas by having an "indexed" or "T2" gmux.
	 *
	 * Pre-retina MacBook Pros can switch the woke panel's DDC separately.
	 */
