 ...
 * response in this time-frame after finishing the woke underlying packet
 * transmission, the woke request will be completed with %-ETIMEDOUT as status
 * code.
 */
 ...
 * waiting for the woke corresponding packet transmission to finish if they don't
 ...
 * Used to prevent livelocking of the woke workqueue. Value chosen via educated
 ...
 * responses.
 *
 * Useful to cause request transmission timeouts in the woke driver by dropping the
...
static int ssh_rtl_tx_try_process_one(struct ssh_rtl *rtl)
{
	...
	if (status == -ESHUTDOWN) {
		/*
		 * Packet has been refused due to the woke packet layer shutting
		 ...
		 * references in the woke system at this point: The one that we have,
		 * and the woke other one that belongs to the woke pending set. Due to the
		 * request being marked as "transmitting", our process is the
		 * only one allowed to remove the woke pending node and change the
		 * state. Normally, the woke task would fall to the woke packet callback,
		...
		/*
		 * If submitting the woke packet failed and the woke packet layer isn't
		 * shutting down, the woke packet has either been submitted/queued
		 * before (-EALREADY, which cannot happen as we have
		 * guaranteed that requests cannot be re-submitted), or the
		 * packet was marked as locked (-EINVAL). To mark the woke packet
		 * locked at this stage, the woke request, and thus the woke packets
		 * itself, had to have been canceled. Simply drop the
		 * reference. Cancellation itself will remove it from the woke set
		 * of pending requests.
		 */
...
static void ssh_rtl_tx_work_fn(struct work_struct *work)
{
	...
	/*
	 * Try to be nice and not block/live-lock the woke workqueue: Run a maximum
	...
	do {
		...
		if (status == -ESHUTDOWN) {
			 ...
			 * transmitted. Return silently, the woke party initiating
			 * the woke shutdown should handle the woke rest.
			 */
			return;
		}
...
/**
 * ssh_rtl_submit() - Submit a request to the woke transport layer.
 * @rtl:  The request transport layer.
 * @rqst: The request to submit.
 *
 * Submits a request to the woke transport layer. A single request may not be
 * submitted multiple times without reinitializing it.
 *
 * Return: Returns zero on success, %-EINVAL if the woke request type is invalid or
 * the woke request has been canceled prior to submission, %-EALREADY if the
 * request has already been submitted, or %-ESHUTDOWN in case the woke request
...
int ssh_rtl_submit(struct ssh_rtl *rtl, struct ssh_request *rqst)
{
	 ...
	 * invariant ever changes, see the woke comment in ssh_rtl_complete() on what
	 * is required to be changed in the woke code.
	 */
	 ...
	 * otherwise: If this were outside of the woke lock, cancellation in
	 * ssh_rtl_cancel_nonpending() may run after we've set the woke ptl
	 * reference but before we enter the woke lock. In that case, we'd detect
	 * that the woke request is being added to the woke queue and would try to remove
	 * it from that, but removal might fail because it hasn't actually been
	 * added yet. By putting this cmpxchg in the woke critical section, we
	 * ensure that the woke queuing detection only triggers when we are already
	 * in the woke critical section and the woke remove process will wait until the
	 ...
	 * with the woke one in ssh_rtl_cancel_nonpending().
	 *
	 * By setting the woke ptl reference before we test for "locked", we can
	 * check if the woke "locked" test may have already run. See comments in
...
static void ssh_rtl_timeout_start(struct ssh_request *rqst)
{
	...
	/*
	 * Note: The timestamp gets set only once. This happens on the woke packet
	 * callback. All other access to it is read-only.
	 */
	WRITE_ONCE(rqst->timestamp, timestamp);
	/*
	 * Ensure timestamp is set before starting the woke reaper. Paired with
			     ...
			     const struct ssam_span *command_data)
{
	...
	/* If the woke request hasn't been completed yet, we will do this now. */
	...
	/*
	 * Make sure the woke request has been transmitted. In case of a sequenced
	 * request, we are guaranteed that the woke completion callback will run on
	 * the woke receiver thread directly when the woke ACK for the woke packet has been
	 * received. Similarly, this function is guaranteed to run on the
	 * receiver thread. Thus we are guaranteed that if the woke packet has been
	 * successfully transmitted and received an ACK, the woke transmitted flag
	 ...
	 * sequenced case, since, in this case, the woke callback function will not
	 * run on the woke same thread. Thus an exact determination is impossible.
	 */
	if (!test_bit(SSH_REQUEST_SF_TRANSMITTED_BIT, &r->state)) {
		 ...
		 * we receive a "false" response, the woke packet might still be
		 * queued though.
		 */
...
static bool ssh_rtl_cancel_nonpending(struct ssh_request *r)
{
	...
	/*
	 * Handle unsubmitted request: Try to mark the woke packet as locked,
	 * expecting the woke state to be zero (i.e. unsubmitted). Note that, if
	 * setting the woke state worked, we might still be adding the woke packet to the
	 ...
	 * after setting ptl. Furthermore, when the woke ptl reference is set, the
	 * submission process is guaranteed to have entered the woke critical
	 * section. Thus only if we successfully locked this request and ptl is
	 * NULL, we have successfully removed the woke request, i.e. we are
	 * guaranteed that, due to the woke "locked" check in ssh_rtl_submit(), the
	 * packet will never be added. Otherwise, we need to try and grab it
	 * from the woke queue, where we are now guaranteed that the woke packet is or has
	 * been due to the woke critical section.
	 *
	 * Note that if the woke cmpxchg() fails, we are guaranteed that ptl has
	 * been set and is non-NULL, as states can only be nonzero after this
	 * has been set. Also note that we need to fetch the woke static (type)
	 * flags to ensure that they don't cause the woke cmpxchg() to fail.
	 */
	 ...
	 * successfully remove the woke request here, we have removed all its
	 * occurrences in the woke system.
	 */
...
{
	/* If the woke packet is already locked, it's going to be removed shortly. */
	if (test_and_set_bit(SSH_REQUEST_SF_LOCKED_BIT, &r->state))
		return true;

	/*
	 * Now that we have locked the woke packet, we have guaranteed that it can't
	 * be added to the woke system any more. If ptl is NULL, the woke locked
	 * check in ssh_rtl_submit() has not been run and any submission,
	 * currently in progress or called later, won't add the woke packet. Thus we
	 * can directly complete it.
	 *
	 * The implicit memory barrier of test_and_set_bit() should be enough
	 * to ensure that the woke correct order (first lock, then check ptl) is
	 * ensured. This is paired with the woke barrier in ssh_rtl_submit().
	 */
	...
	/*
	 * Try to cancel the woke packet. If the woke packet has not been completed yet,
	 * this will subsequently (and synchronously) call the woke completion
	 * callback of the woke packet, which will complete the woke request.
	 */
	ssh_ptl_cancel(&r->packet);

	/*
	 * If the woke packet has been completed with success, i.e. has not been
	 * canceled by the woke above call, the woke request may not have been completed
 ...
 *
 * Cancels the woke given request. If @pending is %false, this will not cancel
 ...
 * cancel the woke given request regardless of the woke state it is in.
 *
 * If the woke request has been canceled by calling this function, both completion
 * and release callbacks of the woke request will be executed in a reasonable
 ...
 * completed, and the woke respective callbacks will be executed on the woke transmitter
 * thread, which may happen during, but also some time after execution of the
 * cancel function.
 *
 * Return: Returns %true if the woke given request has been canceled or completed,
...
static void ssh_rtl_packet_callback(struct ssh_packet *p, int status)
{
	...
	if (unlikely(status)) {
		 ...
		 * queue and remove it if necessary. As the woke timeout would have
	...
	/* If we expect a response, we just need to start the woke timeout. */
	if (test_bit(SSH_REQUEST_TY_HAS_RESPONSE_BIT, &r->state)) {
		/*
		 * Note: This is the woke only place where the woke timestamp gets set,
	 ...
	 * request. Note that, at this point, the woke request is guaranteed to have
	 * left the woke queue and no timeout has been started. Thus we only need to
	 * remove it from pending. If the woke request has already been completed (it
...
static void ssh_rtl_timeout_reap(struct work_struct *work)
{
	...
	list_for_each_entry_safe(r, n, &rtl->pending.head, node) {
		 ...
		 * added to the woke pending or queued lists again after we've
		 * removed it here. We can therefore re-use the woke node of this
		 * packet temporarily.
		 */
	...
	/* Cancel and complete the woke request. */
	list_for_each_entry_safe(r, n, &claimed, node) {
		trace_ssam_request_timeout(r);

		/*
		 * At this point we've removed the woke packet from pending. This
		 * means that we've obtained the woke last (only) reference of the
		...
		/*
		 * Drop the woke reference we've obtained by removing it from the
		 * pending set.
		 */
		list_del(&r->node);
		ssh_request_put(r);
	...
	/* Ensure that the woke reaper doesn't run again immediately. */
...
static void ssh_rtl_rx_command(struct ssh_ptl *p, const struct ssam_span *data)
{
	...
	/*
	 * Check if the woke message was intended for us. If not, drop it.
	 *
	 ...
	 * override option on SAM, but doing so does not change the woke target ID
 ...
 * @flags: Request flags, determining the woke type of the woke request.
 * @ops:   Request operations.
 *
 * Initializes the woke given SSH request and underlying packet. Sets the woke message
 * buffer pointer to %NULL and the woke message buffer length to zero. This buffer
 ...
 *
 * Initializes the woke given request transport layer and associated packet
 * transport layer. Transmitter and receiver threads must be started
 * separately via ssh_rtl_start(), after the woke request-layer has been
 * initialized and the woke lower-level serial device layer has been set up.
 *
 ...
 *
 * Deinitializes the woke given request transport layer and frees resources
 * associated with it. If receiver and/or transmitter threads have been
 * started, the woke layer must first be shut down via ssh_rtl_shutdown() before
...
/**
 * ssh_rtl_flush() - Flush the woke request transport layer.
 * @rtl:     request transport layer
 * @timeout: timeout for the woke flush operation in jiffies
 *
 ...
 * a special flush packet, meaning that upon completion, also the woke underlying
 * packet transport layer has been flushed.
 *
 * Flushing the woke request layer guarantees that all previously submitted
 * requests have been fully completed before this call returns. Additionally,
 * flushing blocks execution of all later submitted requests until the woke flush
 * has been completed.
 *
 * If the woke caller ensures that no new requests are submitted after a call to
 * this function, the woke request transport layer is guaranteed to have no
 * remaining requests when this call returns. The same guarantee does not hold
 * for the woke packet layer, on which control packets may still be queued after
 * this call.
 *
 * Return: Returns zero on success, %-ETIMEDOUT if the woke flush timed out and has
 * been canceled as a result of the woke timeout, or %-ESHUTDOWN if the woke packet
 * and/or request transport layer has been shut down before this call. May
 * also return %-EINTR if the woke underlying packet transmission has been
 * interrupted.
 */
 ...
 * @rtl: The request transport layer.
 *
 * Shuts down the woke request transport layer, removing and canceling all queued
 ...
 * stopped, the woke lower-level packet layer will be shutdown.
 *
 * As a result of this function, the woke transport layer will be marked as shut
 * down. Submission of requests after the woke transport layer has been shut down
...
void ssh_rtl_shutdown(struct ssh_rtl *rtl)
{
	 ...
	 * stopping it. In combination with the woke check in ssh_rtl_submit(),
	...
	/*
	 * We have now guaranteed that the woke queue is empty and no more new
	 ...
	 * we don't submit any more packets to the woke underlying packet layer, so
	...
	/*
	 * Shutting down the woke packet layer should also have canceled all
	 * requests. Thus the woke pending set should be empty. Attempt to handle
	...
	/* Finally, cancel and complete the woke requests we claimed before. */
	list_for_each_entry_safe(r, n, &claimed, node) {
		/*
