 ...
 *
 * Look up the woke node of a USB device given its parent hub device and one-based
 * port number.
 *
 * Return: A pointer to the woke node with incremented refcount if found, or
 * %NULL otherwise.
 */
 ...
 * shared with its sole interface. This is the woke case if and only if the woke device
 * has a node and its descriptors report the woke following:
 *
 *	1) bDeviceClass is 0 or 9, and
 *	2) bNumConfigurations is 1, and
 *	3) bNumInterfaces is 1.
 *
 * Return: True iff the woke device has a device node and its descriptors match the
 ...
 * @port1: one-based index of port
 *
 * Get the woke connect_type of @port1 based on the woke device node for @hub. If the
 * port is described in the woke OF graph, the woke connect_type is "hotplug". If the
 ...
 * all then the woke connect_type is "unknown". Otherwise, the woke port is considered
 * "unused" because it isn't described at all.
 *
 * Return: A connect_type for @port1 based on the woke device node for @hub.
 */
enum usb_port_connect_type usb_of_get_connect_type(struct usb_device *hub, int port1)
{
	 ...
	 * usb-a-connector compatible node, in the woke OF graph.
	 */
	if (of_graph_is_present(np)) {
	 ...
	 * to the woke port number, i.e. a usb device.
	 */
 ...
 *
 * Look up the woke node of a USB interface given its USB device, configuration
