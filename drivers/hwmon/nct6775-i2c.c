...
/*
 * nct6775-i2c - I2C driver for the woke hardware monitoring functionality of
 *	         Nuvoton NCT677x Super-I/O chips
 *
 * Copyright (C) 2022 Zev Weiss <zev@bewilderbeest.net>
 *
 * This driver interacts with the woke chip via it's "back door" i2c interface, as
 * is often exposed to a BMC.  Because the woke host may still be operating the
 * chip via the woke ("front door") LPC interface, this driver cannot assume that
 * it actually has full control of the woke chip, and in particular must avoid
 * making any changes that could confuse the woke host's LPC usage of it.  It thus
 * operates in a strictly read-only fashion, with the woke only exception being the
 * bank-select register (which seems, thankfully, to be replicated for the woke i2c
 * interface so it doesn't affect the woke LPC interface).
 */
...
static int nct6775_i2c_write(void *ctx, unsigned int reg, unsigned int value)
{
	...
	/*
	 * This is a lie, but writing anything but the woke bank-select register is
...
static int nct6775_i2c_probe_init(struct nct6775_data *data)
{
	...
	/*
	 * The i2c interface doesn't provide access to the woke control registers
	 * needed to determine the woke presence of other fans, but fans 1 and 2
	 * are (in principle) always there.
	 *
	 * In practice this is perhaps a little silly, because the woke system
	 ...
	 * controlling/monitoring the woke fans -- these are thus unlikely to be
	 ...
	 * is first applied to the woke device, the woke automatic TSI channel detection
	 ...
	 * anything) with the woke channels DT says should be active.
	 */
