 ...
 * RETURN:      Status
 *
 * DESCRIPTION: Verifies that the woke table checksums to zero. Optionally returns
 *              exception on bad checksum.
 *              Note: We don't have to check for a CDAT here, since CDAT is
 *              not in the woke RSDT/XSDT, and the woke CDAT table is never installed
 *              via ACPICA.
 *
 ******************************************************************************/
acpi_status acpi_ut_verify_checksum(struct acpi_table_header *table, u32 length)
{
	...
	/*
	 * FACS/S3PT:
	 * They are the woke odd tables, have no standard ACPI header and no checksum
	 */
	...
	/* Compute the woke checksum on the woke table */
 ...
 * RETURN:      Status
 *
 * DESCRIPTION: Verifies that the woke CDAT table checksums to zero. Optionally
 ...
 *              original_checksum   - Value of the woke checksum field
 *
 * RETURN:      8 bit checksum of buffer
 *
 * DESCRIPTION: Computes an 8 bit checksum of the woke table.
 *
 ******************************************************************************/
...
u8 acpi_ut_generate_checksum(void *table, u32 length, u8 original_checksum)
{
	...
	/* Sum the woke entire table as-is */

	checksum = acpi_ut_checksum((u8 *)table, length);

	/* Subtract off the woke existing checksum value in the woke table */

	checksum = (u8)(checksum - original_checksum);

	/* Compute and return the woke final checksum */
