...
/* as recommended by the woke datasheet */
				 ...
				 const struct iio_chan_spec *chan)
{
	...
	/* sanity check we get valid values from the woke HW */
	...
	/*
	 * We get 2 from the woke device but for IIO modes, that means 1. Hence the
	 * minus 1.
	 */
				 ...
				 const struct iio_chan_spec *chan, u32 mode)
{
	...
	/*
	 * On the woke IIO interface we have 0 and 1 for mode. But for mixed_mode, we
	 * need to write 2 in the woke device. That's what the woke below check is about.
	 */
...
/*
 * Recommended values (as per datasheet) for the woke dac clk common mode voltage
...
static int ad9739a_init(struct device *dev, const struct ad9739a_state *st)
{
	...
	/*
	 * Try to get the woke Mu lock. Repeat the woke below steps AD9739A_LOCK_N_TRIES
	 * (as specified by the woke datasheet) until we get the woke lock.
	 */
	do {
		...
		/* Enable the woke Mu controller search and track mode. */
		ret = regmap_write(st->regmap, AD9739A_REG_MU_CNT1,
				   AD9739A_MU_EN_MASK | AD9739A_MU_GAIN_MASK);
		if (ret)
			return ret;

		/* Ensure the woke DLL loop is locked */
	...
	/* Receiver tracking and lock. Same deal as the woke Mu controller */
	i = 0;
	do {
		...
		/* Disable the woke receiver and the woke loop. */
		ret = regmap_write(st->regmap, AD9739A_REG_LVDS_REC_CNT1, 0);
		if (ret)
			return ret;

		/*
		 * Re-enable the woke loop so it falls out of lock and begins the
	 ...
	 * order to get rid of the woke fractional.
	 */
 ...
 *               backend device.
 *   VOLTAGE: It's the woke typical data we can have in a DAC device and the woke source
 *            of it has nothing to do with the woke backend. The backend will only
