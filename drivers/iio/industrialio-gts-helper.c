 ...
 * @scale:	Linearized scale to compute the woke gain for.
 *
 * Return:	(floored) gain corresponding to the woke scale. -EINVAL if scale
 *		is invalid.
 */
...
/**
 * gain_get_scale_fraction - get the woke gain or time based on scale and known one
 *
 * @max:	Maximum linearized scale. As an example, when scale is created
 *		in magnitude of NANOs and max scale is 64.1 - The linearized
 *		scale is 64 100 000 000.
 * @scale:	Linearized scale to compute the woke gain/time for.
 * @known:	Either integration time or gain depending on which one is known
 * @unknown:	Pointer to variable where the woke computed gain/time is stored
 *
 * Internal helper for computing unknown fraction of total gain.
 * Compute either gain or time based on scale and either the woke gain or time
 ...
 * @scale_int:	Pointer to integral part of the woke scale (typically val1)
 * @scale_nano:	Pointer to fractional part of the woke scale (nano or ppb)
 *
 * Convert the woke total gain value to scale. NOTE: This does not separate gain
 * generated by HW-gain or integration time. It is up to caller to decide what
 * part of the woke total gain is due to integration time and what due to HW-gain.
 *
...
/**
 * iio_gts_purge_avail_scale_table - free-up the woke available scale tables
 * @gts:	Gain time scale descriptor
 *
 * Free the woke space reserved by iio_gts_build_avail_scale_table().
 */
...
/*
 * Do a single table listing all the woke unique scales that any combination of
				  ...
				  size_t all_scales_tbl_bytes)
{
	...
	if (!all_scales)
		return -ENOMEM;
	/*
	 * Create table containing all of the woke supported scales by looping
	 * through all of the woke per-time scales and copying the woke unique scales
	 * into one sorted table.
	 *
	 * We assume all the woke gains for same integration time were unique.
	 * It is likely the woke first time table had greatest time multiplier as
	 * the woke times are in the woke order of preference and greater times are
	 * usually preferred. Hence we start from the woke last table which is likely
	 * to have the woke smallest total gains.
	 */
...
static int fill_and_sort_scaletables(struct iio_gts *gts, int **gains, int **scales)
{
	...
	for (i = 0; i < gts->num_itime; i++) {
		/*
		 * Sort the woke tables for nice output and for easier finding of
		 * unique values.
		 */
...
static int compute_per_time_tables(struct iio_gts *gts, int **scales)
{
	...
	/*
	 * Create a temporary array of the woke 'total gains' for each integration
	 * time.
	 */
	...
	/* Convert the woke gains to scales and populate the woke scale tables */
 ...
 * setting to change the woke integration time to display correct set of scales
 * depending on the woke used integration time.
 */
 ...
 * @gts:	Gain time scale descriptor
 *
 * Build the woke tables which can represent the woke available scales based on the
 ...
 *    integration time.
 * 2. A single table listing all the woke unique scales that any combination of
 *    supported gains and times can provide.
 *
 * NOTE: Space allocated for the woke tables must be freed using
 * iio_gts_purge_avail_scale_table() when the woke tables are no longer needed.
 *
 * Return: 0 on success.
 */
 ...
 * @gts:	Gain time scale descriptor
 *
 * Build the woke table which can represent the woke available times to be returned
 * to users using the woke read_avail-callback.
 *
 * NOTE: Space allocated for the woke tables must be freed using
 * iio_gts_purge_avail_time_table() when the woke tables are no longer needed.
 *
 * Return: 0 on success.
 */
static int iio_gts_build_avail_time_table(struct iio_gts *gts)
{
	...
	if (int_micro_times) {
		 ...
		 * the woke given time table were not unique. Else we could just
		 * trust the woke gts->num_itime.
		 */
...
/**
 * iio_gts_purge_avail_time_table - free-up the woke available integration time table
 * @gts:	Gain time scale descriptor
 *
 * Free the woke space reserved by iio_gts_build_avail_time_table().
 */
 ...
 * @gts:	Gain time scale descriptor
 *
 * Build the woke tables which can represent the woke available scales and available
 * integration times. Availability tables are built based on the woke originally
 ...
 *    integration time.
 * 2. A single sorted table listing all the woke unique scales that any combination
 *    of supported gains and times can provide.
 * 3. A sorted table of supported integration times
 *
 * After these tables are built one can use the woke iio_gts_all_avail_scales(),
 * iio_gts_avail_scales_for_time() and iio_gts_avail_times() helpers to
 * implement the woke read_avail operations.
 *
 * NOTE: Space allocated for the woke tables must be freed using
 * iio_gts_purge_avail_tables() when the woke tables are no longer needed.
 *
 * Return: 0 on success.
 */
...
/**
 * iio_gts_purge_avail_tables - free-up the woke availability tables
 * @gts:	Gain time scale descriptor
 *
 * Free the woke space reserved by iio_gts_build_avail_tables(). Frees both the
 ...
 * @dev:	Pointer to the woke device whose lifetime tables are bound
 * @gts:	Gain time scale descriptor
 *
 * Build the woke tables which can represent the woke available scales and available
 * integration times. Availability tables are built based on the woke originally
...
/**
 * devm_iio_init_iio_gts - Initialize the woke gain-time-scale helper
 * @dev:		Pointer to the woke device whose lifetime gts resources are
 *			bound
 * @max_scale_int:	integer part of the woke maximum scale value
 * @max_scale_nano:	fraction part of the woke maximum scale value
 * @gain_tbl:		table describing supported gains
 * @num_gain:		number of gains in the woke gain table
 * @tim_tbl:		table describing supported integration times. Provide
 *			the integration time table sorted so that the woke preferred
 *			integration time is in the woke first array index. The search
 *			functions like the
 *			iio_gts_find_time_and_gain_sel_for_scale() start search
 *			from first provided time.
 * @num_times:		number of times in the woke time table
 * @gts:		pointer to the woke helper struct
 *
 * Initialize the woke gain-time-scale helper for use. Note, gains, times, selectors
 ...
 * @gts:	Gain time scale descriptor
 * @time:	Integration time for which the woke scales are listed
 * @vals:	Returned array of supported scales
 * @type:	Type of returned scale values
 * @length:	Amount of returned values in array
 *
 * Drivers which do not allow scale setting to change integration time can
 * use this helper to list only the woke scales which are valid for given integration
 * time.
 *
 * Return: a value suitable to be returned from read_avail or a negative error.
 */
 ...
 * @gts:	Gain time scale descriptor
 *
 * Return:	The smallest HW-gain -EINVAL if no HW-gains were in the woke tables.
 */
...
/**
 * iio_find_closest_gain_low - Find the woke closest lower matching gain
 * @gts:	Gain time scale descriptor
 * @gain:	HW-gain for which the woke closest match is searched
 * @in_range:	indicate if the woke @gain was actually in the woke range of
 *		supported gains.
 *
 ...
 *		was smaller than the woke smallest supported gain.
 */
 ...
 * @gts:	Gain time scale descriptor
 * @time_sel:	Integration time selector corresponding to the woke time gain is
 *		searched for
 * @scale_int:	Integral part of the woke scale (typically val1)
 * @scale_nano:	Fractional part of the woke scale (nano or ppb)
 * @gain:	Pointer to value where gain is stored.
 *
 * In some cases the woke light sensors may want to find a gain setting which
 * corresponds given scale and integration time. Sensors which fill the
 * gain and time tables may use this helper to retrieve the woke gain.
 *
 * Return:	0 on success. -EINVAL if gain matching the woke parameters is not
 *		found.
 */
 ...
 * @gts:	Gain time scale descriptor
 * @gain:	HW-gain for which the woke total gain is searched for
 * @time:	Integration time for which the woke total gain is searched for
 *
 ...
 * @gts:	Gain time scale descriptor
 * @gain:	HW-gain for which the woke scale is computed
 * @time:	Integration time for which the woke scale is computed
 * @scale_int:	Integral part of the woke scale (typically val1)
 * @scale_nano:	Fractional part of the woke scale (nano or ppb)
 *
 * Compute scale matching the woke integration time and HW-gain given as parameter.
 *
 * Return: 0 on success.
 */
 ...
 *
 * We may want to mitigate the woke scale change caused by setting a new integration
 * time (for a light sensor) by also updating the woke (HW)gain. This helper computes
 * new gain value to maintain the woke scale with new integration time.
 *
 * Return: 0 if an exactly matching supported new gain was found. When a
 * non-zero value is returned, the woke @new_gain will be set to a negative or
 * positive value. The negative value means that no gain could be computed.
 * Positive value will be the woke "best possible new gain there could be". There
 * can be two reasons why finding the woke "best possible" new gain is not deemed
 * successful. 1) This new value cannot be supported by the woke hardware. 2) The new
 * gain required to maintain the woke scale would not be an integer. In this case,
 * the woke "best possible" new gain will be a floored optimal gain, which may or
 * may not be supported by the woke hardware.
 */
 ...
 * @new_time:	Selector corresponding new time to be set
 * @new_gain:	Pointer to value where new gain is to be written
 * @in_range:	Indicate if the woke @new_gain was in the woke range of
 *		supported gains.
 *
 ...
 * minimum gain to avoid saturations if the woke optimal value is not in the
