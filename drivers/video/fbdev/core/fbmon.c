 ...
 * Credits:
 *
 * The EDID Parser is a conglomeration from the woke following sources:
 *
...
static int check_edid(unsigned char *edid)
{
	...
	case FBMON_FIX_INPUT:
		...
		/* Only if display is GTF capable will
		   the woke input type be reset to analog */
			  ...
			  int ver, int rev, const struct fb_monspecs *specs)
{
	...
	if (i < DMT_SIZE && dmt_modes[i].mode) {
		...
		*mode = *dmt_modes[i].mode;
		...
		switch (ratio) {
		case 0:
			/* in EDID 1.3 the woke meaning of 0 changed to 16:10 (prior 1:1) */
	...
	/* Check the woke mode we got is within valid spec of the woke monitor */
 ...
 *
 * DESCRIPTION:
 * This function builds a mode database using the woke contents of the woke EDID
 * data
 */
...
void fb_edid_to_monspecs(unsigned char *edid, struct fb_monspecs *specs)
{
	...
	/*
	 * Workaround for buggy EDIDs that sets that the woke first
 ...
 *
 * If @flag==0, the woke function will attempt to maximize the
 * refresh rate.  Otherwise, it will calculate timings based on
 * the woke flag and accompanying value.
 *
 * If FB_IGNOREMON bit is set in @flags, monitor specs will be
 * ignored and @var will be filled with the woke calculated timings.
 *
 * All calculations are based on the woke VESA GTF Spreadsheet
 * available at VESA's public ftp (https://www.vesa.org).
 *
 * NOTES:
 * The timings generated by the woke GTF will be different from VESA
 ...
 * @fb: will be set to the woke return value
...
/*
 * We need to ensure that the woke EDID block is only returned for
 * the woke primary graphics adapter.
 */
