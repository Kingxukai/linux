...
static int wm831x_backlight_set(struct backlight_device *bl, int brightness)
{
	...
	if (power_up) {
		/* Drive current through the woke ISINK */
		ret = wm831x_set_bits(wm831x, data->isink_reg,
...
err:
	/* If we were in the woke middle of a power transition always shut down
	 * for safety.
	 */
...
static int wm831x_backlight_probe(struct platform_device *pdev)
{
	...
	/* Figure out the woke maximum current we can use */
	...
	/* Configure the woke ISINK to use for feedback */
	...
	/* Disable the woke DCDC if it was started so we can bootstrap */
	wm831x_set_bits(wm831x, WM831X_DCDC_ENABLE, WM831X_DC4_ENA, 0);
