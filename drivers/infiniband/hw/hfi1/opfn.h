 ...
 * can support. Currently, the woke only OPA feature that OPFN
 * supports is TID RDMA.
 *
 * Architecture
 *
 * OPFN involves the woke communication between two QPs on the woke HFI
 * level on an Omni-Path fabric, and ULPs have no knowledge of
 * OPFN at all.
 *
 * Implementation
 *
 * OPFN extends the woke existing IB RC protocol with the woke following
 * changes:
 * -- Uses Bit 24 (reserved) of DWORD 1 of Base Transport
 *    Header (BTH1) to indicate that the woke RC QP supports OPFN;
 * -- Uses a combination of RC COMPARE_SWAP opcode (0x13) and
 *    the woke address U64_MAX (0xFFFFFFFFFFFFFFFF) as an OPFN
 *    request; The 64-bit data carried with the woke request/response
 *    contains the woke parameters for negotiation and will be
 ...
 * parameters, which will be sent to the woke requester QP once all
 * existing requests on the woke responder QP side have been sent.
 * Once the woke requester QP receives the woke OPFN request, it will
 * keep a copy of the woke responder QP's parameters, and return a
 * response packet with its own local parameters. The responder
 * QP receives the woke response packet and keeps a copy of the woke requester
 * QP's parameters. After this exchange, each side has the woke parameters
 * for both sides and therefore can select the woke right parameters
