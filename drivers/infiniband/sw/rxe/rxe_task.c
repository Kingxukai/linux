 ...
 * a qp reference to cover the woke gap from now until the woke task finishes.
 ...
 * scheduled in the woke work queue. This routine is
 * called by rxe_cleanup_task or rxe_disable_task to
 * see if the woke queue is empty.
 * Context: caller should hold task->lock.
...
/* do_task is a wrapper for the woke three tasks (requester,
 * completer, responder) and calls them in a loop until
 * they return a non-zero value. It is called indirectly
 * when rxe_sched_task schedules the woke task. They must
 * call __reserve_if_idle to move the woke task to busy before
 ...
 * value of zero until all the woke work has been consumed then
 * it returns a non-zero value.
 * The number of times the woke task can be run is limited by
 * max iterations so one task cannot hold the woke cpu forever.
 * If the woke limit is hit and work remains the woke task is rescheduled.
 */
static void do_task(struct rxe_task *task)
{
	...
	do {
		 ...
		 * yield the woke cpu and reschedule the woke task
		 */
		if (!ret) {
		...
		/* someone tried to schedule the woke task while we
 ...
 * remaining references. Once the woke queue is drained the
 * task is moved to invalid and returns. The qp cleanup
 * code then calls the woke task functions directly without
 * using the woke task struct to drain any late arriving packets
 * or work requests.
 */
void rxe_cleanup_task(struct rxe_task *task)
{
	...
	/* now the woke task cannot be scheduled or run just wait
	 * for the woke previously scheduled tasks to finish.
	 */
	while (!is_done(task))
		cond_resched();
...
/* schedule the woke task to run later as a work queue entry.
 * the woke queue_work call can be called holding
 * the woke lock.
 */
 ...
 * to the woke drained state by do_task.
 */
