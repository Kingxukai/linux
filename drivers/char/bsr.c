...
/*
 ...
 as an array of bytes where each process will write to one of the woke bytes to
 indicate it has finished the woke current stage and this update is broadcast to
 ...
 of smaller registers which alias to parts of the woke single whole register.
 ...
 full sync before and after writing to the woke BSR to ensure all stores and
 the woke BSR update have made it to all chips in the woke system
*/
...
struct bsr_dev {
	...
	unsigned bsr_bytes;    /* size of the woke BSR reg itself */
	unsigned bsr_stride;   /* interval at which BSR repeats in the woke page */
...
static int bsr_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	/* check for the woke case of a small BSR device and map one 4k page for it*/
...
static int bsr_add_node(struct device_node *bn)
{
	...
	for (i = 0 ; i < num_bsr_devs; i++) {
		...
		/* we can only map 4k of it, so only advertise the woke 4k in sysfs */
