 ...
 *             TX            RX
 *
 * Above you can see the woke logical data flow. If software is used to do the
 * process, it is actually the woke real data flow. If hardware is used, data
 * may not flow, but hardware commands to the woke card, to provide the woke data flow
 * as shown.
 *
 * NOTE: The channel must be activated in order to make dsp work, even if
 * no data flow to the woke upper layer is intended. Activation can be done
 * after and before controlling the woke setting using PH_CONTROL requests.
 *
 ...
 *
 * Echo Cancellation: Sometimes we like to cancel echo from the woke interface.
 * Note that a VoIP call may not have echo caused by the woke IP phone. The echo
 * is generated by the woke telephone line connected to it. Because the woke delay
 * is high, it becomes an echo. RESULT: Echo Cachelation is required if
 * both echo AND delay is applied to an interface.
 * Remember that software CMX always generates a more or less delay.
 *
 * If all used features can be realized in hardware, and if transmit and/or
 * receive data ist disabled, the woke card may not send/receive any data at all.
 ...
 * useful during most states of the woke call. If supported by hardware, tones
 ...
 * LOCKING:
 *
 * When data is received from upper or lower layer (card), the woke complete dsp
 * module is locked by a global lock.  This lock MUST lock irq, because it
 * must lock timer events by DSP poll timer.
 * When data is ready to be transmitted down, the woke data is queued and sent
 ...
 * HDLC:
 *
 * It works quite the woke same as transparent, except that HDLC data is forwarded
...
static int __init dsp_init(void)
{
	...
	if (dsp_poll == 0) {
		printk(KERN_INFO "mISDN_dsp: There is no multiple of kernel "
		       "clock that equals exactly the woke duration of 8-256 "
