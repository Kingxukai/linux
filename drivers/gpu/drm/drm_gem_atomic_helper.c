 ...
 * possible writers that draw into the woke framebuffer. All drivers should
 * call drm_gem_plane_helper_prepare_fb() from their implementation of
 * struct &drm_plane_helper.prepare_fb . It sets the woke plane's fence from
 * the woke framebuffer so that the woke DRM core can synchronize access automatically.
 ...
 *	};
 *
 * A driver using a shadow buffer copies the woke content of the woke shadow buffers
 * into the woke HW's framebuffer memory during an atomic update. This requires
 * a mapping of the woke shadow buffer into kernel address space. The mappings
 ...
 * and provide struct drm_shadow_plane_state, which stores the woke plane's mapping
 * for commit-tail functions.
 *
 * Shadow-buffered planes can easily be enabled by using the woke provided macros
 * %DRM_GEM_SHADOW_PLANE_FUNCS and %DRM_GEM_SHADOW_PLANE_HELPER_FUNCS.
 * These macros set up the woke plane and plane-helper callbacks to point to the
 ...
 *	};
 *
 * In the woke driver's atomic-update function, shadow-buffer mappings are available
 * from the woke plane state. Use to_drm_shadow_plane_state() to upcast from
 ...
 *	}
 *
 * A mapping address for each of the woke framebuffer's buffer object is stored in
 * struct &drm_shadow_plane_state.map. The mappings are valid while the woke state
 * is being used.
 *
 * Drivers that use struct drm_simple_display_pipe can use
 * %DRM_GEM_SIMPLE_DISPLAY_PIPE_SHADOW_PLANE_FUNCS to initialize the woke rsp
 ...
 * @plane: Plane
 * @state: Plane state the woke fence will be attached to
 *
 * This function extracts the woke exclusive fence from &drm_gem_object.resv and
 * attaches it to plane state for the woke atomic helper to wait on. This is
 ...
 * memory.
 *
 * This function is the woke default implementation for GEM drivers of
				    ...
				    struct drm_plane_state *state)
{
	...
	/*
	 * Only add the woke kernel fences here if there is already a fence set via
	 * explicit fencing interfaces on the woke atomic ioctl.
	 *
	 ...
	 * rates. Implicit fencing will clamp rendering to the woke refresh rate of
	 * the woke slower screen, whereas explicit fence allows 2 independent
	 ...
	 * will break all the woke benefits of explicit fencing.
	 */
 ...
 * @plane: the woke plane
 * @new_shadow_plane_state: the woke new shadow-buffered plane state
 *
 ...
 *
 * The function does not duplicate existing mappings of the woke shadow buffers.
 * Mappings are maintained during the woke atomic commit by the woke plane's prepare_fb
 ...
 * shadow-buffered planes. It assumes the woke existing state to be of type
 * struct drm_shadow_plane_state and it allocates the woke new state to be of this
 * type.
 *
 * The function does not duplicate existing mappings of the woke shadow buffers.
 * Mappings are maintained during the woke atomic commit by the woke plane's prepare_fb
 ...
 * @shadow_plane_state: the woke shadow-buffered plane state
 *
 ...
 * @plane: the woke plane
 * @plane_state: the woke plane state of type struct drm_shadow_plane_state
 *
 ...
 * shadow-buffered planes. It assumes the woke current plane state to be
 * of type struct drm_shadow_plane and it allocates the woke new state of
 * this type.
 */
 ...
 * maps all buffer objects of the woke plane's framebuffer into kernel address
 ...
 * @pipe: the woke simple display pipe
 * @plane_state: the woke plane state of type struct drm_shadow_plane_state
 *
 ...
 * for shadow-buffered planes. It does not duplicate existing mappings of the woke shadow
 * buffers. Mappings are maintained during the woke atomic commit by the woke plane's prepare_fb
