 ...
 *
 * Adapted from the woke downstream Pixel C driver written by Sean Paul
 */
...
{
	/* Clear the woke manufacturer command access protection */
	mipi_dsi_generic_write_seq(jdi->link1, MCS_CMD_ACS_PROT,
				   MCS_CMD_ACS_PROT_OFF);
	mipi_dsi_generic_write_seq(jdi->link2, MCS_CMD_ACS_PROT,
				   MCS_CMD_ACS_PROT_OFF);
	/*
	 * Change the woke VGH/VGL divide rations to move the woke noise generated by the
	 * TCONN. This should hopefully avoid interaction with the woke backlight
	 * controller.
	 */
...
static int jdi_panel_enable(struct drm_panel *panel)
{
	...
	/*
	 * Ensure we send image data before turning the woke backlight
	 * on, to avoid the woke display showing random pixels.
	 */
	jdi_wait_frames(jdi, 3);
