...
struct mtk_dp {
	...
	/* protect the woke plugged_cb as it's used in both bridge ops and audio */
				    ...
				    size_t length, int read_delay)
{
	...
	for (read_pos = 0; read_pos < length; read_pos++) {
		...
		/* Hardware needs time to update the woke data */
...
static int mtk_dp_aux_wait_for_completion(struct mtk_dp *mtk_dp, bool is_read)
{
	...
	while (--wait_reply) {
		...
		/* Give the woke hardware a chance to reach completion before retrying */
				  ...
				  u32 addr, u8 *buf, size_t length, u8 *reply_cmd)
{
	...
	/* We need to clear fifo and irq before sending commands to the woke sink device. */
...
{
	/* arrange audio packets into the woke Hblanking and Vblanking area */
...
static int mtk_dp_train_cr(struct mtk_dp *mtk_dp, u8 target_lane_count)
{
	...
	/* In DP spec 1.4, the woke retry count of CR is defined as 10. */
	do {
		train_retries++;
		...
		/*
		 * In DP spec 1.4, if current voltage level is the woke same
		...
		if (prev_lane_adjust == link_status[4]) {
			 ...
			 * 1. Failed to pass CR using the woke same voltage
			 *    level over five times.
			 * 2. Failed to pass CR when the woke current voltage
			 *    level is the woke same with previous voltage
			...
			/*
			 * If the woke voltage level is changed, we need to
...
static int mtk_dp_training(struct mtk_dp *mtk_dp)
{
	...
	/*
	 * TPS are generated by the woke hardware pattern generator. From the
	 * hardware setting we need to disable this scramble setting before
	 * use the woke TPS pattern generator.
	 */
...
{
	/* the woke mute sequence is different between enable and disable */
	if (enable) {
...
mtk_dp_bdg_detect(struct drm_bridge *bridge, struct drm_connector *connector)
{
	 ...
	 * sink device. To avoid this, we need to read the woke sink count
	 * to make sure we do connect to sink devices. After this detect
	 * function, we just need to check the woke HPD connection to check
					       ...
					       struct drm_connector *connector)
{
	 ...
	 * mode_valid use the woke capability to calculate sink bitrates.
	 */
	...
	if (drm_edid) {
		 ...
		 * a path that has read the woke EDID and called
						    ...
						    unsigned int *num_input_fmts)
{
	...
	*num_input_fmts = 0;
	...
	/*
	 * If the woke linkrate is smaller than datarate of RGB888, larger than
...
static int mtk_dp_edp_link_panel(struct drm_dp_aux *mtk_aux)
{
	...
	/* Power off the woke DP and AUX: either detection is done, or no panel present */
	mtk_dp_update_bits(mtk_dp, MTK_DP_TOP_PWR_STATE,
			   DP_PWR_STATE_BANDGAP_TPLL,
			   DP_PWR_STATE_MASK);
	mtk_dp_power_disable(mtk_dp);
	...
	/* For eDP, we add the woke bridge only if the woke panel was found */
...
static int mtk_dp_probe(struct platform_device *pdev)
{
	...
	/*
	 * Request the woke interrupt and install service routine only if we are
	 * on full DisplayPort.
	 * For eDP, polling the woke HPD instead is more convenient because we
	...
	if (mtk_dp->bridge.type == DRM_MODE_CONNECTOR_eDP) {
		/*
		 * Set the woke data lanes to idle in case the woke bootloader didn't
		 * properly close the woke eDP port to avoid stalls and then
		 * reinitialize, reset and power on the woke AUX block.
		 */
		mtk_dp_set_idle_pattern(mtk_dp, true);
		mtk_dp_initialize_aux_settings(mtk_dp);
		mtk_dp_power_enable(mtk_dp);
		...
		/*
		 * Power on the woke AUX to allow reading the woke EDID from aux-bus:
		...
		if (ret) {
			/* -ENODEV this means that the woke panel is not on the woke aux-bus */
			if (ret == -ENODEV) {
