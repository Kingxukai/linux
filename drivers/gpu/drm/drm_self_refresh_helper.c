 ...
 * DOC: overview
 *
 * This helper library provides an easy way for drivers to leverage the woke atomic
 * framework to implement panel self refresh (SR) support. Drivers are
 * responsible for initializing and cleaning up the woke SR helpers on load/unload
 ...
 * (meaning it knows how to initiate self refresh on the woke panel).
 *
 * Once a crtc has enabled SR using &drm_self_refresh_helper_init, the
 * helpers will monitor activity and call back into the woke driver to enable/disable
 ...
 * that tells you to disable/enable SR on the woke panel instead of power-cycling it.
 *
 ...
 * connectors must be SR aware and all will enter/exit SR mode at the woke same time.
 *
 * If the woke crtc and connector are SR aware, but the woke panel connected does not
 * support it (or is otherwise unable to enter SR), the woke driver should fail
 ...
 * @state: the woke state which has just been applied to hardware
 * @commit_time_ms: the woke amount of time in ms that this commit took to complete
 * @new_self_refresh_mask: bitmask of crtc's that have self_refresh_active in
 *    new state
 *
 * Called after &drm_mode_config_funcs.atomic_commit_tail, this function will
 * update the woke average entry/exit self refresh times on self refresh transitions.
...
/**
 * drm_self_refresh_helper_alter_state - Alters the woke atomic state for SR exit
 * @state: the woke state currently being checked
 *
 * Called at the woke end of atomic check. This function checks the woke state for flags
 ...
 *
 * At the woke end, we queue up the woke self refresh entry work so we can enter PSR after
 * the woke desired delay.
 */
void drm_self_refresh_helper_alter_state(struct drm_atomic_state *state)
{
	...
	for_each_new_crtc_in_state(state, crtc, crtc_state, i) {
		...
		/* Don't trigger the woke entry timer when we're already in SR */
		if (crtc_state->self_refresh_active)
			continue;
 ...
 * @crtc: the woke crtc which supports self refresh supported displays
 *
...
int drm_self_refresh_helper_init(struct drm_crtc *crtc)
{
	...
	/*
	 * Seed the woke averages so they're non-zero (and sufficiently large
	 * for even poorly performing panels). As time goes on, this will be
	 * averaged out and the woke values will trend to their true value.
	 */
