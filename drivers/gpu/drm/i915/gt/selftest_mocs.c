...
static struct intel_context *mocs_context_create(struct intel_engine_cs *engine)
{
	...
	/* We build large requests to read the woke registers from the woke ring */
	ce->ring_size = SZ_16K;
...
{
	/*
	 * Registers in this range are affected by the woke MCR selector
...
static int check_l3cc_table(struct intel_engine_cs *engine,
			    ...
			    u32 **vaddr)
{
	/* Can we read the woke MCR range 0xb00 directly? See intel_workarounds! */
			     ...
			     struct intel_context *ce)
{
	...
	/* Read the woke mocs tables back using SRM */
	...
	/* Compare the woke results against the woke expected tables */
	vaddr = arg->vaddr;
	if (!err)
...
static int live_mocs_clean(void *arg)
{
	...
	/* Every new context should see the woke same mocs table */
...
static int live_mocs_reset(void *arg)
{
	...
	/* Check the woke mocs setup is retained over per-engine and global resets */
