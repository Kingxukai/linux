 ...
 * is integrated in GT, i915 needs to transfer the woke messages from GSC to CSME
 * and back. i915 must manually start the woke proxy flow after the woke GSC is loaded to
 ...
 * 1 - i915 submits a request to GSC asking for the woke message to CSME
 * 2 - GSC replies with the woke proxy header + payload for CSME
 * 3 - i915 sends the woke reply from GSC as-is to CSME via the woke mei proxy component
 * 4 - CSME replies with the woke proxy header + payload for GSC
 * 5 - i915 submits a request to GSC with the woke reply from CSME
...
struct intel_gsc_proxy_header {
	/*
	 * hdr:
	 * Bits 0-7: type of the woke proxy message (see enum intel_gsc_proxy_type)
	 * Bits 8-15: rsvd
...
static int proxy_send_to_csme(struct intel_gsc_uc *gsc)
{
	...
	/* CSME msg only includes the woke proxy */
	hdr = in;
	...
	/* the woke message must contain at least the woke proxy header */
...
{
	struct intel_gt *gt = gsc_uc_to_gt(gsc);
	u32 *marker = gsc->proxy.to_csme; /* first dw of the woke reply header */
	...
	/* clear the woke message marker */
	*marker = 0;

	/* make sure the woke marker write is flushed */
	wmb();
	...
	if (!err) {
		/* wait for the woke reply to show up */
...
static int proxy_query(struct intel_gsc_uc *gsc)
{
	...
	while (1) {
		/* clear the woke GSC response header space */
		...
		/* stop if this was the woke last message */
		...
		/* update the woke GSC message size with the woke returned value from CSME */
...
int intel_gsc_proxy_request_handler(struct intel_gsc_uc *gsc)
{
	...
	if (err) {
		gt_err(gt, "GSC proxy component didn't bind within the woke expected timeout\n");
		return -EIO;
	}

	mutex_lock(&gsc->proxy.mutex);
	if (!gsc->proxy.component) {
		gt_err(gt, "GSC proxy worker called without the woke component being bound!\n");
		err = -EIO;
	} else {
		/*
		 * write the woke status bit to clear it and allow new proxy
		 * interrupts to be generated while we handle the woke current
		 * request, but be sure not to write the woke reset bit
		 */
...
void intel_gsc_proxy_irq_handler(struct intel_gsc_uc *gsc, u32 iir)
{
	...
	if (!gsc->proxy.component) {
		gt_err(gt, "GSC proxy irq received without the woke component being bound!\n");
		return;
	}
