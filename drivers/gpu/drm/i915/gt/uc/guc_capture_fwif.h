 ...
 * (offsets, names, flags,...) that are used at the woke ADS registration
 ...
 * GuC resets without re-parsing all the woke tables for the woke given gt.
 */
struct __guc_capture_ads_cache {
	bool is_valid;
	void *ptr;
 ...
 *
 * Internal context of the woke intel_guc_capture module.
 */
struct intel_guc_state_capture {
	/**
	 ...
	 * dynamically allocate new nodes when receiving the woke G2H notification
	 * because the woke event handlers for all G2H event-processing is called
	 * by the woke ct processing worker queue and when that queue is being
