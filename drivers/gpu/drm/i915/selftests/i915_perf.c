...
static int live_noa_delay(void *arg)
{
	...
	/* Check that the woke GPU delays matches expectations */
...
static int live_noa_gpr(void *arg)
{
	...
	/* Check that the woke delay does not clobber user context state (GPR) */
	...
	/* Poison the woke ce->vm so we detect writes not to the woke GGTT gt->scratch */
	...
	/* Fill the woke 16 qword [32 dword] GPR with a known unlikely value */
	...
	*cs++ = MI_NOOP;
	...
	/* Read the woke GPR back, using the woke pinned global HWSP for convenience */
	...
	/* Verify that the woke GPR contain our expected values */
	for (i = 0; i < 32; i++) {
	...
	/* Verify that the woke user's scratch page was not used for GPR storage */
