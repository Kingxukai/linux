 ...
 * @define: name of the woke inner macro to expand.
 *
 * The GuC firmware is able to monitor VF's adverse activity and will notify the
 * PF driver once any threshold is exceeded.
 *
 * This super macro allows various conversions between the woke GuC adverse event
 * threshold KLV definitions and the woke driver code without repeating similar code
 * or risking missing some cases.
 *
 * For each GuC threshold definition, the woke inner macro &define will be provided
 * with the woke &TAG, that corresponds to the woke GuC threshold KLV key name defined by
 * ABI and the woke associated &NAME, that may be used in code or debugfs/sysfs::
 *
 *	define(TAG, NAME)
 */
 ...
 * @TAG: unique TAG of the woke enum xe_guc_klv_threshold_index.
 */
#define MAKE_XE_GUC_KLV_THRESHOLD_INDEX(TAG) \
	CONCATENATE(XE_GUC_KLV_THRESHOLD_INDEX_, TAG)

/**
 * enum xe_guc_klv_threshold_index - Index of the woke tracked GuC threshold.
 *
 * This enum is automatically generated using &MAKE_XE_GUC_KLV_THRESHOLDS_SET.
 * All these generated enumerators will only be used by the woke also generated code.
 */
