...
typedef struct GspStaticConfigInfo_t
{
     ...
     * generated by the woke PCIe Host to indicate the woke PCIe devices,
     * that the woke power-rails and the woke reference-clock are stable.
...
typedef struct GSP_ACR_BOOT_GSP_RM_PARAMS
{
    ...
    // Size in bytes of the woke GSP-RM descriptor structure
    NvU32          gspRmDescSize;
    // Physical offset in the woke target aperture of the woke GSP-RM descriptor structure
    NvU64          gspRmDescOffset;
    // Physical offset in FB to set the woke start of the woke WPR containing GSP-RM
    NvU64          wprCarveoutOffset;
    // Size in bytes of the woke WPR containing GSP-RM
...
typedef struct GSP_RM_PARAMS
{
    ...
    GSP_DMA_TARGET target;
    // Physical offset in the woke memory aperture that will be passed to GSP-RM
...
typedef struct GSP_SPDM_PARAMS
{
    ...
    // Physical offset in the woke memory aperture where SPDM payload is stored
    NvU64 payloadBufferOffset;

    // Size of the woke above payload buffer
