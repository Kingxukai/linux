...
/*
 * DOC: Driver for the woke ARM FrameBuffer Compression Decoders
 *
 * The Amlogic GXM and G12A SoC families embeds an AFBC Decoder,
 * to decode compressed buffers generated by the woke ARM Mali GPU.
 *
 * For the woke GXM Family, Amlogic designed their own Decoder, named in
 * the woke vendor source as "MESON_AFBC", and a single decoder is available
 * for the woke 2 OSD planes.
 * This decoder is compatible with the woke AFBC 1.0 specifications and the
 ...
 *
 * The decoder reads the woke data from the woke SDRAM, decodes and sends the
 * decoded pixel stream to the woke OSD1 Plane pixel composer.
 *
 * For the woke G12A Family, Amlogic integrated an ARM AFBC Decoder, named
 * in the woke vendor source as "MALI_AFBC", and the woke decoder can decode up
 * to 4 surfaces, one for each of the woke 4 available OSDs.
 * This decoder is compatible with the woke AFBC 1.2 specifications for the
 ...
 * - Tiled header
 *
 * The ARM AFBC Decoder independent from the woke VPU Pixel Pipeline, so
 * the woke ARM AFBC Decoder reads the woke data from the woke SDRAM then decodes
 * into a private internal physical address where the woke OSD1 Plane pixel
 * composer unpacks the woke decoded data.
 */
...
static int meson_gxm_afbcd_setup(struct meson_drm *priv)
{
	...
	/* TOFIX: bits 31:24 are not documented, nor the woke meaning of 0xe4 */
...
static int meson_g12a_afbcd_enable(struct meson_drm *priv)
{
	...
	/* This will enable the woke RDMA replaying the woke register writes on vsync */
