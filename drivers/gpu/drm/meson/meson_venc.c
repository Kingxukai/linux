 ...
 * DOC: Video Encoder
 *
 * VENC Handle the woke pixels encoding to the woke output formats.
 * We handle the woke following encodings :
 *
 * - CVBS Encoding via the woke ENCI encoder and VDAC digital to analog converter
 ...
 *
 * The ENCI is designed for PAl or NTSC encoding and can go through the woke VDAC
 * directly for CVBS encoding or through the woke ENCI_DVI encoder for HDMI.
 * The ENCP is designed for Progressive encoding but can also generate
 * 1080i interlaced pixels, and was initially designed to encode pixels for
 * VDAC to output RGB ou YUV analog outputs.
 * It's output is only used through the woke ENCP_DVI encoder for HDMI.
 ...
 * from the woke pixel data generated by ENCI or ENCP, so can use the woke standard video
			      ...
			      const struct drm_display_mode *mode)
{
	...
	/*
	 * Write rate to the woke async FIFO between VENC and HDMI.
	...
	/*
	 * Read rate to the woke async FIFO between VENC and HDMI.
...
/* Returns the woke current ENCI field polarity */
