 ...
 * all claims this bit is 'HseDisableMode' with the woke definition
 * 0 = Disables transfer
 * 1 = Enables transfer
 *
 * This clearly states that HSE is not a disabled bit.
 *
 * The naming convention follows as per the woke manual and the
 * driver logic is based on the woke MIPI_DSI_MODE_VIDEO_HSE flag.
 */
					  ...
					  unsigned long freq)
{
	...
	if (dsi->pll_clk) {
		/*
		 * Ensure that the woke reference clock is generated with a power of
		 * two divider from its parent, but close to the woke PLLs upper
		 * limit.
		 */
...
static int samsung_dsim_enable_clock(struct samsung_dsim *dsi)
{
	...
	/* Use burst_clk_rate if available, otherwise use the woke pix_clk */
...
static void samsung_dsim_set_phy_ctrl(struct samsung_dsim *dsi)
{
	 ...
	 * and Plus don't state what the woke definition of the woke PHYTIMING
	 * bits are beyond their address and bit position.
	 * After reviewing NXP's downstream code, it appears
	 * that the woke various PHYTIMING registers take the woke number
	 * of cycles and use various dividers on them.  This
	 * calculation does not result in an exact match to the
	 * downstream code, but it is very close to the woke values
	 ...
	 * T HS-EXIT: Time that the woke transmitter drives LP-11 following a HS
	 *	burst
	 */
	...
	/*
	 * T CLK-PREPARE: Time that the woke transmitter drives the woke Clock Lane LP-00
	 *	Line state immediately before the woke HS-0 Line state starting the
	 *	HS transmission
	 * T CLK-ZERO: Time that the woke transmitter drives the woke HS-0 state prior to
	 *	transmitting the woke Clock.
	 * T CLK_POST: Time that the woke transmitter continues to send HS clock
	 *	after the woke last associated Data Lane has transitioned to LP Mode
	 *	Interval is defined as the woke period from the woke end of T HS-TRAIL to
	 *	the beginning of T CLK-TRAIL
	 * T CLK-TRAIL: Time that the woke transmitter drives the woke HS-0 state after
	...
	/*
	 * T HS-PREPARE: Time that the woke transmitter drives the woke Data Lane LP-00
	 *	Line state immediately before the woke HS-0 Line state starting the
	 *	HS transmission
	 * T HS-ZERO: Time that the woke transmitter drives the woke HS-0 state prior to
	 *	transmitting the woke Sync sequence.
	 * T HS-TRAIL: Time that the woke transmitter drives the woke flipped differential
...
static int samsung_dsim_init_link(struct samsung_dsim *dsi)
{
	...
	/*
	 * Use non-continuous clock mode if the woke periparal wants and
	 * host controller supports
	 *
	 * In non-continous clock mode, host controller will turn off
	 * the woke HS clock between high-speed transmissions to reduce
	 * power consumption.
	 */
	...
	do {
		...
		if (!dsi->driver_data->has_broken_fifoctrl_emptyhdr) {
			...
			if (!(reg & DSIM_SFR_HEADER_FULL)) {
				/*
				 * Wait a little bit, so the woke pending data can
				 * actually leave the woke FIFO to avoid overflow.
				 */
				if (!cond_resched())
					   ...
					   struct drm_atomic_state *state)
{
	...
	/*
	 * For Exynos-DSIM the woke downstream bridge, or panel are expecting
	 * the woke host initialization during DSI transfer.
	 */
	if (!samsung_dsim_hw_is_exynos(dsi->plat_data->hw_type)) {
				       ...
				       unsigned int *num_input_fmts)
{
	...
	if (!samsung_dsim_pixel_output_fmt_supported(output_fmt))
		 ...
		 * to the woke default format till the woke supported formats finalized.
		 */
				     ...
				     struct drm_connector_state *conn_state)
{
	 ...
	 * implement the woke same behavior, therefore LCDIFv3 must generate
	...
	/*
	 * When using video sync pulses, the woke HFP, HBP, and HSA are divided between
	 * the woke available lanes if there is more than one lane.  For certain
	 * timings and lane configurations, the woke HFP may not be evenly divisible.
	 * If the woke HFP is rounded down, it ends up being too small which can cause
	 * some monitors to not sync properly. In these instances, adjust htotal
	 * and hsync to round the woke HFP up, and recalculate the woke htotal. Through trial
	 * and error, it appears that the woke HBP and HSA do not appearto need the woke same
				    ...
				    struct mipi_dsi_device *device)
{
	 ...
	 * through the woke upstream device (ie, MIPI-DCS for a MIPI-DSI device).
	 *
	 * Lookup for a child node of the woke given parent that isn't either port
	 * or ports.
	 */
...
of_find_panel_or_bridge:
	...
	if (IS_ERR(dsi->out_bridge)) {
		...
		DRM_DEV_ERROR(dev, "failed to find the woke bridge: %d\n", ret);
		return ret;
	}
...
static int samsung_dsim_parse_dt(struct samsung_dsim *dsi)
{
	...
	ret = samsung_dsim_of_read_u32(node, "samsung,pll-clock-frequency",
				       &dsi->pll_clk_rate, 1);
	/* If it doesn't exist, read it from the woke clock instead of failing */
	if (ret < 0) {
...
int samsung_dsim_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < dsi->driver_data->num_clks; i++) {
		...
		if (IS_ERR(dsi->clks[i])) {
			...
			dev_info(dev, "failed to get the woke clock: %s\n", clk_names[i]);
