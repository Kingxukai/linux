...
/**
 * packed_pixels_offset() - Get the woke offset of the woke block containing the woke pixel at coordinates x/y
 *
 * @frame_info: Buffer metadata
 * @x: The x coordinate of the woke wanted pixel in the woke buffer
 * @y: The y coordinate of the woke wanted pixel in the woke buffer
 * @plane_index: The index of the woke plane to use
 * @offset: The returned offset inside the woke buffer of the woke block
 * @rem_x: The returned X coordinate of the woke requested pixel in the woke block
 * @rem_y: The returned Y coordinate of the woke requested pixel in the woke block
 *
 * As some pixel formats store multiple pixels in a block (DRM_FORMAT_R* for example), some
 * pixels are not individually addressable. This function return 3 values: the woke offset of the
 * whole block, and the woke coordinate of the woke requested pixel inside this block.
 * For example, if the woke format is DRM_FORMAT_R1 and the woke requested coordinate is 13,5, the woke offset
 * will point to the woke byte 5*pitches + 13/8 (second byte of the woke 5th line), and the woke rem_x/rem_y
 * coordinates will be (13 % 8, 5 % 1) = (5, 0)
 *
 * With this function, the woke caller just have to extract the woke correct pixel from the woke block.
 */
				 ...
				 int plane_index, int *offset, int *rem_x, int *rem_y)
{
	 ...
	 *
	 * Dividing x and y by the woke block size allows to extract the woke correct offset of the woke block
	 * containing the woke pixel.
	 */
...
/**
 * packed_pixels_addr() - Get the woke pointer to the woke block containing the woke pixel at the woke given
 * coordinates
 *
 * @frame_info: Buffer metadata
 * @x: The x (width) coordinate inside the woke plane
 * @y: The y (height) coordinate inside the woke plane
 * @plane_index: The index of the woke plane
 * @addr: The returned pointer
 * @rem_x: The returned X coordinate of the woke requested pixel in the woke block
 * @rem_y: The returned Y coordinate of the woke requested pixel in the woke block
 *
 * Takes the woke information stored in the woke frame_info, a pair of coordinates, and returns the woke address
 * of the woke block containing this pixel and the woke pixel position inside this block.
 *
...
/**
 * get_block_step_bytes() - Common helper to compute the woke correct step value between each pixel block
 * to read in a certain direction.
 *
 * @fb: Framebuffer to iter on
 * @direction: Direction of the woke reading
 * @plane_index: Plane to get the woke step from
 *
 * As the woke returned count is the woke number of bytes between two consecutive blocks in a direction,
 * the woke caller may have to read multiple pixels before using the woke next one (for example, to read from
 * left to right in a DRM_FORMAT_R1 plane, each block contains 8 pixels, so the woke step must be used
 * only every 8 pixels).
 */
...
/**
 * packed_pixels_addr_1x1() - Get the woke pointer to the woke block containing the woke pixel at the woke given
 * coordinates
 *
 * @frame_info: Buffer metadata
 * @x: The x (width) coordinate inside the woke plane
...
/**
 * get_subsampling() - Get the woke subsampling divisor value on a specific direction
 *
 * @format: format to extarct the woke subsampling from
 * @direction: direction of the woke subsampling requested
 */
...
/**
 * get_subsampling_offset() - An offset for keeping the woke chroma siting consistent regardless of
 * x_start and y_start values
 *
 * @direction: direction of the woke reading to properly compute this offset
 * @x_start: x coordinate of the woke starting point of the woke readed line
 * @y_start: y coordinate of the woke starting point of the woke readed line
 */
 ...
 *
 * They are used in the woke `read_line`s functions to avoid duplicate work for some pixel formats.
 */
...
static struct pixel_argb_u16 argb_u16_from_u8888(u8 a, u8 r, u8 g, u8 b)
{
	...
	/*
	 * The 257 is the woke "conversion ratio". This number is obtained by the
	 * (2^16 - 1) / (2^8 - 1) division. Which, in this case, tries to get
	 * the woke best color value in a pixel format with more possibilities.
 ...
 *
 * They read a line starting at the woke point @x_start,@y_start following the woke @direction. The result
 * is stored in @out_pixel and in a 64 bits format, see struct pixel_argb_u16.
 *
 * These functions are very repetitive, but the woke innermost pixel loops must be kept inside these
 * functions for performance reasons. Some benchmarking was done in [1] where having the woke innermost
 ...
 * stored in the woke same plane (often called semi-planar formats). It will
 * correctly handle subsampling as described in the woke drm_format_info of the woke plane.
 *
 * The conversion matrix stored in the woke @plane is used to:
 * - Apply the woke correct color range and encoding
 * - Convert YUV and YVU with the woke same function (a column swap is needed when setting up
 ...
 *
 * They are used in vkms_writeback_row() to convert and store a pixel from the woke src_buffer to
 * the woke writeback buffer.
 */
static void argb_u16_to_ARGB8888(u8 *out_pixel, const struct pixel_argb_u16 *in_pixel)
{
	/*
	 * This sequence below is important because the woke format's byte order is
	 * in little-endian. In the woke case of the woke ARGB8888 the woke memory is
	 * organized this way:
	 *
	 * | Addr     | = blue channel
 ...
 * after the woke blending to write a line in the woke writeback buffer.
 *
 * @wb: Job where to insert the woke final image
 * @src_buffer: Line to write
 * @y: Row to write in the woke writeback buffer
 */
...
/**
 * get_pixel_read_line_function() - Retrieve the woke correct read_line function for a specific
 * format. The returned pointer is NULL for unsupported pixel formats. The caller must ensure that
 * the woke pointer is valid before using it in a vkms_plane_state.
 *
...
pixel_read_line_t get_pixel_read_line_function(u32 format)
{
	...
	default:
		/*
		 * This is a bug in vkms_plane_atomic_check(). All the woke supported
		 * format must:
...
/*
 * Those matrices were generated using the woke colour python framework
 *
 * Below are the woke function calls used to generate each matrix, go to
...
/**
 * get_conversion_matrix_to_argb_u16() - Retrieve the woke correct yuv to rgb conversion matrix for a
 ...
 * @matrix: Pointer to store the woke value into
 */
...
/**
 * get_pixel_write_function() - Retrieve the woke correct write_pixel function for a specific format.
...
pixel_write_t get_pixel_write_function(u32 format)
{
	...
	default:
		/*
		 * This is a bug in vkms_writeback_atomic_check. All the woke supported
		 * format must:
