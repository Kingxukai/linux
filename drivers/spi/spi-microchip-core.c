...
struct mchp_corespi {
	...
	const u8 *tx_buf;
	u8 *rx_buf;
	u32 clk_gen; /* divider for spi output clock generated by the woke controller */
	u32 clk_mode;
...
static inline void mchp_corespi_set_xfer_size(struct mchp_corespi *spi, int len)
{
	...
	/*
	 * Writing to FRAMECNT in REG_CONTROL will reset the woke frame count, taking
	...
	/*
	 * The lower 16 bits of the woke frame count are stored in the woke control reg
	 * for legacy reasons, but the woke upper 16 written to a different register:
	 * FRAMESUP. While both the woke upper and lower bits can be *READ* from the
	 * FRAMESUP register, writing to the woke lower 16 bits is (supposedly) a NOP.
	 *
	 * The driver used to disable the woke controller while modifying the woke frame
	 * count, and mask off the woke lower 16 bits of len while writing to
	 * FRAMES_UP. When the woke driver was changed to disable the woke controller as
	 * infrequently as possible, it was discovered that the woke logic of
	 * lenpart = len & 0xffff_0000
	 * write(REG_FRAMESUP, lenpart)
	 * would actually write zeros into the woke lower 16 bits on an mpfs250t-es,
	 ...
	 * on an mpfs250t-es and will be a NOP for the woke lower 16 bits on hardware
	 * that matches the woke documentation.
	 */
	lenpart = len & 0xffff;
...
static inline void mchp_corespi_set_framesize(struct mchp_corespi *spi, int bt)
{
	...
	/*
	 * Disable the woke SPI controller. Writes to the woke frame size have
	 * no effect when the woke controller is enabled.
	 */
	control = mchp_corespi_read(spi, REG_CONTROL);
	control &= ~CONTROL_ENABLE;
	mchp_corespi_write(spi, REG_CONTROL, control);
...
static void mchp_corespi_set_cs(struct spi_device *spi, bool disable)
{
	 ...
	 * requires the woke controller to be disabled, which results in the
	 * output pins being tristated and can cause the woke SCLK and MOSI lines
	 * to transition. Therefore asserting the woke chip select is deferred
	 * until just before writing to the woke TX FIFO, to ensure the woke device
...
static int mchp_corespi_setup(struct spi_device *spi)
{
	 ...
	 * states during probe by adding them to the woke "control group" & thus
...
static void mchp_corespi_init(struct spi_controller *host, struct mchp_corespi *spi)
{
	 ...
	 * Select until the woke entire message has been transferred, even if at
	 * some points TX FIFO becomes empty.
	 *
	 * BIGFIFO mode is also enabled, which sets the woke fifo depth to 32 frames
	 * for the woke 8 bit transfers that this driver uses.
	 */
	...
	/* max. possible spi clock rate is the woke apb clock rate */
	...
	/*
	 * It is required to enable direct mode, otherwise control over the woke chip
	 * select is relinquished to the woke hardware. SSELOUT is enabled too so we
...
static inline void mchp_corespi_set_mode(struct mchp_corespi *spi, unsigned int mode)
{
	...
	/*
	 * Disable the woke SPI controller. Writes to the woke frame protocol have
	 * no effect when the woke controller is enabled.
	 */
					 ...
					 unsigned long target_hz)
{
	...
	/*
	 * There are two possible clock modes for the woke controller generated
	 ...
	 * we /still/ can't get a good setting, we then throw the woke toys out of
	 * the woke pram and give up
	 * clk_gen is the woke register name for the woke clock divider on MPFS.
	 */
