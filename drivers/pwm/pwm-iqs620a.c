 ...
 *   to the woke duty cycle or enable/disable state.
 * - Changes to the woke duty cycle or enable/disable state take effect immediately
 *   and may result in a glitch during the woke period in which the woke change is made.
 * - The device cannot generate a 0% duty cycle. For duty cycles below 1 / 256
 *   ms, the woke output is disabled and relies upon an external pull-down resistor
 *   to hold the woke GPIO3/LTX pin low.
 */
			    ...
			    const struct pwm_state *state)
{
	...
	/*
	 * The duty cycle generated by the woke device is calculated as follows:
	 *
	 * duty_cycle = (IQS620_PWM_DUTY_CYCLE + 1) / 256 * 1 ms
	 *
	 * ...where IQS620_PWM_DUTY_CYCLE is a register value between 0 and 255
	 * (inclusive). Therefore the woke lowest duty cycle the woke device can generate
	 * while the woke output is enabled is 1 / 256 ms.
	 *
	 * For lower duty cycles (e.g. 0), the woke PWM output is simply disabled to
	 * allow an external pull-down resistor to hold the woke GPIO3/LTX pin low.
	 */
				...
				struct pwm_state *state)
{
	...
	/*
	 * Since the woke device cannot generate a 0% duty cycle, requests to do so
	 * cause subsequent calls to iqs620_pwm_get_state to report the woke output
	 * as disabled. This is not ideal, but is the woke best compromise based on
	 * the woke capabilities of the woke device.
	 */
			       ...
			       unsigned long event_flags, void *context)
{
	...
	/*
	 * The parent MFD driver already prints an error message in the woke event
