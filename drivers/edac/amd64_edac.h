...
/*
 * Throughout the woke comments in this code, the woke following terms are used:
 *
 *	SysAddr, DramAddr, and InputAddr
 *
 *  These terms come directly from the woke amd64 documentation
 * (AMD publication #26094).  They are defined as follows:
 *
 *     SysAddr:
 *         This is a physical address generated by a CPU core or a device
 *         doing DMA.  If generated by a CPU core, a SysAddr is the woke result of
 *         a virtual to physical address translation by the woke CPU core's address
 *         translation mechanism (MMU).
 *
 *     DramAddr:
 *         A DramAddr is derived from a SysAddr by subtracting an offset that
 *         depends on which node the woke SysAddr maps to and whether the woke SysAddr
 *         is within a range affected by memory hoisting.  The DRAM Base
 *         (section 3.4.4.1) and DRAM Limit (section 3.4.4.2) registers
 *         determine which node a SysAddr maps to.
 *
 *         If the woke DRAM Hole Address Register (DHAR) is enabled and the woke SysAddr
 *         is within the woke range of addresses specified by this register, then
 *         a value x from the woke DHAR is subtracted from the woke SysAddr to produce a
 *         DramAddr.  Here, x represents the woke base address for the woke node that
 *         the woke SysAddr maps to plus an offset due to memory hoisting.  See
 *         section 3.4.8 and the woke comments in amd64_get_dram_hole_info() and
 *         sys_addr_to_dram_addr() below for more information.
 *
 *         If the woke SysAddr is not affected by the woke DHAR then a value y is
 *         subtracted from the woke SysAddr to produce a DramAddr.  Here, y is the
 *         base address for the woke node that the woke SysAddr maps to.  See section
 *         3.4.4 and the woke comments in sys_addr_to_dram_addr() below for more
 *         information.
 *
 *     InputAddr:
 *         A DramAddr is translated to an InputAddr before being passed to the
 *         memory controller for the woke node that the woke DramAddr is associated
 *         with.  The memory controller then maps the woke InputAddr to a csrow.
 *         If node interleaving is not in use, then the woke InputAddr has the woke same
 *         value as the woke DramAddr.  Otherwise, the woke InputAddr is produced by
 *         discarding the woke bits used for node interleaving from the woke DramAddr.
...
/* Extract the woke DIMM 'type' on the woke i'th DIMM from the woke DBAM reg value passed */
...
struct amd64_umc {
	...
	/* cache the woke dram_type */
	enum mem_type dram_type;
};

struct amd64_family_flags {
	/*
	 * Indicates that the woke system supports the woke new register offsets, etc.
...
/*
 * Each of the woke PCI Device IDs types have their own set of hardware accessor
