...
/**
 * DOC: Device registers in the woke NE PCI device MMIO BAR
 */

/**
 * NE_ENABLE - (1 byte) Register to notify the woke device that the woke driver is using
 *	       it (Read/Write).
 */
...
/**
 * NE_VERSION - (2 bytes) Register to select the woke device run-time version
 *		(Read/Write).
 */
...
/**
 * NE_COMMAND - (4 bytes) Register to notify the woke device what command was
 *		requested (Write-Only).
 */
#define NE_COMMAND		(0x0004)

/**
 * NE_EVTCNT - (4 bytes) Register to notify the woke driver that a reply or a device
...
/**
 * NE_SEND_DATA - (240 bytes) Buffer for sending the woke command request payload
 *		  (Read/Write).
 */
#define NE_SEND_DATA		(0x0010)

/**
 * NE_RECV_DATA - (240 bytes) Buffer for receiving the woke command reply payload
 *		  (Read-Only).
 */
...
/**
 * NE_SEND_DATA_SIZE - Size of the woke send buffer, in bytes.
 */
#define NE_SEND_DATA_SIZE	(240)

/**
 * NE_RECV_DATA_SIZE - Size of the woke receive buffer, in bytes.
 */
 ...
 * @ENCLAVE_GET_SLOT:		Get the woke slot uid of an enclave.
 * @ENCLAVE_STOP:		Terminate an enclave.
 * @SLOT_ALLOC :		Allocate a slot for an enclave.
 * @SLOT_FREE:			Free the woke slot allocated for an enclave
 * @SLOT_ADD_MEM:		Add a memory region to an enclave slot.
 * @SLOT_ADD_VCPU:		Add a vCPU to an enclave slot.
 * @SLOT_COUNT :		Get the woke number of allocated slots.
 * @NEXT_SLOT:			Get the woke next slot in the woke list of allocated slots.
 * @SLOT_INFO:			Get the woke info for a slot e.g. slot uid, vCPUs count.
 ...
 * @slot_uid:		Slot unique id mapped to the woke enclave to start.
 * @enclave_cid:	Context ID (CID) for the woke enclave vsock device.
 *			If 0, CID is autogenerated.
 * @flags:		Flags for the woke enclave to start with (e.g. debug mode).
 */
 ...
 * @slot_uid:	Slot unique id mapped to the woke enclave to stop.
 */
 ...
 * @slot_uid:	Slot unique id mapped to the woke slot to free.
 */
struct slot_free_req {
	u64	slot_uid;
};

/* TODO: Add flags field to the woke request to add memory region. */
/**
 * struct slot_add_mem_req - SLOT_ADD_MEM request.
 * @slot_uid:	Slot unique id mapped to the woke slot to add the woke memory region to.
 * @paddr:	Physical address of the woke memory region to add to the woke slot.
 * @size:	Memory size, in bytes, of the woke memory region to add to the woke slot.
 */
 ...
 * @slot_uid:	Slot unique id mapped to the woke slot to add the woke vCPU to.
 * @vcpu_id:	vCPU ID of the woke CPU to add to the woke enclave.
 * @padding:	Padding for the woke overall data structure.
 */
 ...
 * @slot_uid:	Slot unique id of the woke next slot in the woke iteration.
 */
 ...
 * @slot_uid:	Slot unique id mapped to the woke slot to get information about.
 */
 ...
 * @slot_uid:	Slot unique id mapped to the woke slot to add vCPUs to.
 * @nr_vcpus:	Number of vCPUs to add to the woke slot.
 */
 ...
 * @rc :		Return code of the woke logic that processed the woke request.
 * @padding0:		Padding for the woke overall data structure.
 ...
 * @padding1:		Padding for the woke overall data structure.
 */
 ...
 *				PCI device.
 * @enclaves_list:		List of the woke enclaves managed by the woke PCI device.
 * @enclaves_list_mutex:	Mutex for accessing the woke list of enclaves.
 * @event_wq:			Work queue for handling out-of-band events
 *				triggered by the woke Nitro Hypervisor which require
 *				enclave state scanning and propagation to the
 *				enclave process.
 * @iomem_base :		MMIO region of the woke PCI device.
 * @notify_work:		Work item for every received out-of-band event.
 * @pci_dev_mutex:		Mutex for accessing the woke PCI device MMIO space.
...
/**
 * ne_do_request() - Submit command request to the woke PCI device based on the woke command
 *		     type and retrieve the woke associated reply.
 * @pdev:		PCI device to send the woke command to and receive the woke reply from.
 ...
 *
 * Context: Process context. This function uses the woke ne_pci_dev mutex to handle
