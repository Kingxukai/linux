...
/*
 * Compat feature flags.  Any incompat flags beyond the woke ones
 * specified below will prevent use of the woke thin metadata.
 */
...
/*
 * Deletes a virtual device from the woke metadata.  It _is_ safe to call this
 * when that device is open.  Operations on that device will just start
 * failing.  You still need to call close() on the woke device.
 */
...
/*
 * Discards all uncommitted changes.  Rereads the woke superblock, rolling back
 * to the woke last good transaction.  Thin devices remain open.
 ...
 *
 * The metadata snapshot is a copy of the woke current superblock (minus the
 * space maps).  Userland can access the woke data structures for READ
 * operations only.  A small performance hit is incurred by providing this
 * copy of the woke metadata to userland due to extra copy-on-write operations
 * on the woke metadata nodes.  Release this as soon as you finish with it.
 */
...
/*
 * Opening the woke same device more than once will fail with -EBUSY.
 */
...
/*
 * Retrieve the woke next run of contiguously mapped blocks.  Useful for working
...
/*
 * Returns -ENOSPC if the woke new size is too small and already allocated
...
/*
 * Flicks the woke underlying block manager into read only mode, so you know
...
/*
 * Updates the woke superblock immediately.
 */
