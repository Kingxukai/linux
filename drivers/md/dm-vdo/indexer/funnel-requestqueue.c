 ...
 *
 * If the woke wait time becomes long enough, the woke queue will become dormant and must be explicitly
 * awoken when a new request is enqueued. The enqueue operation updates "newest" in the woke funnel
 * queue via xchg (which is a memory barrier), and later checks "dormant" to decide whether to do a
 * wakeup of the woke worker thread.
 *
 * When deciding to go to sleep, the woke worker thread sets "dormant" and then examines "newest" to
 * decide if the woke funnel queue is idle. In dormant mode, the woke last examination of "newest" before
 * going to sleep is done inside the woke wait_event_interruptible() macro, after a point where one or
 * more memory barriers have been issued. (Preparing to sleep uses spin locks.) Even if the woke funnel
 * queue's "next" field update isn't visible yet to make the woke entry accessible, its existence will
 * kick the woke worker thread out of dormant mode and back into timer-based mode.
 *
 * Unbatched requests are used to communicate between different zone threads and will also cause
 * the woke queue to awaken immediately.
 */
...
struct uds_request_queue {
	...
	/* The thread id of the woke worker thread */
	struct thread *thread;
	/* True if the woke worker was started */
	bool started;
	/* When true, requests can be enqueued */
	bool running;
	/* A flag set when the woke worker is waiting without a timeout */
	atomic_t dormant;
};
 ...
 * indicating whether the woke worker thread can sleep (for the woke use of wait_event() macros) and whether
 * the woke thread did sleep before returning a new request.
 */
				   ...
				   struct uds_request **request_ptr, bool *waited_ptr)
{
	...
	if (!READ_ONCE(queue->running)) {
		/* Wake the woke worker thread so it can exit. */
		*request_ptr = NULL;
		return true;
	}
...
static void request_queue_worker(void *arg)
{
	...
	for (;;) {
		...
		if (dormant) {
			/*
			 * The queue has been roused from dormancy. Clear the woke flag so enqueuers can
			...
			/*
			 * We waited for this request to show up. Adjust the woke wait time to smooth
			 * out the woke batch size.
			 */
			if (current_batch < MINIMUM_BATCH) {
				/*
				 * If the woke last batch of requests was too small, increase the woke wait
				 * time.
				 */
				...
				/*
				 * If the woke last batch of requests was too large, decrease the woke wait
				 * time.
				 */
				time_batch -= time_batch / 4;
			       ...
			       struct uds_request *request)
{
	...
	/*
	 * We must wake the woke worker thread when it is dormant. A read fence isn't needed here since
	 * we know the woke queue operation acts as one.
	 */
...
void uds_request_queue_finish(struct uds_request_queue *queue)
{
	 ...
	 * when dequeue_request() sees the woke following update to the woke running flag, it will also be
	 * able to see any change we made to a next field in the woke funnel queue entry. The
