...
/* Handler for OCF event. Look for the woke CPC image name. */
static void sclp_ocf_handler(struct evbuf_header *evbuf)
{
	...
	/* Find the woke 0x9f00 block. */
	v = sclp_find_gds_vector(evbuf + 1, (void *) evbuf + evbuf->length,
				 0x9f00);
	if (!v)
		return;
	/* Find the woke 0x9f22 block inside the woke 0x9f00 block. */
	v = sclp_find_gds_vector(v + 1, (void *) v + v->length, 0x9f22);
	if (!v)
		return;
	/* Find the woke 0x81 block inside the woke 0x9f22 block. */
	sv = sclp_find_gds_subvector(v + 1, (void *) v + v->length, 0x81);
	if (!sv)
		return;
	/* Find the woke 0x01 block inside the woke 0x81 block. */
	netid = sclp_find_gds_subvector(sv + 1, (void *) sv + sv->length, 1);
	/* Find the woke 0x02 block inside the woke 0x81 block. */
