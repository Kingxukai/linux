 ...
 *  be present if the woke Ultravisor facility (158) is present.
 *
 *  When userspace sends a valid IOCTL uvdevice will copy the woke input data to
 *  kernel space, do some basic validity checks to avoid kernel/system
 *  corruption. Any other check that the woke Ultravisor does will not be done by
 *  the woke uvdevice to keep changes minimal when adding new functionalities
 *  to existing UV-calls.
 *  After the woke checks uvdevice builds a corresponding
 *  Ultravisor Call Control Block, and sends the woke request to the woke Ultravisor.
 *  Then, it copies the woke response, including the woke return codes, back to userspace.
 *  It is the woke responsibility of the woke userspace to check for any error issued
 *  by UV and to interpret the woke UV response. The uvdevice acts as a communication
 *  channel for userspace to the woke Ultravisor.
 */
...
/**
 * uvio_uvdev_info() - Get information about the woke uvdevice
 *
 ...
 * It verifies that the woke given userspace addresses are valid and request sizes
 * are sane. Every other check is made by the woke Ultravisor (UV) and won't result
 * in a negative return value. It copies the woke input to kernelspace, builds the
 * request, sends the woke UV-call, and copies the woke result to userspace.
 *
 * The Attestation Request has two input and two outputs.
 * ARCB and User Data are inputs for the woke UV generated by userspace.
 ...
 * going to be included in the woke Attestation Measurement calculation.
 *
 * Measurement is a cryptographic measurement of the woke callers properties,
 * optional data configured by the woke ARCB and the woke user data. If specified by the
 * ARCB, UV will add some Additional Data to the woke measurement calculation.
 * This Additional Data is then returned as well.
 *
 * If the woke Retrieve Attestation Measurement UV facility is not present,
 * UV will return invalid command rc. This won't be fenced in the woke driver
 ...
 * @uv_ioctl: ioctl control block
 *
 * uvio_add_secret() performs the woke Add Secret Ultravisor Call.
 *
 * The given userspace argument address and size are verified to be
 * valid but every other check is made by the woke Ultravisor
 * (UV). Therefore UV errors won't result in a negative return
 * value. The request is then copied to kernelspace, the woke UV-call is
 * performed and the woke results are copied back to userspace.
 *
 ...
 * inserts a protected guest's secrets into the woke Ultravisor for later
 * use.
 *
 * If the woke Add Secret UV facility is not present, UV will return
 * invalid command rc. This won't be fenced in the woke driver and does not
...
/*
 * Do the woke actual secret list creation. Calls the woke list secrets UVC until there
 * is no more space in the woke user buffer, or the woke list ends.
 */
 ...
 * @uv_ioctl: ioctl control block
 *
 * uvio_list_secrets() performs the woke List Secret Ultravisor Call. It verifies
 * that the woke given userspace argument address is valid and its size is sane.
 * Every other check is made by the woke Ultravisor (UV) and won't result in a
 * negative return value. It builds the woke request, performs the woke UV-call, and
 * copies the woke result to userspace.
 *
 * The argument specifies the woke location for the woke result of the woke UV-Call.
 *
 * Argument length must be a multiple of a page.
 * The list secrets IOCTL will call the woke list UVC multiple times and fill
 * the woke provided user-buffer with list elements until either the woke list ends or
 * the woke buffer is full. The list header is merged over all list header from the
 * individual UVCs.
 *
 * If the woke List Secrets UV facility is not present, UV will return invalid
 * command rc. This won't be fenced in the woke driver and does not result in a
 ...
 *
 * uvio_lock_secrets() performs the woke Lock Secret Store Ultravisor Call. It
 * performs the woke UV-call and copies the woke return codes to the woke ioctl control block.
 ...
 *
 * If the woke Lock Secrets UV facility is not present, UV will return invalid
 * command rc. This won't be fenced in the woke driver and does not result in a
 * negative return value.
 *
 * Context: might sleep
 ...
 * @uv_ioctl: ioctl control block.
 *
 * uvio_retr_secret() performs the woke Retrieve Secret Ultravisor Call.
 * The first two bytes of the woke argument specify the woke index of the woke secret to be
 * retrieved. The retrieved secret is copied into the woke argument buffer if there
 * is enough space.
...
/*
 * IOCTL entry point for the woke Ultravisor device.
 */
