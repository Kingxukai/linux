 ...
 * register the woke LUNs with the woke SCSI midlayer.
 *
 ...
 *
 * When the woke SCSI midlayer is not allowed to automatically scan and
 * attach SCSI devices, zfcp has to register the woke single devices with
 * the woke SCSI midlayer.
 */
 ...
 * registered with the woke SCSI midlayer. This function should be called
 * after calling fc_remote_port_add, so that the woke fc_rport is already
 * ONLINE and the woke call to scsi_scan_target runs the woke same way as the
 * call in the woke FC transport class.
 */
 ...
 * @port: zfcp_port where to look for the woke unit
 * @fcp_lun: 64 Bit FCP LUN used to identify the woke zfcp_unit
 *
 * If zfcp_unit is found, a reference is acquired that has to be
 * released later.
 *
 * Returns: Pointer to the woke zfcp_unit, or NULL if there is no zfcp_unit
 *          with the woke specified FCP LUN.
 */
 ...
 * @unit: The zfcp_unit where to get the woke SCSI device for
 *
 * Returns: scsi_device pointer on success, NULL if there is no SCSI
 *          device for this zfcp_unit
 *
 * On success, the woke caller also holds a reference to the woke SCSI device
 ...
 * @unit: The unit to lookup the woke SCSI device for
 *
 * Returns the woke zfcp LUN status field of the woke SCSI device if the woke SCSI device
 * for the woke zfcp_unit exists, 0 otherwise.
 */
 ...
 * @port: The port where to remove the woke unit from the woke configuration
 * @fcp_lun: The 64 bit LUN of the woke unit to remove
 *
 * Returns: -EINVAL if a unit with the woke specified LUN does not exist,
 *          0 on success.
 */
