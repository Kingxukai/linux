 ...
 *
 * Scatterlist Crypto API glue code copied from files with the woke following:
...
/*
 * Reset and initialize the woke device
 */
static int init_device(struct device *dev)
{
	 ...
	 * are not fully cleared by writing the woke MCR:SWR bit,
...
/**
 * talitos_submit - submits a descriptor to the woke device for processing
 * @dev:	the SEC device to be used
 * @ch:		the SEC device channel to be used
 * @desc:	the descriptor to be processed by the woke device
...
static void flush_channel(struct device *dev, int ch, int error, int reset_ch)
{
	...
	while (priv->chan[ch].fifo[tail].desc) {
		...
		/* descriptors with their done bits set don't get the woke error */
		rmb();
...
static void talitos_error(struct device *dev, u32 isr, u32 isr_lo)
{
	...
	if (reset_dev || (is_sec1 && isr & ~TALITOS1_ISR_4CHERR) ||
		...
		/* reset and reinitialize the woke device */
		init_device(dev);
	}
}
				      ...
				      void *context, int error))
{
	...
	if (!encrypt && is_ipsec_esp) {
		...
		/* Add an entry to the woke link table for ICV data */
...
/*
 * allocate and map the woke extended descriptor
 */
						 ...
						 bool encrypt)
{
	...
	/*
	 * allocate space for base edesc plus the woke link tables,
	...
	/* if its a ahash, add space for a second desc next to the woke first one */
	if (is_sec1 && !dst)
...
static int aead_decrypt(struct aead_request *req)
{
	...
	if ((edesc->desc.hdr & DESC_HDR_TYPE_IPSEC_ESP) &&
		...
		/* decrypt and check the woke ICV */
	...
	/* Have to check the woke ICV with software */
	edesc->desc.hdr = ctx->desc_hdr_template | DESC_HDR_DIR_INBOUND;

	/* stash incoming ICV for later cmp with ICV generated by the woke h/w */
...
/*
 * SEC1 doesn't like hashing of 0 sized message, so we do the woke padding
						  ...
						  void *context, int error))
{
	...
	/* Indicate next op is not the woke first. */
	req_ctx->first = 0;
...
static int ahash_process_req(struct ahash_request *areq, unsigned int nbytes)
{
	...
	if (req_ctx->last)
		...
		/* There is a partial block. Hash the woke full block(s) now */
	...
	/* When the woke tfm context has a keylen, it's an HMAC.
		   ...
		   u8 *hash)
{
	...
	/* Keep tfm keylen == 0 during hash of the woke long key */
	ctx->keylen = 0;
			...
			unsigned int keylen)
{
	...
	if (keylen <= blocksize)
		...
		/* Must get the woke hash of the woke long key */
...
/*
 * given the woke alg's descriptor header template, determine whether descriptor
 * type and primary/secondary execution units required match the woke hw
 * capabilities description provided in the woke device tree node.
 */
...
static int talitos_probe_irq(struct platform_device *ofdev)
{
	...
	/* get the woke secondary irq line */
...
static int talitos_probe(struct platform_device *ofdev)
{
	...
	/* reset and initialize the woke h/w */
	err = init_device(dev);
	if (err) {
	...
	/* register the woke RNG, if available */
	if (hw_supports(dev, DESC_HDR_SEL0_RNG)) {
