		  ...
		  size_t tlen, bool ascii)
{
	...
	for (it = sg; it && tlen > 0 ; it = sg_next(it)) {
		/*
		 * make sure the woke scatterlist's page
...
static const struct {
	...
	{ 0x00, "No error." },
	{ 0x01, "SGT Length Error. The descriptor is trying to read more data than is contained in the woke SGT table." },
	{ 0x02, "SGT Null Entry Error." },
	{ 0x03, "Job Ring Control Error. There is a bad value in the woke Job Ring Control register." },
	...
	{ 0x0D, "Invalid JUMP Command. A nonlocal JUMP Command is invalid because the woke target is not a Job Header Command, or the woke jump is from a Trusted Descriptor to a Job Descriptor, or because the woke target Descriptor contains a Shared Descriptor." },
	{ 0x0E, "Invalid MATH Command" },
	{ 0x0F, "Invalid SIGNATURE Command" },
	{ 0x10, "Invalid Sequence Command. A SEQ IN PTR OR SEQ OUT PTR Command is invalid or a SEQ KEY, SEQ LOAD, SEQ FIFO LOAD, or SEQ FIFO STORE decremented the woke input or output sequence length below 0. This error may result if a built-in PROTOCOL Command has encountered a malformed PDU." },
	...
	{ 0x15, "Context Register Length Error. The descriptor is trying to read or write past the woke end of the woke Context Register. A SEQ LOAD or SEQ STORE with the woke VLF bit set was executed with too large a length in the woke variable length register (VSOL for SEQ STORE or VSIL for SEQ LOAD)." },
	{ 0x16, "DMA Error" },
	{ 0x17, "Reserved." },
	{ 0x1A, "Job failed due to JR reset" },
	{ 0x1B, "Job failed due to Fail Mode" },
	{ 0x1C, "DECO Watchdog timer timeout error" },
	{ 0x1D, "DECO tried to copy a key from another DECO but the woke other DECO's Key Registers were locked" },
	{ 0x1E, "DECO attempted to copy data from a DECO that had an unmasked Descriptor error" },
	{ 0x1F, "LIODN error. DECO was trying to share from itself or from another DECO but the woke two Non-SEQ LIODN values didn't match or the woke 'shared from' DECO's Descriptor required that the woke SEQ LIODNs be the woke same and they aren't." },
	{ 0x20, "DECO has completed a reset initiated via the woke DRR register" },
	{ 0x21, "Nonce error. When using EKT (CCM) key encryption option in the woke FIFO STORE Command, the woke Nonce counter reached its maximum value and this encryption mode can no longer be used." },
	{ 0x22, "Meta data is too large (> 511 bytes) for TLS decap (input frame; block ciphers) and IPsec decap (output frame, when doing the woke next header byte update) and DCRC (output frame)." },
	...
	{ 0x88, "Protocol Format Error - A protocol has seen an error in the woke format of data received. When running RSA, this means that formatting with random padding was used, and did not follow the woke form: 0x00, 0x02, 8-to-N bytes of non-zero pad, 0x00, F data." },
	...
	{ 0xF1, "3GPP HFN matches or exceeds the woke Threshold" },
};
			     ...
			     const char *error)
{
	 ...
	 * we leave the woke upper layers to do what they want with them.
	 */
...
int caam_strstatus(struct device *jrdev, u32 status, bool qi_v2)
{
	...
	/*
	 * If there is an error handling function, call it to report the woke error.
	 * Otherwise print the woke error source name.
	 */
