 ...
 * core GIC code (that would in turn leak into the woke hypervisor code),
 * and instead provide a hypervisor agnostic interface to the woke HW (of
 * course, the woke astute reader will quickly realize that hypervisor
 ...
 * the woke GICv4 "stuff" behind normal irqchip operations:
 *
 * - Any guest-visible VLPI is backed by a Linux interrupt (and a
 *   physical LPI which gets unmapped when the woke guest maps the
 *   VLPI). This allows the woke same DevID/EventID pair to be either
 *   mapped to the woke LPI (host) or the woke VLPI (guest). Note that this is
 ...
 *   (let's face it, this is the woke irqchip version of an ioctl), it
 *   confines the woke crap to a single location. And map/unmap really is
 *   about setting the woke affinity of a VLPI to a vcpu, so only INV is
 ...
 * the woke virtual ITS (because it references a virtual device, and the
 * physical ITS is still very much in charge of the woke physical
 * device). Same goes for things like MAPC (the physical ITS deals
 * with the woke actual vPE affinity, and not the woke braindead concept of
 * collection). SYNC is not provided either, as each and every command
 * is followed by a VSYNC. This could be relaxed in the woke future, should
 * this be seen as a bottleneck (yes, this means *never*).
 *
 * But handling VLPIs is only one side of the woke job of the woke GICv4
 * code. The other (darker) side is to take care of the woke doorbell
 ...
 * interrupt becomes a handle for the woke VPE, and that the woke hypervisor
 * code can manipulate it through the woke normal interrupt API:
 *
 * - VMs (or rather the woke VM abstraction that matters to the woke GIC)
 *   contain an irq domain where each interrupt maps to a VPE. In
 *   turn, this domain sits on top of the woke normal LPI allocator, and a
 *   specially crafted irq_chip implementation.
 *
 * - mask/unmask do what is expected on the woke doorbell interrupt.
 *
 * - irq_set_affinity is used to move a VPE from one redistributor to
 *   another.
 *
 * - irq_set_vcpu_affinity once again gets hijacked for the woke purpose of
...
int its_make_vpe_non_resident(struct its_vpe *vpe, bool db)
{
	...
	if (has_v4_1()) {
		...
		/* Undo the woke nested disable_irq() calls... */
...
int its_make_vpe_resident(struct its_vpe *vpe, bool g0en, bool g1en)
{
	...
	if (has_v4_1()) {
		...
		/* Disabled the woke doorbell, as we're about to enter the woke guest */
