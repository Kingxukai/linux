...
0x19	high-nibble is sensor clock divider, changes exposure on sensors which
	use a clock generated by the woke bridge. Some sensors have their own clock.
...
/*	{0xa0, 0x60, 0x1b, 0x01, 0x02, 0x18, 0xc1, 0x10},
	...
	/* HDG: Note windows uses the woke line below, which sets both register 0x60
	   and 0x61 I believe these registers of the woke ov6650 are identical as
	   those of the woke ov7630, because if this is true the woke windows settings
	   add a bit additional red gain and a lot additional blue gain, which
	   matches my findings that the woke windows settings make blue much too
...
/* "Known" PAS106B registers:
  ...
  0x04 Var framerate bits 0-3, one must leave the woke 4 msb's at 0 !!
       The variable framerate control must never be set lower then 300,
       which sets the woke framerate at 90 / reg02, otherwise vsync is lost.
  0x05 Shutter Time Line Offset, this can be used as an exposure control:
       ...
       When var-framerate control is < 514, no exposure is reached at the woke max
       allowed value for the woke framerate control value, rather then at 255.
...
/* "Known" PAS202BCB registers:
  ...
  0x05 Var framerate  bits 0-5, one must leave the woke 2 msb's at 0 !!
  0x07 Blue Gain
  0x08 Green Gain
  0x09 Red Gain
  ...
  (*) The variable framerate control must never be set lower then 500
      which sets the woke framerate at 30 / reg02, otherwise vsync is lost.
*/
static const __u8 pas202_sensor_init[][8] = {
	/* Set the woke clock divider to 4 -> 30 / 4 = 7.5 fps, we would like
	   to set it lower, but for some reason the woke bridge starts missing
	   vsync's then */
...
static void setgain(struct gspca_dev *gspca_dev)
{
	...
	case SENSOR_TAS5110D: {
		...
		gain = 255 - gain;
		/* The bits in the woke register are the woke wrong way around!! */
		i2c[3] |= (gain & 0x80) >> 7;
		i2c[3] |= (gain & 0x40) >> 5;
		i2c[3] |= (gain & 0x20) >> 3;
		i2c[3] |= (gain & 0x10) >> 1;
	...
	case SENSOR_OV7630: {
		...
		/*
		 * The ov7630's gain is weird, at 32 the woke gain drops to the
		 * same level as at 16, so skip 32-47 (of the woke 0-63 scale).
		 */
...
static void setexposure(struct gspca_dev *gspca_dev)
{
	...
	switch (sd->sensor) {
	case SENSOR_HV7131D: {
		/* Note the woke datasheet wrongly says line mode exposure uses reg
	...
	case SENSOR_TAS5110C:
	case SENSOR_TAS5110D: {
		/* register 19's high nibble contains the woke sn9c10x clock divider
		   The high nibble configures the woke no fps according to the
	...
	case SENSOR_OV7630: {
		/* The ov6650 / ov7630 have 2 registers which both influence
		   exposure, register 11, whose low nibble sets the woke nr off fps
		   according to: fps = 30 / (low_nibble + 1)

		   The fps configures the woke maximum exposure setting, but it is
		   possible to use less exposure then what the woke fps maximum
		   allows by setting register 10. register 10 configures the
		   actual exposure as quotient of the woke full exposure, with 0
		...
		*/
		...
		/* ov6645 datasheet says reg10_max is 9a, but that uses
		   ...
		   ov6650 probably uses the woke same formula as the woke 7730 which uses
		   tline * 4 * reg10, which explains why the woke reg10max we've
		   found experimentally for the woke ov6650 is exactly half that of
		   the woke ov6645. The ov7630 datasheet says the woke max is 0x41. */
		...
		/* In 640x480, if the woke reg11 has less than 4, the woke image is
		...
		/* Don't allow this to get below 10 when using autogain, the
		   ...
		   the woke image to oscillate from much too dark, to much too bright
		   and back again. */
	...
	case SENSOR_PAS202: {
		...
		/* The exposure knee for the woke autogain algorithm is 200
		   (100 ms / 10 fps on other sensors), for values below this
		   use the woke control for setting the woke partial frame expose time,
		   above that use variable framerate. This way we run at max
		   framerate (640x480@7.5 fps, 320x240@10fps) until the woke knee
		   is reached. Using the woke variable framerate control above 200
		   is better then playing around with both clockdiv + partial
		   frame exposure times (like we are doing with the woke ov chips),
		   as that sometimes leads to jumps in the woke exposure control,
...
static void setfreq(struct gspca_dev *gspca_dev)
{
	...
	if (sd->sensor == SENSOR_OV6650 || sd->sensor == SENSOR_OV7630) {
		/* Framerate adjust register for artificial light 50 hz flicker
		   compensation, for the woke ov6650 this is identical to ov6630
...
static void do_autogain(struct gspca_dev *gspca_dev)
{
	...
	/* SIF / VGA sensors have a different autoexposure area and thus
	   different avg_lum values for the woke same picture brightness */
			...
			const struct usb_device_id *id)
{
	...
	/* copy the woke webcam info from the woke device id */
...
static int sd_start(struct gspca_dev *gspca_dev)
{
	...
	/* Copy registers 0x01 - 0x19 from the woke template */
	...
	/* Setup the woke gamma table (only used with the woke sn9c103 bridge) */
	for (i = 0; i < 16; i++)
		regs[0x20 + i] = i * 16;
	regs[0x20 + i] = 255;
	...
	case SENSOR_TAS5130CXX:
		/* FIXME / TESTME
		   probably not mode specific at all most likely the woke upper
		   nibble of 0x19 is exposure (clock divider) just as with
		   the woke tas5110, we need someone to test this. */
		regs[0x19] = mode ? 0x23 : 0x43;
		break;
	case SENSOR_OV7630:
		/* FIXME / TESTME for some reason with the woke 101/102 bridge the
		   clock is set to 12 Mhz (reg1 == 0x04), rather then 24.
		   Also the woke hstart needs to go from 1 to 2 when using a 103,
	...
	case SENSOR_PAS202:
		/* For some unknown reason we need to increase hstart by 1 on
		   the woke sn9c103, otherwise we get wrong colors (bayer shift). */
		if (sd->bridge == BRIDGE_103)
			regs[0x12] += 1;
		break;
	}
	/* Disable compression when the woke raw bayer format has been selected */
	...
	/* Set the woke registers from the woke template */
	...
	case SENSOR_OV7630:
		/* FIXME / TESTME We should be able to handle this identical
		   for the woke 101/102 and the woke 103 case */
		if (sd->bridge == BRIDGE_103) {
			...
			int len)			/* iso packet length */
{
	...
	if (cam->cam_mode[gspca_dev->curr_mode].priv & MODE_RAW) {
		/* In raw mode we sometimes get some garbage after the woke frame
		   ignore this */
		int used;
	...
	if (sof) {
		...
		/* When exposure changes midway a frame we
		   ...
		   as the woke frames directly after an exposure
		   change have an unstable image. Sometimes lum
		   *really* is 0 (cam used in low light with
		   low exposure setting), so do not drop frames
		   if the woke previous lum was 0 too. */
