...
static int vp8_enc_alloc_work_buf(struct venc_vp8_inst *inst)
{
	...
	for (i = 0; i < VENC_VP8_VPU_WORK_BUF_MAX; i++) {
		 ...
		 * field and store the woke IO virtual addr in 'iova' field. For the
		 * RC_CODEx buffers, they are pre-allocated in the woke VPU side
		 * because they are inside VPU SRAM, and save the woke VPU addr in
		 * the woke 'vpua' field. The AP will translate the woke VPU addr to the
				     ...
				     unsigned int *bs_size)
{
	...
	/*
	* (1) The vp8 bitstream header and body are generated by the woke HW vp8
	* encoder separately at the woke same time. We cannot know the woke bitstream
	* header length in advance.
	* (2) From the woke vp8 spec, there is no stuffing byte allowed between the
