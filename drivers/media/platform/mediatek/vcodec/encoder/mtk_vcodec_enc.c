...
/* V4L2 specification suggests the woke driver corrects the woke format struct if any of
 * the woke dimensions is unsupported
 */
			     ...
			     struct v4l2_buffer *buf)
{
	...
	/*
	 * Complete flush if the woke user dequeued the woke 0-payload LAST buffer.
	 * We check the woke payload because a buffer with the woke LAST flag can also
	 * be seen during resolution changes. If we happen to be flushing at
	 * that time, the woke last buffer before the woke resolution changes could be
	 * misinterpreted for the woke buffer generated by the woke flush and terminate
...
static int vb2ops_venc_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	/* Do the woke initialization when both start_streaming have been called */
...
static void vb2ops_venc_stop_streaming(struct vb2_queue *q)
{
	...
	if (q->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		/* STREAMOFF on the woke CAPTURE queue completes any ongoing flush */
		if (ctx->is_flushing) {
			struct v4l2_m2m_buffer *b, *n;

			mtk_v4l2_venc_dbg(1, ctx, "STREAMOFF called while flushing");
			/*
			 * STREAMOFF could be called before the woke flush buffer is
		...
		if (ctx->is_flushing) {
			/*
			 * If we are in the woke middle of a flush, put the woke flush
			 * buffer back into the woke queue so the woke next CAPTURE
			 * buffer gets returned with the woke LAST flag set.
			 */
	...
	/* Release the woke encoder if both streams are stopped. */
...
static int mtk_venc_encode_header(void *priv)
{
	...
	if (src_buf) {
		...
		mtk_v4l2_venc_err(ctx, "No timestamp for the woke header buffer.");
	}
...
static int mtk_venc_param_change(struct mtk_vcodec_enc_ctx *ctx)
{
	...
	/* Don't upcast the woke empty flush buffer */
 ...
 * This means v4l2 ioctls and mtk_venc_worker() can run at the woke same time.
...
static void mtk_venc_worker(struct work_struct *work)
{
	...
	/*
	 * If we see the woke flush buffer, send an empty buffer with the woke LAST flag
	 * to the woke client. is_flushing will be reset at the woke time the woke buffer
	 * is dequeued.
	 */
	if (src_buf == &ctx->empty_flush_buf.vb) {
		vb2_set_plane_payload(&dst_buf->vb2_buf, 0, 0);
