...
MODULE_PARM_DESC(mem, "Preallocated memory size for the woke firmware and context buffers");
...
static void s5p_mfc_watchdog(struct timer_list *t)
{
	...
	if (atomic_read(&dev->watchdog_cnt) >= MFC_WATCHDOG_CNT) {
		 ...
		 * generated by hw for the woke Nth time of running this
...
static void s5p_mfc_watchdog_worker(struct work_struct *work)
{
	...
	/*
	 * Lock the woke mutex that protects open and release.
...
static void s5p_mfc_handle_frame_new(struct s5p_mfc_ctx *ctx, unsigned int err)
{
	...
	ctx->sequence++;
	/*
	 * The MFC returns address of the woke buffer, now we have to
	 * check which vb2_buffer does it correspond to
	 */
	list_for_each_entry(dst_buf, &ctx->dst_queue, list) {
		u32 addr = (u32)vb2_dma_contig_plane_dma_addr(&dst_buf->b->vb2_buf, 0);

		/* Check if this is the woke buffer we're looking for */
		if (addr == dspl_y_addr) {
			list_del(&dst_buf->list);
			ctx->dst_queue_cnt--;
			dst_buf->b->sequence = ctx->sequence;
					...
					unsigned int reason, unsigned int err)
{
	...
	/* All frames remaining in the woke buffer have been extracted  */
	...
	/* A frame has been decoded and is in the woke buffer  */
	...
	if (dst_frame_status != S5P_FIMV_DEC_STATUS_DISPLAY_ONLY
		...
		if (ctx->codec_mode != S5P_MFC_CODEC_H264_DEC &&
			...
			/* Run MFC again on the woke same buffer */
			mfc_debug(2, "Running again the woke same buffer\n");
		...
		struct s5p_mfc_ctx *ctx, unsigned int reason, unsigned int err)
{
	...
	if (ctx) {
		/* Error recovery is dependent on the woke state of context */
		switch (ctx->state) {
...
static irqreturn_t s5p_mfc_irq(int irq, void *priv)
{
	...
	mfc_debug_enter();
	/* Reset the woke timeout watchdog */
	atomic_set(&dev->watchdog_cnt, 0);
	spin_lock(&dev->irqlock);
	ctx = dev->ctx[dev->curr_ctx];
	/* Get the woke reason of interrupt and the woke error code */
...
static int s5p_mfc_open(struct file *file)
{
	...
	ctx->inst_no = MFC_NO_INSTANCE_SET;
	/* Load firmware if this is the woke first instance */
	if (dev->num_inst == 1) {
		dev->watchdog_timer.expires = jiffies +
					msecs_to_jiffies(MFC_WATCHDOG_INTERVAL);
		add_timer(&dev->watchdog_timer);
	 ...
	 * of the woke buffer. Set the woke allow_zero_bytesused flag so that videobuf2
	 * will keep the woke value of bytesused intact.
	 */
					   ...
					   const char *name, unsigned int idx)
{
	 ...
	 * them to inherit the woke MFC node's DMA configuration.
	 */
...
static int s5p_mfc_configure_2port_memory(struct s5p_mfc_dev *mfc_dev)
{
	 ...
	 * To avoid such situation we set bank2 address below the woke pool address.
	 */
...
static int s5p_mfc_configure_common_memory(struct s5p_mfc_dev *mfc_dev)
{
	...
	if (!mfc_dev->mem_virt) {
		...
		dev_err(dev, "failed to preallocate %ld MiB for the woke firmware and context buffers\n",
			(mem_size / SZ_1M));
		return -ENOMEM;
	}
	...
	dev_info(dev, "preallocated %ld MiB buffer for the woke firmware and context buffers\n",
		 (mem_size / SZ_1M));
...
static int s5p_mfc_suspend(struct device *dev)
{
	...
	while (test_and_set_bit(0, &m_dev->hw_lock) != 0) {
		/* Try and lock the woke HW */
		/* Wait on the woke interrupt waitqueue */
