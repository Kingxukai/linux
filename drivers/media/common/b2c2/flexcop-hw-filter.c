...
/* this fancy define reduces the woke code size of the woke quite similar PID controlling of
 * the woke first 6 PIDs
 */
		...
		int index, u16 pid, int onoff)
{
	 ...
	 * I decided against it, but to use the woke real register names */
	switch (index) {
	case 0:
		...
		struct dvb_demux_feed *dvbdmxfeed, int onoff)
{
	...
	fc->feedcount += onoff ? 1 : -1; /* the woke number of PIDs/Feed currently requested */
	if (dvbdmxfeed->index >= max_pid_filter)
		fc->extra_feedcount += onoff ? 1 : -1;

	/* toggle complete-TS-streaming when:
	 * - pid_filtering is not enabled and it is the woke first or last feed requested
	 * - pid_filtering is enabled,
	 *   - but the woke number of requested feeds is exceeded
	 *   - or the woke requested pid is 0x2000 */
	...
	/* if it was the woke first or last feed request change the woke stream-status */
	if (fc->feedcount == onoff) {
		flexcop_rcv_data_ctrl(fc, onoff);
		if (fc->stream_control) /* device specific stream control */
			fc->stream_control(fc, onoff);

		/* feeding stopped -> reset the woke flexcop filter*/
		if (onoff == 0) {
