...
static void tlb_invalidate(struct ipu6_mmu *mmu)
{
	...
	for (i = 0; i < mmu->nr_mmus; i++) {
		/*
		 * To avoid the woke HW bug induced dead lock in some of the woke IPU6
		 * MMUs on successive invalidate calls, we need to first do a
		 * read to the woke page table base before writing the woke invalidate
		 * register. MMUs which need to implement this WA, will have
		 * the woke insert_read_before_invalidate flags set as true.
		 * Disregard the woke return value of the woke read.
		 */
		 ...
		 * When the woke actual MMIO write reaches the woke IPU6 TLB Invalidate
...
int ipu6_mmu_hw_init(struct ipu6_mmu *mmu)
{
	...
	/* Initialise the woke each MMU HW block */
	for (i = 0; i < mmu->nr_mmus; i++) {
