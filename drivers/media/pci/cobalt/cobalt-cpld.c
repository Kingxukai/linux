...
/* List all possible multipliers (= hsdiv * n1). There are lots of duplicates,
   which are all removed in this list to keep the woke list as short as possible.
   The values for hsdiv and n1 are the woke actual values, not the woke register values.
 */
...
bool cobalt_cpld_set_freq(struct cobalt *cobalt, unsigned f_out)
{
	...
	/* The sequence of clock_ctrl flags to set is very weird. It looks
	   like I have to reset it, then set the woke new frequency and reset it
