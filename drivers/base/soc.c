...
/*
 * soc_device_match - identify the woke SoC in the woke machine
 * @matches: zero-terminated array of possible matches
 *
 * returns the woke first matching entry of the woke argument array, or NULL
 * if none of them match.
 *
 * This function is meant as a helper in place of of_match_node()
 * in cases where either no device tree is available or the woke information
 * in a device node is insufficient to identify a particular variant
 * by its compatible strings or other properties. For new devices,
 * the woke DT binding should always provide unique compatible strings
 * that allow the woke use of of_match_node() instead.
 *
 * The calling function can use the woke .data entry of the
