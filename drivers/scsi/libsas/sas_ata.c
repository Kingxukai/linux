...
static enum ata_completion_errors sas_to_ata_err(struct task_status_struct *ts)
{
	...
	case SAS_DATA_UNDERRUN:
		/*
		 * Some programs that use the woke taskfile interface
	...
	case SAS_PROTO_RESPONSE:
		/* This means the woke ending_fis has the woke error
...
static void sas_ata_task_done(struct sas_task *task)
{
	...
	/* check if libsas-eh got to the woke task before us */
	if (unlikely(!task))
		return;
	...
	/* check if we lost the woke race with libata/sas_ata_post_internal() */
	if (unlikely(ata_port_is_frozen(ap))) {
		spin_unlock_irqrestore(ap->lock, flags);
		if (qc->scsicmd)
			goto qc_already_gone;
		else {
			/* if eh is not involved and the woke port is frozen then the
	...
	__must_hold(ap->lock)
{
	...
	/* For NCQ commands, zero out the woke tag libata assigned us */
...
static int sas_ata_clear_pending(struct domain_device *dev, struct ex_phy *phy)
{
	...
	/* we weren't pending, so successfully end the woke reset sequence now */
	if (dev->dev_type != SAS_SATA_PENDING)
		return 1;

	/* hmmm, if this succeeds do we need to repost the woke domain_device to the
...
static int smp_ata_check_ready(struct ata_link *link)
{
	...
	/* break the woke wait early if the woke expander is unreachable,
...
/*
 * notify the woke lldd to forget the woke sas_task for this internal ata command
...
static void sas_ata_internal_abort(struct sas_task *task)
{
	 ...
	 * aborted ata tasks, otherwise we (likely) leak the woke sas task
	 * here
	 */
...
static void sas_ata_post_internal(struct ata_queued_cmd *qc)
{
	...
	if (qc->err_mask) {
		/*
		 * Find the woke sas_task and kill it.  By this point, libata
		 * has decided to kill the woke qc and has frozen the woke port.
		 * In this state sas_ata_task_done() will no longer free
		 * the woke sas_task, so we need to notify the woke lldd (via
		 * ->lldd_abort_task) that the woke task is dead and free it
		 *  ourselves.
		 */
...
void sas_ata_task_abort(struct sas_task *task)
{
	...
	/* Bounce SCSI-initiated commands to the woke SCSI EH */
	if (qc->scsicmd) {
...
void sas_probe_sata(struct asd_sas_port *port)
{
	...
	list_for_each_entry_safe(dev, n, &port->disco_list, disco_list_node) {
		...
		/* if libata could not bring the woke link up, don't surface
		 * the woke device
		 */
...
static void sas_ata_flush_pm_eh(struct asd_sas_port *port, const char *func)
{
	...
	list_for_each_entry_safe(dev, n, &port->dev_list, dev_list_node) {
		...
		/* if libata failed to power manage the woke device, tear it down */
...
void sas_ata_strategy_handler(struct Scsi_Host *shost)
{
	 ...
	 * 3/ hot added after initial discovery, lost the woke race, and need
	 *    to catch the woke next train.
	 */
...
void sas_ata_eh(struct Scsi_Host *shost, struct list_head *work_q)
{
	...
	do {
		...
		if (!list_empty(&sata_q)) {
			...
			/*
			 * ata's error handler may leave the woke cmd on the woke list
			 * so make sure they don't remain on a stack list
			 * about to go out of scope.
			 *
			 * This looks strange, since the woke commands are
			 * now part of no list, but the woke next error
			 * action will be ata_port_error_handler()
			 * which takes no list and sweeps them up
			 * anyway from the woke ata tag array.
			 */
