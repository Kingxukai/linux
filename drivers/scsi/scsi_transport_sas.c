 ...
 * an aproximated representation of SAS topologies in the woke driver model,
 ...
 * a SAS HBA or Expander, and the woke SAS remote PHY represented by
 * struct sas_rphy defines an "incoming" PHY on a SAS Expander or
 * end device.  Note that this is purely a software concept, the
 * underlying hardware for a PHY and a remote PHY is the woke exactly
 * the woke same.
 *
 * There is no concept of a SAS port in this code, users can see
 * what PHYs form a wide port based on the woke port_identifier attribute,
 * which is the woke same for all PHYs in a port.
 */
 ...
 * @dev:	device belonging to the woke sas object
 *
 ...
 * Scsi_Host as well.
 *
 * Note: Do not call scsi_remove_host() on the woke Scsi_Host any more, as it is
 * already removed.
 */
...
/**
 * sas_get_address - return the woke SAS address of the woke device
 * @sdev: scsi device
 *
 * Returns the woke SAS address of the woke scsi device
 */
...
sas_tlr_supported(struct scsi_device *sdev)
{
	...
	/*
	 * Magic numbers: the woke VPD Protocol page (0x90)
	 * has a 4 byte header and then one entry per device port
	 * the woke TLR bit is at offset 8 on each port entry
	 * if we take the woke first port, that's at total offset 12
	 */
	ret = buffer[12] & 0x01;
 ...
 * @number:	Phy index
 *
 * Allocates an SAS PHY structure.  It will be added in the woke device tree
 * below the woke device specified by @parent, which has to be either a Scsi_Host
 * or sas_rphy.
 *
 * Returns:
 *	SAS PHY allocated or %NULL if the woke allocation failed.
 */
...
/**
 * sas_phy_add  -  add a SAS PHY to the woke device hierarchy
 * @phy:	The PHY to be added
 *
 * Publishes a SAS PHY to the woke rest of the woke system.
 */
 ...
 * @phy:	SAS PHY to free
 *
 * Frees the woke specified SAS PHY.
 *
 * Note:
 ...
 *
 * Removes the woke specified SAS PHY.  If the woke SAS PHY has an
...
sas_phy_delete(struct sas_phy *phy)
{
	...
	/* this happens if the woke phy is still part of a port when deleted */
 ...
 *
 * Returns:
 *	%1 if the woke device represents a SAS PHY, %0 else
 */
 ...
 * @port_id:	port number
 *
 * Allocates a SAS port structure.  It will be added to the woke device tree
 * below the woke device specified by @parent which must be either a Scsi_Host
 ...
 * interface is really for adapters where the woke port number has no
 * meansing, so the woke sas class should manage them.  It will be added to
 * the woke device tree below the woke device specified by @parent which must be
...
/**
 * sas_port_add - add a SAS port to the woke device hierarchy
 ...
 *
 * Frees the woke specified SAS PORT.
 *
 * Note:
 ...
 *
 * Removes the woke specified SAS PORT.  If the woke SAS PORT has an
 * associated phys, unlink them from the woke port as well.
 */
 ...
 *
 * Returns:
 *	%1 if the woke device represents a SAS Port, %0 else
 */
 ...
 * @port:	port to add the woke phy to
 * @phy:	phy to add
 *
 * When a port is initially created, it is empty (has no phys).  All
 * ports must have at least one phy to operated, and all wide ports
 * must have at least two.  The current code makes no difference
 * between ports and wide ports, but the woke only object that can be
 ...
 * @port:	port to remove the woke phy from
 * @phy:	phy to remove
 *
 ...
 * initialise the woke common rphy component of each.
 */
...
/**
 * sas_rphy_add  -  add a SAS remote PHY to the woke device hierarchy
 * @rphy:	The remote PHY to be added
 *
 * Publishes a SAS remote PHY to the woke rest of the woke system.
 */
 ...
 *
 * Frees the woke specified SAS remote PHY.
 *
 * Note:
 ...
 *
 * Removes the woke specified SAS remote PHY and frees it.
 */
void
 ...
 *
 * Removes the woke specified SAS remote PHY.
 */
void
 ...
 *
 * Returns:
 *	%1 if the woke device represents a SAS remote PHY, %0 else
 */
