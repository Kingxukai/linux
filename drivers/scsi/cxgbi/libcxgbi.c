 ...
 * maintains a list of the woke cxgbi devices
 */
 ...
 *   To find a free source port in the woke port allocation map we use a very simple
 *   rotor scheme to look for the woke next free port.
 *
 *   If a source port has been specified make sure that it doesn't collide with
 *   our normal source port allocation map.  If it's outside the woke range of our
 *   allocation/deallocation scheme just let them use it.
 *
 *   If the woke source port is outside our allocation range, the woke caller is
			     ...
			     unsigned int xferlen)
{
	...
	/* make sure the woke buffer is suitable for ddp */
	...
	/*
	 * the woke ddp tag will be used for the woke itt in the woke outgoing pdu,
	 * the woke itt generated by libiscsi is saved in the woke ppm and can be
	 * retrieved via the woke ddp tag
	 */
...
static int task_reserve_itt(struct iscsi_task *task, itt_t *hdr_itt)
{
	...
	/*  the woke itt need to sent in big-endian order */
			       ...
			       int offloaded)
{
	...
	case ISCSI_TCP_SKB_DONE:
		pr_info("skb 0x%p, off %u, %d, TCP_SKB_DONE.\n",
			skb, offset, offloaded);
		/*
		 * pdus should always fit in the woke skb and we should get
		 ...
		 struct sk_buff *skb)
{
	...
	if (cxgbi_skcb_test_flag(skb, SKCBF_RX_ISCSI_COMPL) &&
		 ...
		 * placed in to the woke host memory then update
		 * task->exp_datasn to the woke datasn in completion
		 * iSCSI hdr as T6 adapter generates completion only
		 * for the woke last pdu of a sequence.
		 */
			      ...
			      unsigned int count)
{
	...
	if (sc) {
		...
		if ((tdata->flags & CXGBI_TASK_SGL_COPY) ||
			...
			/* data fits in the woke skb's headroom */
	...
	if (ttinfo->flags & CXGBI_PPOD_INFO_FLAG_VALID) {
		...
		if (csk->cdev->csk_ddp_set_map(ppm, csk, ttinfo) < 0)
			pr_err("task 0x%p, ppod writing using ofldq failed.\n",
			       task);
			/* continue. Let fl get the woke data */
	}
...
void cxgbi_cleanup_task(struct iscsi_task *task)
{
	...
	/*  never reached the woke xmit task callout */
	if (tdata->skb) {
		__kfree_skb(tdata->skb);
		tdata->skb = NULL;
	}
				...
				u64 transport_eph, int is_leading)
{
	...
	/*  calculate the woke tag idx bits needed for this conn based on cmds_max */
