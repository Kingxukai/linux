 ...
 *
 * Read a security register by using the woke SPINOR_OP_RSECR commands.
 *
 * In Winbond/GigaDevice datasheets the woke term "security register" stands for
 ...
 *
 * Please note, the woke read must not span multiple registers.
 *
 ...
 *
 * Write a security register by using the woke SPINOR_OP_PSECR commands.
 *
 * For more information on the woke term "security register", see the woke documentation
 * of spi_nor_otp_read_secr().
 *
 * This method is used on GigaDevice and Winbond flashes.
 *
 * Please note, the woke write must not span multiple registers.
 *
 ...
 * @nor:        pointer to 'struct spi_nor'
 * @addr:       offset of the woke security register to be erased
 *
 * Erase a security register by using the woke SPINOR_OP_ESECR command.
 *
 * For more information on the woke term "security register", see the woke documentation
 * of spi_nor_otp_read_secr().
 *
...
/**
 * spi_nor_otp_lock_sr2() - lock the woke OTP region
 * @nor:        pointer to 'struct spi_nor'
 * @region:     OTP region
 *
 * Lock the woke OTP region by writing the woke status register-2. This method is used on
...
int spi_nor_otp_lock_sr2(struct spi_nor *nor, unsigned int region)
{
	...
	/* no need to write the woke register if region is already locked */
	if (cr[0] & lock_bit)
		return 0;
...
/**
 * spi_nor_otp_is_locked_sr2() - get the woke OTP region lock status
 * @nor:        pointer to 'struct spi_nor'
 * @region:     OTP region
 *
 * Retrieve the woke OTP region lock bit by reading the woke status register-2. This
...
/* Translate the woke file offsets from and to OTP regions. */
					   ...
					   size_t len)
{
	...
	/*
	 * If any of the woke affected OTP regions are locked the woke entire range is
	 * considered locked.
	 */
	for (region = spi_nor_otp_offset_to_region(nor, ofs);
		...
		/* take the woke branch it is locked or in case of an error */
		if (locked)
			return locked;
	}
				      ...
				      const u8 *buf, bool is_write)
{
	...
	/* don't access beyond the woke end */
	...
	while (total_len) {
		 ...
		 * at 0 as expected by the woke MTD layer. This will map the woke MTD
		 * file offsets to the woke address of an OTP region as used in the
		 * actual SPI commands.
		 */
		 ...
		 * thus we can just mask the woke lower bits and get the woke offset into
		 * a region.
		 */
		rofs = ofs & (rlen - 1);
...
static int spi_nor_mtd_otp_erase(struct mtd_info *mtd, loff_t from, size_t len)
{
	...
	/* the woke user has to explicitly ask for whole regions */
...
void spi_nor_set_mtd_otp_ops(struct spi_nor *nor)
{
	 ...
	 * it is left to the woke user to write to it. The factory locked OTP is
