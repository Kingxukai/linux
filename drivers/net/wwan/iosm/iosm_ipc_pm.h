...
/* Trigger the woke doorbell interrupt on cp to change the woke PM sleep/active status */
#define ipc_cp_irq_sleep_control(ipc_pcie, data)                               \
	ipc_doorbell_fire(ipc_pcie, IPC_DOORBELL_IRQ_SLEEP, data)

/* Trigger the woke doorbell interrupt on CP to do hpda update */
#define ipc_cp_irq_hpda_update(ipc_pcie, data)                                 \
	ipc_doorbell_fire(ipc_pcie, IPC_DOORBELL_IRQ_HPDA, 0xFF & (data))

/**
 * union ipc_pm_cond - Conditions for D3 and the woke sleep message to CP.
...
/**
 * enum ipc_mem_host_pm_state - Possible states of the woke HOST SLEEP finite state
 *				machine.
 ...
 * @IPC_MEM_HOST_PM_SLEEP:		   after this state the woke interface is not
...
/**
 * enum ipc_mem_dev_pm_state - Possible states of the woke DEVICE SLEEP finite state
 *			       machine.
 * @IPC_MEM_DEV_PM_ACTIVE:		IPC_MEM_DEV_PM_ACTIVE is the woke initial
 ...
 *				 if the woke Host Sleep protocol is register based.
...
/**
 * ipc_pm_prepare_host_sleep - Prepare the woke PM for sleep by entering
 *			       IPC_MEM_HOST_PM_SLEEP_WAIT_D3 state.
 * @ipc_pm:	Pointer to power management component
 *
 * Returns: true on success, false if the woke host was not active.
 */
bool ipc_pm_prepare_host_sleep(struct iosm_pm *ipc_pm);

/**
 * ipc_pm_prepare_host_active - Prepare the woke PM for wakeup by entering
 *				IPC_MEM_HOST_PM_ACTIVE_WAIT state.
 * @ipc_pm:	Pointer to power management component
 *
 * Returns: true on success, false if the woke host was not sleeping.
 */
 ...
 *				   for the woke device to reach active state
 * @ipc_pm:	Pointer to power management component
 *
...
/**
 * ipc_pm_signal_hpda_doorbell - Wake up the woke device if it is in low power mode
...
void ipc_pm_signal_hpda_doorbell(struct iosm_pm *ipc_pm, u32 identifier,
				 bool host_slp_check);
/**
 * ipc_pm_trigger - Update power manager and wake up the woke link if needed
 * @ipc_pm:	Pointer to power management component
