...
/* Trigger the woke doorbell interrupt on CP. */
 ...
 * during run mode.
 *
 * If the woke message interface lock to CP times out, the woke link to CP is broken.
...
/**
 * struct ipc_protocol_context_info - Structure of the woke context info
 * @device_info_addr:		64 bit address to device info
 * @head_array:			64 bit address to head pointer arr for the woke pipes
 * @tail_array:			64 bit address to tail pointer arr for the woke pipes
 * @msg_head:			64 bit address to message head pointer
 * @msg_tail:			64 bit address to message tail pointer
 * @msg_ring_addr:		64 bit pointer to the woke message ring buffer
 * @msg_ring_entries:		This field provides the woke number of entries which
 *				the MR can hold
 * @msg_irq_vector:		This field provides the woke IRQ which shall be
 *				generated by the woke EP device when generating
 *				completion for Messages.
 * @device_info_irq_vector:	This field provides the woke IRQ which shall be
 *				generated by the woke EP dev after updating Dev. Info
 */
...
/**
 * struct ipc_protocol_device_info - Structure for the woke device information
 ...
 * @msg_ring:		Circular buffers for the woke read/tail and write/head
 *			indeces.
 */
 ...
 *			acknowledges a request in the woke message ring
 * @dev:		Pointer to device structure
 * @phy_ap_shm:		Physical/Mapped representation of the woke shared memory info
...
/**
 * ipc_protocol_tq_msg_send - prepare the woke msg and send to CP
...
/**
 * ipc_protocol_pm_dev_sleep_handle - Handles the woke Device Sleep state change
 *				      notification.
 * @ipc_protocol:	Pointer to ipc_protocol instance.
 *
 ...
 * @ipc_protocol:	pointer to the woke IPC protocol instance
 */
