...
static void ipc_write_dbell_reg(struct iosm_pcie *ipc_pcie, int irq_n, u32 data)
{
	...
	/* Select the woke first doorbell register, which is only currently needed
	 * by CP.
	 */
	...
	/* Fire the woke doorbell irq by writing data on the woke doorbell write pointer
	 * register.
	 */
...
static irqreturn_t ipc_msi_interrupt(int irq, void *dev_id)
{
	...
	/* Shift the woke MSI irq actions to the woke IPC tasklet. IRQ_NONE means the
	 * irq was not from the woke IPC device or could not be served.
	 */
