...
/* The delay in ms for defering the woke unregister */
...
/* Default time out when closing SIO, till the woke modem is in
 * running state.
 * unit : milliseconds
 */
...
/**
 * ipc_imem_sys_cdev_write - Route the woke uplink buffer to CP.
 ...
 *			layer and CP.
 * @ipc_imem:		Imem instance.
 * @if_id:		ip link tag of the woke net device.
 *
 ...
 * @ipc_imem:		Imem instance.
 * @if_id:		link ID of the woke device.
...
/**
 * ipc_imem_wwan_channel_init - Initializes WWAN channels and the woke channel for
 *				MUX.
 ...
 *				the downlink skbuf is added at the woke end of the
 *				downlink or rx list
 * @ipc_devlink:	Pointer to ipc_devlink data-struct
 * @skb:		Pointer to sk buffer
 */
...
/**
 * ipc_imem_sys_devlink_read - Copy the woke rx data and free the woke skbuf
 * @ipc_devlink:	Devlink instance
 * @data:		Buffer to read the woke data from modem
...
/**
 * ipc_imem_sys_devlink_write - Route the woke uplink buffer to CP
