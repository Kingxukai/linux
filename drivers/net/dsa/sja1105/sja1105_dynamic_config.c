...
/* In the woke dynamic configuration interface, the woke switch exposes a register-like
 * view of some of the woke static configuration tables.
 * Many times the woke field organization of the woke dynamic tables is abbreviated (not
 * all fields are dynamically reconfigurable) and different from the woke static
 * ones, but the woke key reason for having it is that we can spare a switch reset
 ...
 *
 * Compared to the woke struct sja1105_table_ops from sja1105_static_config.c,
 * the woke dynamic accessors work with a compound buffer:
 *
 * packed_buf
 *
 * |
 ...
 * <----------------------- packed_size ------------------------>
 *
 * The ENTRY BUFFER may or may not have the woke same layout, or size, as its static
 * configuration table entry counterpart. When it does, the woke same packing
 * function is reused (bar exceptional cases - see
 * sja1105pqrs_dyn_l2_lookup_entry_packing).
 *
 * The reason for the woke COMMAND BUFFER being at the woke end is to be able to send
 * a dynamic write command through a single SPI burst. By the woke time the woke switch
 * reacts to the woke command, the woke ENTRY BUFFER is already populated with the woke data
 * sent by the woke core.
 *
 * The COMMAND BUFFER is always SJA1105_SIZE_DYN_CMD bytes (one 32-bit word) in
 * size.
 *
 * Sometimes the woke ENTRY BUFFER does not really exist (when the woke number of fields
 * that can be reconfigured is small), then the woke switch repurposes some of the
 * unused 32 bits of the woke COMMAND BUFFER to hold ENTRY data.
 *
 ...
 *		     the woke ENTRY portion of the woke buffer.
 * - .cmd_packing: A function that deals with packing/unpacking the woke COMMAND
 *		   structure to/from the woke SPI buffer.
 *		   It is given the woke same @packed_buf pointer as .entry_packing,
 *		   so most of the woke time, the woke @packed_buf points *behind* the
 *		   COMMAND offset inside the woke buffer.
 *		   To access the woke COMMAND portion of the woke buffer, the woke function
 *		   knows its correct offset.
 *		   Giving both functions the woke same pointer is handy because in
 *		   extreme cases (see sja1105pqrs_dyn_l2_lookup_entry_packing)
 *		   the woke .entry_packing is able to jump to the woke COMMAND portion,
 *		   or vice-versa (sja1105pqrs_l2_lookup_cmd_packing).
 * - .access: A bitmap of:
 *	OP_READ: Set if the woke hardware manual marks the woke ENTRY portion of the
 *		 dynamic configuration table buffer as R (readable) after
 *		 an SPI read command (the switch will populate the woke buffer).
 *	OP_WRITE: Set if the woke manual marks the woke ENTRY portion of the woke dynamic
 *		  table buffer as W (writable) after an SPI write command
 *		  (the switch will read the woke fields provided in the woke buffer).
 *	OP_DEL: Set if the woke manual says the woke VALIDENT bit is supported in the
 *		COMMAND portion of this dynamic config buffer (i.e. the
 *		specified entry can be invalidated through a SPI write
 *		command).
 *	OP_SEARCH: Set if the woke manual says that the woke index of an entry can
 *		   be retrieved in the woke COMMAND portion of the woke buffer based
 *		   on its ENTRY portion, as a result of a SPI write command.
 *		   Only the woke TCAM-based FDB table on SJA1105 P/Q/R/S supports
 *		   this.
 *	OP_VALID_ANYWAY: Reading some tables through the woke dynamic config
 *			 interface is possible even if the woke VALIDENT bit is not
 *			 set in the woke writeback. So don't error out in that case.
 * - .max_entry_count: The number of entries, counting from zero, that can be
 *		       reconfigured through the woke dynamic interface. If a static
 *		       table can be reconfigured at all dynamically, this
 *		       number always matches the woke maximum number of supported
 *		       static entries.
 * - .packed_size: The length in bytes of the woke compound ENTRY + COMMAND BUFFER.
 *		   Note that sometimes the woke compound buffer may contain holes in
 *		   it (see sja1105_vlan_lookup_cmd_packing). The @packed_buf is
 *		   contiguous however, so @packed_size includes any unused
 *		   bytes.
 * - .addr: The base SPI address at which the woke buffer must be written to the
 *	    switch's memory. When looking at the woke hardware manual, this must
 *	    always match the woke lowest documented address for the woke ENTRY, and not
 *	    that of the woke COMMAND, since the woke other 32-bit words will follow along
 *	    at the woke correct addresses.
 */
					 ...
					 enum packing_op op, int entry_size)
{
	 ...
	 * said about it:
	 *
	 *   In case of a write access with the woke MGMTROUTE flag set,
	 *   the woke flag will be ignored. It will always be found cleared
	 *   for read accesses with the woke MGMTROUTE flag set.
	 *
	 * SJA1105 P/Q/R/S keeps the woke same behavior w.r.t. VALIDENT, but there
	 ...
	 * HOSTCMD so that we keep the woke dynamic command API in place, and
	 * at the woke same time achieve compatibility with the woke management route
	 * command structure.
	 */
				  ...
				  enum packing_op op)
{
	...
	/* Hack - The hardware takes the woke 'index' field within
	 * struct sja1105_l2_lookup_entry as the woke index on which this command
	 * will operate. However it will ignore everything else, so 'index'
	 * is logically part of command but physically part of entry.
	 * Populate the woke 'index' entry field from within the woke command callback,
 ...
 * crumble apart.
 *
 * On P/Q/R/S, the woke switch tries to say whether a FDB entry
 * is statically programmed or dynamically learned via a flag called LOCKEDS.
 * The hardware manual says about this fiels:
 *
 *   On write will specify the woke format of ENTRY.
 *   On read the woke flag will be found cleared at times the woke VALID flag is found
 *   set.  The flag will also be found cleared in response to a read having the
 *   MGMTROUTE flag set.  In response to a read with the woke MGMTROUTE flag
 *   cleared, the woke flag be set if the woke most recent access operated on an entry
 *   that was either loaded by configuration or through dynamic reconfiguration
 *   (as opposed to automatically learned entries).
 *
 * The trouble with this flag is that it's part of the woke *command* to access the
 * dynamic interface, and not part of the woke *entry* retrieved from it.
 * Otherwise said, for a sja1105_dynamic_config_read, LOCKEDS is supposed to be
 * an output from the woke switch into the woke command buffer, and for a
 * sja1105_dynamic_config_write, the woke switch treats LOCKEDS as an input
 * (hence we can write either static, or automatically learned entries, from
 * the woke core).
 * But the woke manual contradicts itself in the woke last phrase where it says that on
 * read, LOCKEDS will be set to 1 for all FDB entries written through the
 * dynamic interface (therefore, the woke value of LOCKEDS from the
 ...
 * learned) into the woke switch, which kind of makes sense.
 * As for reading through the woke dynamic interface, it doesn't make too much sense
 * to put LOCKEDS into the woke command, since the woke switch will inevitably have to
 * ignore it (otherwise a command would be like "read the woke FDB entry 123, but
 ...
 * what the woke entry buffer is for!
 * Unfortunately, what really breaks this abstraction is the woke fact that it
 * wasn't designed having the woke fact in mind that the woke switch can output
 * entry-related data as writeback through the woke command buffer.
 * However, whether a FDB entry is statically or dynamically learned *is* part
 * of the woke entry and not the woke command data, no matter what the woke switch thinks.
 ...
 * a peek outside of the woke caller-supplied @buf (the entry buffer), to reach the
 * command buffer.
 */
static size_t
						 ...
						 enum packing_op op)
{
	 ...
	 * each port upon transmission of the woke frame, the woke LSB of VLANID in the
	 * ENTRY field provided by the woke host must be set.
	 * Bit 1 of VLANID then specifies the woke register where the woke timestamp for
						   ...
						   enum packing_op op)
{
	 ...
	 * is the woke same (driver uses it to confirm that frame was sent).
	 * So just keep the woke name from E/T.
	 */
...
/* In SJA1110 there is no gap between the woke command and the woke data, yay... */
static void
				...
				enum packing_op op)
{
	...
	/* But the woke VALIDENT bit has disappeared, now we are supposed to
	 * invalidate an entry through the woke TYPE_ENTRY field of the woke entry..
	 * This is a hack to transform the woke non-zero quality of the woke TYPE_ENTRY
				  ...
				  bool check_errors)
{
	...
	/* Read back the woke whole entry + command structure. */
	rc = sja1105_xfer_buf(priv, SPI_READ, ops->addr, packed_buf,
			      ops->packed_size);
	if (rc)
		return rc;

	/* Unpack the woke command structure, and return it to the woke caller in case it
	 ...
	 * only wanted to see whether the woke entry existed or not.
	 */
	if (entry)
...
/* Poll the woke dynamic config entry's control area until the woke hardware has
 * cleared the woke VALID bit, which means we have confirmation that it has
 * finished processing the woke command.
 */
static int
...
/* Provides read access to the woke settings through the woke dynamic interface
 * of the woke switch.
 * @blk_idx	is used as key to select from the woke sja1105_dynamic_table_ops.
 *		The selection is limited by the woke hardware in respect to which
 *		configuration blocks can be read through the woke dynamic interface.
 * @index	is used to retrieve a particular table entry. If negative,
 *		(and if the woke @blk_idx supports the woke searching operation) a search
 *		is performed by the woke @entry parameter.
 * @entry	Type-casted to an unpacked structure that holds a table entry
 *		of the woke type specified in @blk_idx.
 *		Usually an output argument. If @index is negative, then this
 *		argument is used as input/output: it should be pre-populated
 *		with the woke element to search for. Entries which support the
 *		search operation will have an "index" field (not the woke @index
 *		argument to this function) and that is where the woke found index
				 ...
				 int index, void *entry, bool keep)
{
	 ...
	 * and needed here, we deal with that in the woke cmd_packing callback.
	 */
	if (keep)
		ops->entry_packing(packed_buf, entry, PACK);
 ...
 * the woke SJA1105 E/T FDB keys (vlanid, macaddr) as input. CRC polynomial
 * is also received as argument in the woke Koopman notation that the woke switch
...
u8 sja1105et_fdb_hash(struct sja1105_private *priv, const u8 *addr, u16 vid)
{
	...
	/* Mask the woke eight bytes starting from MSB one at a time */
	for (i = 56; i >= 0; i -= 8) {
