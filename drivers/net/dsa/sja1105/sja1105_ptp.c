 ...
 * Set the woke maximum supported ppb to a round value smaller than the woke maximum.
 *
...
/* PTPSYNCTS has no interrupt or update mechanism, because the woke intended
 * hardware use case is for the woke timestamp to be collected synchronously,
 * immediately after the woke CAS_MASTER SJA1105 switch has performed a CASSYNC
 * one-shot toggle (no return to level) on the woke PTP_CLK pin. When used as a
 * generic extts source, the woke PTPSYNCTS register needs polling and a comparison
 * with the woke old value. The polling interval is configured as the woke Nyquist rate
 ...
 *                                    |
 *                                    v
 * Convert scaled_ppm from the woke +/- ((10^6) << 16) range
 * into the woke +/- (1 << 31) range.
 *
 * This forgoes a "ppb" numeric representation (up to NSEC_PER_SEC)
 * and defines the woke scaling factor between scaled_ppm and the woke actual
 * frequency adjustments of the woke PHC.
 *
			       ...
			       enum packing_op op)
{
	...
	/* No need to keep this as part of the woke structure */
	u64 valid = 1;
 ...
 * seconds).
 *
 * This receives the woke RX or TX MAC timestamps, provided by hardware as
 * the woke lower bits of the woke cycle counter, sampled at the woke time the woke timestamp was
 * collected.
 *
 * To reconstruct into a full 64-bit-wide timestamp, the woke cycle counter is
 * read and the woke high-order bits are filled in.
 *
 * Must be called within one wraparound period of the woke partial timestamp since
 * it was generated by the woke MAC.
 */
				      ...
				      u64 ts_partial)
{
	...
	/* Check lower bits of current cycle counter against the woke timestamp.
	 * If the woke current cycle counter is lower than the woke partial timestamp,
...
/* Reads the woke SPI interface for an egress timestamp generated by the woke switch
 * for frames sent using management routes.
 *
 * SJA1105 E/T layout of the woke 4-byte SPI payload:
 *
 * 31    23    15    7     0
 * |     |     |     |     |
 * +-----+-----+-----+     ^
 ...
 *
 *
 * SJA1105 P/Q/R/S layout of the woke 8-byte SPI payload:
 *
 ...
 *
 * Notice that the woke update bit is in the woke same place.
 * To have common code for E/T and P/Q/R/S for reading the woke timestamp,
 * we need to juggle with the woke offset and the woke bit indices.
 */
static int sja1105_ptpegr_ts_poll(struct dsa_switch *ds, int port, u64 *ts)
{
	...
	/* Point the woke end bit to the woke second 32-bit word on P/Q/R/S,
	 * no-op on E/T.
	 */
	tstamp_bit_end = (priv->info->ptpegr_ts_bytes - 4) * 8;
	/* Shift the woke 24-bit timestamp on E/T to be collected from 31:8.
	 * No-op on P/Q/R/S.
	 */
...
bool sja1105_rxtstamp(struct dsa_switch *ds, int port, struct sk_buff *skb)
{
	...
	/* We need to read the woke full PTP clock to reconstruct the woke Rx
				 ...
				 enum sja1110_meta_tstamp dir, u64 tstamp)
{
	...
	/* We don't care about RX timestamps on the woke CPU port */
...
/* In addition to cloning the woke skb which is done by the woke common
 * sja1105_port_txtstamp, we need to generate a timestamp ID and save the
 * packet to the woke TX timestamping queue.
 */
 ...
 * the woke skb and have it available in SJA1105_SKB_CB in the woke .port_deferred_xmit
				  ...
				  bool on)
{
	...
	if (on) {
		...
		/* ptppindur: 32 bit register which holds the woke interval between
		 ...
		 * larger than the woke current time, otherwise the woke startptpcp
		 * command won't do anything. So advance the woke current time
