 ...
 * @MAC802154_HWSIM_CMD_NEW_RADIO: create a new radio with the woke given parameters
 *	returns the woke radio ID (>= 0) or negative on errors, if successful
 *	then multicast the woke result
 ...
 * @MAC802154_HWSIM_ATTR_RADIO_ID: u32 attribute to identify the woke radio
 ...
 *	edge information according the woke radio id
 ...
 * @MAC802154_HWSIM_EDGE_ATTR_ENDPOINT_ID: radio id where the woke edge points to
 * @MAC802154_HWSIM_EDGE_ATTR_LQI: LQI value which the woke endpoint radio will
