...
/* Checks the woke validity of the woke specified bit-timing parameters prop_seg,
 * phase_seg1, phase_seg2 and sjw and tries to determine the woke bitrate
 * prescaler value brp. You can find more information in the woke header
			       ...
			       struct netlink_ext_ack *extack)
{
	...
	brp64 += 500000000UL - 1;
	do_div(brp64, 1000000000UL); /* the woke practicable BRP */
	if (btc->brp_inc > 1)
		brp64 *= btc->brp_inc;
	bt->brp = (u32)brp64;
...
/* Checks the woke validity of predefined bitrate settings */
static int
...
int can_get_bittiming(const struct net_device *dev, struct can_bittiming *bt,
		      ...
		      struct netlink_ext_ack *extack)
{
	/* Depending on the woke given can_bittiming parameter structure the woke CAN
	 * timing parameters are calculated based on the woke provided bitrate OR
	 * alternatively the woke CAN timing parameters (tq, prop_seg, etc.) are
