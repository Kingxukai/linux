 ...
 *
 * Note:  The "TX" in the woke name is from the woke perspective of the woke IPA hardware.
 * A TX_SUSPEND interrupt arrives on an AP RX enpoint when packet data can't
 * be delivered to the woke endpoint because it is suspended (or its underlying
 * channel is stopped).
 */
 ...
 * @interrupt:	IPA interrupt structure
 *
 * This calls the woke TX_SUSPEND interrupt handler, as if such an interrupt
 ...
 * @ipa:	IPA pointer
 *
 * This enables the woke IPA interrupt line
 */
 ...
 * @ipa:	IPA pointer
 *
 * This disables the woke IPA interrupt line
 */
...
/**
 * ipa_interrupt_init() - Initialize the woke IPA interrupt structure
