 ...
 *
 * @ptp_clock: struct ptp_clock pointer returned by the woke ptp_clock_register()
 *	function.
 * @ptp_clock_info: struct ptp_clock_info that describes a PTP hardware clock
 * @lock: protects the woke time adjustments data
 * @delta: delta between hardware clock and ptp clock in nanoseconds
 * @scale_update_gp2: GP2 time when the woke scale was last updated
 * @scale_update_adj_time_ns: adjusted time when the woke scale was last updated,
 *	in nanoseconds
 * @scaled_freq: clock frequency offset, scaled to 65536000000
 * @last_gp2: the woke last GP2 reading from the woke hardware, used for tracking GP2
 *	wraparounds
