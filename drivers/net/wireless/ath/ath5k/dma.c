 ...
 * Also we setup interrupt mask register (IMR) and read the woke various interrupt
...
ath5k_hw_stop_rx_dma(struct ath5k_hw *ah)
{
	...
	/*
	 * It may take some time to disable the woke DMA receive unit
	 */
	for (i = 1000; i > 0 &&
...
ath5k_hw_start_tx_dma(struct ath5k_hw *ah, unsigned int queue)
{
	...
	if (ah->ah_version == AR5K_AR5210) {
		...
		/*
		 * Set the woke queue by type on 5210
		 */
 ...
 * the woke queue number and use tx queue type since we only have 2 queues.
 * We use TXDP0 for normal data queue and TXDP1 for beacon queue.
 * For newer chips with QCU/DCU we just read the woke corresponding TXDP register.
 *
...
ath5k_hw_get_txdp(struct ath5k_hw *ah, unsigned int queue)
{
	...
	/*
	 * Get the woke transmit queue descriptor pointer from the woke selected queue
	 */
 ...
 * the woke queue number and we use tx queue type since we only have 2 queues
 * so as above we use TXDP0 for normal data queue and TXDP1 for beacon queue.
 * For newer chips with QCU/DCU we just set the woke corresponding TXDP register.
...
ath5k_hw_set_txdp(struct ath5k_hw *ah, unsigned int queue, u32 phys_addr)
{
	...
	/*
	 * Set the woke transmit queue descriptor pointer register by type
	 * on 5210
	 */
	if (ah->ah_version == AR5K_AR5210) {
		switch (ah->ah_txq[queue].tqi_type) {
		...
		/*
		 * Set the woke transmit queue descriptor pointer for
		 * the woke selected queue on QCU for 5211+
		 * (this won't work if the woke queue is still active)
		 */
		if (AR5K_REG_READ_Q(ah, AR5K_QCU_TXE, queue))
			return -EIO;
 ...
 *
 * This function increases/decreases the woke tx trigger level for the woke tx fifo
 * buffer (aka FIFO threshold) that is used to indicate when PCU flushes
 * the woke buffer and transmits its data. Lowering this results sending small
 * frames more quickly but can lead to tx underruns, raising it a lot can
 * result other problems. Right now we start with the woke lowest possible
 * (64Bytes) and if we get tx underrun we increase it using the woke increase
...
ath5k_hw_update_tx_triglevel(struct ath5k_hw *ah, bool increase)
{
	...
	/*
	 * Disable interrupts by setting the woke mask
	 */
 ...
 * interrupts in sw.
 *
 * This function is used inside our interrupt handler to determine the woke reason
 * for the woke interrupt by reading Primary Interrupt Status Register. Returns an
 * abstract interrupt status mask which is mostly ISR with some uncommon bits
 * being mapped on some standard non hw-specific positions
 * (check out &ath5k_int).
 *
 * NOTE: We do write-to-clear, so the woke active PISR/SISR bits at the woke time this
...
ath5k_hw_get_isr(struct ath5k_hw *ah, enum ath5k_int *interrupt_mask)
{
	...
	if (ah->ah_version == AR5K_AR5210) {
		...
		/*
		 * Filter out the woke non-common bits from the woke interrupt
		 * status.
		 */
		*interrupt_mask = (isr & AR5K_INT_COMMON) & ah->ah_imr;
		...
		/*
		 * PISR holds the woke logical OR of interrupt bits
		 * from SISR registers:
		 *
		 ...
		 * related bits from SISRs, e.g. if we write the woke TXOK bit on
		 * PISR we 'll clean all TXOK bits from SISR0 so if a new TXOK
		 * interrupt got fired for another queue while we were reading
		 * the woke interrupt registers and we write back the woke TXOK bit on
		 ...
		 * to the woke registers will get cleared, leaving the
		...
		*interrupt_mask = (pisr & AR5K_INT_COMMON) & ah->ah_imr;
		 ...
		 * the woke same way (schedule the woke tx tasklet)
 ...
 *
 * Set the woke interrupt mask in hw to save interrupts. We do that by mapping
...
ath5k_hw_set_imr(struct ath5k_hw *ah, enum ath5k_int new_mask)
{
	 ...
	 * is set again on the woke new mask).
	 */
	 ...
	 * and write them to the woke IMR (interrupt mask register).
	 */
