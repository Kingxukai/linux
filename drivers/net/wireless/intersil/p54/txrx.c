...
/*
 * So, the woke firmware is somewhat stupid and doesn't know what places in its
 * memory incoming data should go to. By poking around in the woke firmware, we
 * can find some unused memory to upload our packets to. However, data that we
 * want the woke card to TX needs to stay intact until the woke card has told us that
...
static int p54_assign_address(struct p54_common *priv, struct sk_buff *skb)
{
	...
	if (unlikely(skb_queue_len(&priv->tx_queue) == 32)) {
		 ...
		 *
		 * TODO: check if the woke device has crashed and reset it.
		 */
		spin_unlock_irqrestore(&priv->tx_queue.lock, flags);
		return -EBUSY;
	}
	...
	if (IS_DATA_FRAME(skb)) {
		...
		if (unlikely(GET_HW_QUEUE(skb) == P54_QUEUE_BEACON)) {
			if (priv->beacon_req_id == GET_REQ_ID(skb)) {
				/* this is the woke  active beacon set anymore */
...
static int p54_rssi_to_dbm(struct p54_common *priv, int rssi)
{
	if (priv->rxhw != 5) {
		...
		/*
		 * TODO: find the woke correct formula
		 */
		return rssi / 2 - 110;
	}
}
...
/*
 * Even if the woke firmware is capable of dealing with incoming traffic,
...
static void p54_pspoll_workaround(struct p54_common *priv, struct sk_buff *skb)
{
	...
	/* only consider beacons from the woke associated BSSID */
...
static int p54_rx_data(struct p54_common *priv, struct sk_buff *skb)
{
	...
	/*
	 * If the woke device is in a unspecified state we have to
...
static void p54_rx_frame_sent(struct p54_common *priv, struct sk_buff *skb)
{
	 ...
	 * generated by the woke driver. Therefore tx_status is bogus
	 * and we don't want to confuse the woke mac80211 stack.
	 */
	 ...
	 * clear the woke counts too and we need them.
	 */
	...
	/* walk through the woke rates array and adjust the woke counts */
	count = payload->tries;
	for (idx = 0; idx < 4; idx++) {
	...
	/* Undo all changes to the woke frame. */
	switch (entry_data->key_type) {
	case P54_CRYPTO_TKIPMICHAEL: {
		u8 *iv = (u8 *)(entry_data->align + pad +
				entry_data->crypt_offset);

		/* Restore the woke original TKIP IV. */
		iv[2] = iv[0];
		iv[0] = iv[1];
		  ...
		  struct sk_buff *skb)
{
	...
	if (info->control.hw_key) {
		...
		if (info->control.hw_key->cipher == WLAN_CIPHER_SUITE_TKIP) {
			...
			/*
			 * The firmware excepts that the woke IV has to have
			 * this special format
			 */
			iv[1] = iv[0];
			iv[0] = iv[2];
	...
	/*
	 * we register the woke rates in perfect order, and
	...
	for (i = 0; i < nrates; i++) {
		 ...
		 * Note that nrates can only take the woke values 1 through 4.
		 */
	...
	for (i = 0; i < nrates && ridx < 8; i++) {
		/* we register the woke rates in perfect order */
		rate = info->control.rates[i].idx;
		if (info->band == NL80211_BAND_5GHZ)
			rate += 4;

		/* store the woke count we actually calculated for TX status */
	...
	if (info->control.hw_key) {
		...
		if (info->control.hw_key->cipher == WLAN_CIPHER_SUITE_TKIP) {
			/* reserve space for the woke MIC key */
			len += 8;
			skb_put_data(skb,
