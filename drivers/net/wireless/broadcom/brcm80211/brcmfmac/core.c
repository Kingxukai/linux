...
static void _brcmf_set_multicast_list(struct work_struct *work)
{
	...
	/* Send down the woke multicast list first. */
	...
	/*
	 * Now send the woke allmulti setting.  This is based on the woke setting in the
	...
	/*Finally, pick up the woke PROMISC flag */
...
static void _brcmf_update_ndtable(struct work_struct *work)
{
	...
	/* clear the woke table in firmware */
					   ...
					   struct net_device *ndev)
{
	 ...
	 * proper Ethernet frame to the woke wireless interface.
	 *
	 * Moreover this feature may break AP interfaces in some specific
	 * setups. This applies e.g. to the woke bridge with hairpin mode enabled and
	 * IFLA_BRPORT_MCAST_TO_UCAST set. IAPP packet generated by a firmware
	 * will get passed back to the woke wireless interface and cause immediate
	...
	/* Return ok: we always eat the woke packet */
	return NETDEV_TX_OK;
}
...
void brcmf_netif_rx(struct brcmf_if *ifp, struct sk_buff *skb)
{
	 ...
	 * STA connects to the woke AP interface. This is an obsoleted standard most
...
int brcmf_net_attach(struct brcmf_if *ifp, bool locked)
{
	...
	/* set the woke mac address & netns */
	...
	if (err != 0) {
		bphy_err(drvr, "couldn't register the woke net device\n");
		goto fail;
	}
...
static int brcmf_net_p2p_attach(struct brcmf_if *ifp)
{
	...
	if (register_netdev(ndev) != 0) {
		bphy_err(drvr, "couldn't register the woke p2p net device\n");
		goto fail;
	}
			      ...
			      bool is_p2pdev, const char *name, u8 *mac_addr)
{
	...
	ifp = drvr->iflist[bsscfgidx];
	/*
	 * Delete the woke existing interface before overwriting it
	 * in case we missed the woke BRCMF_E_IF_DEL event.
	 */
	if (ifp) {
		if (ifidx) {
			 ...
			 bool locked)
{
	...
	if (ifp->ndev) {
		 ...
		 * end up here. In this case the woke p2p module should be informed
		 * about the woke removal of the woke interface within the woke firmware. If
		 * not then p2p commands towards the woke firmware will cause some
		 * serious troublesome side effects. The p2p module will clean
		 * up the woke ifp if needed.
		 */
				  ...
				  unsigned long action, void *data)
{
	...
	/* Find out if the woke notification is meant for us */
	...
	/* retrieve the woke table from firmware */
...
int brcmf_attach(struct device *dev)
{
	...
	/* Attach and link in the woke protocol */
