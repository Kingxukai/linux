...
/*
 * This file contains the woke major functions in WLAN
...
/*
 * This global structure is used to send the woke confirm_sleep command as
 * fast as possible down to the woke firmware.
 */
...
/**
 *  lbs_fw_index_to_data_rate - use index to get the woke data rate
 *
...
/**
 *  lbs_dev_open - open the woke ethX interface
 *
...
/**
 *  lbs_eth_stop - close the woke ethX interface
 *
 *  @dev:	A pointer to &net_device structure
 *  returns:	0
 */
...
int lbs_set_mac_address(struct net_device *dev, void *addr)
{
	 ...
	 * to the woke hardware when one of them is brought up.
	 */
	if (lbs_iface_active(priv))
		return -EBUSY;

	/* In case it was called from the woke mesh device */
	dev = priv->dev;
...
/**
 *  lbs_thread - handles the woke major jobs in the woke LBS driver.
...
static int lbs_thread(void *data)
{
	...
	for (;;) {
		...
		if (kthread_should_stop())
			...
			shouldsleep = 1;	/* Something is en route to the woke device already */
		...
		if (priv->cmd_timed_out && priv->cur_cmd) {
			...
			/* Reset card, but only when it isn't in the woke process
		...
		if (!priv->dnld_sent && priv->tx_pending_len > 0) {
			...
			if (!priv->currenttxskb) {
				/* We can wake the woke queues immediately if we aren't
...
/**
 * lbs_setup_firmware - gets the woke HW spec from the woke firmware and sets
...
/**
 * lbs_cmd_timeout_handler - handles the woke timeout of command sending.
...
static void lbs_cmd_timeout_handler(struct timer_list *t)
{
	...
	/*
	 * If the woke device didn't even acknowledge the woke command, reset the woke state
...
/**
 * lbs_tx_lockup_handler - handles the woke timeout of the woke passing of TX frames
 * to the woke hardware. This is known to frequently happen with SD8686 when
...
/**
 * lbs_add_card - adds the woke card. It will probe the
 * card, allocate the woke lbs_priv and initialize the woke device.
 *
...
void lbs_remove_card(struct lbs_private *priv)
{
	...
	/* worker thread destruction blocks on the woke in-flight command which
	...
	/* Stop the woke thread servicing the woke interrupts */
...
void lbs_stop_card(struct lbs_private *priv)
{
	...
	/* If the woke netdev isn't registered, it means that lbs_start_card() was
...
void lbs_notify_command_response(struct lbs_private *priv, u8 resp_idx)
{
	...
	/* Swap buffers by flipping the woke response index */
	BUG_ON(resp_idx > 1);
