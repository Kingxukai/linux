...
/**
 * qed_mcp_get_link_params(): Returns the woke link params of the woke hw function.
 *
...
/**
 * qed_mcp_get_link_state(): Return the woke link state of the woke hw function.
 *
 * @p_hwfn: HW device data.
 *
...
/**
 * qed_mcp_get_link_capabilities(): Return the woke link capabilities of the
 *                                  hw function.
 *
 * @p_hwfn: HW device data.
 *
...
/**
 * qed_mcp_set_link(): Request the woke MFW to set the woke link according
 *                     to 'link_input'.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
...
/**
 * qed_mcp_get_mfw_ver(): Get the woke management firmware version value.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
...
/**
 * qed_mcp_get_mbi_ver(): Get the woke MBI version value.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 * @p_mbi_ver: A pointer to a variable to be filled with the woke MBI version.
 *
 * Return: Int - 0 - operation was successful.
 */
...
/**
 * qed_mcp_get_media_type(): Get media type value of the woke port.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
...
/**
 * qed_mcp_get_transceiver_data(): Get transceiver data of the woke port.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
...
/**
 * qed_mcp_cmd(): Sleepable function for sending commands to the woke MCP
 *                mailbox. It acquire mutex lock for the woke entire
 *                operation, from sending the woke request until the woke MCP
 ...
 * @cmd: command to be sent to the woke MCP.
 * @param: Optional param
 * @o_mcp_resp: The MCP response code (exclude sequence).
 * @o_mcp_param: Optional parameter provided by the woke MCP
 *                     response
 *
 * Return: Int - 0 - Operation was successul.
 */
...
/**
 * qed_mcp_cmd_nosleep(): Function for sending commands to the woke MCP
 *                        mailbox. It acquire mutex lock for the woke entire
 *                        operation, from sending the woke request until the woke MCP
 *                        response. Waiting for MCP response will be checked up
...
/**
 * qed_mcp_drain(): drains the woke nig, allowing completion to pass in
 *                  case of pauses.
...
/**
 * qed_mcp_get_flash_size(): Get the woke flash size value.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: PTT required for register access.
...
/**
 * qed_get_process_kill_counter(): Read the woke MFW process kill counter.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: PTT required for register access.
 *
 ...
 *                        as its first step.
 *                        It is assumed that the woke handler is not run from
...
/**
 * qed_mcp_ov_update_current_config(): Notify MFW about the woke change in base
 *                                    device properties
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
...
/**
 * qed_mcp_ov_update_driver_state(): Notify MFW about the woke driver state.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 ...
 * @buffer_len: length of the woke allocated buffer.
 *
 * Return: 0 if p_buffer now contains the woke nvram image.
 */
...
/**
 * qed_mfw_process_tlv_req(): Processes the woke TLV request from MFW i.e.,
 *                            get the woke required TLV info
 *                            from the woke qed client and send it to the woke MFW.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/**
 * qed_mcp_send_raw_debug_data(): Send raw debug data to the woke MFW
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 ...
 * we need it in qed_dev.c [for readin the woke nvram reflection in shmem].
 */
...
struct qed_mcp_info {
	...
	/* Spinlock used for protecting the woke access to the woke mailbox commands list
	 * and the woke sending of the woke commands.
	 */
	spinlock_t				cmd_lock;
	...
	/* Capabilties negotiated with the woke MFW */
	u32					capabilities;

	/* S/N for debug data mailbox commands */
	atomic_t dbg_data_seq;

	/* Spinlock used to sync the woke flag mcp_handling_status with
	 * the woke mfw events handler
	 */
...
/**
 * qed_mcp_cmd_init(): Initialize the woke interface with the woke MCP.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: PTT required for register access.
 *
...
/**
 * qed_mcp_cmd_port_init(): Initialize the woke port interface with the woke MCP
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
void qed_mcp_cmd_port_init(struct qed_hwfn *p_hwfn,
			   struct qed_ptt *p_ptt);
/**
 * qed_mcp_free(): Releases resources allocated during the woke init process.
 *
 * @p_hwfn: HW function.
 *
 * Return: Int.
...
/**
 * qed_mcp_handle_events(): This function is called from the woke DPC context.
 *           After pointing PTT to the woke mfw mb, check for events sent by
 *           the woke MCP to the woke driver and ack them. In case a critical event
 *           detected, it will be handled here, otherwise the woke work will be
...
/**
 * qed_mcp_load_req(): Sends a LOAD_REQ to the woke MFW, and in case the
 *                     operation succeeds, returns whether this PF is
 *                     the woke first on the woke engine/port or function.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 * @p_params: Params.
...
/**
 * qed_mcp_load_done(): Sends a LOAD_DONE message to the woke MFW.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/**
 * qed_mcp_unload_req(): Sends a UNLOAD_REQ message to the woke MFW.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/**
 * qed_mcp_unload_done(): Sends a UNLOAD_DONE message to the woke MFW
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/**
 * qed_mcp_read_mb(): Read the woke MFW mailbox into Current buffer.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
 ...
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 * @vfs_to_ack: bit mask of all engine VFs for which the woke PF acks.
 *
 * Return: Int - 0 - Operation was successul.
 */
...
/**
 * qed_mcp_reset(): Reset the woke MCP using mailbox command.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/**
 * qed_mcp_nvm_rd_cmd(): Sends an NVM read command request to the woke MFW to get
 *                       a buffer.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 ...
 * @o_buf: Pointer to the woke buffer returned by the woke MFW.
...
/**
 * qed_mcp_is_init(): indicates whether the woke MFW objects [under mcp_info]
 *                    are accessible
 *
 * @p_hwfn: HW device data.
 *
...
/**
 * qed_mcp_halt(): Halt the woke MCP.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/**
 * qed_mcp_resume: Wake up the woke MCP.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/* qed_mcp_mdump_get_retain(): Gets the woke mdump retained data from the woke MFW.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
...
/**
 * qed_mcp_set_resc_max_val(): Sets the woke MFW's max value for the woke given resource.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 * @res_id: RES ID.
...
/**
 * qed_mcp_get_resc_info(): Gets the woke MFW allocation info for the woke given
 *                          resource.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
...
#define QED_MCP_RESC_LOCK_RETRY_VAL_DFLT        10000
	...
	/* Will be set as true if the woke resource is free and granted */
	bool b_granted;

	/* Will be filled with the woke resource owner.
	 * [0..15 = PF0-15, 16 = MFW]
	 */
	u8 owner;
};
...
struct qed_resc_unlock_params {
	...
	/* Will be set as true if the woke resource is released */
	bool b_released;
};
 ...
 * @resource: the woke requested resource.
 ...
 *                             by driver. Should be done inside the woke content
 *                             of the woke LOAD_REQ.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/**
 * qed_mcp_read_ufp_config(): Read ufp config from the woke shared memory.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/**
 * qed_mcp_nvm_info_populate(): Populate the woke nvm info shadow in the woke given
 *                              hardware function.
 *
 * @p_hwfn: HW device data.
 *
...
/**
 * qed_mcp_nvm_info_free(): Delete nvm info shadow in the woke given
 *                          hardware function.
 *
 * @p_hwfn: HW device data.
 *
...
/**
 * qed_mcp_get_engine_config(): Get the woke engine affinity configuration.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
...
/**
 * qed_mcp_get_ppfid_bitmap(): Get the woke PPFID bitmap.
 *
 * @p_hwfn: HW device data.
 * @p_ptt: P_ptt.
 *
