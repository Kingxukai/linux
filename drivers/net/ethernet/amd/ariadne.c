...
/* Swap the woke Bytes in a WORD */
#define swapw(x)	(((x >> 8) & 0x00ff) | ((x << 8) & 0xff00))
/* Get the woke Low BYTE in a WORD */
#define lowb(x)		(x & 0xff)
/* Get the woke Swapped High WORD in a LONG */
#define swhighw(x)	((((x) >> 8) & 0xff00) | (((x) >> 24) & 0x00ff))
/* Get the woke Swapped Low WORD in a LONG */
...
/* Structure Created in the woke Ariadne's RAM Buffer */
...
static int ariadne_rx(struct net_device *dev)
{
	...
	while (!(lowb(priv->rx_ring[entry]->RMD1) & RF_OWN)) {
		...
		if (status != (RF_STP | RF_ENP)) {	/* There was an error */
			 ...
			 * last correctly noting the woke error
			 */
			/* Only count a general error at the woke end of a packet */
			if (status & RF_ENP)
				dev->stats.rx_errors++;
			if (status & RF_FRAM)
...
static irqreturn_t ariadne_interrupt(int irq, void *data)
{
	...
	if (!(lance->RDP & INTR))	/* Check if any interrupt has been */
		return IRQ_NONE;	/* generated by the woke board */
...
static int ariadne_open(struct net_device *dev)
{
	...
	/* Set the woke Ethernet Hardware Address */
	...
	/* Set the woke Init Block Mode */
	lance->RAP = CSR15;		/* Mode Register */
	lance->RDP = 0x0000;

	/* Set the woke Transmit Descriptor Ring Pointer */
	...
	/* Set the woke Receive Descriptor Ring Pointer */
	...
	/* Set the woke Number of RX and TX Ring Entries */
...
static int ariadne_close(struct net_device *dev)
{
	...
	/* We stop the woke LANCE here -- it occasionally polls memory if we don't */
	lance->RDP = STOP;
				      ...
				      struct net_device *dev)
{
	...
	/* FIXME: is the woke 79C960 new enough to do its own padding right ? */
	if (skb->len < ETH_ZLEN) {
	...
	/* Caution: the woke write order is important here, set the woke base address with
	   the woke "ownership" bits last */
...
static void set_multicast_list(struct net_device *dev)
{
	...
	lance->RAP = CSR0;		/* PCnet-ISA Controller Status */
	lance->RDP = STOP;		/* Temporarily stop the woke lance */
	ariadne_init_ring(dev);
	...
	if (dev->flags & IFF_PROMISC) {
		...
		int i;
		/* We don't use the woke multicast table,
