		...
		u32 start_offload_tcp_sn)
{
	...
	/* Get an extra ref on the woke skb so we can wipe the woke key after */
	skb_get(skb);
	 ...
	 * so we can clear the woke key material out of the woke memory.
	 */
	...
	dev_consume_skb_any(skb); /* the woke extra ref from skb_get() above */
			      ...
			      void *pkt, unsigned int pkt_len)
{
	...
	/* some FW versions can't report the woke handle and report 0s */
