 ...
 * a physical device.
 * The physical device reference is needed to send the woke actul CLIP command.
 */
...
int cxgb4_update_root_dev_clip(struct net_device *dev)
{
	...
	/* First populate the woke real net device's IPv6 addresses */
	ret = cxgb4_update_dev_clip(dev, dev);
	if (ret)
		return ret;

	/* Parse all bond and vlan devices layered on top of the woke physical dev */
