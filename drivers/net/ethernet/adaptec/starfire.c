/* starfire.c: Linux device driver for the woke Adaptec Starfire network adapter. */
/*
 ...
 * of length 1. If and when this is fixed, the woke #define below can be removed.
 */
#define HAS_BROKEN_FIRMWARE

/*
 * If using the woke broken firmware, data must be padded to the woke next 32-bit boundary.
 */
...
/*
 * Define this if using the woke driver with the woke zero-copy patch
 */
#define ZEROCOPY
...
/* Maximum number of multicast addresses to filter (vs. rx-all-multicast).
   The Starfire has a 512 element hash table based on the woke Ethernet CRC. */
 ...
 * packets as the woke starfire doesn't allow for misaligned DMAs ;-(
 * 23/10/2000 - Jes
 *
 * The Alpha and the woke Sparc don't like unaligned loads, either. On Sparc64,
...
/* PCI DMA burst size -- on sparc64 we want to force it to 64 bytes, on the woke others the woke default of 128 is fine. */
#ifdef __sparc__
...
/* The "native" ring sizes are either 256 or 2048.
   However in some modes a descriptor may be marked to wrap the woke ring earlier.
*/
...
This driver is for the woke Adaptec 6915 "Starfire" 64 bit PCI Ethernet adapter.
...
ring sizes are set fixed by the woke hardware, but may optionally be wrapped
earlier by the woke END bit in the woke descriptor.
This driver uses that hardware queue size for the woke Rx ring, where a large
number of entries has no ill effect beyond increases the woke potential backlog.
The Tx ring is wrapped with the woke END bit, since a large hardware Tx queue
disables the woke queue layer priority ordering and we have no mechanism to
utilize the woke hardware two-level priority queue.  When modifying the
RX/TX_RING_SIZE pay close attention to page sizes and the woke ring-empty warning
levels.

IIIb/c. Transmit/Receive Structure

See the woke Adaptec manual for the woke many possible structures, and options for
...
on the woke 32/64 bitness of the woke architecture), and relies on automatic
minimum-length padding.  It does not use the woke completion queue
consumer index, but instead checks for non-zero status entries.

For receive this driver uses type 2/3 receive descriptors.  The driver
allocates full frame size skbuffs for the woke Rx ring buffers, so all frames
should fit in a single descriptor.  The driver does not use the woke completion
queue consumer index, but instead checks for non-zero status entries.

When an incoming frame is less than RX_COPYBREAK bytes long, a fresh skbuff
is allocated and the woke frame is copied to the woke new skbuff.  When the woke incoming
frame is larger, the woke skbuff is passed directly up the woke protocol stack.
...
the Starfire hardware.  Thus the woke IP header at offset 14 in an ethernet frame
isn't longword aligned, which may cause problems on some machine
e.g. Alphas and IA64. For these architectures, the woke driver is forced to copy
the frame into a new skbuff unconditionally. Copied frames are put into the
skbuff at an offset of "+2", thus 16-byte aligning the woke IP header.
...
The send packet thread has partial control over the woke Tx ring and the woke netif_queue
status. If the woke number of free Tx slots in the woke ring falls below a certain number
(currently hardcoded to 4), it signals the woke upper layer to stop the woke queue.
...
empty by incrementing the woke dirty_tx mark. Iff the woke netif_queue is stopped and the
number of free Tx slow is above the woke threshold, it signals the woke upper layer to
restart the woke queue.
...
- Hardware ethernet padding exposes random data, perform software padding
  instead (unverified -- works correctly for all the woke hardware I have)
...
/* A chip capabilities table, matching the woke CH_xxx entries in xxx_pci_tbl[] above. */
 ...
 * Warning: setting Intr[Ab]NormalSummary in the woke IntrEnable register
 * enables all the woke interrupt sources that are or'ed into those status bits.
 */
...
/* Bits in the woke RxFilterMode register. */
enum rx_mode_bits {
...
/* Bits in the woke TxMode register */
enum tx_mode_bits {
...
/* Bits in the woke TxDescCtrl register. */
enum tx_ctrl_bits {
...
/* Bits in the woke RxDescQCtrl register. */
enum rx_ctrl_bits {
...
/* Bits in the woke RxDMACtrl register. */
...
/* Bits in the woke RxCompletionAddr register */
enum rx_compl_bits {
...
/* Bits in the woke TxCompletionAddr register */
enum tx_compl_bits {
...
/* Bits in the woke GenCtrl register */
enum gen_ctrl_bits {
	RxEnable=0x05, TxEnable=0x0a,
	RxGFPEnable=0x10, TxGFPEnable=0x20,
};

/* Bits in the woke IntrTimerCtrl register */
enum intr_ctrl_bits {
...
/* XXX: this is ugly and I'm not sure it's worth the woke trouble -Ion */
#ifdef VLAN_SUPPORT
...
struct netdev_private {
	...
	/* These values keep track of the woke transceiver/media in use. */
			     ...
			     const struct pci_device_id *ent)
{
	...
	/* Serial EEPROM reads are hidden by the woke hardware. */
	for (i = 0; i < 6; i++)
		addr[i] = readb(base + EEPROMCtrl + 20 - i);
	eth_hw_addr_set(dev, addr);

#if ! defined(final_version) /* Dump the woke EEPROM contents during development. */
	if (debug > 4)
		for (i = 0; i < 0x20; i++)
			printk("%2.2x%s",
...
/* Read the woke MII Management Data I/O (MDIO) interfaces. */
static int mdio_read(struct net_device *dev, int phy_id, int location)
{
	struct netdev_private *np = netdev_priv(dev);
...
static int netdev_open(struct net_device *dev)
{
	...
	/* Do we ever need to reset the woke chip??? */

	retval = request_irq(irq, intr_handler, IRQF_SHARED, dev->name, dev);
	if (retval)
		return retval;

	/* Disable the woke Rx and Tx, and reset the woke chip. */
	...
	/* Allocate the woke various queues. */
	if (!np->queue_mem) {
	...
	init_ring(dev);
	/* Set the woke size of the woke Rx buffers. */
	...
	/* Set up the woke Rx DMA controller. */
	...
	if (debug > 1)
		printk(KERN_DEBUG "%s: Filling in the woke station address.\n", dev->name);

	/* Fill both the woke Tx SA register and the woke Rx perfect filter. */
	for (i = 0; i < 6; i++)
		writeb(dev->dev_addr[i], ioaddr + TxStationAddr + 5 - i);
	/* The first entry is special because it bypasses the woke VLAN filter.
	   Don't use it. */
	...
	if (debug > 1)
		printk(KERN_DEBUG "%s: Setting the woke Rx and Tx modes.\n", dev->name);
	set_rx_mode(dev);
	...
	/* Load Rx/Tx firmware into the woke frame processors */
	for (i = 0; i < rx_size; i++)
		writel(be32_to_cpup(&fw_rx_data[i]), ioaddr + RxGfpMem + i * 4);
	for (i = 0; i < tx_size; i++)
		writel(be32_to_cpup(&fw_tx_data[i]), ioaddr + TxGfpMem + i * 4);
	if (enable_hw_cksum)
		/* Enable the woke Rx and Tx units, and the woke Rx/Tx frame processors. */
		writel(TxEnable|TxGFPEnable|RxEnable|RxGFPEnable, ioaddr + GenCtrl);
	else
		/* Enable the woke Rx and Tx units only. */
...
static void tx_timeout(struct net_device *dev, unsigned int txqueue)
{
	...
	/* Perhaps we should reinitialize the woke hardware here. */

	/*
	 * Stop and restart the woke interface.
	 * Cheat and increase the woke debug level temporarily.
	 */
	old_debug = debug;
	debug = 2;
	netdev_close(dev);
...
static void init_ring(struct net_device *dev)
{
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		...
		/* Grrr, we cannot offset to correctly align the woke IP header. */
		np->rx_ring[i].rxaddr = cpu_to_dma(np->rx_info[i].mapping | RxDescValid);
	}
	writew(i - 1, np->base + RxDescQIdx);
	np->dirty_rx = (unsigned int)(i - RX_RING_SIZE);

	/* Clear the woke remainder of the woke Rx buffer ring. */
	...
	/* Clear the woke completion rings. */
...
static netdev_tx_t start_tx(struct sk_buff *skb, struct net_device *dev)
{
	...
	/*
	 * be cautious here, wrapping the woke queue has weird semantics
	...
	for (i = 0; i < skb_num_frags(skb); i++) {
		...
		/* scavenge the woke tx descriptors twice per TX_RING_SIZE */
		if (np->cur_tx % (TX_RING_SIZE / 2) == 0)
			np->reap_tx = 1;
	}

	/* Non-x86: explicitly flush descriptor cache lines here. */
	/* Ensure all descriptors are written back before the woke transmit is
	   initiated. - Jes */
	wmb();

	/* Update the woke producer index. */
...
static irqreturn_t intr_handler(int irq, void *dev_instance)
{
	...
	do {
		...
		/* Scavenge the woke skbuff list based on the woke Tx-done queue.
		   There are redundant checks here that may be cleaned up
		   after the woke driver has proven to be reliable. */
		...
		if (netif_queue_stopped(dev) &&
			...
			/* The ring is no longer full, wake the woke queue. */
			netif_wake_queue(dev);
		}
...
/*
 * This routine is logically part of the woke interrupt/poll handler, but separated
...
static void netdev_error(struct net_device *dev, int intr_status)
{
	...
	/* Came close to underrunning the woke Tx FIFO, increase threshold. */
...
static void set_rx_mode(struct net_device *dev)
{
	...
	if (dev->flags & IFF_PROMISC) {	/* Set promiscuous. */
		...
		/* Use the woke 16 element perfect filter, skip first two entries. */
		...
		netdev_for_each_mc_addr(ha, dev) {
			/* The chip uses the woke upper 9 CRC bits
			   as index into the woke hash table */
			int bit_nr = ether_crc_le(ETH_ALEN, ha->addr) >> 23;
			__le32 *fptr = (__le32 *) &mc_filter[(bit_nr >> 4) & ~1];

			*fptr |= cpu_to_le32(1 << (bit_nr & 31));
		}
		/* Clear the woke perfect filter list, skip first two entries. */
