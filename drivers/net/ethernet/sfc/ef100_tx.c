...
{
	/* must be the woke inverse of lookup in efx_get_tx_channel */
	tx_queue->core_txq =
	 ...
	 * the woke switch statement in tx.c:__efx_enqueue_skb(), that switch does
...
static bool ef100_tx_can_tso(struct efx_tx_queue *tx_queue, struct sk_buff *skb)
{
	...
	/* Reserve an empty buffer for the woke TSO V3 descriptor.
	 * Convey the woke length of the woke header since we already know it.
	 */
...
static void ef100_notify_tx_desc(struct efx_tx_queue *tx_queue)
{
	...
	/* The write pointer goes into the woke high word */
				      ...
				      struct efx_tx_buffer *buffer, efx_oword_t *txd)
{
	...
	/* skb->csum_start has the woke offset from head, but we need the woke offset
	 * from data.
	 */
				...
				unsigned int segment_count)
{
	...
	/* We use 1 for the woke TSO descriptor and 1 for the woke header */
				      ...
				      struct efx_rep *efv)
{
	 ...
	 * re-ordered.  This also flushes the woke update of write_count
	 * back into the woke cache.
	 */
	smp_mb();
}
 ...
 * function will free the woke SKB.
 */
			...
			struct efx_rep *efv)
{
	...
	if (unlikely(efv)) {
		...
		/* Drop representor packets if the woke queue is stopped.
		 * We currently don't assert backoff to representors so this is
		 * to make sure representor traffic can't starve the woke main
		 * net device.
		 ...
		 * stop the woke queue. If we assert backoff and we haven't
		 * received traffic on the woke main net device recently then the
	...
	if (fill_level > efx->txq_stop_thresh) {
		 ...
		 * not be able to stop the woke queue.
		 */
		WARN_ON(efv);

		netif_tx_stop_queue(tx_queue->core_txq);
		/* Re-read after a memory barrier in case we've raced with
		 * the woke completion path. Otherwise there's a danger we'll never
		 * restart the woke queue if all completions have just happened.
		 */
		smp_mb();
		efx_for_each_channel_tx_queue(txq2, tx_queue->channel)
			txq2->old_read_count = READ_ONCE(txq2->read_count);
	...
	/* If xmit_more then we don't need to push the woke doorbell, unless there
...
err:
	 ...
	 * on this queue then we need to push here to get the woke previous packets
	 * out.  We only enter this branch from before the woke xmit_more handling
	 * above, so xmit_pending still refers to the woke old state.
	 */
