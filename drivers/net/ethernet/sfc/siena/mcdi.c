 ...
 * things) the woke raw error code, by means of efx_siena_mcdi_display_error.
...
static void efx_mcdi_ev_death(struct efx_nic *efx, int rc)
{
	...
	if (efx_mcdi_complete_sync(mcdi)) {
		...
		/* Consume the woke status word since efx_siena_mcdi_rpc_finish() won't */
		 ...
		 * efx_siena_mcdi_poll_reboot(), the woke MC reboot wasn't noticed but the
