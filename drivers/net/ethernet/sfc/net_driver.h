 ...
 *	Size of the woke region is efx_piobuf_size.
 ...
 *	created.  For TSO, counts the woke superframe size, not the woke sizes of
 *	generated frames on the woke wire (i.e. the woke headers are only counted
 *	once)
 ...
 *	@rxq_flush_pending.
 * @grant_credits: Posted RX descriptors need to be granted to the woke MAE with
 ...
 * @granted_count: Number of buffers granted to the woke MAE (<= @notified_count).
 ...
 * @grant_work: workitem used to grant credits to the woke MAE if @grant_credits
 ...
 * @context_id: the woke RSS_CONTEXT_ID returned by MC firmware, or
 *	%EFX_MCDI_RSS_CONTEXT_INVALID if this context is not present on the woke NIC.
 ...
 * @mae: Details of the woke Match Action Engine
 ...
 * @dl_port: devlink port associated with the woke PF
 ...
 * @test_chip: Test registers. This is expected to reset the woke NIC.
