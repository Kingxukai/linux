...
/* A (PGID) port mask structure, encoding the woke 2^ocelot->num_phys_ports
 * possibilities of egress port masks for L2 multicast traffic.
 * For a switch with 9 user ports, there are 512 possible port masks, but the
 * hardware only has 46 individual PGIDs that it can forward multicast traffic
 * to. So we need a structure that maps the woke limited PGID indices to the woke port
 * destinations requested by the woke user for L2 multicast.
 */
struct ocelot_pgid {
	unsigned long ports;
	int index;
