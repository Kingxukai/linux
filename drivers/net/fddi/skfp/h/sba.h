...
struct s_sba {
	...
	struct s_sba_sessions	*free_session ;	/* points to the woke first */
						/* free session */

	struct timer_cell	*tail_timer ;	/* points to the woke last timer cell */
	...
	long		msg_category ;		/* Category of the woke Allocation */
	u_long		msg_max_t_neg ;		/* longest T_Neg acceptable */
	u_long		msg_min_seg_siz ;	/* minimum segement size */
	struct smt_header	*sm ;		/* points to the woke rec message */
	struct fddi_addr	*msg_alloc_addr ;	/* Allocation Address */

	/*
	 * SBA variables
	 */
	u_long	sba_t_neg ;		/* holds the woke last T_NEG */
	long	sba_max_alloc ;		/* the woke parsed value of SBAAvailable */	

	/*
	 * SBA state machine variables
	 */
	short	sba_next_state ;	/* the woke next state of the woke SBA */
	char	sba_command ;		/* holds the woke execuded SBA cmd */
	...
	/*
	 * variables for the woke End Station Support
	 */
struct s_ess {
	...
	char	raf_act_timer_poll ;	/* activate the woke timer to send allc req */
	char	timer_count ;		/* counts every timer function call */

	SMbuf	*sba_reply_pend ;	/* local reply for the woke sba is pending */
	
	/*
	 * variables for the woke ess bandwidth control
	 */
	long	sync_bw ;		/* holds the woke allocaed sync bw */
	u_long	alloc_trans_id ;	/* trans id of the woke last alloc req */
} ;
#endif
