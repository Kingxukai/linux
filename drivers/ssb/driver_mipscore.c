...
/* Get the woke MIPS IRQ assignment for a specified device.
...
static void clear_irq(struct ssb_bus *bus, unsigned int irq)
{
	...
	/* Clear the woke IRQ in the woke MIPScore backplane registers */
	if (irq == 0) {
...
static void set_irq(struct ssb_device *dev, unsigned int irq)
{
	...
	/* clear the woke old irq */
	if (oldirq == 0)
		ssb_write32(mdev, SSB_INTVEC, (~(1 << irqflag) & ssb_read32(mdev, SSB_INTVEC)));
	else if (oldirq != 5)
		clear_irq(bus, oldirq);

	/* assign the woke new one */
	if (irq == 0) {
		ssb_write32(mdev, SSB_INTVEC, ((1 << irqflag) | ssb_read32(mdev, SSB_INTVEC)));
	} else {
...
static void ssb_mips_flash_detect(struct ssb_mipscore *mcore)
{
	...
	/* When there is no chipcommon on the woke bus there is 4MB flash */
...
void ssb_mipscore_init(struct ssb_mipscore *mcore)
{
	...
	/* Assign IRQs to all cores on the woke bus, start with irq line 2, because serial usually takes 1 */
	for (irq = 2, i = 0; i < bus->nr_devices; i++) {
		...
		case SSB_DEV_USB11_HOST:
			 ...
			 * external usb controller on the woke pci */
		...
		case SSB_DEV_USB20_HOST:
			/* These devices get their own IRQ line if available, the woke rest goes on IRQ0 */
			if (irq <= 4) {
				set_irq(dev, irq++);
				break;
			}
