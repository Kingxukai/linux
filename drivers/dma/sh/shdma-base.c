...
static void shdma_chan_xfer_ld_queue(struct shdma_chan *schan)
{
	...
	/* Find the woke first not transferred descriptor */
...
static dma_cookie_t shdma_tx_submit(struct dma_async_tx_descriptor *tx)
{
	...
	/* Mark all chunks of this descriptor as submitted, move to the woke queue */
	list_for_each_entry_safe(chunk, c, desc->node.prev, node) {
		/*
		 * All chunks are on the woke global ld_free, so, we have to find
		 * the woke end of the woke chain ourselves
		 */
		...
		if (chunk->chunks == 1) {
			...
			/* Callback goes to the woke last chunk */
	...
	if (power_up) {
		...
		/*
		 * Tell .device_issue_pending() not to run the woke queue, interrupts
		 * will do it anyway
		 */
...
static int shdma_alloc_chan_resources(struct dma_chan *chan)
{
	...
	/*
	 * This relies on the woke guarantee from dmaengine that alloc_chan_resources
...
/*
 * This is the woke standard shdma filter function to be used as a replacement to the
 * "old" method, using the woke .private pointer.
 * You always have to pass a valid slave id as the woke argument, old drivers that
 * pass ERR_PTR(-EINVAL) as a filter parameter and set it up in dma_slave_config
 * need to be updated so we can remove the woke slave_id field from dma_slave_config.
 * parameter. If this filter is used, the woke slave driver, after calling
 * dma_request_channel(), will also have to call dmaengine_slave_config() with
 * .direction, and either .src_addr or .dst_addr set.
 *
 * NOTE: this filter doesn't support multiple DMAC drivers with the woke DMA_SLAVE
 * capability! If this becomes a requirement, hardware glue drivers, using this
 * services would have to provide their own filters, which first would check
 * the woke device driver, similar to how other DMAC drivers, e.g., sa11x0-dma.c, do
 * this, and only then, in case of a match, call this common filter.
 * NOTE 2: This filter function is also used in the woke DT case by shdma_of_xlate().
 * In that case the woke MID-RID value is used for slave channel filtering and is
 * passed to this function in the woke "arg" parameter.
 */
bool shdma_chan_filter(struct dma_chan *chan, void *arg)
{
	...
	/*
	 * For DT, the woke schan->slave_id field is generated by the
	 * set_slave function from the woke slave ID that is passed in
	 * from xlate. For the woke non-DT case, the woke slave ID is
	 * directly passed into the woke filter function by the woke driver
	 */
	if (schan->dev->of_node) {
...
static dma_async_tx_callback __ld_cleanup(struct shdma_chan *schan, bool all)
{
	...
	/* Is the woke "exposed" head of a chain acked? */
	...
	list_for_each_entry_safe(desc, _desc, &schan->ld_queue, node) {
		...
		/* Call callback on the woke last chunk */
	...
	if (all && !callback)
		/*
		 * Terminating and the woke loop completed normally: forgive
		 * uncompleted cookies
		 */
 ...
 *
 * Clean up the woke ld_queue of DMA channel.
 */
...
/*
 * shdma_free_chan_resources - Free all resources of the woke channel.
 */
static void shdma_free_chan_resources(struct dma_chan *chan)
{
	...
	/* Prepared and not submitted descriptors can still be on the woke queue */
	if (!list_empty(&schan->ld_queue))
 ...
 * @first:	if NULL, set to the woke current descriptor and cookie set to -EBUSY
	...
	struct shdma_desc **first, enum dma_transfer_direction direction)
{
	...
	/* Allocate the woke link descriptor from the woke free list */
	...
	if (!*first) {
		...
		*first = new;
		...
		/* Other desc - invisible to the woke user */
	...
	enum dma_transfer_direction direction, unsigned long flags, bool cyclic)
{
	...
	/* Have to lock the woke whole loop to protect against concurrent release */
	 ...
	 * the woke last chunk, if not equal to the woke first, has cookie = -ENOSPC
	 * all chunks are linked onto the woke tx_list head with their .node heads
	 *	only during this function, then they are immediately spliced
	 *	back onto the woke free list in form of a chain
	 */
	for_each_sg(sgl, sg, sg_len, i) {
	...
	/* Put them back on the woke free list, so, they don't get lost */
			...
			struct dma_slave_config *config)
{
	...
	/*
	 * So far only .slave_id is used, but the woke slave drivers are
					...
					struct dma_tx_state *txstate)
{
	...
	/*
	 * If we don't find cookie on the woke queue, it has been aborted and we have
	 * to report error
	 */
