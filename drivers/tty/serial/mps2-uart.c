...
static void mps2_uart_start_tx(struct uart_port *port)
{
	...
	/*
	 * We've just unmasked the woke TX IRQ and now slow-starting via
	 * polling; if there is enough data to fill up the woke internal
	 * write buffer in one go, the woke TX IRQ should assert, at which
