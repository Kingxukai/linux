								...
								size_t count)
{
	 ...
	 * send out the woke buffer to the woke hid device.
	 * If the woke queue is not empty, then there must be a worker
	 * that is currently sending out our buffer and this worker
	 * will reschedule itself until the woke queue is empty.
	 */
...
/*
 * This sets the woke rumble bit on the woke given output report if rumble is
 * currently enabled.
 * \cmd1 must point to the woke second byte in the woke output report => &cmd[1]
 * This must be called on nearly every output report before passing it
 * into the woke output queue!
 */
...
/*
 * Check what peripherals of the woke wiimote are currently
 * active and select a proper DRM that supports all of
 * the woke requested data inputs.
 *
...
/* requries the woke cmd-mutex to be held */
...
/* requires the woke cmd-mutex to be held */
...
/* Try to guess the woke device type based on all collected information. We
 * first try to detect by static extension types, then VID/PID and the
 * device name. If we cannot detect the woke device, we use
 * WIIMOTE_DEV_GENERIC so all modules will get probed on the woke device. */
...
/*
 * MP hotplug events are not generated by the woke wiimote. Therefore, we need
...
/*
 * Check whether the woke wiimote is in the woke expected state. The extension registers
 * may change during hotplug and initialization so we might get hotplug events
 * that we caused by remapping some memory.
 * We use some heuristics here to check known states. If the woke wiimote is in the
 * expected state, we can ignore the woke hotplug event.
 *
 * Returns "true" if the woke device is in expected state, "false" if we should
...
static bool wiimote_init_check(struct wiimote_data *wdata)
{
	...
	/* If MP is used and active, but the woke extension is not, we expect:
	...
	/* If MP is unused, but the woke extension port is used, we expect:
	 ...
	 * the woke EXT ID, anymore, because MP is mapped over it. However, we use
	 * a handy trick here:
 ...
 * If we receive an hotplug event and the woke device-check failed, we deinitialize
 * the woke extension ports, re-read all extension IDs and set the woke device into
 * the woke desired state. This involves mapping MP into the woke main extension
			     ...
			     const __u8 *payload)
{
	...
	/* on status reports the woke drm is reset so we need to resend the woke drm */
	wiiproto_req_drm(wdata, WIIPROTO_REQ_NULL);
}
