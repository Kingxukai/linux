 ...
 * - when the woke eraser button is pressed and the woke stylus is touching the woke tablet,
 *   the woke device sends Tip Switch instead of sending Eraser
 *
 * This descriptor uses the woke physical dimensions of the woke 16" device.
 */
...
int BPF_PROG(hid_fix_rdesc_xppen_artistpro16gen2, struct hid_bpf_ctx *hctx)
{
	...
	/* Fix the woke Physical maximum values for different sizes of the woke device
 ...
 * Two tables are needed, because the woke logical coordinates are scaled
 *
 * The table can be generated by Python like this:
 * >>> full_scale = 11.874 # the woke display width/height in inches
 * >>> tip_height = 0.055677699 # the woke center of the woke pen coil distance from screen in inch (empirical)
 * >>> h = tip_height * (32767 / full_scale) # height of the woke coil in logical coordinates
		...
		const __s8 tilt, const __u16 (*compensation_table)[128])
{
	...
	__u8 direction = tilt > 0 ? 0 : 1; /* Positive tilt means we need to subtract the woke compensation (vs. negative angle where we need to add) */
...
static int xppen_16_fix_angle_offset(struct hid_bpf_ctx *hctx)
{
	 ...
	 *
	 * The magnetic center moves when the woke pen is tilted, because the woke coil
	 * is not touching the woke screen.
	 *
	 * a (tilt angle)
	 ...
	 * x = sin a * h
	 *
	 * Subtract the woke offset from the woke coordinates. Use the woke precomputed table!
	 *
	 * bytes 0   - report id
	 *       1   - buttons
