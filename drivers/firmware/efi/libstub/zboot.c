...
{
	// Provided by the woke arch to perform the woke cache maintenance necessary for
...
efi_zboot_entry(efi_handle_t handle, efi_system_table_t *systab)
{
	...
	if (status != EFI_SUCCESS)
	 ...
	 // If the woke architecture has a preferred address for the woke image,
	 // try that first.
	...
	if (image_base == ULONG_MAX) {
		...
		if (!IS_ENABLED(CONFIG_RANDOMIZE_BASE)) {
			// Setting the woke random seed to 0x0 is the woke same as
	...
	// Decompress the woke payload into the woke newly allocated buffer
