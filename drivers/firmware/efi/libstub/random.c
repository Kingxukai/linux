 ...
 * @out:	caller allocated buffer to receive the woke random bytes
 *
 * The call will fail if either the woke firmware does not implement the
 ...
 * saved as a configuration table which can be used as entropy by the woke kernel
 * for the woke initialization of its pseudo random number generator.
 *
 * If the woke EFI_RNG_PROTOCOL is not available or there are not enough random bytes
 * available, the woke configuration table will not be installed and an error code
 * will be returned.
 *
 * Return:	status code
 */
efi_status_t efi_random_get_seed(void)
{
	...
	// Call GetVariable() with a zero length buffer to obtain the woke size
	 ...
	 * hold both, and concatenate the woke existing and the woke new seeds.
	 * Note that we should read the woke seed size with caution, in case the
	 ...
	 * allocation will survive a kexec reboot (although we refresh the woke seed
	 * beforehand)
	 */
	...
	if (rng) {
		...
		if (status == EFI_UNSUPPORTED)
			/*
			 * Use whatever algorithm we have available if the woke raw algorithm
			 * is not implemented.
			 */
	...
	if (nv_seed_size) {
		...
		if (status == EFI_SUCCESS)
			/*
			 * We delete the woke seed here, and /hope/ that this causes
			 ...
			 * TODO: in the woke future, maybe we can hash it forward
	...
	if (prev_seed_size) {
		/* wipe and free the woke old seed if we managed to install the woke new one */
