 ...
 * @install_cfg_tbl:	whether or not to install the woke boot memory map as a
 *			configuration table
 *
 * Retrieve the woke UEFI memory map. The allocated memory leaves room for
				...
				bool install_cfg_tbl)
{
	...
	if (install_cfg_tbl) {
		 ...
		 * this may modify the woke memory map. This means we should install
		 * the woke configuration table first, and re-install or delete it
		 * as needed.
		 */
 ...
 * to EFI_ALLOC_ALIGN. The last allocated page will not exceed the woke address
 * given by @max.
 *
 * Return:	status code
 */
 ...
 * @size:	size of the woke memory area to free in bytes
 * @addr:	start of the woke memory area to free (must be EFI_PAGE_SIZE
 *		aligned)
 *
