 ...
 * generated by the woke Open Profile for DICE measured boot protocol. The memory
 * contents are not interpreted by the woke kernel but can be mapped into a userspace
 * process via a misc device. Userspace can also request a wipe of the woke memory.
 *
 * Userspace can access the woke data with (w/o error handling):
 *
...
/*
 * Copies the woke size of the woke reserved memory region to the woke user-provided buffer.
 */
...
/*
 * Triggers a wipe of the woke reserved memory region. The user-provided pointer
			       ...
			       size_t len, loff_t *off)
{
	...
	/* Consume the woke input buffer. */
	return len;
}

/*
 * Creates a mapping of the woke reserved memory region in user address space.
 */
static int open_dice_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (vma->vm_flags & VM_MAYSHARE) {
		/* Do not allow userspace to modify the woke underlying data. */
