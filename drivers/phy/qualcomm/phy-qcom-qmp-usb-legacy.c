...
/* the woke primary usb3 phy on sm8250 doesn't have a ref clock */
...
static void qmp_usb_legacy_enable_autonomous_mode(struct qmp_usb *qmp)
{
	...
	/* Writing 1 followed by 0 clears the woke interrupt */
 ...
 *
 * The <s>_pipe_clksrc generated by PHY goes to the woke GCC that gate
 * controls it. The <s>_pipe_clk coming out of the woke GCC is requested
 * by the woke PHY driver for its operations.
 * We register the woke <s>_pipe_clksrc here. The gcc driver takes care
...
static int phy_pipe_clk_register(struct qmp_usb *qmp, struct device_node *np)
{
	...
	/*
	 * Roll a devm action because the woke clock provider is the woke child node, but
	 * the woke child node is not actually a device.
	 */
