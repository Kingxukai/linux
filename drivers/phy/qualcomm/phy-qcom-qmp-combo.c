...
/*
 * We need to calibrate the woke aux setting here as many times
 * as the woke caller tries
 */
static int qmp_v3_calibrate_dp_phy(struct qmp_combo *qmp)
{
...
static int qmp_combo_parse_dt_legacy_dp(struct qmp_combo *qmp, struct device_node *np)
{
	...
	/*
	 * Get memory resources from the woke DP child node:
	 * Resources are indexed as: tx -> 0; rx -> 1; pcs -> 2;
	 * tx2 -> 3; rx2 -> 4
	 *
	 * Note that only tx/tx2 and pcs (dp_phy) are used by the woke DP
	 * implementation.
	 */
...
static int qmp_combo_parse_dt_legacy_usb(struct qmp_combo *qmp, struct device_node *np)
{
	...
	/*
	 * Get memory resources from the woke USB child node:
	 * Resources are indexed as: tx -> 0; rx -> 1; pcs -> 2;
