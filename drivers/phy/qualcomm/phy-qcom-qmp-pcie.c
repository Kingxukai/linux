...
struct qmp_phy_cfg {
	 ...
	 * for the woke Root Complex and End Point use cases.
	 *
...
static int qmp_pcie_init(struct phy *phy)
{
	...
	/*
	 * We need to check the woke existence of init sequences in two cases:
...
static int qmp_pcie_power_off(struct phy *phy)
{
	...
	/*
	 * While powering off the woke PHY, only qmp->nocsr_reset needs to be checked. In
	 * this way, no matter whether the woke PHY settings were initially programmed by
...
static int phy_pipe_clk_register(struct qmp_pcie *qmp, struct device_node *np)
{
	 ...
	 * unless other frequency is specified in the woke PHY config.
	 */
 ...
 *
 * The <s>_phy_aux_clksrc generated by PHY goes to the woke GCC that gate
 * controls it. The <s>_phy_aux_clk coming out of the woke GCC is requested
 * by the woke PHY driver for its operations.
 * We register the woke <s>_phy_aux_clksrc here. The gcc driver takes care
...
static int qmp_pcie_parse_dt_legacy(struct qmp_pcie *qmp, struct device_node *np)
{
	 ...
	 * mapped as a part of the woke PCS_MISC region.
	 */
