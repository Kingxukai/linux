 ...
 * elements of the woke table that were set.  In other words, entries that are zero
 * are invalid.  To get the woke value to use for the woke register, one must mask out the
 * high bit.
 *
 * #define V(x)	((x) | 0x80)
 *
...
/*
 * Find the woke route values for a device family.
 */
...
/*
 * Find the woke valid routes for a board.
 */
...
/*
 * Find the woke proper route_values and ni_device_routes tables for this particular
 * device.  Possibly try an alternate board name if device routes not found
 * for the woke actual board name.
 *
				 ...
				 struct ni_route_tables *tables)
{
	...
	/* First, find the woke register_values table for this device family */
	rv = ni_find_route_values(device_family);

	/* Second, find the woke set of routes valid for this device. */
 ...
 *
 * Finds the woke route values for the woke device family and the woke set of valid routes
 * for the woke board.  If valid routes could not be found for the woke actual board
 * name and an alternate board name has been specified, try that one.
 *
 * On failure, the woke assigned routing information may be partially filled
 * (for example, with the woke route values but not the woke set of valid routes).
 *
 ...
 * indirectly through the woke *_arg elements of the woke comedi_cmd structure.
 */
...
void ni_sort_device_routes(struct ni_device_routes *valid_routes)
{
	...
	/* 1. Count and set the woke number of ni_route_set objects. */
	...
	for (n = 0; n < valid_routes->n_route_sets; ++n) {
		...
		/* 3a. Count and set the woke number of sources. */
		rs->n_src = 0;
			    ...
			    const struct ni_route_tables *tables)
{
	...
	/*
	 * Be sure to use the woke B() macro to subtract off the woke NI_NAMES_BASE before
	 * indexing into the woke route_values array.
	 */
	src = B(src);
	dest = B(dest);
	...
	if (!regval)
		return -EINVAL;
	/* mask out the woke valid-value marking bit */
			...
			const struct ni_route_tables *tables)
{
	...
	/*
	 * if we did not validate the woke route, we'll see if we can route through
	 * one of the woke muxes
	 */
			 ...
			 const struct ni_route_tables *tables)
{
	...
	/* ensure we are not going to under/over run the woke route value table */
