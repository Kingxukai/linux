 ...
 * populates the woke frame info's when required.*/
    ...
    unsigned int *bds_factor)
{
	...
	/* delta in pixels to account for rounding margin in the woke calculation */
	unsigned int delta = max_bds_factor * max_rounding_margin;

	/* Assert if the woke resolutions are not set */
    ...
    struct ia_css_frame_info *vf_info)
{
	...
	/*
	 * Set up the woke info of the woke input frame with
	 * the woke ISP required resolution
	 */
	...
	*bds_out_info = *in_info;
	...
	/* bayer ds and fractional ds cannot be enabled at the woke same time,
	...
	/*
	   ...
	   * instead it has an output port. However, the woke output is
	   * generated by vf_veceven module in which we might have
	   * a downscaling (by 1x, 2x, or 4x). Because the woke resolution
	...
	*out_info = *vf_info;
	 ...
	 * out and vf infos will be the woke same. Otherwise, we set out resolution
    ...
    int stream_config_left_padding)
{
	...
	if (mode == IA_CSS_BINARY_MODE_VIDEO) {
		...
		at the woke same time, so we disable bds_out_info when
    ...
    struct ia_css_frame_info *vf_info)
{
	...
	assert(pipe);
	assert(in_info);
	/* Note: if the woke following assert fails, the woke number of ports has been
    ...
    struct ia_css_frame_info *vf_info)
{
	...
	/* the woke in_info is only used for resolution to enable
	   bayer down scaling. */
