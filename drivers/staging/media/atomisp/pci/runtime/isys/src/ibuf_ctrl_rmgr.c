    ...
    uint32_t	*start_addr)
{
	...
	/* Check if there is an available un-used handle with the woke size
	 * that will fulfill the woke request.
	 */
	...
	if (!input_buffer_found) {
		 ...
		 * request. Allocate a new handle with the woke requested size.
		 */
