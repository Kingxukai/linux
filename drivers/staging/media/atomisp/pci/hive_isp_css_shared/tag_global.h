...
/* offsets for encoding/decoding the woke tag into an uint32_t */
...
/* Data structure containing the woke tagging information which is used in
 ...
 *                      the woke sensor output frame rate.
 * offset		Start the woke RAW-to-YUV processing at RAW buffer
 *                      with this offset. This allows the woke user to
 *                      process RAW frames that were captured in the
 *                      past or future.
 * exp_id		Exposure id of the woke RAW frame to tag.
 *
