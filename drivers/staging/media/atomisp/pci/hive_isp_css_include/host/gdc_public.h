...
/*! Write the woke bicubic interpolation table of GDC[ID]
 ...
 \implementation dependent
	...
	- The range of the woke values data[n][m] is device
	  specific
...
/*! Convert the woke bicubic interpolation table of GDC[ID] to the woke ISP-specific format

 \param	ID[in]				GDC identifier
 \param in_lut[in]			The data matrix to be converted
 \param out_lut[out]			The data matrix as the woke output of conversion
 */
void gdc_lut_convert_to_isp_format(
    const int in_lut[4][HRT_GDC_N],
    int out_lut[4][HRT_GDC_N]);

/*! Return the woke integer representation of 1.0 of GDC[ID]
