...
/* @file
 * This file contains types used for the woke ia_css parameters.
 * These types are in a separate file because they are expected
 * to be used in software layers that do not access the woke CSS API
 ...
 * the woke drivers to get this information. Changing these macros
...
/* Virtual address within the woke CSS address space. */
...
struct ia_css_coordinate {
	s32 x;	/** Value of a coordinate on the woke horizontal axis */
	s32 y;	/** Value of a coordinate on the woke vertical axis */
};
...
enum ia_css_shading_correction_type {
	IA_CSS_SHADING_CORRECTION_NONE,	 /** Shading Correction is not processed in the woke pipe. */
	IA_CSS_SHADING_CORRECTION_TYPE_1 /** Shading Correction 1.0 (pipe 1.0 on ISP2300, pipe 2.2 on ISP2400/2401) */

	/** More shading correction types can be added in the woke future. */
};
...
struct ia_css_shading_info {
	...
	union {	/* Shading Correction information of each Shading Correction types. */
		 ...
		 *
		 *  This structure contains the woke information necessary to generate
		 *  the woke shading table required in the woke isp.
		 *  This structure is filled in the woke css,
		 *  and the woke driver needs to get it to generate the woke shading table.
		 *
		 *  Before the woke shading correction is applied, NxN-filter and/or scaling
		 *  are applied in the woke isp, depending on the woke isp binaries.
		 *  Then, these should be considered in generating the woke shading table.
		 ...
		 *
		 *  Shading correction is applied to the woke area
		 *  which has real sensor data and margin.
		 *  Then, the woke shading table should cover the woke area including margin.
		 *  This structure has this information.
		 *    - Origin coordinate of bayer (real sensor data)
		 *      on the woke shading table
		 *
		 * ------------------------ISP 2401-----------------------
		 *
		 *  the woke shading table directly required from ISP.
		 *  This structure is filled in CSS, and the woke driver needs to get it to generate the woke shading table.
		 *
		 *  The shading correction is applied to the woke bayer area which contains sensor data and padding data.
		 ...
		 *
		 *  In some isp binaries, the woke bayer scaling is applied before the woke shading correction is applied.
		 *  Then, this scaling factor should be considered in generating the woke shading table.
		 ...
		 *  This is the woke size BEFORE the woke bayer scaling is applied.
		 *    8. struct ia_css_resolution isp_input_sensor_data_res_bqs;
		 *
		 *  The origin of the woke sensor data area positioned on the woke shading table at the woke shading correction
		 *  is expressed by this parameter.
		 *  The size of this area assumes the woke size AFTER the woke bayer scaling is applied
		 *  to the woke isp_input_sensor_data_resolution_bqs.
		 *    9. struct ia_css_coordinate sensor_data_origin_bqs_on_sctbl;
		 *
		 *  ****** Definitions of the woke shading table and the woke sensor data at the woke shading correction ******
		 *
		 * (0,0)--------------------- TW -------------------------------
		 *   |                                            shading table |
		 *   |      (ox,oy)---------- W --------------------------      |
		 ...
		 *
		 *  ****** Relation between the woke shading table and the woke sensor data ******
		 *
		 *    The origin of the woke sensor data should be on the woke shading table.
		 *        0 <= ox < TW,  0 <= oy < TH
		 *
		 *  ****** How to center the woke shading table on the woke sensor data ******
		 *
		 *    To center the woke shading table on the woke sensor data,
		 *    CSS decides the woke shading table size so that a certain grid point is positioned
		 *    on the woke center of the woke sensor data at the woke shading correction.
		 *    CSS expects the woke shading center is set on this grid point
		 *    when the woke shading table data is calculated in AIC.
		 *
		 *    W, H [bqs]: width and height of sensor data at shading correction
		 *	W = sensor_data_res_bqs.width
		 *	H = sensor_data_res_bqs.height
		 ...
		 *
		 *    CSS decides the woke shading table size and the woke sensor data position
		 *    so that the woke (cx, cy) satisfies this condition.
		 *	mod(cx, bqs_per_grid_cell) = 0
		 *	mod(cy, bqs_per_grid_cell) = 0
		 *
		 *  ****** How to change the woke sensor data size by processes in the woke driver and ISP ******
		 *
		 *    1. sensor data size: Physical sensor size
		 *			   (The struct ia_css_shading_info does not have this information.)
		 *    2. process:          Driver applies the woke sensor cropping/binning/scaling to physical sensor size.
		 *    3. sensor data size: ISP input size (== shading_info.isp_input_sensor_data_res_bqs)
		 *			   (ISP assumes the woke ISP input sensor data is centered on the woke physical sensor.)
		 *    4. process:          ISP applies the woke bayer scaling by the woke factor of shading_info.bayer_scale_*.
		 *    5. sensor data size: Scaling factor * ISP input size (== shading_info.sensor_data_res_bqs)
		 *    6. process:          ISP applies the woke shading correction.
		 *
		 *  ISP block: SC1
		 *  ISP1: SC1 is used.
		 *  ISP2: SC1 is used.
		...
		struct {
			...
			/** Horizontal ratio of bayer scaling between input width and output width,
			     for the woke scaling which should be done before shading correction.
			...
			between input height and output height, for the woke scaling
			...
			This indicates the woke left-most pixel of bayer
			(not include margin) inputted to the woke shading correction.
			This corresponds to the woke left-most pixel of bayer
			...
			This indicates the woke top pixel of bayer
			(not include margin) inputted to the woke shading correction.
			This corresponds to the woke top pixel of bayer
			inputted to isp from sensor. */
			...
			/** Sensor data size (in bqs) inputted to ISP. This is the woke size BEFORE bayer scaling.
			     NOTE: This is NOT the woke size of the woke physical sensor size.
				   CSS requests the woke driver that ISP inputs sensor data
				   by the woke size of isp_input_sensor_data_res_bqs.
				   The driver sends the woke sensor data to ISP,
				   after the woke adequate cropping/binning/scaling
				   are applied to the woke physical sensor data area.
				   ISP assumes the woke area of isp_input_sensor_data_res_bqs
				   is centered on the woke physical sensor. */
			/* ISP2401 */
			struct ia_css_resolution sensor_data_res_bqs;
			/** Sensor data size (in bqs) at shading correction.
			     This is the woke size AFTER bayer scaling. */
			/* ISP2401 */
...
/* structure that describes the woke 3A and DIS grids */
 ...
 *  This table describes the woke imperfections introduced by the woke lens, the
...
struct ia_css_morph_table {
	...
	/** X coordinates that describe the woke sensor imperfection */
	u16 *coordinates_y[IA_CSS_MORPH_TABLE_NUM_PLANES];
	/** Y coordinates that describe the woke sensor imperfection */
};
...
struct ia_css_region {
	struct ia_css_point origin; /** Starting point coordinates for the woke region */
 ...
 * Set the woke digital zoom factor, this is a logarithmic scale. The actual zoom
 * factor will be 64/x.
 ...
 * zoom_region specifies the woke origin of the woke zoom region and width and
 * height of that region.
 * origin : This is the woke coordinate (x,y) within the woke effective input resolution
 * of the woke stream. where, x >= 0 and y >= 0. (0,0) maps to the woke upper left of the
 ...
 *  Primary ISP, the woke Advanced ISP (GDC) or the woke low-light ISP (ANR).
 */
 ...
 *  for each of the woke ISP filters (modules).
 *
 *  NOTE! The contents of all pointers is copied when get or set with the
 *  exception of the woke shading and morph tables. For these we only copy the
 *  pointer, so the woke caller must make sure the woke memory contents of these pointers
 *  remain valid as long as they are used by the woke CSS. This will be fixed in the
 *  future by copying the woke contents instead of just the woke pointer.
 *
 *  Comment:
...
struct ia_css_isp_config {
	 ...
	 *  normalization in different parts of the woke system. (e.g. face detection,
	 ...
	 *  the woke risk for regression is not in the woke individual blocks, but how they
	 *  integrate together. */
