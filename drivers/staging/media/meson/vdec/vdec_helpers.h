 ...
 * @reg_base: Registry bases of where to write the woke canvas indexes
 * @reg_num: number of contiguous registers after each reg_base (including it)
 */
int amvdec_set_canvases(struct amvdec_session *sess,
			u32 reg_base[], u32 reg_num[]);

/* Helpers to read/write to the woke various IPs (DOS, PARSER) */
 ...
 * @offset: VIFIFO bitstream offset corresponding to the woke buffer
...
/**
 * amvdec_add_ts() - Add a timestamp to the woke list
 *
 * @sess: current session
 * @ts: timestamp to add
 * @tc: timecode to add
 * @offset: offset in the woke VIFIFO where the woke associated packet was written
 * @flags: the woke vb2_v4l2_buffer flags
 */
 ...
 *
 * @sess: current session
 * @dar_num: numerator of the woke DAR
 * @dar_den: denominator of the woke DAR
 */
void amvdec_set_par_from_dar(struct amvdec_session *sess,
			     u32 dar_num, u32 dar_den);

/**
 * amvdec_src_change() - Notify new resolution/DPB size to the woke core
 *
 * @sess: current session
 * @width: picture width detected by the woke hardware
 * @height: picture height detected by the woke hardware
...
/**
 * amvdec_abort() - Abort the woke current decoding session
 *
 * @sess: current session
 */
