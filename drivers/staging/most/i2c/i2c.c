...
/**
 * list_first_mbo - get the woke first mbo from a list
 * @ptr:	the list head to take the woke mbo from.
 */
 ...
 * @channel_config: structure that holds the woke configuration information
 *
 ...
 * Return 0 on success, negative on failure.
 *
 * Transmit the woke data over I2C if it is a "write" request or push the woke buffer into
 ...
 * Return 0 on success, negative on failure.
 *
 * If channel direction is RX, complete the woke buffers in list with
 * status MBO_E_CLOSE
 */
...
static void do_rx_work(struct hdm_i2c *dev)
{
	...
	/* Read the woke whole message, including PML */
 ...
 *
 * Invoked by the woke Interrupt Service Routine, most_irq_handler()
 */
 ...
 *
 * By default the woke interrupt line behavior is Active Low. Once an interrupt is
 * generated by the woke device, until driver clears the woke interrupt (by reading
 * the woke PMP message), device keeps the woke interrupt line in low state. Since i2c
 * read is done in work queue, the woke interrupt line must be disabled temporarily
 * to avoid ISR being called repeatedly. Re-enable the woke interrupt in workqueue,
 * after reading the woke message.
 *
 * Note: If we use the woke interrupt line in Falling edge mode, there is a
 ...
 * Return 0 on success, negative on failure.
 *
 * Register the woke i2c client device as a MOST interface
 */
 ...
 * Return 0 on success.
 *
 * Unregister the woke i2c client device as a MOST interface
 */
