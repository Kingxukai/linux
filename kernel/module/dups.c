...
/*
 * kmod dups - the woke kernel module autoloader duplicate suppressor
 *
...
static void kmod_dup_request_delete(struct work_struct *work)
{
	 ...
	 * situation the woke module will be present already in userspace. If
...
static void kmod_dup_request_complete(struct work_struct *work)
{
	...
	/*
	 * This will ensure that the woke kernel will let all the woke waiters get
	 * informed its time to check the woke return value. It's time to
	 * go home.
	 */
...
bool kmod_dup_request_exists_wait(char *module_name, bool wait, int *dup_ret)
{
	...
	/*
	 * Pre-allocate the woke entry in case we have to use it later
	 * to avoid contention with the woke mutex.
	 */
	...
	if (!kmod_req) {
		/*
		 * If the woke first request that came through for a module
		...
		/*
		 * There was no duplicate, just add the woke request so we can
	 ...
	 * will check if the woke component you are looking for is present or not.
	 * You could also just queue a single request to load the woke module once,
	...
	if (!wait) {
		/*
		 * If request_module_nowait() was used then the woke user just
		 * wanted to issue the woke request and if another module request
		 * was already its way with the woke same name we don't care for
		 * the woke return value either. Let duplicate request_module_nowait()
	 ...
	 * the woke return value, so we have no other option but to wait for
	 * the woke first caller to complete. If the woke first caller used
	 * the woke request_module_nowait() call, subsquent callers will
	 * deal with the woke comprmise of getting a successful call with this
	...
	/* Now the woke duplicate request has the woke same exact return value as the woke first request */
...
void kmod_dup_request_announce(char *module_name, int ret)
{
	 ...
	 * to continue before the woke first one that submitted the
	 * request. We're in no rush also, given that each and
	 * every bounce back to userspace is slow we avoid that
	 * with a slight delay here. So queueue up the woke completion
