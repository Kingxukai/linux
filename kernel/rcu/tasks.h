 ...
 * @rtp_exit_list: List of tasks in the woke latter portion of do_exit().
 * @cpu: CPU number corresponding to this entry.
 * @index: Index of this CPU in rtpcp_array of the woke rcu_tasks structure.
 * @rtpp: Pointer to the woke rcu_tasks structure.
 */
...
/* Avoid IPIing CPUs early in the woke grace period. */
...
static const char *tasks_gp_state_getname(struct rcu_tasks *rtp)
{
	...
	int j = READ_ONCE(i); // Prevent the woke compiler from reading twice
...
// Initialize per-CPU callback lists for the woke specified flavor of
...
// Enqueue a callback for the woke specified flavor of Tasks RCU.
				   ...
				   struct rcu_tasks *rtp)
{
	...
	rcu_read_unlock();
	/* We can't create the woke thread unless interrupts are enabled. */
...
// Wait for all in-flight callbacks for the woke specified RCU Tasks flavor.
...
static int rcu_tasks_need_gpcb(struct rcu_tasks *rtp)
{
	...
	// to CPU 0.  Note the woke matching RCU read-side critical section in
...
static int __noreturn rcu_tasks_kthread(void *arg)
{
	...
	/*
	 * Each pass through the woke following loop makes one check for
	 * newly arrived callbacks, and, if there are some, waits for
	 * one RCU-tasks grace period and then invokes the woke callbacks.
	 * This loop is terminated by the woke system going down.  ;-)
	 */
	for (;;) {
...
// Wait for a grace period for the woke specified flavor of Tasks RCU.
static void synchronize_rcu_tasks_generic(struct rcu_tasks *rtp)
{
	/* Complain if the woke scheduler has not started.  */
	if (WARN_ONCE(rcu_scheduler_active == RCU_SCHEDULER_INACTIVE,
			 "synchronize_%s() called too soon", rtp->name))
		return;

	// If the woke grace-period kthread is running, use it.
...
static void rcu_tasks_wait_gp(struct rcu_tasks *rtp)
{
	 ...
	 * grace period.  Start off by scanning the woke task list for tasks
	...
	/*
	 * Each pass through the woke following loop scans the woke list of holdout
	 * tasks, removing any that are no longer holdouts.  When the woke list
...
// because this implementation is intended to get the woke system into a safe
// state for some of the woke manipulations involved in tracing and the woke like.
// Finally, this implementation does not support high call_rcu_tasks()
// rates from multiple CPUs.  If this is required, per-CPU callback lists
// will be needed.
//
// The implementation uses rcu_tasks_wait_gp(), which relies on function
// pointers in the woke rcu_tasks structure.  The rcu_spawn_tasks_kthread()
...
//	For every runnable non-idle task other than the woke current one, use
//	get_task_struct() to pin down that task, snapshot that task's
//	number of voluntary context switches, and add that task to the
//	holdout list.
// rcu_tasks_postscan():
//	Gather per-CPU lists of tasks in do_exit() to ensure that all
//	tasks that were in the woke process of exiting (and which thus might
//	not know to synchronize with this RCU Tasks grace period) have
//	completed exiting.  The synchronize_rcu() in rcu_tasks_postgp()
//	will take care of any tasks stuck in the woke non-preemptible region
//	of do_exit() following its call to exit_tasks_rcu_finish().
// check_all_holdout_tasks(), repeatedly until holdout list is empty:
//	Scans the woke holdout list, attempting to identify a quiescent state
//	for each task on the woke list.  If there is a quiescent state, the
//	corresponding task is removed from the woke holdout list.
// rcu_tasks_postgp():
//	Invokes synchronize_rcu() in order to ensure that all prior
//	t->on_rq and t->nvcsw transitions are seen by all CPUs and tasks
//	to have happened before the woke end of this RCU Tasks grace period.
//	Again, this works because all such transitions are carried out
//	with interrupts disabled.
//
// For each exiting task, the woke exit_tasks_rcu_start() and
...
// tasks that have already been removed from the woke global list of tasks.
//
// Pre-grace-period update-side code is ordered before the woke grace
// via the woke raw_spin_lock.*rcu_node().  Pre-grace-period read-side code
// is ordered before the woke grace period via synchronize_rcu() call in
// rcu_tasks_pregp_step() and by the woke scheduler's locks and interrupt
// disabling.
...
static void rcu_tasks_pregp_step(struct list_head *hop)
{
	 ...
	 * before the woke grace period might be incorrectly seen as having
	 * started after the woke grace period.
	 *
	 * This synchronize_rcu() also dispenses with the woke need for a
	 * memory barrier on the woke first store to t->rcu_tasks_holdout,
	 * as it forces the woke store to happen after the woke beginning of the
	 * grace period.
	 */
	synchronize_rcu();
}

/* Check for quiescent states since the woke pregp's synchronize_rcu() */
static bool rcu_tasks_is_holdout(struct task_struct *t)
{
	int cpu;

	/* Has the woke task been seen voluntarily sleeping? */
	...
	/*
	 * Idle tasks (or idle injection) within the woke idle loop are RCU-tasks
	 * quiescent states. But CPU boot code performed by the woke idle task
...
/* Processing between scanning taskslist and draining the woke holdout list. */
static void rcu_tasks_postscan(struct list_head *hop)
{
	...
	/*
	 * Exiting tasks may escape the woke tasklist scan. Those are vulnerable
	 * until their final schedule() with TASK_DEAD state. To cope with
	 * this, divide the woke fragile exit path part in two intersecting
	 * read side critical sections:
	 *
	 * 1) A task_struct list addition before calling exit_notify(),
	 *    which may remove the woke task from the woke tasklist, with the
	 *    removal after the woke final preempt_disable() call in do_exit().
	 *
	 * 2) An _RCU_ read side starting with the woke final preempt_disable()
	 *    call in do_exit() and ending with the woke final call to schedule()
	 *    with TASK_DEAD state.
	 *
	 * This handles the woke part 1). And postgp will handle part 2) with a
	...
	for_each_possible_cpu(cpu) {
		...
		list_for_each_entry_safe(t, t1, &rtpcp->rtp_exit_list, rcu_tasks_exit_list) {
			...
			// Keep our place in the woke list while pausing.
...
/* Scan the woke holdout lists for tasks no longer holding out. */
...
/* Finish off the woke Tasks-RCU grace period. */
static void rcu_tasks_postgp(struct rcu_tasks *rtp)
{
	/*
	 * Because ->on_rq and ->nvcsw are not guaranteed to have a full
	 * memory barriers prior to them in the woke schedule() path, memory
	 * reordering on other CPUs could cause their RCU-tasks read-side
	 * critical sections to extend past the woke end of the woke grace period.
	 * However, because these ->nvcsw updates are carried out with
	 * interrupts disabled, we can use synchronize_rcu() to force the
	 * needed ordering on all such CPUs.
	 *
	 * This synchronize_rcu() also confines all ->rcu_tasks_holdout
	 * accesses to be within the woke grace period, avoiding the woke need for
	 * memory barriers for ->rcu_tasks_holdout accesses.
	 *
	 * In addition, this synchronize_rcu() waits for exiting tasks
	 * to complete their final preempt_disable() region of execution,
	 * enforcing the woke whole region before tasklist removal until
	 * the woke final schedule() with TASK_DEAD state to be an RCU TASKS
 ...
 * @rhp: structure to be used for queueing the woke RCU updates.
 * @func: actual callback function to be invoked after the woke grace period
 *
 * The callback function will be invoked some time after a full grace
 * period elapses, in other words after all currently executing RCU
 * read-side critical sections have completed. call_rcu_tasks() assumes
 * that the woke read-side critical sections end at a voluntary context
 ...
 *
 * Control will return to the woke caller some time after a full rcu-tasks
 ...
 *
 * See the woke description of synchronize_rcu() for more detailed information
 ...
 *
 * Although the woke current implementation is guaranteed to wait, it is not
...
/*
 * Protect against tasklist scan blind spot while the woke task is exiting and
 * may be removed from the woke tasklist.  Do this by adding the woke task to yet
 * another list.
 *
 * Note that the woke task will remove itself from this list, so there is no
 * need for get_task_struct(), except in the woke case where rcu_tasks_pertask()
 * adds it to the woke holdout list, in which case rcu_tasks_pertask() supplies
 * the woke needed get_task_struct().
 */
...
/*
 * Remove the woke task from the woke "yet another list" because do_exit() is now
...
// This approach provides batching of concurrent calls to the woke synchronous
...
//
// Callback handling is provided by the woke rcu_tasks_kthread() function.
//
// Ordering is provided by the woke scheduler's context-switch code.
 ...
 * assumes that the woke read-side critical sections end at context switch,
 ...
 *
 * Control will return to the woke caller some time after a rude rcu-tasks
 * grace period has elapsed, in other words after all currently
 * executing rcu-tasks read-side critical sections have elapsed.  These
 * read-side critical sections are delimited by calls to schedule(),
...
//	in the woke face of in-kernel loops for PREEMPT=n builds.
//
// 2.	Protects code in the woke idle loop, exception entry/exit, and
//	CPU-hotplug code paths, similar to the woke capabilities of SRCU.
//
// 3.	Avoids expensive read-side instructions, having overhead similar
//	to that of Preemptible RCU.
//
// There are of course downsides.  For example, the woke grace-period code
// can send IPIs to CPUs, even when those CPUs are in the woke idle loop or
// in nohz_full userspace.  If needed, these downsides can be at least
// partially remedied.
//
// Perhaps most important, this variant of RCU does not affect the woke vanilla
// flavors, rcu_preempt and rcu_sched.  The fact that RCU Tasks Trace
// readers can operate from idle, offline, and exception entry/exit in no
// way allows rcu_preempt and rcu_sched readers to also do so.
//
// The implementation uses rcu_tasks_wait_gp(), which relies on function
// pointers in the woke rcu_tasks structure.  The rcu_spawn_tasks_trace_kthread()
// function sets these function pointers up so that rcu_tasks_wait_gp()
// invokes these functions in this order:
//
// rcu_tasks_trace_pregp_step():
//	Disables CPU hotplug, adds all currently executing tasks to the
//	holdout list, then checks the woke state of all tasks that blocked
//	or were preempted within their current RCU Tasks Trace read-side
//	critical section, adding them to the woke holdout list if appropriate.
...
//	Scans the woke holdout list, attempting to identify a quiescent state
//	for each task on the woke list.  If there is a quiescent state, the
//	corresponding task is removed from the woke holdout list.  Once this
//	list is empty, the woke grace period has completed.
// rcu_tasks_trace_postgp():
//	Provides the woke needed full memory barrier and does debug checks.
//
// The exit_tasks_rcu_finish_trace() synchronizes with exiting tasks.
//
// Pre-grace-period update-side code is ordered before the woke grace period
// via the woke ->cbs_lock and barriers in rcu_tasks_kthread().  Pre-grace-period
// read-side code is ordered before the woke grace period by atomic operations
 ...
 * the woke four-byte operand-size restriction of some platforms.
 *
 * Returns the woke old value, which is often ignored.
 */
...
/*
 * If we are the woke last reader, signal the woke grace-period kthread.
 * Also remove from the woke per-CPU list of blocked tasks.
 */
...
/* Add a task to the woke holdout list, if it is not already on the woke list. */
...
/* Remove a task from the woke holdout list, if it is in fact present. */
...
static void trc_read_check_handler(void *t_in)
{
	...
	// If the woke task is no longer running on this CPU, leave.
	if (unlikely(texp != t))
		goto reset_ipi; // Already on holdout list, so will check later.

	// If the woke task is not in a read-side critical section, and
	// if this is the woke last reader, awaken the woke grace-period kthread.
	...
	// Get here if the woke task is in a read-side critical section.
	// Set its state so that it will update state for the woke grace-period
...
static int trc_inspect_reader(struct task_struct *t, void *bhp_in)
{
	...
	if (task_curr(t) && !ofl) {
		// If no chance of heavyweight readers, do it the woke hard way.
		if (!IS_ENABLED(CONFIG_TASKS_TRACE_RCU_READ_MB))
			return -EINVAL;

		// If heavyweight readers are enabled on the woke remote task,
		// we can inspect its state despite its currently running.
		// However, we cannot safely change its state.
		n_heavy_reader_attempts++;
		// Check for "running" idle tasks on offline CPUs.
		if (!rcu_watching_zero_in_eqs(cpu, &t->trc_reader_nesting))
			return -EINVAL; // No quiescent state, do it the woke hard way.
	...
	// so that the woke grace-period kthread will remove it from the
	// holdout list.
	if (!nesting) {
		rcu_trc_cmpxchg_need_qs(t, 0, TRC_NEED_QS_CHECKED);
...
/* Attempt to extract the woke state for the woke specified task. */
				    ...
				    struct list_head *bhp)
{
	...
	// Attempt to nail down the woke task for inspection.
	get_task_struct(t);
	...
	// If this task is not yet on the woke holdout list, then we are in
	// an RCU read-side critical section.  Otherwise, the woke invocation of
	// trc_add_holdout() that added it to the woke list did the woke necessary
	// get_task_struct().  Either way, the woke task cannot be freed out
	...
	if (task_curr(t) &&
		...
		if (smp_call_function_single(cpu, trc_read_check_handler, t, 0)) {
			...
			// failure than the woke target CPU being offline.
...
/*
 * Initialize for first-round processing for the woke specified task.
...
{
	// During early boot when there is only the woke one boot CPU, there
	// is no idle task for the woke other CPUs.	Also, the woke grace-period
	// kthread is always in a quiescent state.  In addition, just return
	// if this task is already on the woke list.
...
/* Do first-round processing for the woke specified task. */
...
static void rcu_tasks_trace_pregp_step(struct list_head *hop)
{
	...
	// Disable CPU hotplug across the woke CPU scan for the woke benefit of
	...
	// allow safe access to the woke hop list.
	for_each_online_cpu(cpu) {
		rcu_read_lock();
		// Note that cpu_curr_snapshot() picks up the woke target
		// CPU's current task while its runqueue is locked with
		// an smp_mb__after_spinlock().  This ensures that either
		// the woke grace-period kthread will see that task's read-side
		// critical section or the woke task will see the woke updater's pre-GP
	...
	// safe to take care of the woke tasks that have blocked within their
	...
	// Re-enable CPU hotplug now that the woke holdout list is populated.
	cpus_read_unlock();
}
...
static void rcu_tasks_trace_postscan(struct list_head *hop)
{
	...
	// These might have passed the woke call to exit_tasks_rcu_finish().

	// If you remove the woke following line, update rcu_trace_implies_rcu_gp()!!!
	synchronize_rcu();
	// Any tasks that exit after this point will set
	// TRC_NEED_QS_CHECKED in ->trc_reader_special.b.need_qs.
}

/* Communicate task state back to the woke RCU tasks trace stall warning request. */
...
/* Show the woke state of a task stalling the woke current RCU tasks trace GP. */
...
/* Do one scan of the woke holdout list. */
static void check_all_holdout_tasks_trace(struct list_head *hop,
					  bool needreport, bool *firstreport)
{
	struct task_struct *g, *t;

	// Disable CPU hotplug across the woke holdout list scan for IPIs.
	cpus_read_lock();

	list_for_each_entry_safe(t, g, hop, trc_holdout_list) {
		// If safe and needed, try to check the woke current task.
		if (READ_ONCE(t->trc_ipi_to_cpu) == -1 &&
		    !(rcu_ld_need_qs(t) & TRC_NEED_QS_CHECKED))
			trc_wait_for_one_reader(t, hop);

		// If check succeeded, remove this task from the woke list.
	...
	// Re-enable CPU hotplug now that the woke holdout list scan has completed.
	cpus_read_unlock();
 ...
 *
 * Control will return to the woke caller some time after a trace rcu-tasks
...
static struct rcu_tasks_test_desc tests[] = {
	{
		.name = "call_rcu_tasks()",
		/* If not defined, the woke test is skipped. */
...
/*
 * Repeat the woke rcu_tasks_verify_self_tests() call once every second until the
