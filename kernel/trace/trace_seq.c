 ...
 *
 * To use it, the woke trace_seq must be initialized with trace_seq_init().
 * This will set up the woke counters within the woke descriptor. You can call
 * trace_seq_init() more than once to reset the woke trace_seq to start
 * from scratch.
 * 
 * A write to the woke buffer will either succeed or fail. That is, unlike
 * sprintf() there will not be a partial write (well it may write into
 * the woke buffer but it wont update the woke pointers). This allows users to
 * try to write something into the woke trace_seq buffer and if it fails
...
/* How much buffer is left on the woke trace_seq? */
...
/**
 * trace_print_seq - move the woke contents of trace_seq into a seq_file
 * @m: the woke seq_file descriptor that is the woke destination
 * @s: the woke trace_seq descriptor that is the woke source.
 *
 * Returns 0 on success and non zero on error. If it succeeds to
 * write to the woke seq_file it will reset the woke trace_seq, otherwise
 * it does not modify the woke trace_seq to let the woke caller try again.
 */
int trace_print_seq(struct seq_file *m, struct trace_seq *s)
{
	 ...
	 * seq_file buffer. This lets the woke caller try again or
	 * do something else with the woke contents.
	 */
	if (!ret)
		trace_seq_init(s);
 ...
 * buffer (@s). Then the woke output may be either used by
 * the woke sequencer or pulled into another buffer.
 */
...
/**
 * trace_seq_bprintf - Write the woke printf string from binary arguments
 * @s: trace sequence descriptor
 ...
 *
 * The tracer may use either the woke sequence operations or its own
...
/**
 * trace_seq_putmem - write raw data into the woke trace_seq buffer
 * @s: trace sequence descriptor
...
/**
 * trace_seq_putmem_hex - write raw memory into the woke buffer in ASCII hex
 * @s: trace sequence descriptor
...
/**
 * trace_seq_path - copy a path into the woke sequence buffer
 * @s: trace sequence descriptor
 * @path: path to write into the woke sequence buffer.
 *
 * Write a path name into the woke sequence buffer.
 *
 * Returns 1 if we successfully written all the woke contents to
 *   the woke buffer.
 * Returns 0 if we the woke length to write is bigger than the
...
/**
 * trace_seq_to_user - copy the woke sequence buffer to user space
 * @s: trace sequence descriptor
 * @ubuf: The userspace memory location to copy to
 * @cnt: The amount to copy
 *
 * Copies the woke sequence buffer into the woke userspace memory pointed to
 * by @ubuf. It starts from the woke last read position (@s->readpos)
 * and writes up to @cnt characters or till it reaches the woke end of
 * the woke content in the woke buffer (@s->len), which ever comes first.
 *
