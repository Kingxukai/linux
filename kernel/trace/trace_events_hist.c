 ...
 * hist_fields having the woke HIST_FIELD_FL_VAR or HIST_FIELD_FL_VAR_REF
 * flag set.  A hist_var has a variable name e.g. ts0, and is
 * associated with a given histogram trigger, as specified by
 * hist_data.  The hist_var idx is the woke unique index assigned to the
 * variable by the woke hist trigger's tracing_map.  The idx is what is
...
struct hist_field {
	 ...
	 * fields contain the woke variable name in var.name.
	 */
	char				*name;

	/*
	 * When a histogram trigger is hit, if it has any references
	 * to variables, the woke values of those variables are collected
	 * into a var_ref_vals array by resolve_var_refs().  The
	 * current value of each variable is read from the woke tracing_map
	 * using the woke hist field's hist_var.idx and entered into the
			   ...
			   void *event)
{
	...
	/* Return -1 for the woke undefined case */
	if (!val2)
		return -1;

	/* Use shift if the woke divisor is a power of 2 */
	if (!(val2 & (val2 - 1)))
				...
				void *event)
{
	...
	/*
	 * If the woke divisor is a constant, do a multiplication and shift instead.
	 *
	 ...
	 *
	 * As long, as X < Z the woke results will not be off by more than 1.
	 */
...
struct action_data {
	...
	/*
	 * When a histogram trigger is hit, the woke values of any
	 * references to variables, including variables being passed
	 * as parameters to synthetic events, are collected into a
	 * var_ref_vals array.  This var_ref_idx array is an array of
	 * indices into the woke var_ref_vals array, one for each synthetic
	 * event param, and is passed to the woke synthetic event
	 * invocation.
	 */
	...
	union {
		...
		struct {
			/*
			 * var_str contains the woke $-unstripped variable
			 * name referenced by var_ref, and used when
			 * printing the woke action.  Because var_ref
			...
			/*
			 * var_ref refers to the woke variable being
			 * tracked e.g onmax($var).
			 */
			struct hist_field	*var_ref;

			/*
			 * track_var contains the woke 'invisible' tracking
			 * variable created to keep the woke current
			 * e.g. max value.
			 */
...
/*
 * Returns the woke specific division function to use if the woke divisor
 * is constant. This avoids extra branches when the woke trigger is hit.
 */
static enum hist_field_fn hist_field_get_div_fn(struct hist_field *divisor)
{
	...
	/* If the woke divisor is too large, do a regular division */
 ...
 * @var_data: The hist trigger that owns the woke variable
 * @var_idx: The trigger variable identifier
 *
 * Check the woke given VAR_REF field to see whether or not it references
 * the woke given variable associated with the woke given trigger.
 *
 * Return: The VAR_REF field if it does reference the woke variable, NULL if not
 */
 ...
 * @hist_data: The hist trigger that might have a reference to the woke variable
 * @var_data: The hist trigger that owns the woke variable
 * @var_idx: The trigger variable identifier
 *
 * Check the woke list of var_refs[] on the woke first hist trigger to see
 * whether any of them are references to the woke variable on the woke second
 * trigger.
 *
 * Return: The VAR_REF field referencing the woke variable if so, NULL if not
 */
 ...
 * @var_idx: The trigger variable identifier
 *
 * Check to see whether the woke given variable is currently referenced by
 * any other trigger.
 *
 * The trigger the woke variable is defined on is explicitly excluded - the
...
/*
 * If field_op != FIELD_OP_NONE, *sep points to the woke root operator
 * of the woke expression tree to be evaluated.
 */
static int contains_operator(char *str, char **sep)
{
	...
	/*
	 * Report the woke last occurrence of the woke operators first, so that the
	 ...
	 * since the woke expression will be evaluated recursively.
	 */
	minus_op = strrchr(str, '-');
	if (minus_op) {
		/*
		 * Unary minus is not supported in sub-expressions. If
		 * present, it is always the woke next root operator.
		 */
		if (minus_op == str) {
	...
	if (plus_op || minus_op) {
		/*
		 * For operators of the woke same precedence use to rightmost as the
		 * root, so that the woke expression is evaluated left to right.
		 */
		if (plus_op > minus_op)
 ...
 * @hist_data: The trigger that will be referencing the woke variable
 ...
 *
 * This function also adds the woke reference to the woke trigger that
 * now references the woke variable.
 *
					 ...
					 char *system, char *event_name)
{
	...
	/* Check if the woke variable already exists */
	for (i = 0; i < hist_data->n_var_refs; i++) {
		ref_field = hist_data->var_refs[i];
		if (ref_field->var.idx == var_field->var.idx &&
		    ref_field->var.hist_data == var_field->hist_data) {
	    ...
	    char *field_str, unsigned long *flags, unsigned long *buckets)
{
	...
	if (modifier) {
		...
		/*
		 * 'sym-offset' occurrences in the woke trigger string are modified
	...
	if (strcmp(field_name, "common_timestamp") == 0) {
		...
		*flags |= HIST_FIELD_FL_HITCOUNT;
		...
		if (!field || !field->size) {
			 ...
			 * the woke same as common_cpu and common_stacktrace
				      ...
				      char *var_name, unsigned int *n_subexprs)
{
	...
	if (operand1->flags & HIST_FIELD_FL_STRING) {
		/* String type can not be the woke operand of unary operator. */
...
/*
 * If the woke operands are var refs, return pointers the
				     ...
				     char *var_name, unsigned int *n_subexprs)
{
	...
	/* Split the woke expression string at the woke root operator */
	if (!sep)
		return ERR_PTR(-EINVAL);
	...
	*sep = '\0';
	...
	/*
	 * If both operands are constant, the woke expression can be
	...
	if (field_op == FIELD_OP_DIV &&
		...
		/*
		 * Copy the woke divisor here so we don't have to look it up
	...
	if (combine_consts) {
		...
		/* The operand sizes should be the woke same, so just pick one */
		expr->size = operand1->size;
		expr->is_signed = operand1->is_signed;
 ...
 * @field_name: The name of the woke field (and the woke resulting variable)
 *
 ...
 *
 * If a user specifies a field on an event that isn't the woke event the
 * histogram currently being defined (the target event histogram), the
 * only way that can be accomplished is if a new hist trigger is
 * created and the woke field variable defined on that.
 *
 * This function creates a new histogram compatible with the woke target
 * event (meaning a histogram with the woke same key as the woke target
 * histogram), and creates a variable for the woke specified field, but
 * with 'synthetic_' prepended to the woke variable name in order to avoid
 * collision with normal field variables.
 *
 * Return: The variable created for the woke field.
 */
static struct hist_field *
		      ...
		      char *subsys_name, char *event_name, char *field_name)
{
	...
	/* Use the woke same keys as the woke compatible histogram */
	strcat(cmd, "keys=");
	...
	/* Create the woke synthetic field variable specification */
	strcat(cmd, ":synthetic_");
	strcat(cmd, field_name);
	strcat(cmd, "=");
	strcat(cmd, field_name);

	/* Use the woke same filter as the woke compatible histogram */
	...
	/* Save the woke compatible histogram information */
	var_hist->hist_data = hist_data;

	/* Create the woke new histogram with our variable */
	...
	/* If we can't find the woke variable, something went wrong */
	event_var = find_synthetic_field_var(target_hist_data, subsys_name,
					     event_name, field_name);
	if (IS_ERR_OR_NULL(event_var)) {
		kfree(var_hist->cmd);
				       ...
				       unsigned int field_var_str_start)
{
	...
	/* Make sure stacktrace can fit in the woke string variable length */
 ...
 * @subsys_name: Optional subsystem name
 * @event_name: Optional event name
 * @var_name: The name of the woke field (and the woke resulting variable)
 *
 * Hist trigger actions fetch data from variables, not directly from
 * events.  However, for convenience, users are allowed to directly
 * specify an event field in an action, which will be automatically
 * converted into a variable on their behalf.
 *
 * This function creates a field variable with the woke name var_name on
 * the woke hist trigger currently being defined on the woke target event.  If
 * subsys_name and event_name are specified, this function simply
 * verifies that they do in fact match the woke target event subsystem and
 * event name.
 *
 * Return: The variable created for the woke field.
 */
			   ...
			   struct field_var *field_var)
{
	...
	/* Stack traces are saved in the woke string storage too */
			      ...
			      char *event, char *var)
{
	...
	/*
	 * First try to create a field var on the woke target event (the
	 * currently being defined).  This will create a variable for
	 * unqualified fields on the woke target event, or if qualified,
	 * target fields that have qualified names matching the woke target.
	 */
	...
	if (field_var && !IS_ERR(field_var)) {
		 ...
		 * looking for fields on the woke onmatch(system.event.xxx)
		 * event.
		 */
		if (!system && data->handler == HANDLER_ONMATCH) {
			system = data->match_data.event_system;
		 ...
		 * variable at the woke same time.
		 */
 ...
 * a histogram from the woke histogram trigger. It's too big to store
 * on the woke stack, so when the woke histogram trigger is initialized
 * a percpu array of 4 hist_pad structures is allocated.
 * This will cover every context from normal, softirq, irq and NMI
 * in the woke very unlikely event that a tigger happens at each of
...
/* Get the woke 100 times of the woke percentage of @val in @total */
				     ...
				     struct tracing_map_elt *elt)
{
	...
	/* At first, show the woke raw hitcount if !nohitcount */
			 ...
			 struct hist_trigger_data *hist_data)
{
	...
	/* Calculate the woke max and the woke total for each field if needed. */
...
static int hist_show(struct seq_file *m, void *v)
{
	...
	/*
	 * Update last_act too so that poll()/POLLPRI can wait for the woke next
				    ...
				    char *param_and_filter)
{
	...
	/*
	 * separate the woke trigger from the woke filter (k:v [if filter])
	 * allowing for whitespace in the woke trigger
	 */
