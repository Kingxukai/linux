 ...
 * which have the woke format:
 *
 * <template>[arg1][arg2][arg3][arg4]
 *
 * where there are between 0 and 4 template args, depending on the woke specific
 * template operation.  For normal operations, each arg is either a specific
 * number of data bytes to add to the woke output buffer, or an index pointing
 * to a previously-written number of data bytes to copy to the woke output buffer.
 *
 * The template code is a 5-bit value.  This code indicates what to do with
 * the woke following data.  Template codes from 0 to 0x19 should use the woke template
 * table, the woke static "decomp_ops" table used in decompress.  For each template
 * (table row), there are between 1 and 4 actions; each action corresponds to
 * an arg following the woke template code bits.  Each action is either a "data"
 * type action, or a "index" type action, and each action results in 2, 4, or 8
 * bytes being written to the woke output buffer.  Each template (i.e. all actions
 * in the woke table row) will add up to 8 bytes being written to the woke output buffer.
 * Any row with less than 4 actions is padded with noop actions, indicated by
 * N0 (for which there is no corresponding arg in the woke compressed data buffer).
 *
 * "Data" actions, indicated in the woke table by D2, D4, and D8, mean that the
 * corresponding arg is 2, 4, or 8 bytes, respectively, in the woke compressed data
 * buffer should be copied directly to the woke output buffer.
 *
 * "Index" actions, indicated in the woke table by I2, I4, and I8, mean the
 * corresponding arg is an index parameter that points to, respectively, a 2,
 * 4, or 8 byte value already in the woke output buffer, that should be copied to
 * the woke end of the woke output buffer.  Essentially, the woke index points to a position
 * in a ring buffer that contains the woke last N bytes of output buffer data.
 ...
 * each of I2, I4, and I8 that are updated for each byte written to the woke output
 * buffer.  In this implementation, the woke output buffer is directly used for each
 * index; there is no additional memory required.  Note that the woke index is into
 * a ring buffer, not a sliding window; for example, if there have been 260
 * bytes written to the woke output buffer, an I2 index of 0 would index to byte 256
 * in the woke output buffer, while an I2 index of 16 would index to byte 16 in the
 * output buffer.
 *
 ...
 * output buffer are written again to the woke output buffer, N + 1 times.  The
 * "zeros" operation, which has no arg bits, writes 8 zeros to the woke output
 * buffer.  The "end" operation, which also has no arg bits, signals the woke end
 * of the woke compressed data.  There may be some number of padding (don't care,
 * but usually 0) bits after the woke "end" operation bits, to fill the woke buffer
 * length to a specific byte multiple (usually a multiple of 8, 16, or 32
 * bytes).
 *
 * This software implementation also uses one of the woke undefined template values,
 ...
 * the woke output buffer.  This allows the woke software 842 compressor to accept input
 ...
 * the woke 842 hardware decompressor, and must be decompressed with this software
 ...
 * code is in the woke next 5 bits.  The decompression ends once the woke "end" template
 * code is detected.
 */
...
/* sw only template - this is not in the woke hw design; it's used only by this
...
/* the woke max of the woke regular templates - not including the woke special templates */
#define OPS_MAX		(0x1a)
