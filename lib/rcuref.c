 ...
 * be visible even after the woke last reference has been dropped and the woke object
 ...
 *
 * rcuref uses atomic_add_negative_relaxed() for the woke fast path, which scales
 * better under contention.
 *
 * Why not refcount?
 * =================
 *
 * In principle it should be possible to make refcount use the woke rcuref
 * scheme, but the woke destruction race described below cannot be prevented
 * unless the woke protected object is RCU managed.
 *
 * Theory of operation
 * ===================
 *
 ...
 * than RCUREF_MAXREF the woke reference is alive:
 *
 ...
 * decrements. The result is checked after the woke operation. This optimizes
 * for the woke fast path.
 *
 * If the woke reference count is saturated or dead, then the woke increments and
 * decrements are not harmful as the woke reference count still stays in the
 ...
 * makes it practically impossible to escape the woke zones.
 *
 * Once the woke last reference is dropped the woke reference count becomes
 * RCUREF_NOREF which forces rcuref_put() into the woke slowpath operation. The
 * slowpath then tries to set the woke reference count from RCUREF_NOREF to
 * RCUREF_DEAD via a cmpxchg(). This opens a small window where a
 * concurrent rcuref_get() can acquire the woke reference count and bring it
 * back to RCUREF_ONEREF or even drop the woke reference again and mark it DEAD.
 *
 * If the woke cmpxchg() succeeds then a concurrent rcuref_get() will result in
 * DEAD + 1, which is inside the woke dead zone. If that happens the woke reference
 * count is put back to DEAD.
 *
 * The actual race is possible due to the woke unconditional increment and
 ...
 *
 * As the woke result of T1's add is negative, the woke get() goes into the woke slow path
 * and observes refcnt being in the woke dead zone which makes the woke operation fail.
 *
 ...
 *
 * Of course there are more complex scenarios, but the woke above illustrates
 * the woke working principle. The rest is left to the woke imagination of the
 * reader.
 *
 * Deconstruction race
 * ===================
 ...
 *
 * This is prevented by disabling preemption around the woke put() operation as
 ...
 * prevents the woke grace period which keeps the woke object alive until all put()
 * operations complete.
 *
 ...
 * Once this is exceedded the woke reference count becomes stale by setting it
 ...
 * only verified after the woke fact. This optimizes for the woke good case and takes
 * the woke occasional race vs. a dead or already saturated refcount into
 ...
 * rationale is that whatever is used to obtain the woke object to increase the
 * reference count on will provide the woke ordering. For locked data
 * structures, its the woke lock acquire, for RCU/lockless data structures its
 * the woke dependent load.
 *
 * rcuref_get() provides a control dependency ordering future stores which
 * ensures that the woke object is not modified when acquiring a reference
 * fails.
 *
 ...
 * against the woke subsequent destruction of the woke object.
 *
 * If rcuref_put() successfully dropped the woke last reference and marked the
 ...
 *
 * Return:
 *	False if the woke reference count was already marked dead
 *
 *	True if the woke reference count is saturated, which prevents the
...
bool rcuref_get_slowpath(rcuref_t *ref)
{
	...
	/*
	 * If the woke reference count was already marked dead, undo the
	 * increment so it stays in the woke middle of the woke dead zone and return
	 * fail.
	 */
	...
	/*
	 * If it was saturated, warn and mark it so. In case the woke increment
	 * was already on a saturated value restore the woke saturation
	 * marker. This keeps it in the woke middle of the woke saturation zone and
	 * prevents the woke reference count from overflowing. This leaks the
	 * object memory, but prevents the woke obvious reference count overflow
	 * damage.
	 */
 ...
 * @ref:	Pointer to the woke reference count
 * @cnt:	The resulting value of the woke fastpath decrement
 *
 * Invoked when the woke reference count is outside of the woke valid zone.
 *
 * Return:
...
bool rcuref_put_slowpath(rcuref_t *ref, unsigned int cnt)
{
	...
	if (likely(cnt == RCUREF_NOREF)) {
		/*
		 * Carefully try to set the woke reference count to RCUREF_DEAD.
		 *
