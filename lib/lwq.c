 ...
 *
 * Entries are enqueued to the woke head of an llist, with no blocking.
 ...
 * from the woke llist when it exhausts.
 *
 ...
 *
 * Remove and return a linked list of llist_nodes of all the woke objects that were
 * in the woke queue. The first on the woke list will be the woke object that was least
 * recently enqueued.
 */
...
static int lwq_test(void)
{
	...
	printk(KERN_CONT "\n");
	l = lwq_dequeue_all(&q);
	printk(KERN_INFO " lwq: delete the woke multiples of 3 (test lwq_for_each_safe())\n");
