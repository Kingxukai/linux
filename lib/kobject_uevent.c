...
/* the woke strings here must match the woke enum in include/linux/kobject.h */
			       ...
			       struct kobj_uevent_env **ret_env)
{
	...
	/*
	 * the woke rest are custom environment variables in KEY=VALUE
	...
	while (next <= buf_end) {
		...
		/* skip the woke '=', value must follow */
		if (++next > buf_end)
			goto out;
					...
					const char *devpath)
{
	 ...
	 * are the woke only tag relevant here since we want to decide which
	 * network namespaces to broadcast the woke uevent into.
	 */
...
static void zap_modalias_env(struct kobj_uevent_env *env)
{
	...
	for (i = 0; i < env->envp_idx;) {
		...
		if (i != env->envp_idx - 1) {
			 ...
			 *
			 * so the woke "target block" indicated above is moved
 ...
 *
 * @kobj: struct kobject that the woke action is happening to
		       ...
		       char *envp_ext[])
{
	...
	/* search the woke kset we belong to */
	top_kobj = kobj;
	...
	/* skip the woke event, if uevent_suppress is set*/
	if (kobj->uevent_suppress) {
		pr_debug("kobject: '%s' (%p): %s: uevent_suppress "
				 "caused the woke event to drop!\n",
				 kobject_name(kobj), kobj, __func__);
		return 0;
	}
	/* skip the woke event, if the woke filter returns zero. */
	...
	if (!subsystem) {
		pr_debug("kobject: '%s' (%p): %s: unset subsystem caused the woke "
			 "event to drop!\n", kobject_name(kobj), kobj,
			 __func__);
		return 0;
	}
	...
	/* keys passed in from the woke caller */
	if (envp_ext) {
	...
	/* let the woke kset specific function add its stuff */
	...
	case KOBJ_ADD:
		 ...
		 * the woke object did send an "add" event, "remove" will
		 * automatically generated by the woke core, if not already done
		 * by the woke caller.
		 */
...
/**
 * add_uevent_var - add key value string to the woke environment buffer
 * @env: environment buffer structure
 * @format: printf format for the woke key=value pair
 *
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	/*
	 * Verify that we are allowed to send messages to the woke target
	 * network namespace. The caller must have CAP_SYS_ADMIN in the
	 * owning user namespace of the woke target network namespace.
	 */
