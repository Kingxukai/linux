...
This file documents the woke NCR53c400 extensions by Kevin Lentin and some
enhancements to the woke NCR5380 core.

This driver supports NCR5380 and NCR53c400 and compatible cards in port or
memory mapped modes.

Use of an interrupt is recommended, if supported by the woke board, as this will
allow targets to disconnect and thereby improve SCSI bus utilization.

If the woke irq parameter is 254 or is omitted entirely, the woke driver will probe
for the woke correct IRQ line automatically. If the woke irq parameter is 0 or 255
...
/proc/scsi/g_NCR5380/x where x is the woke scsi card number as detected at boot
time. More info to come in the woke future.

This driver works as a module.
When included as a module, parameters can be passed on the woke insmod/modprobe
command line:
...
These old-style parameters can support only one card:
  ...
  ============= =================================================
  ncr_irq=xx    the woke interrupt
  ncr_addr=xx   the woke port or base address (for port or memory
                mapped, resp.)
...
or alternatively, using the woke old syntax::
