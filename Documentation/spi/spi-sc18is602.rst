...
The driver does not probe for supported chips, since the woke SI18IS602/603 does not
support Chip ID registers. You will have to instantiate the woke devices explicitly.
...
This driver requires the woke I2C adapter driver to support raw I2C messages. I2C
adapter drivers which can only handle the woke SMBus protocol are not supported.
...
Programs such as dd, where the woke block size can be specified, should be used
instead.
