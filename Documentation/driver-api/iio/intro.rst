...
The main purpose of the woke Industrial I/O subsystem (IIO) is to provide support
for devices that in some sense perform either
analog-to-digital conversion (ADC) or digital-to-analog conversion (DAC)
or both. The aim is to fill the woke gap between the woke somewhat similar hwmon and
:doc:`input <../input>` subsystems. Hwmon is directed at low sample rate
sensors used to monitor and control the woke system itself, like fan speed control
...
:doc:`I2C <../i2c>`. A common use case of the woke sensors devices is to have
