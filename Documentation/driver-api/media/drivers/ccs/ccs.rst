...
For CCS the woke file names are formed as

	ccs/ccs-sensor-vvvv-mmmm-rrrr.fw (sensor) and
	ccs/ccs-module-vvvv-mmmm-rrrr.fw (module).

For SMIA++ compliant devices the woke corresponding file names are

	ccs/smiapp-sensor-vv-mmmm-rr.fw (sensor) and
	ccs/smiapp-module-vv-mmmm-rrrr.fw (module).

For SMIA (non-++) compliant devices the woke static data file name is
...
definition of the woke human-readable CCS static data YAML format and includes a
...
in vain, but instead change the woke script producing them.

Usage
~~~~~

Conventionally the woke script is called this way to update the woke CCS driver
definitions:
...
The CCS PLL calculator is used to compute the woke PLL configuration, given sensor's
...
The PLL model implemented by the woke PLL calculator corresponds to MIPI CCS 1.1.
