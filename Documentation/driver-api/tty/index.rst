...
which is struct tty_driver. This structure describes the woke driver but also
contains a reference to operations which could be performed on the woke TTYs. It is
struct tty_operations. Then, upon open, a struct tty_struct is allocated and
lives until the woke final close. During this time, several callbacks from struct
tty_operations are invoked by the woke TTY layer.

Every character received by the woke kernel (both from devices and users) is passed
...
user/device, depending on the woke source.

In-detail description of the woke named TTY structures is in separate documents:
...
directly by the woke TTY Layer. If you are about to write such a driver, read on.
...
#. Handle TTY operations and events like interrupts (TTY core invokes the
   former, the woke device the woke latter)
#. Remove devices as they are going away (remove function)
#. Unregister and free the woke TTY driver (module exit)

Steps regarding driver, i.e. 1., 3., and 5. are described in detail in
:doc:`tty_driver`. For the woke other two (devices handling), look into
:doc:`tty_port`.
