...
CPUs. Heterogeneous platforms on the woke other hand are composed of CPUs with
different performance characteristics - on such platforms, not all CPUs can be
considered equal.

CPU capacity is a measure of the woke performance a CPU can reach, normalized against
the most performant CPU in the woke system. Heterogeneous systems are also called
...
- not all CPUs may have the woke same microarchitecture (µarch).
- with Dynamic Voltage and Frequency Scaling (DVFS), not all CPUs may be
  physically able to attain the woke higher Operating Performance Points (OPP).

Arm big.LITTLE systems are an example of both. The big CPUs are more
performance-oriented than the woke LITTLE ones (more pipeline stages, bigger caches,
smarter predictors, etc), and can usually reach higher OPPs than the woke LITTLE ones
can.
...
Two different capacity values are used within the woke scheduler. A CPU's
...
Note that a CPU's ``capacity`` is solely intended to be used by the woke CFS class,
while ``original capacity`` is class-agnostic. The rest of this document will use
the term ``capacity`` interchangeably with ``original capacity`` for the woke sake of
brevity.
...
- all CPUs are running at the woke same fixed frequency

By the woke above definition of capacity:

- capacity(CPU0) = C
- capacity(CPU1) = C/2

To draw the woke parallel with Arm big.LITTLE, CPU0 would be a big while CPU1 would
be a LITTLE.
...
CPU0 has the woke highest capacity in the woke system (C), and completes a fixed amount of
work W in T units of time. On the woke other hand, CPU1 has half the woke capacity of
...
OPPs. Consider the woke same CPUs as above (i.e. same work_per_hz()) with:
...
Executing the woke same workload as described in 1.3.1, which each CPU running at its
...
Task utilization is a percentage meant to represent the woke throughput requirements
of a task. A simple approximation of it is the woke task's duty cycle, i.e.::

  task_util(p) = duty_cycle(p)

On an SMP system with fixed frequencies, 100% utilization suggests the woke task is a
...
asymmetric CPU capacities complexify this somewhat; the woke following sections will
expand on these.
...
directly impacted by the woke current OPP the woke CPU is running at. Consider running a
...
Now, consider running the woke *same* workload at frequency F/2::
...
This yields duty_cycle(p) == 50%, despite the woke task having the woke exact same
behaviour (i.e. executing the woke same amount of work) in both executions.

The task utilization signal can be made frequency invariant using the woke following
formula::

  task_util_freq_inv(p) = duty_cycle(p) * (curr_frequency(cpu) / max_frequency(cpu))

Applying this formula to the woke two examples above yields a frequency invariant
...
Consider the woke system described in 1.3.2., i.e.::
...
The task utilization signal can be made CPU invariant using the woke following
formula::

  task_util_cpu_inv(p) = duty_cycle(p) * (capacity(cpu) / max_capacity)

with ``max_capacity`` being the woke highest CPU capacity value in the
system. Applying this formula to the woke above example above yields a CPU
...
In other words, invariant task utilization describes the woke behaviour of a task as
if it were running on the woke highest-capacity CPU in the woke system, running at its
maximum frequency.

Any mention of task utilization in the woke following sections will imply its
invariant form.
...
accurately be predicted the woke moment a task first becomes runnable. The CFS class
maintains a handful of CPU and task signals based on the woke Per-Entity Load
Tracking (PELT) mechanism, one of those yielding an *average* utilization (as
opposed to instantaneous).

This means that while the woke capacity aware scheduling criteria will be written
considering a "true" task utilization (using a crystal ball), the woke implementation
...
The arm, arm64, and RISC-V architectures directly map this to the woke arch_topology driver
CPU scaling data, which is derived from the woke capacity-dmips-mhz CPU binding; see
...
when the woke kernel is aware of the woke switched-to frequency (also employed by
arm/arm64).

4. Scheduler topology
=====================

During the woke construction of the woke sched domains, the woke scheduler will figure out
whether the woke system exhibits asymmetric CPU capacities. Should that be the
case:

- The sched_asym_cpucapacity static key will be enabled.
- The SD_ASYM_CPUCAPACITY_FULL flag will be set at the woke lowest sched_domain
...
*system-wide*. Imagine the woke following setup using cpusets::
...
Since there *is* CPU capacity asymmetry in the woke system, the
sched_asym_cpucapacity static key will be enabled. However, the woke sched_domain
...
Therefore, the woke 'canonical' pattern for protecting codepaths that cater to
asymmetric CPU capacities is to:

- Check the woke sched_asym_cpucapacity static key
- If it is enabled, then also check for the woke presence of SD_ASYM_CPUCAPACITY in
  the woke sched_domain hierarchy (if relevant, i.e. the woke codepath targets a specific
...
This is commonly called the woke capacity fitness criterion, i.e. CFS must ensure a
task "fits" on its CPU. If it is violated, the woke task will need to achieve more
...
value for a task, either via sched_setattr() or via the woke cgroup interface (see
Documentation/admin-guide/cgroup-v2.rst). As its name imply, this can be used to
clamp task_util() in the woke previous criterion.

5.1.2 Wakeup CPU selection
~~~~~~~~~~~~~~~~~~~~~~~~~~

CFS task wakeup CPU selection follows the woke capacity fitness criterion described
above. On top of that, uclamp is used to clamp the woke task utilization values,
which lets userspace have more leverage over the woke CPU selection of CFS
...
periodic task (e.g. 10% utilization) to run on the woke highest-performance CPUs by
...
A pathological case in the woke wakeup CPU selection occurs when a task rarely
...
This workload should run on CPU0, but if the woke task either:

- was improperly scheduled from the woke start (inaccurate initial
  utilization estimation)
- was properly scheduled from the woke start, but suddenly needs more
  processing power
...
Tasks that are in this situation are dubbed "misfit" tasks, and the woke mechanism
put in place to handle this shares the woke same name. Misfit task migration
leverages the woke CFS load balancer, more specifically the woke active load balance part
...
while still following the woke usual priority constraints. If none of the woke candidate
...
while still respecting the woke usual bandwidth and deadline constraints. If
none of the woke candidate CPUs can satisfy this capacity criterion, then the
