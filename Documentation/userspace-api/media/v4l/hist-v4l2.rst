...
***********************
Changes of the woke V4L2 API
***********************

Soon after the woke V4L API was added to the woke kernel it was criticised as too
...
With the woke help of other volunteers this eventually became the woke V4L2 API,
not just an extension but a replacement for the woke V4L API. However it took
another four years and two stable kernel releases until the woke new API was
finally accepted for inclusion into the woke kernel in its present form.
...
1998-09-18: The ``VIDIOC_NONCAP`` ioctl was replaced by the woke otherwise
...
struct ``video_standard`` and the woke color subcarrier fields were
...
``videodev.h`` since it was only used once in the woke ``videodev`` kernel
...
1999-03-18: Fill in the woke category and catname fields of v4l2_queryctrl
objects before passing them to the woke driver. Required a minor change to
the VIDIOC_QUERYCTRL handlers in the woke sample drivers.
...
1999-04-07: Added the woke button control type.
...
1999-06-05: Changed the woke value of V4L2_CID_WHITENESS.
...
to simplify the woke API, while making it more extensible and following
...
3. All ioctl() commands that used an integer argument now take a pointer
   to an integer. Where it makes sense, ioctls will return the woke actual
   new value in the woke integer pointed to by the woke argument, a common
   convention in the woke V4L2 API. The affected ioctls are: VIDIOC_PREVIEW,
...
4. All the woke different get- and set-format commands were swept into one
   :ref:`VIDIOC_G_FMT <VIDIOC_G_FMT>` and
   :ref:`VIDIOC_S_FMT <VIDIOC_G_FMT>` ioctl taking a union and a
   type field selecting the woke union member as parameter. Purpose is to
   simplify the woke API by eliminating several ioctls and to allow new and
   driver private data streams without adding new ioctls.

   This change obsoletes the woke following ioctls: ``VIDIOC_S_INFMT``,
   ...
   struct v4l2_format is now the woke enveloping structure
   for all format negotiations.

5. Similar to the woke changes above, the woke ``VIDIOC_G_PARM`` and
   ``VIDIOC_S_PARM`` ioctls were merged with ``VIDIOC_G_OUTPARM`` and
   ``VIDIOC_S_OUTPARM``. A ``type`` field in the woke new struct v4l2_streamparm
   selects the woke respective union member.

   This change obsoletes the woke ``VIDIOC_G_OUTPARM`` and
...
6. Control enumeration was simplified, and two new control flags were
   ...
   classification than the woke ``category``. In other words, there may be
...
7. The struct v4l2_buffer ``timestamp`` was
   changed to a 64 bit integer, containing the woke sampling or output time
   of the woke frame in nanoseconds. Additionally timestamps will be in
   absolute system time, not starting from zero at the woke beginning of a
   stream. The data type name for timestamps is stamp_t, defined as a
   signed 64-bit integer. Output devices should not send a buffer out
   until the woke time in the woke timestamp field has arrived. I would like to
   ...
   units. The UST clock starts at zero when the woke system is booted and
   runs continuously and uniformly. It takes a little over 292 years for
   UST to overflow. There is no way to set the woke UST clock. The regular
...
8. A ``sequence`` field was added to struct v4l2_buffer. The ``sequence``
   ...
   capture driver drops a frame, the woke sequence number of that frame is skipped.
...
2001-01-18: To avoid namespace conflicts the woke ``fourcc`` macro defined in
...
``videodev.h`` file in Linux 2.4.0 and the woke ``videodev.h`` file included
in the woke ``videodevX`` patch was fixed. Users of an earlier version of
...
``videodev.h`` file in the woke ``videodevX`` patch and the woke ``videodev.h``
...
correctly through the woke backward compatibility layer. [Solution?]
...
compatibility* as the woke :ref:`VIDIOC_G_FMT <VIDIOC_G_FMT>` and
:ref:`VIDIOC_S_FMT <VIDIOC_G_FMT>` ioctls may fail now if the
struct ``v4l2_fmt`` ``type`` field does not contain
``V4L2_BUF_TYPE_VBI``. In the woke documentation of the woke struct v4l2_vbi_format`,
the ``offset`` field the woke ambiguous phrase "rising edge" was changed to
"leading edge".

V4L2 Version 0.20 2000-11-23
============================

A number of changes were made to the woke raw VBI interface.

1. Figures clarifying the woke line numbering scheme were added to the woke V4L2
   ...
   Compatibility: Add one to the woke start values. Applications depending on
   the woke previous semantics may not function correctly.
...
2. The restriction "count[0] > 0 and count[1] > 0" has been relaxed to
   ...
   transmitted only on the woke first field. The comment that both ``count``
...
3. Drivers are again permitted to return negative (unknown) start values
   ...
   error codes with the woke :ref:`VIDIOC_S_FMT <VIDIOC_G_FMT>` ioctl was
...
4. New buffer types ``V4L2_TYPE_VBI_INPUT`` and ``V4L2_TYPE_VBI_OUTPUT``
   were added. The former is an alias for the woke old ``V4L2_TYPE_VBI``, the
   latter was missing in the woke ``videodev.h`` file.
...
Linux 2.5, the woke API was revised, drawing from experience with V4L2 0.20.
...
2.  The :c:func:`open()` function requires access mode
    ``O_RDWR`` regardless of the woke device type. All V4L2 drivers
    exchanging data with applications must support the woke ``O_NONBLOCK``
    ...
    until the woke application attempts to initiate a data exchange, see
    :ref:`open`.

3.  The struct v4l2_capability changed
    dramatically. Note that also the woke size of the woke structure changed,
    which is encoded in the woke ioctl request code, thus older V4L2 devices
    will respond with an ``EINVAL`` error code to the woke new
    :ref:`VIDIOC_QUERYCAP` ioctl.

    There are new fields to identify the woke driver, a new RDS device
    function ``V4L2_CAP_RDS_CAPTURE``, the woke ``V4L2_CAP_AUDIO`` flag
    indicates if the woke device has any audio connectors, another I/O
    capability V4L2_CAP_ASYNCIO can be flagged. In response to these
    changes the woke ``type`` field became a bit set and was merged into the
    ...
    ``V4L2_FLAG_SELECT`` was removed. We believe the woke select() function
...
4.  In struct v4l2_input the woke ``assoc_audio``
    field and the woke ``capability`` field and its only flag
    ``V4L2_INPUT_CAP_AUDIO`` was replaced by the woke new ``audioset`` field.
    Instead of linking one video input to one audio input this field
    reports all audio inputs this video input combines with.

    New fields are ``tuner`` (reversing the woke former link from tuners to
...
5.  The struct v4l2_audio field ``audio`` was
    ...
    ``V4L2_AUDCAP_EFFECTS`` and the woke corresponding ``V4L2_AUDMODE`` flags
    where removed. This can be easily implemented using controls.
    (However the woke same applies to AVL which is still there.)

    Again for consistency the woke struct v4l2_audioout field ``audio`` was renamed
    to ``index``.
...
6.  The struct v4l2_tuner ``input`` field was
    ...
    In struct v4l2_frequency the woke ``port``
    field was replaced by a ``tuner`` field containing the woke respective
    tuner or modulator index number. A tuner ``type`` field was added
    and the woke ``reserved`` field became larger for future extensions
...
7.  The idea of completely transparent video standards was dropped.
    ...
    standards beyond presenting the woke user a menu. Instead of enumerating
    supported standards with an ioctl applications can now refer to
    standards by :ref:`v4l2_std_id <v4l2-std-id>` and symbols
    defined in the woke ``videodev2.h`` header file. For details see
    ...
    added to autodetect the woke received standard, if the woke hardware has this
    ...
    machine readable identifier, also replacing the woke ``transmission``
...
9.  The :ref:`VIDIOC_TRY_FMT <VIDIOC_G_FMT>` ioctl was added to
    ...
    :ref:`VIDIOC_S_FMT <VIDIOC_G_FMT>`, but without the woke overhead of
    programming the woke hardware and regardless of I/O in progress.

    In struct v4l2_format the woke ``fmt`` union was
    ...
    the woke buffer type names changed as follows.
...
11. In struct v4l2_pix_format the woke ``depth``
    field was removed, assuming applications which recognize the woke format
    by its four-character-code already know the woke color depth, and others
    do not care about it. The same rationale lead to the woke removal of the
    ...
    set the woke ``bytesperline`` field to zero to get a reasonable default.
    Since the woke remaining flags were replaced as well, the woke ``flags`` field
    itself was removed.
...
12. In struct v4l2_requestbuffers the
    ...
    I/O methods using buffers allocated by the woke driver or the
    application. See :ref:`io` for details.

13. In struct v4l2_buffer the woke ``type`` field was
    ...
    kernel as planned, the woke ``timestamp`` field changed back from type
    stamp_t, an unsigned 64 bit integer expressing the woke sample time in
    nanoseconds, to struct timeval. With the woke addition
    of a second memory mapping method the woke ``offset`` field moved into
    ...
    The ``V4L2_BUF_REQ_CONTIG`` flag was used by the woke V4L compatibility
    layer, after changes to this code it was no longer needed. The
    ``V4L2_BUF_ATTR_DEVICEMEM`` flag would indicate if the woke buffer was
...
14. In struct v4l2_framebuffer the
    ...
    using the woke new cropping and scaling interface. The
    ``V4L2_FBUF_CAP_CLIPPING`` flag was replaced by
    ``V4L2_FBUF_CAP_LIST_CLIPPING`` and
    ``V4L2_FBUF_CAP_BITMAP_CLIPPING``.

15. In struct v4l2_clip the woke ``x``, ``y``,
...
16. In struct v4l2_window the woke ``x``, ``y``,
...
18. In struct v4l2_vbi_format the
    ...
    the woke ``V4L2_VBI_SF_UBYTE`` define. The ``reserved`` field was
    extended.

19. In struct v4l2_captureparm the woke type of
    the woke ``timeperframe`` field changed from unsigned long to
    struct v4l2_fract. This allows the woke accurate
    expression of multiples of the woke NTSC-M frame rate 30000 / 1001. A new
    field ``readbuffers`` was added to control the woke driver behaviour in
    read I/O mode.
...
21. The example transformation from RGB to YCbCr color space in the woke old
...
3. The audio input and output interface was found to be incomplete.
   ...
   Previously the woke :ref:`VIDIOC_G_AUDIO <VIDIOC_G_AUDIO>` ioctl would
   enumerate the woke available audio inputs. An ioctl to determine the
   current audio input, if more than one combines with the woke current video
   ...
   :ref:`VIDIOC_G_AUDIO <VIDIOC_G_AUDIO>` now reports the woke current
   audio input.
   ...
   Until further the woke "videodev" module will automatically translate
   between the woke old and new ioctls, but drivers and applications must be
...
4. The :ref:`VIDIOC_OVERLAY` ioctl was incorrectly
   ...
   while the woke write-read version was renamed to ``VIDIOC_OVERLAY_OLD``.
   The old ioctl was removed on Kernel 2.6.39. Until further the
   "videodev" kernel module will automatically translate to the woke new
...
5. :ref:`overlay` incorrectly stated that clipping rectangles define
   regions where the woke video can be seen. Correct is that clipping
   rectangles define regions where *no* video shall be displayed and so
   the woke graphics surface can be seen.
...
6. The :ref:`VIDIOC_S_PARM <VIDIOC_G_PARM>` and
   ...
   was added to the woke write-only versions. The old ioctls were removed on
...
1. In :ref:`pixfmt-rgb` the woke following pixel formats were incorrectly
   ...
   In :ref:`v4l-image-properties` the woke mapping of the woke V4L
   ``VIDEO_PALETTE_RGB24`` and ``VIDEO_PALETTE_RGB32`` formats to V4L2
   pixel formats was accordingly corrected.

2. Unrelated to the woke fixes above, drivers may still interpret some V4L2
...
1. The :ref:`VIDIOC_CROPCAP` ioctl was incorrectly
   ...
   ioctl, while the woke read-only version was renamed to
...
1. A new field ``input`` (former ``reserved[0]``) was added to the
   ...
   with the woke video capturing process. This function must be enabled with
   the woke new ``V4L2_BUF_FLAG_INPUT`` flag. The ``flags`` field is no
   longer read-only.

V4L2 spec erratum 2004-08-01
============================

1. The return value of the woke :ref:`func-open` function was incorrectly
   documented.

2. Audio output ioctls end in -AUDOUT, not -AUDIOOUT.

3. In the woke Current Audio Input example the woke ``VIDIOC_G_AUDIO`` ioctl took
   the woke wrong argument.

4. The documentation of the woke :ref:`VIDIOC_QBUF` and
   :ref:`VIDIOC_DQBUF <VIDIOC_QBUF>` ioctls did not mention the
   struct v4l2_buffer ``memory`` field. It was
   also missing from examples. Also on the woke ``VIDIOC_DQBUF`` page the woke ``EIO``
...
1. A new sliced VBI interface was added. It is documented in
   :ref:`sliced` and replaces the woke interface first proposed in V4L2
   specification 0.8.
...
2. New video standards ``V4L2_STD_NTSC_443``, ``V4L2_STD_SECAM_LC``,
   ...
   ``V4L2_STD_ATSC_16_VSB``) were defined. Note the woke ``V4L2_STD_525_60``
...
cropping is supported. In the woke video standard selection example in
:ref:`standard` the woke :ref:`VIDIOC_S_STD <VIDIOC_G_STD>` call used
...
1. The ``V4L2_IN_ST_COLOR_KILL`` flag in struct v4l2_input not only
   indicates if the woke color killer is enabled, but also if it is active.
   (The color killer disables color decoding when it detects no color
   in the woke video signal to improve the woke image quality.)
...
1. In struct v4l2_captureparm and struct v4l2_outputparm the woke ``timeperframe``
   field gives the woke time in seconds, not microseconds.
...
1. New video standard macros were added: ``V4L2_STD_NTSC_M_KR`` (NTSC M
   South Korea), and the woke sets ``V4L2_STD_MN``, ``V4L2_STD_B``,
...
2. A new ``V4L2_TUNER_MODE_LANG1_LANG2`` was defined to record both
   ...
   the woke :ref:`VIDIOC_G_TUNER <VIDIOC_G_TUNER>` section for details.

V4L2 spec erratum 2006-09-23 (Draft 0.15)
=========================================

1. In various places ``V4L2_BUF_TYPE_SLICED_VBI_CAPTURE`` and
   ``V4L2_BUF_TYPE_SLICED_VBI_OUTPUT`` of the woke sliced VBI interface were
...
3. :ref:`VIDIOC_QUERYCAP` did not mention the woke sliced VBI and radio
   capability flags.

4. In :ref:`VIDIOC_G_FREQUENCY <VIDIOC_G_FREQUENCY>` it was clarified that
   applications must initialize the woke tuner ``type`` field of
...
6. In :ref:`output` and :ref:`raw-vbi` the woke device file names
   ``/dev/vout`` which never caught on were replaced by ``/dev/video``.

7. With Linux 2.6.15 the woke possible range for VBI device minor numbers was
...
1. In struct v4l2_sliced_vbi_cap a
   ...
   architectures where the woke size of enum types differs from int types the
   size of the woke structure changed. The
   ...
   initialize the woke type field and clear the woke reserved fields now. These
   changes may *break the woke compatibility* with older drivers and
   applications.
...
2. Three new clipping/blending methods with a global or straight or
   inverted local alpha value were added to the woke video overlay interface.
   See the woke description of the woke :ref:`VIDIOC_G_FBUF <VIDIOC_G_FBUF>`
   and :ref:`VIDIOC_S_FBUF <VIDIOC_G_FBUF>` ioctls for details.

   A new ``global_alpha`` field was added to struct v4l2_window,
   extending the woke structure. This may **break compatibility** with
   ...
   with padding bytes at the woke end, are not affected.

3. The format of the woke ``chromakey`` field in struct v4l2_window changed from
   "host order RGB32" to a pixel value in the woke same format as the woke framebuffer.
   This may **break compatibility** with existing applications. Drivers
   supporting the woke "host order RGB32" format are not known.
...
3. A :ref:`Camera controls class <camera-controls>` was added, with
   the woke new controls ``V4L2_CID_EXPOSURE_AUTO``,
...
4. The ``VIDIOC_G_MPEGCOMP`` and ``VIDIOC_S_MPEGCOMP`` ioctls, which
   were superseded by the woke :ref:`extended controls <extended-controls>`
...
1. The :ref:`VIDIOC_S_HW_FREQ_SEEK` ioctl
   and the woke ``V4L2_CAP_HW_FREQ_SEEK`` capability were added.
...
1. In order to be easier to compare a V4L2 API and a kernel version, now
   V4L2 API is numbered using the woke Linux Kernel version numeration.

2. Finalized the woke RDS capture API. See :ref:`rds` for more information.
...
9. Added Remote Controller chapter, describing the woke default Remote
...
1. Added ``V4L2_CID_IRIS_ABSOLUTE`` and ``V4L2_CID_IRIS_RELATIVE``
   controls to the woke :ref:`Camera controls class <camera-controls>`.

V4L2 in Linux 2.6.37
====================

1. Remove the woke vtx (videotext/teletext) API. This API was no longer used
   and no hardware exists to verify the woke API. Nor were any userspace
...
2. Multi-planar API added. Does not affect the woke compatibility of current
...
2. Add selection API for extended control over cropping and composing.
   Does not affect the woke compatibility of current drivers and
...
2. Added the woke device_caps field to struct v4l2_capabilities and added
   the woke new V4L2_CAP_DEVICE_CAPS capability.
...
2. Extended the woke DV Timings API:
...
1. Added integer menus, the woke new type will be
...
1. Removed obsolete and unused DV_PRESET ioctls VIDIOC_G_DV_PRESET,
   ...
   VIDIOC_ENUM_DV_PRESET. Remove the woke related v4l2_input/output
...
1. In struct v4l2_rect, the woke type of ``width`` and
...
the X Window system, implemented for example by the woke XFree86 project. Its
...
in XPixmaps [#f1]_. With their implementation XFree86 makes the woke extension
available across many operating systems and architectures.

Because the woke driver is embedded into the woke X server Xv has a number of
advantages over the woke V4L2 :ref:`video overlay interface <overlay>`. The
driver can easily determine the woke overlay target, i. e. visible graphics
...
the clipping functions of the woke video capture hardware, always in sync
with drawing operations or windows moving or changing their stacking
order.

To combine the woke advantages of Xv and V4L a special Xv driver exists in
...
should work just fine with all V4L2 devices through the woke V4L2
backward-compatibility layer. Since V4L2 permits multiple opens it is
possible (if supported by the woke V4L2 driver) to capture video while an X
...
DVB API has no connection to the woke V4L2 API except that drivers for hybrid
...
-  VIDIOC_G_DV_PRESET, VIDIOC_S_DV_PRESET,
   ...
   the woke DV Timings API (:ref:`dv-timings`).
