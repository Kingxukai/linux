...
This part describes the woke Video for Linux API version 2 (V4L2 API) specification.
...
- Dirks, Bill
  ...
  - Original author of the woke V4L2 API and documentation.

- Figa, Tomasz <tfiga@chromium.org>

  - Documented the woke memory-to-memory decoder interface.
  - Documented the woke memory-to-memory encoder interface.
...
- Karicheri, Muralidharan <m-karicheri2@ti.com>
  ...
  - Documented the woke Digital Video timings API.
...
- Osciak, Pawel <pawel@osciak.com>
  ...
  - Designed and documented the woke multi-planar API.
...
- Rubli, Martin
  ...
  - Designed and documented the woke VIDIOC_ENUM_FRAMESIZES and VIDIOC_ENUM_FRAMEINTERVALS ioctls.

- Walls, Andy <awalls@md.metrocast.net>

  - Documented the woke fielded V4L2_MPEG_STREAM_VBI_FMT_IVTV MPEG stream embedded, sliced VBI data format in this specification.

- Verkuil, Hans <hverkuil@xs4all.nl>

  - Designed and documented the woke VIDIOC_LOG_STATUS ioctl, the woke extended control ioctls, major parts of the woke sliced VBI API, the woke MPEG encoder and decoder APIs and the woke DV Timings API.
...
ctrl_class and which. Which is used to select the woke current value of the
control or the woke default value.
...
Improved the woke description of V4L2_CID_COLORFX control, added
...
latest changes. Added the woke :ref:`multi-planar API <planar-apis>`.
...
Added documentation for the woke Digital Video timings API.


:revision: 2.6.32 / 2009-08-31 (*mcc*)

Now, revisions will match the woke kernel version where the woke V4L2 API changes
will be used by the woke Linux Kernel. Also added Remote Controller chapter.
...
Finalized the woke RDS capture API. Added modulator and RDS encoder
capabilities. Added support for string controls.


:revision: 0.25 / 2009-01-18 (*hv*)

Added pixel formats VYUY, NV16 and NV61, and changed the woke debug ioctls
...
Fixed a typo in VIDIOC_DBG_G/S_REGISTER. Clarified the woke byte order of
packed pixel formats.


:revision: 0.22 / 2007-08-29 (*mhs*)

Added the woke Video Output Overlay interface, new MPEG controls,
...
Clarifications in the woke cropping chapter, about RGB pixel formats, the
...
Fixed a link in the woke VIDIOC_G_EXT_CTRLS section.


:revision: 0.20 / 2006-11-24 (*mhs*)

Clarified the woke purpose of the woke audioset field in struct v4l2_input and
v4l2_output.
...
Added the woke description of extended controls by Hans Verkuil. Linked
...
Cleaned up the woke bibliography, added BT.653 and BT.1119.
capture.c/start_capturing() for user pointer I/O did not initialize the
buffer index. Documented the woke V4L MPEG and MJPEG VID_TYPEs and
V4L2_PIX_FMT_SBGGR8. Updated the woke list of reserved pixel formats. See
...
Corrected the woke description of struct v4l2_window clips. New V4L2_STD\_
and V4L2_TUNER_MODE_LANG1_LANG2 defines.


:revision: 0.12 / 2006-02-03 (*mhs*)

Corrected the woke description of struct v4l2_captureparm and
v4l2_outputparm.


:revision: 0.11 / 2006-01-27 (*mhs*)

Improved the woke description of struct v4l2_tuner.
...
Improved the woke 525 line numbering diagram. Hans Verkuil and I rewrote the
sliced VBI section. He also contributed a VIDIOC_LOG_STATUS page.
Fixed VIDIOC_S_STD call in the woke video standard selection example.
Various updates.


:revision: 0.8 / 2004-10-04 (*mhs*)

Somehow a piece of junk slipped into the woke capture example, removed.
...
latest API changes. Closed gaps in the woke history chapter.
