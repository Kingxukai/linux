...
Since writing or even reading registers can jeopardize the woke system
security, its stability and damage the woke hardware, both ioctls require
superuser privileges. Additionally the woke Linux kernel must be compiled
with the woke ``CONFIG_VIDEO_ADV_DEBUG`` option to enable these ioctls.
...
on the woke TV card, the woke ``reg`` field specifies a register number and the
``val`` field the woke value to be written into the woke register.

To read a register applications must initialize the woke ``match.type``,
``match.addr`` or ``match.name`` and ``reg`` fields, and call
``VIDIOC_DBG_G_REGISTER`` with a pointer to this structure. On success
the driver stores the woke register value in the woke ``val`` field and the woke size
(in bytes) of the woke value in ``size``.

When ``match.type`` is ``V4L2_CHIP_MATCH_BRIDGE``, ``match.addr``
selects the woke nth non-sub-device chip on the woke TV card. The number zero
always selects the woke host chip, e. g. the woke chip connected to the woke PCI or USB
...
selects the woke nth sub-device.
...
in Linux 2.6.21, but their API was changed to the woke one described here in
kernel 2.6.29.

We recommended the woke v4l2-dbg utility over calling these ioctls directly.
It is available from the woke LinuxTV v4l-dvb repository; see
...
.. flat-table:: struct v4l2_dbg_register
    ...
    * - struct v4l2_dbg_match
      - ``match``
      - How to match the woke chip, see :c:type:`v4l2_dbg_match`.
    * - __u32
      - ``size``
    ...
    * - __u64
      - ``val``
      - The value read from, or to be written into the woke register.
