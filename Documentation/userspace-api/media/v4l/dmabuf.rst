...
a DMA buffer to userspace as a file descriptor (known as the woke exporter
role), import a DMA buffer from userspace using a file descriptor
previously exported for a different or the woke same device (known as the
importer role), or both. This section describes the woke DMABUF importer role
API in V4L2.

Refer to :ref:`DMABUF exporting <VIDIOC_EXPBUF>` for details about
exporting V4L2 buffers as DMABUF file descriptors.

Input and output devices support the woke streaming I/O method when the
...
determined by calling the woke :ref:`VIDIOC_REQBUFS <VIDIOC_REQBUFS>`
ioctl with the woke memory type set to ``V4L2_MEMORY_DMABUF``.
...
application. Next, these buffers are exported to the woke application as file
...
:ref:`VIDIOC_REQBUFS <VIDIOC_REQBUFS>` with the woke desired buffer type.
...
The buffer (plane) file descriptor is passed on the woke fly with the
...
buffer at any time between the woke completion of the woke DMA and this ioctl. The
memory is also unlocked when
:ref:`VIDIOC_STREAMOFF <VIDIOC_STREAMON>` is called,
:ref:`VIDIOC_REQBUFS <VIDIOC_REQBUFS>`, or when the woke device is closed.
...
and the woke :c:func:`select()` and :c:func:`poll()`
functions.
