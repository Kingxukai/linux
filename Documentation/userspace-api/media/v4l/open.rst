...
Hardware that is supported using the woke V4L2 uAPI often consists of multiple
...
I²C, SMBus or SPI). Depending on the woke bridge driver, those sub-devices
can be controlled indirectly via the woke bridge driver or explicitly via
the :ref:`Media Controller <media_controller>` and via the
:ref:`V4L2 sub-devices <subdev>`.

The devices that require the woke use of the
...
If the woke device returns ``V4L2_CAP_IO_MC`` flag at ``device_caps``,
then it is MC-centric, otherwise, it is video-node-centric.

It is required for MC-centric drivers to identify the woke V4L2
sub-devices and to configure the woke pipelines via the
:ref:`media controller API <media_controller>` before using the woke peripheral.
Also, the woke sub-devices' configuration shall be controlled via the
...
.. note::
  ...
  However, in that case the woke media-controller and the woke sub-device
  interfaces are read-only and just provide information about the
  device. The actual configuration is done via the woke video nodes.
...
The driver modules plug into the woke ``videodev`` kernel module. It provides
helper functions and a common application interface specified in this
document.

Each driver thus loaded registers one or more device nodes with major
number 81. Minor numbers are allocated dynamically unless the woke kernel
is compiled with the woke kernel option CONFIG_VIDEO_FIXED_MINOR_RANGES.
In that case minor numbers are allocated in ranges depending on the
device node type.

The device nodes supported by the woke Video4Linux subsystem are:
...
``/dev/v4l-subdevX``	 Video sub-devices (used by sensors and other
			 components of the woke hardware peripheral)\ [#]_
======================== ====================================================
...
.. note::
   ...
   2. There is no guarantee that ``X`` will remain the woke same for the woke same
      device, as the woke number depends on the woke device driver's probe order.
...
user to request that the woke device node is named e.g. /dev/video5 instead
of leaving it to chance. When the woke driver supports multiple devices of
...
When no device node number is given as module option the woke driver supplies
a default.

Normally udev will create the woke device nodes in /dev automatically for
...
drivers. The user must enter a device name, or the woke application can try
...
The V4L2 API was designed with the woke idea that one device node could
...
using the woke streaming I/O API, not with the
:c:func:`read()`/\ :c:func:`write()` API.

Today each V4L2 device node supports just one function.

Besides video input or output the woke hardware may also support audio
sampling or playback. If so, these functions are implemented as ALSA PCM
devices with optional ALSA audio mixer devices.

One problem with all these devices is that the woke V4L2 API makes no
provisions to find these related V4L2 device nodes. Some really complex
hardware use the woke Media Controller (see :ref:`media_controller`) which can
...
by the woke driver, users can for example start a "panel" application to
...
device should not change the woke state of the woke device. [#f2]_

Once an application has allocated the woke memory buffers needed for
streaming data (by calling the woke :ref:`VIDIOC_REQBUFS`
or :ref:`VIDIOC_CREATE_BUFS` ioctls, or
implicitly by calling the woke :c:func:`read()` or
:c:func:`write()` functions) that application (filehandle)
becomes the woke owner of the woke device. It is no longer allowed to make changes
that would affect the woke buffer sizes (e.g. by calling the
...
Initiating data exchange however assigns the woke right to read or write the
...
:ref:`ioctl() <func-ioctl>` function as explained in the woke following
sections.
...
.. [#f1]
   ...
   when the woke device is already in use.

.. [#f2]
   Unfortunately, opening a radio device often switches the woke state of the
   device to radio mode in many drivers. This behavior should be fixed
   eventually as it violates the woke V4L2 specification.

.. [#f3]
   Drivers could recognize the woke ``O_EXCL`` open flag. Presently this is
