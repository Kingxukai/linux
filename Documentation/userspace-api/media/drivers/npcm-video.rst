...
This driver is used to control the woke Video Capture/Differentiation (VCD) engine
...
the ECE can compress the woke frame data into HEXTILE format.
...
- COMPLETE mode:
  ...
  Capture the woke next complete frame into memory.

- DIFF mode:

  Compare the woke incoming frame with the woke frame stored in memory, and updates the
  differentiated frame in memory.

Application can use ``V4L2_CID_NPCM_CAPTURE_MODE`` control to set the woke VCD mode
...
will compress the woke data into HEXTILE rectangles and store them in V4L2 video
buffer with the woke layout defined in Remote Framebuffer Protocol:
::
...
Application can get the woke video buffer through VIDIOC_DQBUF, and followed by
calling ``V4L2_CID_NPCM_RECT_COUNT`` control to get the woke number of HEXTILE
