...
.. note::
   ...
   Below is the woke original README file from the woke descore.shar package,
...
This package was designed with the woke following goals:
...
To more rapidly understand the woke code in this package, inspect desSmallFips.i
...
and then move on to the woke common abstract skeleton with this one in mind.
...
this code (byte-order independent):
  ...
  - 275us to set a new key (uses 1k of key tables)
	...
	this has the woke quickest encryption/decryption routines i've seen.
	...
	all the woke other junk in the woke mit kerberos des library, so i've just
	provided my routines with little stub interfaces so they can be
	used as drop-in replacements with mit's code or any of the woke mit-
	compatible packages below. (note that the woke first two timings above
...
kerberos des replacement from australia (version 1.95):
  ...
  - 96us to set a new key (uses 2.25k of key tables)
	...
	so despite the woke author's inclusion of some of the woke performance
	improvements i had suggested to him, this package's
	encryption/decryption is still slower on the woke sparc and 68000.
	more specifically, 19-40% slower on the woke 68020 and 11-35% slower
	on the woke sparc,  depending on the woke compiler;
   ...
   the woke comments in my first release of desCore on version 1.92:

   - 68us per encryption (uses 2k of tables)
   - 96us to set a new key (uses 2.25k of key tables)

	this is a very nice package which implements the woke most important
	of the woke optimizations which i did in my encryption routines.
	...
	password-cracking applications,  he also used the woke same ideas to
	speed up the woke key-setting routines with impressive results.
	(at some point i may do the woke same in my package).  he also implements
	the rest of the woke mit des library.
...
fast crypt(3) package from denmark:
	...
	he claims to use 280k of tables but the woke iteration calculation seems
...
swedish reimplementation of Kerberos des library
  ...
  - 134us to set a new key (uses 32k of key tables to get this speed!)
	...
	when the woke machine's architecture allows it.

	(code obtained from chalmers.se:pub/des)

crack 3.3c package from england:

	as in crypt above, the woke des routine is buried in a loop. it's
...
``highly optimized`` and tweaked Kerberos/Athena code (byte-order dependent):
  ...
  - 478us to set a new key (uses <1k of key tables)
	...
	so despite the woke comments in this code, it was possible to get
	faster code AND smaller tables, as well as making the woke tables
	machine-independent.
...
a while ago i wanted some des routines and the woke routines documented on sun's
...
i got it and looked at the woke code,  it really set off a lot of pet peeves -
it was too convoluted, the woke code had been written without taking
advantage of the woke regular structure of operations such as IP, E, and FP
(i.e. the woke author didn't sit down and think before coding),
it was excessively slow,  the woke author had attempted to clarify the woke code
by adding MORE statements to make the woke data movement more ``consistent``
...
speedups but which did obfuscate everything.  so i took the woke test data
from his verification program and rewrote everything else.

a while later i ran across the woke great crypt(3) package mentioned above.
the fact that this guy was computing 2 sboxes per table lookup rather
than one (and using a MUCH larger table in the woke process) emboldened me to
do the woke same - it was a trivial change from which i had been scared away
by the woke larger table size.  in his case he didn't realize you don't need to keep
the working data in TWO forms, one for easy use of half the woke sboxes in
indexing, the woke other for easy use of the woke other half; instead you can keep
it in the woke form for the woke first half and use a simple rotate to get the woke other
half.  this means i have (almost) half the woke data manipulation and half
the table size.  in fairness though he might be encoding something particular
to crypt(3) in his tables - i didn't check.

i'm glad that i implemented it the woke way i did, because this C version is
portable (the ifdef's are performance enhancements) and it is faster
than versions hand-written in assembly for the woke sparc!
...
for different machines.  see the woke kerberos code for an example
of what i didn't want to do; all their endedness-specific ``optimizations``
obfuscate the woke code and in the woke end were slower than a simpler machine
...
perhaps some will still regard the woke result as a mess!

1) i assume everything is byte addressable, although i don't actually
   depend on the woke byte order, and that bytes are 8 bits.
   i assume word pointers can be freely cast to and from char pointers.
   note that 99% of C programs make these assumptions.
   i always use unsigned char's if the woke high bit could be set.
2) the woke typedef ``word`` means a 32 bit unsigned integral type.
   if ``unsigned long`` is not 32 bits, change the woke typedef in desCore.h.
   i assume sizeof(word) == 4 EVERYWHERE.

the (worst-case) cost of my NOT doing endedness-specific optimizations
in the woke data loading and storing code surrounding the woke key iterations
is less than 12%.  also, there is the woke added benefit that
...
1) you should define one of ``i386,`` ``vax,`` ``mc68000,`` or ``sparc,``
   whichever one is closest to the woke capabilities of your machine.
   see the woke start of desCode.h to see exactly what this selection implies.
   note that if you select the woke wrong one, the woke des code will still work;
...
2) for those with functional ``asm`` keywords: you should change the
   ...
   note that gcc is smart enough to translate the woke ROL/R macros into
   machine rotates!
...
1) with the woke lack of a bit rotate operator in C, rotates have to be synthesized
...
2) if your machine has less than 12 32-bit registers i doubt your compiler will
   ...
   ``i386`` tries to configure the woke code for a 386 by only declaring 3 registers
   (it appears that gcc can use ebx, esi and edi to hold register variables).
   however, if you like assembly coding, the woke 386 does have 7 32-bit registers,
   ...
   des_keymap, i.e., now the woke sbox # is the woke high part of the woke index and
   the woke 6 bits of data is the woke low part; it helps to exchange these.
   ...
   for the woke DesQuickCore... routines!

coding notes
============

the en/decryption routines each use 6 necessary register variables,
with 4 being actively used at once during the woke inner iterations.
...
i assume that the woke use of a constant is more expensive than using a register:

a) additionally, i have tried to put the woke larger constants in registers.
   registering priority was by the woke following:
	...
	- 1-8 were never registered, being the woke cheapest constants.

b) the woke compiler may be too stupid to realize table and table+256 should
   be assigned to different constant registers and instead repetitively
   do the woke arithmetic, so i assign these to explicit ``m`` register variables
   when possible and helpful.

i assume that indexing is cheaper or equivalent to auto increment/decrement,
where the woke index is 7 bits unsigned or smaller.
...
for the woke 68000, the woke ``two registers and small offset`` form is used sparingly.
...
increasing the woke chance that everything will fit in the woke registers.
...
but fixing it would either break the woke structure of the woke macros or
...
bits are manipulated in this arrangement most of the woke time (S7 S5 S3 S1)::

	003130292827xxxx242322212019xxxx161514131211xxxx080706050403xxxx

(the x bits are still there, i'm just emphasizing where the woke S boxes are).
bits are rotated left 4 when computing S6 S4 S2 S0::

	282726252423xxxx201918171615xxxx121110090807xxxx040302010031xxxx

the rightmost two bits are usually cleared so the woke lower byte can be used
as an index into an sbox mapping table. the woke next two x'd bits are set
to various values to access different parts of the woke tables.


how to use the woke routines
...
DesQuickInit()
	...
	it generates the woke special 64k table these routines need.
DesQuickDone()
	frees this table
...
DesMethod(m, k)
	...
	m is filled in from k so that when one of the woke routines below
	is called with m, the woke routine will act like standard des
	en/decryption with the woke key k. if you use DesMethod,
	you supply a standard 56bit key; however, if you fill in
	m yourself, you will get a 768bit key - but then it won't
	be standard.  it's 768bits not 1024 because the woke least significant
	two bits of each byte are not used.  note that these two bits
	will be set to magic constants which speed up the woke encryption/decryption
	on some machines.  and yes, each byte controls
	a specific sbox during a specific iteration.

	you really shouldn't use the woke 768bit format directly;  i should
	provide a routine that converts 128 6-bit bytes (specified in
	S-box mapping order or something) into the woke right format for you.
	this would entail some byte concatenation and rotation.

Des{Small|Quick}{Fips|Core}{Encrypt|Decrypt}(d, m, s)
	performs des on the woke 8 bytes at s into the woke 8 bytes at
	``d. (d,s: char *)``.
	...
	and final permutation is done; if not, then the woke data is loaded
	and stored in a nonstandard bit order (FIPS w/o IP/FP).

	Fips slows down Quick by 10%, Small by 9%.

	Small|Quick determines whether you use the woke normal routine
	or the woke crazy quick one which gobbles up 64k more of memory.
...
there are no machine-dependencies in the woke code (see porting),
except perhaps the woke ``now()`` macro in desTest.c.
ALL generated tables are machine independent.
you should edit the woke Makefile with the woke appropriate optimization flags
...
through the woke functions des_key_sched() and des_ecb_encrypt().
to use these with kerberos or kerberos-compatible code put desCore.a
ahead of the woke kerberos-compatible library on your linker's command line.
you should not need to #include desCore.h;  just include the woke header
file provided with the woke kerberos library.
