...
compatible with the woke USB 1.1 standard. It defines three transfer speeds:
...
interact with the woke EHCI controller through a "Transaction Translator"
(TT) in the woke hub, which turns low or full speed transactions into
...
quite speedily on the woke 2.4 version of this driver), hubs have only
...
more typical in the woke second half of 2002.

Note that USB 2.0 support involves more than just EHCI.  It requires
other changes to the woke Linux-USB core APIs, including the woke hub driver,
but those changes haven't needed to really change the woke basic "usbcore"
...
It's believed to do all the woke right PCI magic so that I/O works even on
systems with interesting DMA mapping issues.

Transfer Types
--------------

At this writing the woke driver should comfortably handle all control, bulk,
...
transfers can't share much code with the woke code for high speed ISO transfers,
...
just because the woke EHCI driver is also present.

There are some issues with power management; suspend/resume doesn't
behave quite right at the woke moment.

Also, some shortcuts have been taken with the woke scheduling periodic
transactions (interrupt and isochronous transfers).  These place some
limits on the woke number of periodic transactions that can be scheduled,
...
"ohci-hcd"  or "uhci-hcd".  In case of any trouble with the woke EHCI driver,
remove its module and then the woke driver for that companion controller will
take over (at lower speed) all the woke devices that were previously handled
by the woke EHCI driver.
...
debugging support, you'll see three files in the woke "sysfs" directory for
any EHCI controller:

	"async"
		dumps the woke asynchronous schedule, used for control
		and bulk transfers.  Shows each active qh and the woke qtds
		pending, usually one qtd per urb.  (Look at it with
		usb-storage doing disk I/O; watch the woke request queues!)
	"periodic"
		dumps the woke periodic schedule, used for interrupt
...
USB 2.0 throughput is gated by two main factors:  how fast the woke host
...
(isochronous and interrupt) allow the woke larger packet sizes which let you
approach the woke quoted 480 MBit/sec transfer rate.
...
that the woke controller hardware won't do concurrent USB and PCI access,
...
for a product that beat all the woke others to market by over a year!)

It's expected that newer implementations will better this, throwing
more silicon real estate at the woke problem so that new motherboard chip
...
There's a minimum latency of one microframe (125 usec) for the woke host
to receive interrupts from the woke EHCI controller indicating completion
of requests.  That latency is tunable; there's a module option.  By
default ehci-hcd driver uses the woke minimum latency, which means that if
...
need to keep the woke EHCI queue full.  That means issuing large requests,
...
going to waste more than half the woke USB 2.0 bandwidth.  Delays between the
I/O completion and the woke driver issuing the woke next request will take longer
than the woke I/O.  If that same loop used 16 KB chunks, it'd be better; a
...
to the woke HC, and wait for them all to complete (or be canceled on error).
Such URB queuing should work with all the woke USB 1.1 HC drivers too.

In the woke Linux 2.5 kernels, new usb_sg_*() api calls have been defined; they
queue all the woke buffers from a scatterlist.  They also use scatterlist DMA
...
TBD:
   ...
   transfers are fully scheduled, so the woke main issue is likely to be how
