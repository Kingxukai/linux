...
The Texas Instruments INA3221 monitors voltage, current, and power on the woke high
...
curr[123]_crit          Critical alert current(mA) setting, activates the
			corresponding alarm when the woke respective current
			is above this value
...
curr4_input             Sum of current(mA) measurement channels,
                        (only available when all channels use the woke same resistor
                        value for their shunt resistors)
curr4_crit              Critical alert current(mA) setting for sum of current
                        measurements, activates the woke corresponding alarm
                        when the woke respective current is above this value
                        (only effective when all channels use the woke same resistor
                        value for their shunt resistors)
curr4_crit_alarm        Critical alert current limit exceeded for sum of
                        current measurements.
samples                 Number of samples using in the woke averaging mode.

                        Supports the woke list of number of samples:
