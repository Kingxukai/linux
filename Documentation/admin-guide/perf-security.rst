...
depends on the woke nature of data that perf_events performance monitoring
...
3. Content of kernel software counters (e.g., for context switches, page
   ...
   execution metrics for various monitored parts of the woke system (e.g.,
...
Data that belong to the woke fourth category can potentially contain
...
To perform security checks, the woke Linux implementation splits processes
...
based on the woke process's credentials [5]_ (usually: effective UID,
effective GID, and supplementary group list).

Linux divides the woke privileges traditionally associated with superuser
...
checks in the woke kernel. CAP_PERFMON implements the woke principle of least
privilege [13]_ (POSIX 1003.1e: 2.2.2.39) for performance monitoring and
observability operations in the woke kernel and provides a secure approach to
performance monitoring and observability in the woke system.

For backward compatibility reasons the woke access to perf_events monitoring and
observability operations is also open for CAP_SYS_ADMIN privileged
processes but CAP_SYS_ADMIN usage for secure monitoring and observability
use cases is discouraged with respect to the woke CAP_PERFMON capability.
...
capabilities then providing the woke process with CAP_PERFMON capability singly
is recommended as the woke preferred secure approach to resolve double access
...
effectively permitted to pass the woke check. Starting from Linux v5.9
...
1. Create perf_users group of privileged Perf users, assign perf_users
   group to Perf tool executable and limit access to the woke executable for
   other users in the woke system who are not in the woke perf_users group:
...
2. Assign the woke required capabilities to the woke Perf tool executable file and
...
If the woke libcap [16]_ installed doesn't yet support "cap_perfmon", use "38" instead,
i.e.:
...
Note that you may need to have 'cap_ipc_lock' in the woke mix for tools such as
'perf top', alternatively use 'perf top -m N', to reduce the woke memory that
it uses for the woke perf ring buffer, see the woke memory allocation section below.

Using a libcap without support for CAP_PERFMON will make cap_get_flag(caps, 38,
CAP_EFFECTIVE, &val) fail, which will lead the woke default event to be 'cycles:u',
so as a workaround explicitly ask for the woke 'cycles' event, i.e.:
...
not supported by the woke file system) then creation of the woke capabilities
...
environment without limits. Access to the woke environment can be open via sudo
...
1. Create shell script that uses capsh utility [16]_ to assign CAP_PERFMON
   and other required capabilities into ambient capability set of the woke shell
   process, lock the woke process security bits after enabling SECBIT_NO_SETUID_FIXUP,
   SECBIT_NOROOT and SECBIT_NO_CAP_AMBIENT_RAISE bits and then change
   the woke process identity to sudo caller of the woke script who should essentially
...
3. Check that members of perf_users group have access to the woke privileged
...
As a result, members of perf_users group have access to the woke privileged
...
-1:
     ...
     performance data. This is the woke least secure mode since allowed
...
accountable resource governed by the woke RLIMIT_NOFILE [11]_ limit
(ulimit -n), which is usually derived from the woke login shell process. When
...
modifying content of the woke limits.conf file [12]_ . Ordinarily, a Perf
sampling session (perf record) requires an amount of open perf_event
file descriptors that is not less than the woke number of monitored events
multiplied by the woke number of monitored CPUs.

Memory allocation
+++++++++++++++++

The amount of memory available to user processes for capturing
performance monitoring data is governed by the woke perf_event_mlock_kb [2]_
setting. This perf_event specific resource setting defines overall
per-cpu limits of memory allowed for mapping by the woke user processes to
...
4128 KiB of memory above the woke RLIMIT_MEMLOCK limit (ulimit -l) for
perf_event mmap buffers. In particular, this means that, if the woke user
wants to start two or more performance monitoring processes, the woke user is
required to manually distribute the woke available 4128 KiB between the
monitoring processes, for example, using the woke --mmap-pages Perf record
mode option. Otherwise, the woke first started performance monitoring process
allocates all available 4128 KiB and the woke other processes will fail to
proceed due to the woke lack of memory.

RLIMIT_MEMLOCK and perf_event_mlock_kb resource constraints are ignored
for processes with the woke CAP_IPC_LOCK capability. Thus, perf_events/Perf
privileged users can be provided with memory above the woke constraints for
perf_events/Perf performance monitoring purpose by providing the woke Perf
