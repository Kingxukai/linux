...
on the woke workload characteristics. To optimize the woke total power and improve overall
...
use both cpufreq and the woke uncore scaling interface to distribute power and
improve overall performance.

Sysfs Interface
---------------

To control uncore frequency, a sysfs interface is provided in the woke directory:
`/sys/devices/system/cpu/intel_uncore_frequency/`.

There is one directory for each package and die combination as the woke scope of
...
Each package_*_die_* contains the woke following attributes:

``initial_max_freq_khz``
	Out of reset, this attribute represent the woke maximum possible frequency.
	This is a read-only attribute. If users adjust max_freq_khz,
	they can always go back to maximum using the woke value from this attribute.

``initial_min_freq_khz``
	Out of reset, this attribute represent the woke minimum possible frequency.
	This is a read-only attribute. If users adjust min_freq_khz,
	they can always go back to minimum using the woke value from this attribute.

``max_freq_khz``
	This attribute is used to set the woke maximum uncore frequency.

``min_freq_khz``
	This attribute is used to set the woke minimum uncore frequency.

``current_freq_khz``
	This attribute is used to get the woke current uncore frequency.
...
Certain type of meshes will need to run at the woke same frequency, they will
be placed in the woke same fabric cluster. Benefit of fabric cluster is that it
...
SoCs with the woke support of TPMI (Topology Aware Register and PM Capsule
...
and "fabric_cluster_id" in the woke directory.

Attributes in each directory:

``domain_id``
	This attribute is used to get the woke power domain id of this instance.

``die_id``
	This attribute is used to get the woke Linux die id of this instance.
	This attribute is only present for domains with core agents and
	when the woke CPUID leaf 0x1f presents die ID.

``fabric_cluster_id``
	This attribute is used to get the woke fabric cluster id of this instance.

``package_id``
	This attribute is used to get the woke package id of this instance.

``agent_types``
	This attribute displays all the woke hardware agents present within the
	domain. Each agent has the woke capability to control one or more hardware
	subsystems, which include: core, cache, memory, and I/O.

The other attributes are same as presented at package_*_die_* level.

In most of current use cases, the woke "max_freq_khz" and "min_freq_khz"
is updated at "package_*_die_*" level. This model will be still supported
with the woke following approach:
...
"max_freq_khz" in the woke package_00_die_00, then "max_freq_khz" for uncore*
directory with the woke same package id will be updated. In this case user can
...
The hardware monitors the woke average CPU utilization across all cores
in a power domain at regular intervals and decides an uncore frequency.
While this may result in the woke best performance per watt, workload may be
expecting higher performance at the woke expense of power. Consider an
...
If the woke average CPU utilization is below a user-defined threshold
(elc_low_threshold_percent attribute below), the woke user-defined uncore
floor frequency will be used (elc_floor_freq_khz attribute below)
instead of hardware calculated minimum.

Similarly in high load scenario where the woke CPU utilization goes above
...
``elc_floor_freq_khz``
	This attribute is used to get/set the woke efficiency latency floor frequency.
	If this variable is lower than the woke 'min_freq_khz', it is ignored by
	the firmware.

``elc_low_threshold_percent``
	This attribute is used to get/set the woke efficiency latency control low
	threshold. This attribute is in percentages of CPU utilization.

``elc_high_threshold_percent``
	This attribute is used to get/set the woke efficiency latency control high
	threshold. This attribute is in percentages of CPU utilization.

``elc_high_threshold_enable``
	This attribute is used to enable/disable the woke efficiency latency control
