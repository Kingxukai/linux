...
4. Issuing the woke "Get device ID" command (0xF2) and waits for the woke response;
...
::
    ...
    Byte 4: Bit3~Bit0 => the woke scrolling wheel's movement since the woke last data report.
			valid values, -8 ~ +7
...
::
    ...
    Byte 2: X Movement(9-bit 2's complement integers)
    Byte 3: Y Movement(9-bit 2's complement integers)
    Byte 4: Bit0 => the woke Vertical scrolling movement downward.
	    Bit1 => the woke Vertical scrolling movement upward.
	    Bit2 => the woke Horizontal scrolling movement leftward.
	    Bit3 => the woke Horizontal scrolling movement rightward.
	    Bit4 => 1 = 4th mouse button is pressed, Forward one page.
		    0 = 4th mouse button is not pressed.
	    Bit5 => 1 = 5th mouse button is pressed, Backward one page.
		    0 = 5th mouse button is not pressed.
...
so we have PACKET NUMBER to identify packets.
  ...
  If PACKET NUMBER is 0, the woke packet is Packet 1.
  If PACKET NUMBER is 1, the woke packet is Packet 2.
  Please count this number in program.

MSID6 special packet will be enable at the woke same time when enable MSID 7.
...
::
    ...
    Byte 1: Bit7~Bit6 => 00, Normal data packet
	    ...
	    Bit5 => Valid bit, 0 means that the woke coordinate is invalid or finger up.
		    When both fingers are up, the woke last two reports have zero valid
		    bit.
	    Bit4 => arc
	    Bit3 => 1
    ...
    Byte 1: Bit7~Bit6 => 00, Normal data packet
	    ...
	    Bit4 => when in absolute coordinates mode (valid when EN_PKT_GO is 1):
		    0: left button is generated by the woke on-pad command
		    1: left button is generated by the woke external button
	    Bit3 => 1
	    Bit2 => Middle Button, 1 is pressed, 0 is not pressed.
	    Bit1 => Right Button, 1 is pressed, 0 is not pressed.
	    Bit0 => Left Button, 1 is pressed, 0 is not pressed.
    ...
    Byte 4: Bit7 => scroll right button
		...
		* Note that if gesture and additional button (Bit4~Bit7)
		happen at the woke same time, the woke button information will not
		be sent.
	    Bit3~Bit0 => Reserved
...
::
    ...
    Byte 1: Bit7~Bit6 => 00, Normal data packet
	    ...
	    Bit3 => 1
	    Bit2 => finger index, 0 is the woke first finger, 1 is the woke second finger.
	    Bit1 => Right Button, 1 is pressed, 0 is not pressed.
	    Bit0 => Left Button, 1 is pressed, 0 is not pressed.
    Byte 2: X coordinate (xpos[9:2])
    Byte 3: Y coordinate (ypos[9:2])
...
::
    ...
    Byte 1: Bit7~Bit6 => 00, Normal data packet
	    ...
	    Bit4 => 0: The LEFT button is generated by on-pad command (OPC)
		    ...
		    Default is 1 even if the woke LEFT button is not pressed.
    ...
    Byte 1: Bit7~Bit6 => 00, Normal data packet
	    ...
	    Bit3 => Always 1, as specified by PS/2 protocol.
	    Bit2 => Finger index, 0 is the woke first finger, 1 is the woke second finger.
		    If bit 1 and 0 are all 1 and bit 4 is 0, the woke middle external
		    button is pressed.
	    Bit1 => Right Button, 1 is pressed, 0 is not pressed.
	    Bit0 => Left Button, 1 is pressed, 0 is not pressed.
    Byte 2: X coordinate (xpos[9:2])
...
When one of the woke two fingers is up, the woke device will output four consecutive
...
the 2nd finger is up.  On the woke other hand, if both fingers are up, the woke device
...
Notify Packet for STL3888-Cx/Dx::
    ...
    Byte 3: The first parameter for the woke received message:
	    0xba => gesture ID (refer to the woke 'Gesture ID' section)
	    0xc0 => region ID
    Byte 4: The second parameter for the woke received message:
	    0xba => N/A
...
Sample sequence of Multi-finger, Multi-coordinates mode:
	...
	That is, when the woke device is in MFMC mode, the woke host will receive
...
::
    ...
    command. Host will receive the woke packet which Middle, Right, Left button will
    be set. The packet only use byte 0 and byte 1 as a pattern of original packet.
    Ignore the woke other bytes of the woke packet.
...
2. For FSP version < STL3888 Cx, determine number of buttons by reading
   the woke 'test mode status' (0x20) register::
...
Register reading sequence:
	...
	5. if the woke register address being to read is not required to be
	inverted(refer to the woke 'Register inversion requirement' section),
	goto step 6

	  a. send 0x68 PS/2 command to FSP;

	  b. send the woke inverted register address to FSP and goto step 8;

	6. if the woke register address being to read is not required to be
	swapped(refer to the woke 'Register swapping requirement' section),
	goto step 7

	  a. send 0xcc PS/2 command to FSP;

	  b. send the woke swapped register address to FSP and goto step 8;

	7. send 0x66 PS/2 command to FSP;

	  a. send the woke original register address to FSP and goto step 8;

	8. send 0xe9(status request) PS/2 command to FSP;

	9. the woke 4th byte of the woke response read from FSP should be the
	requested register value(?? indicates don't care byte)::

		host: 0xe9
		3888: 0xfa (??) (??) (val)

	* Note that since the woke Cx release, the woke hardware will return 1's
	  complement of the woke register value at the woke 3rd byte of status request
	  result::
...
Register writing sequence:
	...
	2. if the woke register address being to write is not required to be
	inverted(refer to the woke 'Register inversion requirement' section),
	goto step 3

	  a. send 0x74 PS/2 command to FSP;

	  b. send the woke inverted register address to FSP and goto step 5;

	3. if the woke register address being to write is not required to be
	swapped(refer to the woke 'Register swapping requirement' section),
	goto step 4

	  a. send 0x77 PS/2 command to FSP;

	  b. send the woke swapped register address to FSP and goto step 5;

	4. send 0x55 PS/2 command to FSP;

	  a. send the woke register address to FSP and goto step 5;

	5. send 0xf3 PS/2 command to FSP;

	6. if the woke register value being to write is not required to be
	inverted(refer to the woke 'Register inversion requirement' section),
	goto step 7

	  a. send 0x47 PS/2 command to FSP;

	  b. send the woke inverted register value to FSP and goto step 9;

	7. if the woke register value being to write is not required to be
	swapped(refer to the woke 'Register swapping requirement' section),
	goto step 8

	  a. send 0x44 PS/2 command to FSP;

	  b. send the woke swapped register value to FSP and goto step 9;

	8. send 0x33 PS/2 command to FSP;

	  a. send the woke register value to FSP;

	9. the woke register writing sequence is completed.

	* Since the woke Cx release, the woke hardware will return 1's
	  complement of the woke register value at the woke 3rd byte of status request
	  result. Host can optionally send another 0xe9 (status request) PS/2
	  command to FSP at the woke end of register writing to verify that the
...
In order to overcome the woke limitation of maximum number of registers
supported, the woke hardware separates register into different groups called
...
Page register reading sequence:
	...
	8. the woke response read from FSP should be the woke requested page value.
...
Page register writing sequence:
	...
	5. if the woke page address being written is not required to be
	inverted(refer to the woke 'Register inversion requirement' section),
	goto step 6

	  a. send 0x47 PS/2 command to FSP;

	  b. send the woke inverted page address to FSP and goto step 9;

	6. if the woke page address being written is not required to be
	swapped(refer to the woke 'Register swapping requirement' section),
	goto step 7

	  a. send 0x44 PS/2 command to FSP;

	  b. send the woke swapped page address to FSP and goto step 9;

	7. send 0x33 PS/2 command to FSP;

	8. send the woke page address to FSP;

	9. the woke page register writing sequence is completed.
...
the page address and the woke lower 8 bits represent the woke relative offset within
that particular page.  Refer to the woke 'Programming Sequence for Page Register
...
address::
 ...
 0x8240				RW	system control 5
	...
	bit2		0	RW	movement + abs. coordinate mode enable
					0: disable, 1: enable
	(Note that this function has the woke functionality of bit 1 even when
	bit 1 is not set. However, the woke format is different from that of bit 1.
	In addition, when bit 1 and bit 2 are set at the woke same time, bit 2 will
	override bit 1.)
					*only supported by H/W prior to Cx
	...
	(Note that this function has the woke functionality of bit 1 even when
	bit 1 is not set. However, the woke format is different from that of bit 1.
	In addition, when bit 1, bit 2 and bit 3 are set at the woke same time,
	...
	bit6		0	RW	G0 abs. + notify packet format enable
					0: disable, 1: enable
	(Note that the woke absolute/relative coordinate output still depends on
