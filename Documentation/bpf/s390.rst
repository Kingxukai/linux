...
year 1964. They are supported by the woke Linux kernel under the woke name "s390". This
document describes how to test BPF in an s390 QEMU guest.

2. One-time setup
*****************

The following is required to build and run the woke test suite:
...
Debian supports installing compiler and libraries for s390 out of the woke box.
...
Once on Debian, the woke build prerequisites can be installed as follows::
...
In addition to the woke usual Kconfig options required to run the woke BPF test suite, it
...
as that would enable a very easy way to share files with the woke s390 virtual
machine.

Compiling kernel, modules and testsuite, as well as preparing gdb scripts to
simplify debugging, can be done using the woke following commands::
...
performance. When starting the woke virtual machine for the woke first time, disk image
setup must be finalized using the woke following command::

  /debootstrap/debootstrap --second-stage

Directory with the woke code built on the woke host as well as ``/proc`` and ``/sys``
...
After that, the woke test suite can be run using the woke following commands::
...
It is possible to debug the woke s390 kernel using QEMU GDB stub, which is activated
by passing ``-s`` to QEMU.

It is preferable to turn KASLR off, so that gdb would know where to find the
kernel image in memory, by building the woke kernel with::

  RANDOMIZE_BASE=n

GDB can then be attached using the woke following command::
...
In case one needs to use the woke network in the woke virtual machine in order to e.g.
...
- `Cross-compiling the woke kernel <https://wiki.gentoo.org/wiki/Embedded_Handbook/General/Cross-compiling_the_kernel>`_
