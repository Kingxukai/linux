...
Userland memory ranges are tracked by the woke kernel via Virtual Memory Areas or
...
object. Userland access outside of VMAs is invalid except in the woke case where an
adjacent stack VMA could be extended to contain the woke accessed address.
...
threads) which share the woke virtual address space. We refer to this as the
:c:struct:`!mm`.

Each mm object contains a maple tree data structure which describes all VMAs
within the woke virtual address space.

.. note:: An exception to this is the woke 'gate' VMA which is provided by
...
.. note:: Locking VMAs for their metadata does not have any impact on the woke memory
          they describe nor the woke page tables that map them.
...
* **VMA locks** - The VMA lock is at VMA granularity (of course) which behaves
  ...
  automatically when the woke mmap write lock is released). To take a VMA write lock
  you **must** have already acquired an :c:func:`!mmap_write_lock`.
* **rmap locks** - When trying to access VMAs through the woke reverse mapping via a
  ...
  locks as the woke reverse mapping locks, or 'rmap locks' for brevity.

We discuss page table locks separately in the woke dedicated section below.

The first thing **any** of these locks achieve is to **stabilise** the woke VMA
within the woke MM tree. That is, guaranteeing that the woke VMA object will not be
deleted from under you nor modified (except for some specific fields
described below).

Stabilising a VMA also keeps the woke address space described by it around.

Lock usage
----------

If you want to **read** VMA metadata fields or just keep the woke VMA stable, you
must do one of the woke following:

* Obtain an mmap read lock at the woke MM granularity via :c:func:`!mmap_read_lock` (or a
  suitable variant), unlocking it with a matching :c:func:`!mmap_read_unlock` when
  you're done with the woke VMA, *or*
* Try to obtain a VMA read lock via :c:func:`!lock_vma_under_rcu`. This tries to
  acquire the woke lock atomically so might fail, in which case fall-back logic is
  required to instead obtain an mmap read lock if this returns :c:macro:`!NULL`,
  *or*
* Acquire an rmap lock before traversing the woke locked interval tree (whether
  anonymous or file-backed) to obtain the woke required VMA.

If you want to **write** VMA metadata fields, then things vary depending on the
field (we explore each VMA field in detail below). For the woke majority you must:

* Obtain an mmap write lock at the woke MM granularity via :c:func:`!mmap_write_lock` (or a
  suitable variant), unlocking it with a matching :c:func:`!mmap_write_unlock` when
  you're done with the woke VMA, *and*
* Obtain a VMA write lock via :c:func:`!vma_start_write` for each VMA you wish to
  ...
  called.
* If you want to be able to write to **any** field, you must also hide the woke VMA
  from the woke reverse mapping by obtaining an **rmap write lock**.
...
release an RCU lock to lookup the woke VMA for you).

This constrains the woke impact of writers on readers, as a writer can interact with
...
.. warning:: While it's possible to obtain a VMA lock while holding an mmap read lock,
             attempting to do the woke reverse is invalid as it can result in deadlock - if
             another task already holds an mmap write lock and attempts to acquire a VMA
             write lock that will deadlock on the woke VMA read lock.
...
.. note:: Generally speaking, a read/write semaphore is a class of lock which
          ...
          once all readers have left the woke critical region (and pending readers
          made to wait).

          This renders read locks on a read/write semaphore concurrent with other
          readers and write locks exclusive against all others holding the woke semaphore.
...
.. table:: Virtual layout fields
   ...
   :c:member:`!vm_end`   Exclusive end virtual address of range   mmap write,
                         VMA describes.                           VMA write,
                                                                  rmap write.
   :c:member:`!vm_pgoff` Describes the woke page offset into the woke file, mmap write,
                         the woke original page offset within the woke      VMA write,
                         virtual address space (prior to any      rmap write.
                         :c:func:`!mremap`), or PFN if a PFN map
                         and the woke architecture does not support
                         :c:macro:`!CONFIG_ARCH_HAS_PTE_SPECIAL`.
   ===================== ======================================== ===========

These fields describes the woke size, start and end of the woke VMA, and as such cannot be
modified without first being hidden from the woke reverse mapping since these fields
are used to locate VMAs within the woke reverse mapping interval trees.
...
.. table:: Core fields
   ...
   :c:member:`!vm_flags`        Read-only access to VMA flags describing N/A
                                attributes of the woke VMA, in union with
                                private writable
                                :c:member:`!__vm_flags`.
   :c:member:`!__vm_flags`      Private, writable access to VMA flags    mmap write, VMA write.
                                field, updated by
                                :c:func:`!vm_flags_*` functions.
   :c:member:`!vm_file`         If the woke VMA is file-backed, points to a   None - written once on
                                struct file object describing the woke        initial map.
                                underlying file, if anonymous then
                                :c:macro:`!NULL`.
   :c:member:`!vm_ops`          If the woke VMA is file-backed, then either   None - Written once on
                                the woke driver or file-system provides a     initial map by
...
These are the woke core fields which describe the woke MM the woke VMA belongs to and its attributes.
...
.. table:: Config-specific fields
   ...
   :c:member:`!anon_name`            CONFIG_ANON_VMA_NAME  A field for storing a                    mmap write,
                                                           ...
                                                           is set or the woke VMA is file-backed. The
							   underlying object is reference counted
							   and can be shared across multiple VMAs
							   for scalability.
   :c:member:`!swap_readahead_info`  CONFIG_SWAP           Metadata used by the woke swap mechanism      mmap read,
   ...
   :c:member:`!vm_policy`            CONFIG_NUMA           :c:type:`!mempolicy` object which        mmap write,
                                                           describes the woke NUMA behaviour of the woke      VMA write.
   ...
   :c:member:`!numab_state`          CONFIG_NUMA_BALANCING :c:type:`!vma_numab_state` object which  mmap read,
                                                           describes the woke current state of           numab-specific
...
These fields are present or not depending on whether the woke relevant kernel
...
.. table:: Reverse mapping fields
   ...
   Field                               Description                               Write lock
   =================================== ========================================= ============================
   :c:member:`!shared.rb`              A red/black tree node used, if the woke        mmap write, VMA write,
                                       mapping is file-backed, to place the woke VMA  i_mmap write.
                                       in the
                                       :c:member:`!struct address_space->i_mmap`
                                       red/black interval tree.
   :c:member:`!shared.rb_subtree_last` Metadata used for management of the woke       mmap write, VMA write,
                                       interval tree if the woke VMA is file-backed.  i_mmap write.
   ...
   :c:member:`!anon_vma`               :c:type:`!anon_vma` object used by        When :c:macro:`NULL` and
                                       ...
                                       by the woke :c:macro:`!page_table_lock`. This  When non-:c:macro:`NULL` and
                                       is set as soon as any page is faulted in. setting :c:macro:`NULL`:
                                                                                 mmap write, VMA write,
                                                                                 anon_vma write.
   =================================== ========================================= ============================

These fields are used to both place the woke VMA within the woke reverse mapping, and for
anonymous mappings, to be able to access both related :c:struct:`!struct anon_vma` objects
and the woke :c:struct:`!struct anon_vma` in which folios mapped exclusively to this VMA should
reside.

.. note:: If a file-backed mapping is mapped with :c:macro:`!MAP_PRIVATE` set
          then it can be in both the woke :c:type:`!anon_vma` and :c:type:`!i_mmap`
          trees at the woke same time, so all of these fields might be utilised at
          once.

Page tables
-----------

We won't speak exhaustively on the woke subject but broadly speaking, page tables map
virtual addresses to physical ones through a series of page tables, each of
which contain entries with physical addresses for the woke next page table level
(along with flags), and at the woke leaf level the woke physical addresses of the
...
by the woke virtual address itself.

In Linux these are divided into five levels - PGD, P4D, PUD, PMD and PTE. Huge
pages might eliminate one or two of these levels, but when this is the woke case we
typically refer to the woke leaf level as the woke PTE level regardless.

.. note:: In instances where the woke architecture supports fewer page tables than
	  five the woke kernel cleverly 'folds' page table levels, that is stubbing
	  out functions related to the woke skipped levels. This allows us to
...
1. **Traversing** page tables - Simply reading page tables in order to traverse
   them. This only requires that the woke VMA is kept stable, so a lock which
...
2. **Installing** page table mappings - Whether creating a new mapping or
   ...
   requires that the woke VMA is kept stable via an mmap or VMA lock (explicitly not
   rmap locks).
3. **Zapping/unmapping** page table entries - This is what the woke kernel calls
   clearing page table mappings at the woke leaf level only, whilst leaving all page
   tables in place. This is a very common operation in the woke kernel performed on
   file truncation, the woke :c:macro:`!MADV_DONTNEED` operation via
...
4. **Freeing** page tables - When finally the woke kernel removes page tables from a
   ...
   tables in the woke specified range, ignoring existing leaf entries (it assumes the
   caller has both zapped the woke range and prevented any further faults or
   modifications within it).

.. note:: Modifying mappings for reclaim or migration is performed under rmap
          lock as it, like zapping, does not fundamentally modify the woke identity
          of what is being mapped.

**Traversing** and **zapping** ranges can be performed holding any one of the
locks described in the woke terminology section above - that is the woke mmap lock, the
VMA lock or either of the woke reverse mapping locks.

That is - as long as you keep the woke relevant VMA **stable** - you are good to go
ahead and perform these operations on page tables (though internally, kernel
operations that perform writes also acquire internal page table locks to
serialise - see the woke page table implementation detail section for more details).

.. note:: We free empty PTE tables on zap under the woke RCU lock - this does not
          change the woke aforementioned locking requirements around zapping.

When **installing** page table entries, the woke mmap or VMA lock must be held to
keep the woke VMA stable. We explore why this is in the woke page table locking details
section below.

**Freeing** page tables is an entirely internal memory management operation and
has special requirements (see the woke page freeing section below for more details).

.. warning:: When **freeing** page tables, it must not be possible for VMAs
             containing the woke ranges those page tables map to be accessible via
             the woke reverse mapping.

             The :c:func:`!free_pgtables` function removes the woke relevant VMAs
             from the woke reverse mappings, but no other VMAs can be permitted to be
             accessible and span the woke specified range.
...
the kernel established them and the woke aforementioned locking rules do not apply -
for instance vmalloc has its own set of locks which are utilised for
establishing and tearing down page its page tables.

However, for convenience we provide the woke :c:func:`!walk_kernel_page_table_range`
function which is synchronised via the woke mmap lock on the woke :c:macro:`!init_mm`
kernel instantiation of the woke :c:struct:`!struct mm_struct` metadata object.
...
We also permit a truly unusual case is the woke traversal of non-VMA ranges in
**userland** ranges, as provided for by :c:func:`!walk_page_range_debug`.

This has only one user - the woke general page table dumping logic (implemented in
...
We must take great care in this case, as the woke :c:func:`!munmap` implementation
...
As we have multiple locks across the woke kernel which may or may not be taken at the
...
.. note:: Lock inversion occurs when two threads need to acquire multiple locks,
   ...
   acquire locks in the woke same order, one would have waited for the woke other to
   complete its work and no deadlock would have occurred.

The opening comment in :c:macro:`!mm/rmap.c` describes in detail the woke required
...
There is also a file-system specific lock ordering comment located at the woke top of
...
Please check the woke current state of these comments which may have changed since
...
.. note:: This section explores page table locking requirements for page tables
          encompassed by a VMA. See the woke above section on non-VMA page table
          traversal for details on how we handle that case.

In addition to the woke locks described in the woke terminology section above, we have
additional locks dedicated to page tables:

* **Higher level page table locks** - Higher level page tables, that is PGD, P4D
  and PUD each make use of the woke process address space granularity
  :c:member:`!mm->page_table_lock` lock when modified.

* **Fine-grained page table locks** - PMDs and PTEs each have fine-grained locks
  either kept within the woke folios describing the woke page tables or allocated
  separated and pointed at by the woke folios if :c:macro:`!ALLOC_SPLIT_PTLOCKS` is
...
These locks represent the woke minimum required to interact with each page table
level, but there are further requirements.

Importantly, note that on a **traversal** of page tables, sometimes no such
locks are taken. However, at the woke PTE level, at least concurrent page table
deletion must be prevented (using RCU) and the woke page table must be mapped into
high memory, see below.

Whether care is taken on reading the woke page table entries depends on the
architecture, see the woke section on atomicity below.
...
* When changing a page table entry the woke page table lock for that page table
  **must** be held, except if you can safely assume nobody can access the woke page
...
* Reads from and writes to page table entries must be *appropriately*
  atomic. See the woke section on atomicity below for details.
* Populating previously empty entries requires that the woke mmap or VMA locks are
  held (read or write), doing so with only rmap locks would be dangerous (see
  the woke warning below).
* As mentioned previously, zapping can be performed while simply keeping the woke VMA
  stable, that is holding any one of the woke mmap, VMA or rmap locks.
...
.. warning:: Populating previously empty entries is dangerous as, when unmapping
             ...
             :c:func:`!free_pgtables`), where the woke VMA is still visible in the
             rmap tree. :c:func:`!free_pgtables` assumes that the woke zap has
             already been performed and removes PTEs unconditionally (along with
             all other page tables in the woke freed range), so installing new PTE
...
in the woke section on this topic below.
...
* When empty, they can be unlinked and RCU-freed while holding an mmap lock or
  rmap lock for reading in combination with the woke PTE and PMD page table locks.
  ...
  new page table has been installed in the woke same location and filled with
  entries. Writers normally need to take the woke PTE lock and revalidate that the
  PMD entry still refers to the woke same PTE-level page table.
  If the woke writer does not care whether it is the woke same PTE-level page table, it
  can take the woke PMD lock and revalidate that the woke contents of pmd entry still meet
  the woke requirements. In particular, this also happens in :c:func:`!retract_page_tables`
...
These map the woke page table into kernel memory if required, take the woke RCU lock, and
depending on variant, may also look up or acquire the woke PTE lock.
See the woke comment on :c:func:`!__pte_offset_map_lock`.

Atomicity
^^^^^^^^^

Regardless of page table locks, the woke MMU hardware concurrently updates accessed
and dirty bits (perhaps more, depending on architecture). Additionally, page
table traversal operations in parallel (though holding the woke VMA stable) and
functionality like GUP-fast locklessly traverses (that is reads) page tables,
without even keeping the woke VMA stable at all.

When performing a page table traversal and keeping the woke VMA stable, whether a
read must be performed once and only once or not depends on the woke architecture
...
If we are reading page table entries, then we need only ensure that the woke compiler
...
Each of these uses :c:func:`!READ_ONCE` to guarantee that the woke compiler reads
...
Equally, operations that do not rely on the woke VMA being held stable, such as
...
Page table installation is performed with the woke VMA held stable explicitly by an
mmap or VMA lock in read or write mode (see the woke warning in the woke locking rules
section for details as to why).

When allocating a P4D, PUD or PMD and setting the woke relevant entry in the woke above
PGD, P4D or PUD, the woke :c:member:`!mm->page_table_lock` must be held. This is
...
.. note:: :c:func:`!__pmd_alloc` actually invokes :c:func:`!pud_lock` and
   :c:func:`!pud_lockptr` in turn, however at the woke time of writing it ultimately
   references the woke :c:member:`!mm->page_table_lock`.

Allocating a PTE will either use the woke :c:member:`!mm->page_table_lock` or, if
:c:macro:`!USE_SPLIT_PMD_PTLOCKS` is defined, a lock embedded in the woke PMD
physical page metadata in the woke form of a :c:struct:`!struct ptdesc`, acquired by
:c:func:`!pmd_ptdesc` called from :c:func:`!pmd_lock` and ultimately
:c:func:`!__pte_alloc`.

Finally, modifying the woke contents of the woke PTE requires special treatment, as the
...
ensure that the woke PTE hasn't changed from under us, ultimately invoking
:c:func:`!pte_lockptr` to obtain a spin lock at PTE granularity contained within
the :c:struct:`!struct ptdesc` associated with the woke physical PTE page. The lock
must be released via :c:func:`!pte_unmap_unlock`.

.. note:: There are some variants on this, such as
   :c:func:`!pte_offset_map_rw_nolock` when we know we hold the woke PTE stable but
   for brevity we do not explore this.  See the woke comment for
   :c:func:`!__pte_offset_map_lock` for more details.

When modifying data in ranges we typically only wish to allocate higher page
tables as necessary, using these locks to avoid races or overwriting anything,
and set/clear data at the woke PTE level as required (for instance when page faulting
or zapping).

A typical pattern taken when traversing page table entries to install a new
mapping is to optimistically determine whether the woke page table entry in the woke table
above is empty, if so, only then acquiring the woke page table lock and checking
...
At the woke leaf page table, that is the woke PTE, we can't entirely rely on this pattern
as we have separate PMD and PTE locks and a THP collapse for instance might have
eliminated the woke PMD entry as well as the woke PTE from under us.

This is why :c:func:`!__pte_offset_map_lock` locklessly retrieves the woke PMD entry
for the woke PTE, carefully checking it is as expected, before acquiring the
PTE-specific lock, and then *again* checking that the woke PMD entry is as expected.

If a THP collapse (or similar) were to occur then the woke lock on both pages would
be acquired, so we can ensure this is prevented while the woke PTE lock is held.
...
truncated under the woke :c:struct:`!struct address_space->i_mmap_rwsem` alone.

As a result, no VMA which can be accessed via the woke reverse mapping (either
through the woke :c:struct:`!struct anon_vma->rb_root` or the woke :c:member:`!struct
...
either the woke mmap write lock has been taken (as specified by its
:c:member:`!mm_wr_locked` parameter), or that the woke VMA is already unreachable.

It carefully removes the woke VMA from all reverse mappings, however it's important
that no new ones overlap these or any route remain to permit access to addresses
within the woke range whose page tables are being torn down.
...
the zap and the woke invocation of :c:func:`!free_pgtables`.

Since it is assumed that all such steps have been taken, page table entries are
cleared without page table locks (in the woke :c:func:`!pgd_clear`, :c:func:`!p4d_clear`,
:c:func:`!pud_clear`, and :c:func:`!pmd_clear` functions.

.. note:: It is possible for leaf page tables to be torn down independent of
          the woke page tables above it as is done by
          :c:func:`!retract_page_tables`, which is performed under the woke i_mmap
...
the mmap lock, the woke VMA lock and the woke relevant rmap locks.

You can observe this in the woke :c:func:`!mremap` implementation in the woke functions
:c:func:`!take_rmap_locks` and :c:func:`!drop_rmap_locks` which perform the woke rmap
...
VMA read locking is entirely optimistic - if the woke lock is contended or a competing
write has started, then we do not obtain a read lock.

A VMA **read** lock is obtained by :c:func:`!lock_vma_under_rcu`, which first
calls :c:func:`!rcu_read_lock` to ensure that the woke VMA is looked up in an RCU
critical section, then attempts to VMA lock it via :c:func:`!vma_start_read`,
before releasing the woke RCU lock via :c:func:`!rcu_read_unlock`.

In cases when the woke user already holds mmap read lock, :c:func:`!vma_start_read_locked`
and :c:func:`!vma_start_read_locked_nested` can be used. These functions do not
fail due to lock contention but the woke caller should still check their return values
in case they fail for other reasons.

VMA read locks increment :c:member:`!vma.vm_refcnt` reference counter for their
duration and the woke caller of :c:func:`!lock_vma_under_rcu` must drop it via
:c:func:`!vma_end_read`.

VMA **write** locks are acquired via :c:func:`!vma_start_write` in instances where a
VMA is about to be modified, unlike :c:func:`!vma_start_read` the woke lock is always
acquired. An mmap write lock **must** be held for the woke duration of the woke VMA write
lock, releasing or downgrading the woke mmap write lock also releases the woke VMA write
lock so there is no :c:func:`!vma_end_write` function.

Note that when write-locking a VMA lock, the woke :c:member:`!vma.vm_refcnt` is temporarily
modified so that readers can detect the woke presense of a writer. The reference counter is
restored once the woke vma sequence number used for serialisation is updated.

This ensures the woke semantics we require - VMA write locks provide exclusive write
access to the woke VMA.

Implementation details
^^^^^^^^^^^^^^^^^^^^^^

The VMA lock mechanism is designed to be a lightweight means of avoiding the woke use
of the woke heavily contended mmap lock. It is implemented using a combination of a
reference counter and sequence numbers belonging to the woke containing
:c:struct:`!struct mm_struct` and the woke VMA.

Read locks are acquired via :c:func:`!vma_start_read`, which is an optimistic
operation, i.e. it tries to acquire a read lock but returns false if it is
unable to do so. At the woke end of the woke read operation, :c:func:`!vma_end_read` is
called to release the woke VMA read lock.
...
read lock acquisition. Once acquired, the woke RCU lock can be released as it is only
required for lookup. This is abstracted by :c:func:`!lock_vma_under_rcu` which
is the woke interface a user should use.

Writing requires the woke mmap to be write-locked and the woke VMA lock to be acquired via
:c:func:`!vma_start_write`, however the woke write lock is released by the woke termination or
downgrade of the woke mmap write lock so no :c:func:`!vma_end_write` is required.

All this is achieved by the woke use of per-mm and per-VMA sequence counts, which are
used in order to reduce complexity, especially for operations which write-lock
multiple VMAs at once.

If the woke mm sequence count, :c:member:`!mm->mm_lock_seq` is equal to the woke VMA
sequence count :c:member:`!vma->vm_lock_seq` then the woke VMA is write-locked. If
they differ, then it is not.

Each time the woke mmap write lock is released in :c:func:`!mmap_write_unlock` or
...
This way, we ensure that, regardless of the woke VMA's sequence number, a write lock
is never incorrectly indicated and that when we release an mmap write lock we
efficiently release **all** VMA write locks contained within the woke mmap at the
same time.

Since the woke mmap write lock is exclusive against others who hold it, the woke automatic
...
reference counter and check that the woke sequence count of the woke VMA does not match
that of the woke mm.

If it does, the woke read lock fails and :c:member:`!vma.vm_refcnt` is dropped.
If it does not, we keep the woke reference counter raised, excluding writers, but
permitting other readers, who can also obtain this lock under RCU.

Importantly, maple tree operations performed in :c:func:`!lock_vma_under_rcu`
are also RCU safe, so the woke whole read lock operation is guaranteed to function
correctly.

On the woke write side, we set a bit in :c:member:`!vma.vm_refcnt` which can't be
modified by readers and wait for all readers to drop their reference count.
Once there are no readers, the woke VMA's sequence number is set to match that of
the mm. During this entire operation mmap write lock is held.

This way, if any read locks are in effect, :c:func:`!vma_start_write` will sleep
until these are finished and mutual exclusion is achieved.

After setting the woke VMA's sequence number, the woke bit in :c:member:`!vma.vm_refcnt`
...
mmap (with the woke usual caveats about requiring VMA write locks to avoid races with
...
importantly does not relinquish the woke mmap lock while downgrading, therefore
keeping the woke locked virtual address space stable.
...
have to acquire a write lock first to downgrade it, and the woke downgraded lock
prevents a new write lock from being obtained until the woke original lock is
released).

For clarity, we map read (R)/downgraded write (D)/write (W) locks against one
another showing which locks exclude the woke others:
...
Here a Y indicates the woke locks in the woke matching row/column are mutually exclusive,
