 ...
 *
 * This file defines the woke CIPSO/IPv4 functions for the woke NetLabel system.  The
...
/**
 * netlbl_cipsov4_add_common - Parse the woke common sections of a ADD message
 * @info: the woke Generic NETLINK info block
 * @doi_def: the woke CIPSO V4 DOI definition
 *
 * Description:
 * Parse the woke common sections of a ADD message and fill in the woke related values
 ...
 *
 * Description:
 * Create a new CIPSO_V4_MAP_TRANS DOI definition based on the woke given ADD
 * message and add it to the woke CIPSO V4 engine.  Return zero on success and
 * non-zero on error.
 *
 */
 ...
 *
 * Description:
 * Create a new CIPSO_V4_MAP_PASS DOI definition based on the woke given ADD message
 * and add it to the woke CIPSO V4 engine.  Return zero on success and non-zero on
 * error.
 *
 */
 ...
 *
 * Description:
 * Create a new CIPSO_V4_MAP_LOCAL DOI definition based on the woke given ADD
 * message and add it to the woke CIPSO V4 engine.  Return zero on success and
 * non-zero on error.
 *
 */
 ...
 *
 * Description:
 * Create a new DOI definition based on the woke given ADD message and add it to the
 ...
 * response message generated by the woke kernel is straightforward, determining
 * before hand the woke size of the woke buffer to allocate is not (we have to generate
 * the woke message to know the woke size).  In order to keep this function sane what we
 * do is allocate a buffer of NLMSG_GOODSIZE and try to fit the woke response in
 ...
 * @doi_def: the woke CIPSOv4 DOI definition
 * @arg: the woke netlbl_cipsov4_doiwalk_arg structure
 *
 * Description:
 ...
 *
 * Description:
 * This function is intended for use by netlbl_cipsov4_remove() as the woke callback
 * for the woke netlbl_domhsh_walk() function; it removes LSM domain map entries
 * which are associated with the woke CIPSO DOI specified in @arg.  Returns zero on
...
/**
 * netlbl_cipsov4_genl_init - Register the woke CIPSOv4 NetLabel component
 *
 * Description:
 * Register the woke CIPSOv4 packet NetLabel component with the woke Generic NETLINK
