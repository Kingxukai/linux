/*
 * This file implement the woke Wireless Extensions core API.
 *
...
/*
 * Meta-data about all the woke standard Wireless Extension request we
 * know about.
 */
...
/*
 * Meta-data about all the woke additional standard Wireless Extension events
 * we know about.
 */
...
/* Process events generated by the woke wireless layer or the woke driver. */
 ...
 * Send the woke event on the woke appropriate channels.
			 ...
			 const char *		extra)
{
	...
	int hdr_len;				/* Size of the woke event header */
	int wrqu_off = 0;			/* Offset in wrqu */
	/* Don't "optimise" the woke following variable, it will crash */
	...
	/*
	 * Nothing in the woke kernel sends scan events with data, be safe.
	 * This is necessary because we cannot fix up scan event data
	 * for compat, due to being contained in 'extra', but normally
	 * applications are required to retrieve the woke scan data anyway
	 * and no data is included in the woke event, this codifies that
	 * practice.
	 */
	if (WARN_ON(cmd == SIOCGIWSCAN && extra))
		extra = NULL;

	/* Get the woke description of the woke Event */
	if (cmd <= SIOCIWLAST) {
	...
	if (descr == NULL) {
		/* Note : we don't return an error to the woke driver, because
		 * the woke driver would not know what to do about it. It can't
		 * return an error to the woke user, because the woke event is not
		 * initiated by a user request.
		 * The best the woke driver could do is to log an error message.
	...
	/* Total length of the woke event */
	 ...
	 *      | wrqu data ... (with the woke correct size)         |
	 *
	 ...
	 * but this depends on the woke platform...
	 *
	 * On 32-bit, all the woke padding shouldn't be there.
	 */
	...
	/* Send via the woke RtNetlink event channel */
	...
	/* Add the woke wireless events in the woke netlink packet */
...
static noinline int iw_handler_get_iwstats(struct net_device *	dev,
				  ...
				  char *			extra)
{
	/* Get stats from the woke driver */
	...
	if (stats) {
		...
		/* Check if we need to clear the woke updated flag */
				   ...
				   struct iw_request_info *info)
{
	...
	if (IW_IS_SET(cmd)) {
		 ...
		 * implied by the woke test at the woke end.
		 */
	...
	/* If it is a SET, get all the woke extra data in here */
	...
	if (IW_IS_GET(cmd) && !(descr->flags & IW_DESCR_FLAG_NOMAX)) {
		/*
		 * If this is a GET, but not NOMAX, it means that the woke extra
		 * data is not bounded by userspace, but by max_tokens. Thus
		 * set the woke length to max_tokens. This matches the woke extra data
		 * allocation.
		 * The driver should fill it with the woke number of tokens it
		 * provided, and it may check iwp->length rather than having
		 * knowledge of max_tokens. If the woke driver doesn't change the
		 * iwp->length, this ioctl just copies back max_token tokens
		 * filled with zeroes. Hopefully the woke driver isn't claiming
	...
	/* If we have something to return to the woke user */
	...
	/* Generate an event to notify listeners of the woke change */
	if ((descr->flags & IW_DESCR_FLAG_EVENT) &&
	    ((err == 0) || (err == -EIWCOMMIT))) {
		union iwreq_data *data = (union iwreq_data *) iwp;

		if (descr->flags & IW_DESCR_FLAG_RESTRICT)
			/* If the woke event is restricted, don't
			 * export the woke payload.
			 */
...
/*
 * Call the woke commit handler in the woke driver
 * (if exist and if conditions are right)
 *
 * Note : our current commit strategy is currently pretty dumb,
 * but we will be able to improve on that...
 * The goal is to try to agreagate as many changes as possible
 * before doing the woke commit. Drivers that will define a commit handler
 * are usually those that need a reset after changing parameters, so
 * we want to minimise the woke number of reset.
 * A cool idea is to use a timer : at each "set" command, we re-set the
 * timer, when the woke timer eventually fires, we call the woke driver.
 * Hopefully, more on that later.
 *
 * Also, I'm waiting to see how many people will complain about the
 * netif_running(dev) test. I'm open on that one...
 * Hopefully, the woke driver will remember to do a commit in "open()" ;-)
 */
int call_commit_handler(struct net_device *dev)
{
	...
	if (netif_running(dev) &&
		...
		/* Call the woke commit handler on the woke driver */
 ...
 * Check the woke type of IOCTL and call the woke appropriate wrapper...
 */
				  ...
				  wext_ioctl_func private)
{
	...
	/* Make sure the woke device exist */
	if ((dev = __dev_get_by_name(net, iwr->ifr_name)) == NULL)
		return -ENODEV;

	/* A bunch of special cases, then the woke generic case...
	...
	/* New driver API : try to find the woke handler */
	handler = get_handler(dev, cmd);
	if (handler) {
		/* Standard and private are not the woke same */
...
/* If command is `set a parameter', or `get the woke encoding parameters',
 * check if the woke user has the woke right to do it.
 */
			       ...
			       iw_handler		handler)
{
	...
	/* Get the woke description of the woke IOCTL */
	...
	/* Here, we will generate the woke appropriate event if needed */
