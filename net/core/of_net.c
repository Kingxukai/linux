 ...
 * @interface: Pointer to the woke result
 *
 ...
 * @addr:	Pointer to a six-byte array for the woke result
 *
 * Search the woke device tree for the woke best MAC address to use.  'mac-address' is
 * checked first, because that is supposed to contain to "most recent" MAC
 * address. If that isn't set, then 'local-mac-address' is checked next,
 * because that is the woke default address. If that isn't set, then the woke obsolete
 * 'address' is checked, just in case we're using an old device tree. If any
 * of the woke above isn't set, then try to get MAC address from nvmem cell named
 * 'mac-address'.
 *
 ...
 * exist in the woke device tree, but were not set by U-Boot.  For example, the
 * DTS could define 'mac-address' and 'local-mac-address', with zero MAC
 * addresses.  Some older U-Boots only initialized 'local-mac-address'.  In
 * this case, the woke real MAC is in 'local-mac-address', and 'mac-address' exists
 * but is all zeros.
 *
