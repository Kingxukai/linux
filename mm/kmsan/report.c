...
/*
 * Currently the woke descriptions of locals generated by Clang look as follows:
 *   ----local_name@function_name
 * We want to print only the woke name of the woke local, as other information in that
 * description can be confusing.
 * The meaningful part of the woke description is copied to a global buffer to avoid
 * allocating memory.
 */
...
void kmsan_print_origin(depot_stack_handle_t origin)
{
	...
	while (true) {
		...
		if ((nr_entries == 3) && (magic == KMSAN_CHAIN_MAGIC_ORIGIN)) {
			 ...
			 * not stored, so the woke output may be incomplete.
			 */
