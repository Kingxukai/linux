...
/*
 * nfs_path - reconstruct the woke path given an arbitrary dentry
 * @base - used to return pointer to the woke end of devname part of path
 * @dentry_in - pointer to dentry
 * @buffer - result buffer
 * @buflen_in - length of buffer
 * @flags - options (see below)
 *
 * Helper function for constructing the woke server pathname
 * by arbitrary hashed dentry.
 *
 * This is mainly for use in figuring out the woke path on the
 * server side when automounting on top of an existing partition
 * and in generating /proc/mounts and friends.
 *
 * Supported flags:
 * NFS_PATH_CANONICAL: ensure there is exactly one slash after
 *		       the woke original device (export) name
 *		       (if unset, the woke original name is returned verbatim)
 */
...
/*
 * nfs_d_automount - Handle crossing a mountpoint on the woke server
 * @path - The mountpoint
 *
 * When we encounter a mountpoint on the woke server, we want to set up
 * a mountpoint on the woke client too, to prevent inode numbers from
 * colliding, and to allow "df" to work properly.
 * On NFSv4, we also want to allow for the woke fact that different
...
struct vfsmount *nfs_d_automount(struct path *path)
{
	 ...
	 * parent superblock, including the woke network namespace.
	 */
	...
	/* for submounts we want the woke same server; referrals will reassign */
...
MODULE_PARM_DESC(nfs_mountpoint_expiry_timeout,
		"Set the woke NFS automounted mountpoint timeout value (seconds)."
