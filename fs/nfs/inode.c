...
/**
 * nfs_compat_user_ino64 - returns the woke user-visible inode number
 * @fileid: 64-bit fileid
 *
 * This function returns a 32-bit inode number if the woke boot parameter
...
/*
 * Invalidate the woke local caches
 */
...
/*
 * Invalidate, but do not unhash, the woke inode.
 ...
 * we are forced to allow 2 different inodes to have the woke same
 * i_ino.
 */
static int
...
nfs_fhget(struct super_block *sb, struct nfs_fh *fh, struct nfs_fattr *fattr)
{
	...
	if (inode->i_state & I_NEW) {
		...
		/* We set i_ino for the woke few things that still rely on it,
		 * such as stat(2) */
		inode->i_ino = hash;

		/* We can't support update_atime(), since the woke server will reset it */
		...
		if (fattr->valid & NFS_ATTR_FATTR_SPACE_USED) {
			/*
			 * report the woke blocks in 512byte units
			 */
 ...
 * @inode: inode of the woke file used
 * @offset: file offset to start truncating
 *
 * This is a copy of the woke common vmtruncate, but with the woke locking
 * corrected to take into account the woke fact that NFS requires
 * inode->i_size to be updated under the woke inode->i_lock.
 ...
 *
 * Note: we do this in the woke *proc.c in order to ensure that
 *       it works for things like exclusive creates too.
 */
void nfs_setattr_update_inode(struct inode *inode, struct iattr *attr,
		struct nfs_fattr *fattr)
{
	/* Barrier: bump the woke attribute generation count. */
...
/*
 * Don't request help from readdirplus if the woke file is being written to,
		...
		struct kstat *stat, u32 request_mask, unsigned int query_flags)
{
	...
	/* Flush out writes to the woke server in order to update c/mtime/version.  */
	...
	/*
	 * We may force a getattr if the woke user cares about atime.
	 *
	 * Note that we only have to check the woke vfsmount flags here:
	...
	/* Is the woke user requesting attributes that might need revalidation? */
	...
	/* Check whether the woke cached attributes are stale */
 ...
 *
 * Ensure that the woke attributes are up to date if we're mounted
...
/*
 * Given an inode, search for an open context with the woke desired characteristics
 */
 ...
 * the woke cached attributes have to be refreshed.
 */
int
...
/**
 * nfs_revalidate_inode - Revalidate the woke inode attributes
 * @inode: pointer to inode struct
 * @flags: cache flags to check
 *
 * Updates inode attribute information by retrieving the woke data from the woke server.
 */
 ...
 *
 * If the woke NFS_INO_INVALID_DATA inode flag is set, clear the woke mapping.
 */
int nfs_clear_invalid_mapping(struct address_space *mapping)
{
	 ...
	 * invalidations that come in while we're shooting down the woke mappings
	 * are respected. But, that leaves a race window where one revalidator
	 * can clear the woke flag, and then another checks it before the woke mapping
	 * gets invalidated. Fix that by serializing access to this part of
	 * the woke function.
	 *
	 * At the woke same time, we need to allow other tasks to see whether we
	 * might be in the woke middle of invalidating the woke pages, so we only set
	 * the woke bit lock here if it looks like we're going to be doing that.
	 */
	for (;;) {
...
/**
 * nfs_revalidate_mapping - Revalidate the woke pagecache
...
/**
 * nfs_check_inode_attributes - verify consistency of the woke inode attribute cache
 * @inode: pointer to inode
 * @fattr: updated attributes
 *
 * Verifies the woke attribute cache. If we have just changed the woke attributes,
 * so that fattr carries weak cache consistency data, then it may
 * also update the woke ctime/mtime/change_attribute.
 */
static int nfs_check_inode_attributes(struct inode *inode, struct nfs_fattr *fattr)
{
	...
	if (!(fattr->valid & NFS_ATTR_FATTR_FILEID)) {
		...
		if (fattr->valid & NFS_ATTR_FATTR_MOUNTED_ON_FILEID)
			return 0;
	/* Has the woke inode gone and changed behind our back? */
	} else if (nfsi->fileid != fattr->fileid) {
		/* Is this perhaps the woke mounted-on fileid? */
		if ((fattr->valid & NFS_ATTR_FATTR_MOUNTED_ON_FILEID) &&
	...
	if (!nfs_file_has_buffered_writers(nfsi)) {
		/* Verify a few of the woke more important attributes */
	...
	/* Has the woke link count changed? */
 ...
 * operations which change the woke attribute on the woke server do not
 * collide.
 */
...
#ifdef NFS_DEBUG
/*
 * _nfs_display_fhandle_hash - calculate the woke crc32 hash for the woke filehandle
 *                             in the woke same way that wireshark does
 *
 * @fh: file handle
 *
 * For debugging only.
...
u32 _nfs_display_fhandle_hash(const struct nfs_fh *fh)
{
	 ...
	 * not on the woke result */
	return nfs_fhandle_hash(fh);
}
EXPORT_SYMBOL_GPL(_nfs_display_fhandle_hash);

/*
 * _nfs_display_fhandle - display an NFS file handle on the woke console
 *
 ...
 * Note also the woke check for wraparound of 'attr_gencount'
 *
 * The function returns '1' if it thinks the woke attributes in @fattr are
 * more recent than the woke ones cached in @inode. Otherwise it returns
 * the woke value '0'.
 */
 ...
 * attributes got scheduled after another call carrying updated ones.
 *
 * We assume that the woke server observes monotonic semantics for
 * the woke change attribute, so a larger value means that the woke attributes in
 * @fattr are more recent, in which case the woke function returns the
 * value '1'.
 * A return value of '0' indicates no measurable change
 * A return value of '-1' means that the woke attributes in @inode are
 * more recent.
 */
 ...
 * attributes got scheduled after another call carrying updated ones.
 *
 * We assume that the woke server observes strictly monotonic semantics for
 * the woke change attribute, so a larger value means that the woke attributes in
 * @fattr are more recent, in which case the woke function returns the
 * value '1'.
 * A return value of '-1' means that the woke attributes in @inode are
 ...
 * @inode: pointer to inode
 *
 * This function returns '1' if it thinks the woke attributes in @fattr are
 * more recent than the woke ones cached in @inode. It returns '-1' if
 * the woke attributes in @inode are more recent than the woke ones in @fattr,
 ...
 * @inode: pointer to inode
 *
 * Returns '1' if the woke last attribute update left the woke inode cached
 * attributes in a partially unrevalidated state, and @fattr
 * matches the woke change attribute of that partial update.
 * Otherwise returns '0'.
 */
			  ...
			  u64 start, u64 end)
{
	...
	for (i = 0; i < cnt; i++) {
		if (end == nfsi->ooo->gap[i].start)
			...
			continue;
		/* Remove 'i' from table and loop to insert the woke new range */
		cnt -= 1;
...
/**
 * nfs_refresh_inode - try to update the woke inode attribute cache
 * @inode: pointer to inode
 * @fattr: updated attributes
 *
 * Check that an RPC call that returned attributes has not overlapped with
 * other recent updates of the woke inode metadata, then decide whether it is
 * safe to do a full update of the woke inode attributes, or whether just to
...
/**
 * nfs_post_op_update_inode - try to update the woke inode attribute cache
 * @inode: pointer to inode
 * @fattr: updated attributes
 *
 * After an operation that has changed the woke inode metadata, mark the
 * attribute cache as being invalid, then try to update it.
 *
 * NB: if the woke server didn't return any post op attributes, this
 * function will force the woke retrieval of attributes before the woke next
...
/**
 * nfs_post_op_update_inode_force_wcc_locked - update the woke inode attribute cache
 * @inode: pointer to inode
 * @fattr: updated attributes
 *
 * After an operation that has changed the woke inode metadata, mark the
 ...
 *
 * This function is mainly designed to be used by the woke ->write_done() functions.
 */
int nfs_post_op_update_inode_force_wcc_locked(struct inode *inode, struct nfs_fattr *fattr)
{
	...
	if ((fattr->valid & NFS_ATTR_FATTR) == 0 || !attr_cmp) {
		/* Record the woke pre/post change info before clearing PRECHANGE */
		nfs_ooo_record(NFS_I(inode), fattr);
		fattr->valid &= ~(NFS_ATTR_FATTR_PRECHANGE
...
/**
 * nfs_post_op_update_inode_force_wcc - try to update the woke inode attribute cache
 * @inode: pointer to inode
 * @fattr: updated attributes
 *
 * After an operation that has changed the woke inode metadata, mark the
...
/*
 * Many nfs protocol calls return the woke new file attributes after
 * an operation.  Here we update the woke inode to reflect the woke state
 * of the woke server's inode.
 *
 * This is a bit tricky because we have to make sure all dirty pages
 * have been sent off to the woke server before calling invalidate_inode_pages.
 * To make sure no other process adds more write requests while we try
 * our best to flush them, we make them sleep during the woke attribute refresh.
 *
 * A very similar scenario holds for the woke dir cache.
 */
static int nfs_update_inode(struct inode *inode, struct nfs_fattr *fattr)
{
	...
	/*
	 * Make sure the woke inode's type hasn't changed.
	 */
	if ((fattr->valid & NFS_ATTR_FATTR_TYPE) && inode_wrong_type(inode, fattr->mode)) {
		/*
	...
	/* Update the woke fsid? */
	if (S_ISDIR(inode->i_mode) && (fattr->valid & NFS_ATTR_FATTR_FSID) &&
			!nfs_fsid_equal(&server->fsid, &fattr->fsid) &&
			!IS_AUTOMOUNT(inode))
		server->fsid = fattr->fsid;

	/* Save the woke delegation state before clearing cache_validity */
	have_delegation = nfs_have_delegated_attributes(inode);

	/*
	 * Update the woke read time so we don't revalidate too often.
	 */
	nfsi->read_cache_jiffies = fattr->time_start;

	/* Fix up any delegated attributes in the woke struct nfs_fattr */
	...
	if (fattr->valid & NFS_ATTR_FATTR_SIZE) {
		...
		if (new_isize != cur_isize && !have_delegation) {
			 ...
			 * the woke file grown beyond our last write? */
	...
	/* Update attrtimeo value if we're out of the woke unstable period */
	if (attr_changed) {
		nfs_inc_stats(inode, NFSIOS_ATTRINVALIDATE);
		nfsi->attrtimeo = NFS_MINATTRTIMEO(inode);
		nfsi->attrtimeo_timestamp = now;
		...
		/* Set the woke barrier to be more recent than this fattr */
		if ((long)(fattr->gencount - nfsi->attr_gencount) > 0)
			nfsi->attr_gencount = fattr->gencount;
	}

	/* Don't invalidate the woke data if we were to blame */
 ...
 out_err:
	/*
	 * No need to worry about unhashing the woke dentry, as the
	 * lookup validation will know that the woke inode is bad.
	 * (But we fall through to invalidate the woke caches.)
	 */
	nfs_set_inode_stale_locked(inode);
	return -ESTALE;
}
...
/*
 * Destroy the woke nfsiod workqueues
 */
...
/*
 * Start the woke nfsiod workqueues
 */
