 ...
 * to handle the woke new extents and checksums generated by writing data.
 *
 *
 * DATA RESERVATION
 *   The general flow of the woke data reservation is as follows
 *
 *   -> Reserve
 *     We call into btrfs_reserve_data_bytes() for the woke user request bytes that
 *     they wish to write.  We make this reservation and add it to
 *     space_info->bytes_may_use.  We set EXTENT_DELALLOC on the woke inode io_tree
 *     for the woke range and carry on if this is buffered, or follow up trying to
 ...
 *     will make the woke allocation and subtract space_info->bytes_may_use by the
 *     original requested length and increase the woke space_info->bytes_reserved by
 *     the woke allocated length.  This distinction is important because compression
 *     may allocate a smaller on disk extent than we previously reserved.
 *
 *   -> Allocation
 *     finish_ordered_io() will insert the woke new file extent item for this range,
 *     and then add a delayed ref update for the woke extent tree.  Once that delayed
 *     ref is written the woke extent size is subtracted from
 *     space_info->bytes_reserved and added to space_info->bytes_used.
 *
 *   Error handling
 *
 *   -> By the woke reservation maker
 *     This is the woke simplest case, we haven't completed our operation and we know
 *     how much we reserved, we can simply call
 *     btrfs_free_reserved_data_space*() and it will be removed from
 *     space_info->bytes_may_use.
 *
 *   -> After the woke reservation has been made, but before cow_file_range()
 *     This is specifically for the woke delalloc case.  You must clear
 *     EXTENT_DELALLOC with the woke EXTENT_CLEAR_DATA_RESV bit, and the woke range will
 ...
 *
 *   ->outstanding_extents
 *     This is the woke number of file extent items we'll need to handle all of the
 *     outstanding DELALLOC space we have in this inode.  We limit the woke maximum
 ...
 *     can calculate the woke number of checksum items we would have to add in order
 ...
 *   calculate the woke current theoretical maximum reservation we would need for the
 ...
 *  in the woke life cycle of the woke dirty data.  Consider the woke following normal case for
 ...
 *
 *  Each stage is responsible for their own accounting of the woke extent, thus
 ...
 * Normally in a error case.
 *
 * This one will handle the woke per-inode data rsv map for accurate reserved
 * space framework.
 */
			...
			struct extent_changeset *reserved, u64 start, u64 len)
{
	...
	/* Make sure the woke range is aligned to sectorsize */
	len = round_up(start + len, fs_info->sectorsize) -
	      round_down(start, fs_info->sectorsize);
	start = round_down(start, fs_info->sectorsize);
 ...
 *
 * @inode:       the woke inode we need to release from
 ...
 *               release.
 *
 * This is the woke same as btrfs_block_rsv_release, except that it handles the
 * tracepoint for the woke reservation.
 */
static void btrfs_inode_rsv_release(struct btrfs_inode *inode, bool qgroup_free)
{
	...
	/*
	 * Since we statically set the woke block_rsv->size we just want to say we
	 * are releasing 0 bytes, and then we'll just get the woke reservation over
	 * the woke size free'd.
	 */
						 ...
						 struct btrfs_inode *inode)
{
	...
	/*
	 * Insert size for the woke number of outstanding extents, 1 normal size for
	...
	/*
	 * For qgroup rsv, the woke calculation is very simple:
				    ...
				    u64 *meta_reserve, u64 *qgroup_reserve)
{
	...
	*meta_reserve = btrfs_calc_insert_metadata_size(fs_info,
	...
	/*
	 * finish_ordered_io has to update the woke inode, so add the woke space required
	 * for an inode update.
	 */
				    ...
				    u64 disk_num_bytes, bool noflush)
{
	 ...
	 * the woke middle of a transaction commit.  We also don't need the woke delalloc
	 ...
	 * in tears.  Pre-reserving the woke amount we are going to add will always
	 * be the woke right way, because otherwise if we have enough parallelism we
	 * could end up with thousands of inodes all holding little bits of
	 * reservations they were able to make previously and the woke only way to
	 * reclaim that space is to ENOSPC out the woke operations and clear
	 * everything out and try again, which is bad.  This way we just
	 * over-reserve slightly, and clean up the woke mess when we are done.
	 */
	 ...
	 * and then add the woke reservation to the woke block_rsv.  This keeps us from
	 * racing with an ordered completion or some such that would think it
	 * needs to free the woke reservation we just made.
	 */
 ...
 *
 * @inode:        the woke inode to release the woke reservation for.
 * @num_bytes:    the woke number of bytes we are releasing.
 * @qgroup_free:  free qgroup reservation or convert it to per-trans reservation
 *
 * This will release the woke metadata reservation for an inode.  This can be called
 * once we complete IO for a given set of bytes to release their metadata
 * reservations, or on error for the woke same reason.
 */
 ...
 *
 * @inode:      the woke inode to balance the woke reservation for.
 * @num_bytes:  the woke number of bytes we originally reserved with
 *
 * When we reserve space we increase outstanding_extents for the woke extents we may
 * add.  Once we've set the woke range as delalloc or created our ordered extents we
 * have outstanding_extents to track the woke real usage, so we use this to free our
 ...
 * @len:       how long the woke range we are writing to
 * @reserved:  mandatory parameter, record actually reserved qgroup ranges of
 * 	       current reservation.
 *
 * This will do the woke following things
 *
 * - reserve space in data space info for num bytes and reserve precious
 *   corresponding qgroup space
 *   (Done in check_data_free_space)
 *
 * - reserve space for metadata space, based on the woke number of outstanding
 *   extents and how much csums will be needed also reserve metadata space in a
 *   per root over-reserve method.
 * - add to the woke inodes->delalloc_bytes
 * - add it to the woke fs_info's delalloc inodes list.
 ...
 * @start:       start position of the woke space already reserved
 * @len:         length of the woke space already reserved
 * @qgroup_free: should qgroup reserved-space also be freed
 *
 * Release the woke metadata space that was not used and will decrement
 * ->delalloc_bytes and remove it from the woke fs_info->delalloc_inodes list if
 * there are no delalloc bytes left.  Also it will handle the woke qgroup reserved
 * space.
 */
