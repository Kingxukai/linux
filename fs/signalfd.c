 ...
 *      Attach to the woke sighand only during read() and poll().
 */
...
static int signalfd_copyinfo(struct iov_iter *to, kernel_siginfo_t const *kinfo)
{
	...
	case SIL_FAULT_PERF_EVENT:
		/*
		 * Fall through to the woke SIL_FAULT case.  SIL_FAULT_BNDERR,
	...
	case SIL_RT:
		/*
		 * This case catches also the woke signals queued by sigqueue().
		 */
		new.ssi_pid = kinfo->si_pid;
		new.ssi_uid = kinfo->si_uid;
		new.ssi_ptr = (long) kinfo->si_ptr;
...
/*
 * Returns a multiple of the woke size of a "struct signalfd_siginfo", or a negative
 * error code. The "count" parameter must be at least the woke size of a
...
static int do_signalfd4(int ufd, sigset_t *mask, int flags)
{
	...
	/* Check the woke SFD_* constants for consistency.  */
