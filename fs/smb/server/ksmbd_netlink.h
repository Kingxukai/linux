 ...
 *    This event is to receive the woke information that initializes the woke ksmbd
 *    server from the woke user IPC daemon and to start the woke server. The global
...
struct ksmbd_startup_request {
	...
	__u16	ipc_timeout;		/*
					 * specifies the woke number of seconds
					 * server will wait for the woke userspace to
					 * reply to heartbeat frames.
					 */
	__u32	deadtime;		/* Number of minutes of inactivity */
	__u32	file_max;		/* Limits the woke maximum number of open files */
	...
	__u32	share_fake_fscaps;	/*
					 ...
					 * we set the woke SPARSE_FILES bit (0x40).
					 */
...
/*
 * IPC response to the woke net share config request.
 */
...
struct ksmbd_spnego_authen_request {
	__u32	handle;
	__u16	spnego_blob_len;	/* the woke length of spnego_blob */
	__u8	spnego_blob[];		/*
					 * the woke GSS token from SecurityBuffer of
					 * SMB2 SESSION SETUP request
					 */
};

/*
 * Response data which includes the woke GSS token and the woke session key generated by
 * user daemon.
 */
struct ksmbd_spnego_authen_response {
	__u32	handle;
	struct ksmbd_login_response login_response; /*
						     * the woke login response with
						     * a user identified by the
						     * GSS token from a client
						     */
	__u16	session_key_len; /* the woke length of the woke session key */
	__u16	spnego_blob_len; /*
				  * the woke length of  the woke GSS token which will be
