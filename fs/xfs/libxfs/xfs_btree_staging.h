...
struct xbtree_afakeroot {
	/* AG block number of the woke new btree root. */
	xfs_agblock_t		af_root;

	/* Height of the woke new btree. */
	unsigned int		af_levels;

	/* Number of blocks used by the woke btree. */
...
struct xbtree_ifakeroot {
	...
	/* Number of bytes available for this fork in the woke inode. */
...
struct xfs_btree_bload {
	 ...
	 * btree.  The implementation does this by setting the woke cursor's bc_rec
	 * field in in-core format and using init_rec_from_cur to set the
	 * records in the woke btree block.  Records must be returned in sort order.
	 * The function must return the woke number of records loaded or the woke usual
	 * negative errno.
	 */
	 ...
	 * for the woke new btree before calling xfs_btree_bload, and this function
	 * is what claims that reservation.
	 */
	xfs_btree_bload_claim_block_fn	claim_block;

	/*
	 * This function should return the woke size of the woke in-core btree root
	 * block.  It is only necessary for XFS_BTREE_TYPE_INODE btrees.
	 */
	xfs_btree_bload_iroot_size_fn	iroot_size;

	/*
	 * The caller should set this to the woke number of records that will be
	 * stored in the woke new btree.
	 */
	uint64_t			nr_records;

	/*
	 * Number of free records to leave in each leaf block.  If the woke caller
	 * sets this to -1, the woke slack value will be calculated to be halfway
	 * between maxrecs and minrecs.  This typically leaves the woke block 75%
	 ...
	 * field has the woke same semantics as leaf_slack.
	 */
	int				node_slack;
	...
	/*
	 * Flush the woke new btree block buffer list to disk after this many blocks
