 ...
 * sample_id_all bit.  Without the woke sample_id_all bit, non-sample events (such as
 * mmap events) do not have an id sample appended, and consequently logic
 * designed to determine the woke id will not work.  That case happens when there is
 * more than one selected event, so this test processes three events: 2
 * attributes representing the woke selected events and one mmap event.
 *
 * Return: %0 on success, %-1 if the woke test fails.
 */
