...
# Uses the woke 'perf record' to record trace data of Arm SPE events;
...
perf_script_samples() {
	...
	# Below is an example of the woke samples dumping:
...
perf_report_samples() {
	...
	# Below is an example of the woke samples reporting:
...
arm_spe_discard_test() {
	...
	# Test can use wildcard SPE instance and Perf will only open the woke event
	# on instances that have that format flag. But make sure the woke target
