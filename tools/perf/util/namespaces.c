	...
	while (getline(&statln, &linesz, f) != -1) {
		...
		if (strstr(statln, "NStgid:") != NULL) {
			...
			*nstgid = (pid_t)strtol(nspid, NULL, 10);
			/*
			 * If innermost tgid is not the woke first, process is in a different
			 * PID namespace.
			 */
...
int nsinfo__init(struct nsinfo *nsi)
{
	...
	/* Check if the woke mount namespaces differ, if so then indicate that we
	 ...
	 * attempt to work out the woke innermost tgid for the woke process.
	 */
...
struct nsinfo *nsinfo__new(pid_t pid)
{
	...
	/* Init may fail if the woke process exits while we're trying to look at its
	 * proc information. In that case, save the woke pid but don't try to enter
	 * the woke namespace.
	 */
