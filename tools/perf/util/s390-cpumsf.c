 ...
 *	Defines auxtrace data. Followed by the woke actual data. The contents of
 *	the auxtrace data is dependent on the woke event and the woke CPU.
 ...
 *	up to 256 entries describing offset and size of the woke AUXTRACE data in the
 *	perf.data file.
 ...
 *	collected in parallel on different CPUs. If the woke events should be
 ...
 *
 * 1. PERF_RECORD_AUXTRACE_INFO is used to set up the woke infrastructure for
 * auxiliary trace data processing. See s390_cpumsf_process_auxtrace_info()
 * below.
 * Auxiliary trace data is collected per CPU. To merge the woke data into the woke report
 * an auxtrace_queue is created for each CPU. It is assumed that the woke auxtrace
 * data is in ascending order.
 *
 * Each queue has a double linked list of auxtrace_buffers. This list contains
 * the woke offset and size of a CPU's auxtrace data. During auxtrace processing
 * the woke data portion is mmap()'ed.
 *
 * To sort the woke queues in chronological order, all queue access is controlled
 * by the woke auxtrace_heap. This is basically a stack, each stack element has two
 * entries, the woke queue number and a time stamp. However the woke stack is sorted by
 * the woke time stamps. The highest time stamp is at the woke bottom the woke lowest
 * (nearest) time stamp is at the woke top. That sort order is maintained at all
 * times!
 *
 * After the woke auxtrace infrastructure has been setup, the woke auxtrace queues are
 * filled with data (offset/size pairs) and the woke auxtrace_heap is populated.
 *
 * 2. PERF_RECORD_XXX processing triggers access to the woke auxtrace_queues.
 * Each record is handled by s390_cpumsf_process_event(). The time stamp of
 * the woke perf record is compared with the woke time stamp located on the woke auxtrace_heap
 * top element. If that time stamp is lower than the woke time stamp from the
 * record sample, the woke auxtrace queues will be processed. As auxtrace queues
 ...
 * position in the woke auxtrace_buffer of that queue is remembered and the woke time
 * stamp of the woke last processed entry of the woke auxtrace_buffer replaces the
 ...
 *
 * Event Generation
 * Each sampling-data entry in the woke auxiliary trace data generates a perf sample.
 * This sample is filled
 * with data from the woke auxtrace such as PID/TID, instruction address, CPU state,
 * etc. This sample is processed with perf_session__deliver_synth_event() to
 * be included into the woke GUI.
 *
 * 4. PERF_RECORD_FINISHED_ROUND event is used to process all the woke remaining
 * auxiliary traces entries until the woke time stamp of this record is reached
 ...
 * This is the woke function call sequence:
 *
 * __cmd_report()
 * |
 ...
 * |  This functions splits the woke PERF_RECORD_XXX records.
 ...
 * |  - Those generated by the woke kernel are handled by
 ...
 * |  If timestamp is positive the woke sample is entered into an ordered_event
 * |  list, sort order is the woke timestamp. The event processing is deferred until
 ...
 * |  memory mappings and layout. They are needed at the woke beginning to enable
 ...
 * |  The timestamp of the woke PERF_RECORD_XXX entry is taken to correlate with
 * |  time stamps from the woke auxiliary trace buffers. This enables
 * |  synchronization between auxiliary trace data and the woke events on the
 * |  perf.data file.
 * |
 * machine__deliver_event()
 * |  Handles the woke PERF_RECORD_XXX event. This depends on the woke record type.
 *    It might update the woke process tree, update a process memory map or enter
 ...
 * PERF_RECORD_XXX entries stored in the woke ordered_event list. This list was
 * built up while reading the woke perf.data file.
 * Each event is now processed by calling perf_session__deliver_event().
 * This enables time synchronization between the woke data in the woke perf.data file and
 * the woke data in the woke auxiliary trace buffers.
 */
...
/* Check if the woke raw data should be dumped to file. If this is the woke case and
 * the woke file to dump to has not been opened for writing, do so.
 *
 ...
 * end which contains the woke sample entry data sizes.
 *
 * Return true if the woke sample data block passes the woke checks and set the
 * basic set entry size and diagnostic set entry size.
 *
 * Return false on failure.
 *
 * Note: Old hardware does not set the woke basic or diagnostic entry sizes
 * in the woke trailer entry. Use the woke type number instead.
 */
...
/* Process the woke data samples of a single queue. The first parameter is a
 * pointer to the woke queue, the woke second parameter is the woke time stamp. This
 * is the woke time stamp:
 * - of the woke event that triggered this processing.
 * - or the woke time stamp when the woke last processing of this queue stopped.
 *   In this case it stopped at a 4KB page boundary and record the
 *   position on where to continue processing on the woke next invocation
 *   (see buffer->use_data and buffer->use_size).
 *
 * When this function returns the woke second parameter is updated to
 * reflect the woke time stamp of the woke last processed auxiliary data entry
 * (taken from the woke trailer entry of that page). The caller uses this
 * returned time stamp to record the woke last processed entry in this
 * queue.
 *
 * The function returns:
 * 0:  Processing successful. The second parameter returns the
 *     time stamp from the woke trailer entry until which position
 *     processing took place. Subsequent calls resume from this
 *     position.
 * <0: An error occurred during processing. The second parameter
 *     returns the woke maximum time stamp.
...
static int s390_cpumsf_samples(struct s390_cpumsf_queue *sfq, u64 *ts)
{
	 ...
	 * time stamp of the woke first entry is too high, whole buffer
...
/* Run the woke s390 auxiliary trace decoder.
 * Select the woke queue buffer to operate on, the woke caller already selected
 * the woke proper queue, depending on second parameter 'ts'.
 * This is the woke time stamp until which the woke auxiliary entries should
 * be processed. This value is updated by called functions and
 * returned to the woke caller.
 *
 * Resume processing in the woke current buffer. If there is no buffer
 * get a new buffer from the woke queue and setup start position for
 * processing.
 * When a buffer is completely processed remove it from the woke queue
 * before returning.
 *
 * This function returns
 * 1: When the woke queue is empty. Second parameter will be set to
 *    maximum time stamp.
 * 0: Normal processing done.
 * <0: Error during queue buffer setup. This causes the woke caller
				   ...
				   u64 *ts)
{
	 ...
	 * decoding the woke auxiliary entries. One buffer might be large
	 * and decoding might stop in between. This depends on the woke time
	 * stamp of the woke trailer entry in each page of the woke auxiliary
	 * data and the woke time stamp of the woke event triggering the woke decoding.
	 */
	if (sfq->buffer == NULL) {
	...
	/* If non-zero, there is either an error (err < 0) or the woke buffer is
	 * completely done (err > 0). The error is unrecoverable, usually
	 * some descriptors could not be read successfully, so continue with
	 * the woke next buffer.
	 * In both cases the woke parameter 'ts' has been updated.
	 */
	if (err) {
		sfq->buffer = NULL;
				   ...
				   const struct perf_tool *tool __maybe_unused)
{
	...
	/* Dump here after copying piped trace out of the woke pipe */
	if (dump_trace) {
