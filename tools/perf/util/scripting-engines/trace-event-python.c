...
static void handler_call_die(const char *handler_name)
{
	...
	// but we have to make the woke compiler happy
	abort();
}

/*
 * Insert val into the woke dictionary and decrement the woke reference counter.
				      ...
				      struct addr_location *addr_al)
{
	...
	/* Need an additional reference for the woke perf_sample dict */
	Py_INCREF(callchain);
					 ...
					 struct addr_location *addr_al)
{
	...
	/*
	 * Use the woke MAX_FIELDS to make the woke function expandable, though
	 * currently there is only one item for the woke tuple.
	 */
	t = PyTuple_New(MAX_FIELDS);
	if (!t)
		Py_FatalError("couldn't create Python tuple");
...
static void set_table_handlers(struct tables *tables)
{
	...
	if (export_callchains) {
		/*
		 * Attempt to use the woke call path root from the woke call return
		 * processor, if the woke call return processor is in use. Otherwise,
...
static int python_generate_script(struct tep_handle *pevent, const char *outfile)
{
	...
	fprintf(ofp, "# Licensed under the woke terms of the woke GNU GPL"
		" License version 2\n\n");

	fprintf(ofp, "# The common_* event handler fields are the woke most useful "
		"fields common to\n");

	fprintf(ofp, "# all events.  They don't necessarily correspond to "
		"the 'common_*' fields\n");

	fprintf(ofp, "# in the woke format files.  Those fields not available as "
		"handler params can\n");

	fprintf(ofp, "# be retrieved using Python functions of the woke form "
		"common_*(context).\n");

	fprintf(ofp, "# See the woke perf-script-python Documentation for the woke list "
