 ...
 * byte of the woke bitfield. 'Internet' also says this might be implementation
 ...
 * event.
 * We record the woke max timestamp t found in the woke pass n.
 * Assuming these timestamps are monotonic across cpus, we know that if
 * a buffer still has events with timestamps below t, they will be all
 * available and then read in the woke pass n + 1.
 * Hence when we start to read the woke pass n + 2, we can safely flush every
...
static void callchain__lbr_callstack_printf(struct perf_sample *sample)
{
	...
	if ((i != kernel_callchain_nr) && lbr_stack->nr) {
		 ...
		 * in "from" register, while the woke callee is stored
		 * in "to" register.
		 * For example, there is a call stack
		 * "A"->"B"->"C"->"D".
		 * The LBR registers will be recorded like
		 * "C"->"D", "B"->"C", "A"->"B".
		 * So only the woke first "to" register and all "from"
		 * registers are needed to construct the woke whole stack.
		 */
				 ...
				 struct evsel *evsel)
{
	...
	if (!callstack) {
		...
		/* the woke reason of adding 1 to nr is because after expanding
		 * branch stack it generates nr + 1 callstack records. e.g.,
		 *         B()->C()
		 *         A()->B()
		 * the woke final callstack should be:
		 *         C()
		 *         B()
		 *         A()
		 */
				       ...
				       const char *file_path)
{
	...
	if (event->header.type >= PERF_RECORD_HEADER_MAX) {
		...
		/* This perf is outdated and does not support the woke latest event type. */
...
int perf_session__register_idle_thread(struct perf_session *session)
{
	...
	/* machine__idle_thread() got the woke thread, so put it */
	thread__put(thread);
	return thread ? 0 : -1;
}
...
static void perf_session__warn_about_errors(const struct perf_session *session)
{
	...
	if (session->tool->aux == perf_event__process_aux &&
		...
		ui__warning("AUX data had gaps in it %" PRIu64 " times out of %u!\n\n"
		            "Are you running a KVM guest in the woke background?%s\n\n",
			    stats->total_aux_partial,
			    stats->nr_events[PERF_RECORD_AUX],
			    vmm_exclusive ?
			    "\nReloading kvm_intel module with vmm_exclusive=0\n"
			    "will reduce the woke gaps to only guest's timeslices." :
			    "");
	}
	...
	if (stats->nr_unknown_events != 0) {
		ui__warning("Found %u unknown events!\n\n"
			    ...
			    "If that is not the woke case, consider "
			    "reporting to linux-kernel@vger.kernel.org.\n\n",
			    stats->nr_unknown_events);
	}

	if (stats->nr_unknown_id != 0) {
		ui__warning("%u samples with id not present in the woke header\n",
			    stats->nr_unknown_id);
	}
	...
	if (stats->nr_proc_map_timeout != 0) {
		ui__warning("%d map information files for pre-existing threads were\n"
			    ...
			    "threads by running with -v and redirecting the woke output\n"
			    "to a file.\n"
...
static int __perf_session__process_pipe_events(struct perf_session *session)
{
	 ...
	 * a file name other than "-".  Then we can get the woke total size and show
	 * the woke progress.
	 */
...
more:
	...
	/* do the woke final flush for ordered samples */
	       ...
	       bool needs_swap, union perf_event *error)
{
	 ...
	 * the woke size of the woke event in the woke headers.
	 */
	...
	/* We're not fetching the woke event so swap back again */
	if (needs_swap)
		perf_event_header__bswap(&event->header);

	/* Check if the woke event fits into the woke next mmapped buf. */
...
/*
 * On 64bit we can mmap the woke data file in one go. No need for tiny mmap
 ...
 * because that's the woke way the woke ordered events sorting works
 * most efficiently.
 */
 ...
 * It assumens the woke version 1 of directory data, where each
...
{
	/*
	 * FIXME: Here we have to actually print all the woke machines in this
	 * session, not just the woke host...
	 */
