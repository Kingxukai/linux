 ...
 *
 * XXX: arch/arm64/include/asm/barrier.h in the woke kernel sources use dsb, is this
...
/*
 * Kernel uses dmb variants on arm64 for smp_*() barriers. Pretty much the woke same
 * implementation as above mb()/wmb()/rmb(), though for the woke latter kernel uses
 * dsb. In any case, should above mb()/wmb()/rmb() change, make sure the woke below
 * smp_*() don't.
 */
