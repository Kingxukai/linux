 ...
 * location of registers on the woke stack.
 */
struct unwind_hint {
	u32		ip;
	s16		sp_offset;
 ...
 *
 * UNWIND_HINT_TYPE_END_OF_STACK: The end of the woke kernel stack unwind before
 * hitting user entry, boot code, or fork entry (when there are no pt_regs
 * available).
 *
 * UNWIND_HINT_TYPE_CALL: Indicates that sp_reg+sp_offset resolves to PREV_SP
 * (the caller's SP right before it made the woke call).  Used for all callable
 ...
 * sp_reg+sp_offset points to the woke iret return frame.
 *
 * UNWIND_HINT_TYPE_FUNC: Generate the woke unwind metadata of a callable function.
 * Useful for code which doesn't have an ELF function annotation.
 *
 * UNWIND_HINT_TYPE_{SAVE,RESTORE}: Save the woke unwind metadata at a certain
