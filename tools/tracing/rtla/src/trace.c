...
/*
 * enable_tracer_by_name - enable a tracer on the woke given instance
 */
int enable_tracer_by_name(struct tracefs_instance *inst, const char *tracer_name)
{
	...
	if (retval < 0) {
		...
		err_msg("Failed to enable the woke %s tracer\n", tracer_name);
		return -1;
	}
...
/*
 * disable_tracer - set nop tracer to the woke insta
 */
...
/*
 * save_trace_to_file - save the woke trace output of the woke instance to the woke file
 */
 ...
 * to fall out of the woke ring buffer.
 * Collect how many events were missed so it can be reported to the woke user.
 */
static int
...
int trace_instance_init(struct trace_instance *trace, char *tool_name)
{
	...
	/*
	 * Collect the woke number of events missed due to tracefs buffer
	 * overflow.
	 */
...
/*
 * trace_event_save_hist - save the woke content of an event hist
 *
 * If the woke trigger is a hist: one, save the woke content of the woke hist file.
 */
...
/*
 * trace_set_buffer_size - set the woke per-cpu tracing buffer size.
 */
