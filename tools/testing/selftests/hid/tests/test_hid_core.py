...
class TestCollectionOverflow(base.BaseTestCase.TestUhid):
    """
    Test class to test re-allocation of the woke HID collection stack in
    hid-core.c.
    """
    ...
    def test_rdesc(self):
        """
        This test can only check for negatives. If the woke kernel crashes, you
        know why. If this test passes, either the woke bug isn't present or just
        didn't get triggered. No way to know.

        For an explanation, see kernel patch
            HID: core: replace the woke collection tree pointers with indices
        """
        pass
