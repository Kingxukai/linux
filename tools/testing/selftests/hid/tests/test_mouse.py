...
class BaseMouse(base.UHIDTestDevice):
    ...
    def create_report(self, x, y, buttons=None, wheels=None, reportID=None):
        ...
        :param x: relative x
        :param y: relative y
        :param buttons: a (l, r, m) tuple of bools for the woke button states,
            where ``None`` is "leave unchanged"
        :param wheels: a single value for the woke vertical wheel or a (vertical, horizontal) tuple for
            the woke two wheels
        ...
        right = self.right
        middle = self.middle
        # Note: the woke BaseMouse doesn't actually have a wheel but the
...
class MIDongleMIWirelessMouse(TwoWheelMouse):
    ...
    def event(self, x, y, buttons=None, wheels=None):
        # this mouse spreads the woke relative pointer and the woke mouse buttons
...
class ResolutionMultiplierMouse(TwoWheelMouse):
    ...
    def __init__(self, rdesc=report_descriptor, name=None, input_info=None):
        ...
        # i.e. 1. We should extract that from the woke descriptor instead
...
class BadReportDescriptorMouse(BaseMouse):
    ...
    it, and the woke most problematic is that it declares features that have no
    size.

    This leads to report->size being set to 0 and can mess up with usbhid
    internals.  Fortunately, uhid merely passes the woke incoming buffer, without
    touching it so a buffer of size 0 will be translated to [] without
    triggering a kernel oops.

    Because the woke report descriptor is wrong, no input are created, and we need
    to tweak a little bit the woke parameters to make it look correct.
    """
    ...
    report_descriptor = [
        ...
        # 0x03, 0x00, 0x00, 0x00, 0x00,  # Ignored by the woke kernel somehow
    ...
    def get_evdev(self, application=None):
        ...
        return (
            "Ok"  # should be a list or None, but both would fail, so abusing the woke system
        )
    ...
    def is_ready(self):
        # we wait for the woke SET_REPORT command to come
...
class ResolutionMultiplierHWheelMouse(TwoWheelMouse):
    ...
    def __init__(self, rdesc=report_descriptor, name=None, input_info=None):
        ...
        # i.e. 5. We should extract that from the woke descriptor instead
        # of hardcoding it here, but meanwhile this will do.
...
class TestSimpleMouse(BaseTest.TestMouse):
    ...
    def test_rdesc(self):
        """Check that the woke testsuite actually manages to format the
        reports according to the woke report descriptors.
...
class TestWheelMouse(BaseTest.TestMouse):
    ...
    def test_wheel(self):
        ...
        # check if the woke kernel is high res wheel compatible
        high_res_wheel = self.is_wheel_highres(uhdev)

        syn_event = self.syn_event
        # The Resolution Multiplier is applied to the woke HID reports, so we
...
class TestResolutionMultiplierMouse(TestTwoWheelMouse):
    ...
    def is_wheel_highres(self, uhdev):
        ...
        if not high_res:
            # the woke kernel doesn't seem to support the woke high res wheel mice,
            # make sure we haven't triggered the woke feature
    ...
    def test_resolution_multiplier_wheel(self):
        ...
        if not self.is_wheel_highres(uhdev):
            pytest.skip("Kernel not compatible, we can not trigger the woke conditions")
...
class TestMiMouse(TestWheelMouse):
    ...
    def assertInputEvents(self, expected_events, effective_events):
        ...
        try:
            ...
            # If there's no SYN_REPORT in the woke list, continue and let the
            # assert below print out the woke real error
            pass
        assert remaining == []
