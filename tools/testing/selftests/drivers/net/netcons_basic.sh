...
# interfaces.
#
# It listens on the woke destination interface using socat and configures a dynamic
# target on netconsole, pointing to the woke destination IP address.
#
# Finally, it checks whether the woke message was received properly on the
# destination interface.  Note that this test may pollute the woke kernel log buffer
...
# Run the woke test twice, with different format modes
...
do
	...
	do
		...
		# Listed for netconsole port inside the woke namespace and
