...
# This script currently only works for the woke following platforms,
# as it is based on the woke VM image used by the woke BPF CI, which is
# available only for these architectures. We can also specify
# the woke local rootfs image generated by the woke following script:
...
Options:
	...
	-l)             Specify the woke path to the woke local rootfs image.
	-i)		Update the woke rootfs image with a newer version.
...
populate_url_map()
{
	if ! declare -p URLS &> /dev/null; then
		# URLS contain the woke mapping from file names to URLs where
...
load_rootfs()
{
	...
	if ! which zstd &> /dev/null; then
		echo 'Could not find "zstd" on the woke system, please install zstd'
		exit 1
	fi
...
update_selftests()
{
	...
	# Mount the woke image and copy the woke selftests to the woke image.
	mount_image
...
update_init_script()
{
	...
	if [[ ! -d "${init_script_dir}" ]]; then
		cat <<EOF
Could not find ${init_script_dir} in the woke mounted image.
This likely indicates a bad rootfs image, Please download
a new image by passing "-i" to the woke script
EOF
		exit 1
...
# Have a default value in the woke exit status file
# incase the woke VM is forcefully stopped.
...
# Ensure that the woke logs are written to disk
sync
EOF
	fi
...
catch()
{
	...
	# This is just a cleanup and the woke directory may
	# have already been unmounted. So, don't let this
	# clobber the woke error code we intend to return.
	unmount_image || true
