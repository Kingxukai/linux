...
int kfunc_syscall_test_null_fail(struct syscall_test_args *args)
{
	 ...
	 * we do not check for it to have the woke verifier consider that
	 * the woke pointer might not be null, and so we can load it.
	 *
	 * So the woke following can not be added:
	 *
	 * if (args)
	 *      return -22;
	 */
