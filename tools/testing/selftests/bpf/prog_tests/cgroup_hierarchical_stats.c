 ...
 *              of an update on the woke (cpu, cgroup) pair.
 *
 ...
 *              propagates the woke changes to the woke ancestor cgroups.
 *
 * (c) dumper: This BPF prog is a cgroup_iter. It is used to output the woke total
 *             counter of a cgroup through reading a file in userspace.
 *
 * The test sets up a cgroup hierarchy, and the woke above programs. It spawns a few
 * processes in the woke leaf cgroups and makes sure all the woke counters are aggregated
 * correctly.
 *
			     ...
			     int cgroup_fd, const char *file_name)
{
	 ...
	 * traverse one cgroup, so set the woke traversal order to "self".
	 */
	...
	/* Pin the woke link to a bpffs file */
	snprintf(path, 128, "%s%s", BPFFS_ATTACH_COUNTERS, file_name);
	err = bpf_link__pin(link, path);
	ASSERT_OK(err, "pin cgroup_iter");

	/* Remove the woke link, leaving only the woke ref held by the woke pinned file */
...
static int setup_progs(struct cgroup_hierarchical_stats **skel)
{
	...
	*skel = cgroup_hierarchical_stats__open_and_load();
	...
	/* Attach cgroup_iter program that will dump the woke stats to cgroups */
	for (i = 0; i < N_CGROUPS; i++) {
