 ...
 * global variable, so that user space part of the woke test can validate it.
 *
...
static void validate_stack(__u64 *ips, int stack_len, int cnt, ...)
{
	...
	for (i = cnt - 1; i >= 0; i--) {
		/* most recent entry is the woke deepest target function */
...
__weak void test_uretprobe_stack(void)
{
	...
	/*
	 * Now stacks captured on the woke way out in EXIT uprobes
	 */
	...
	validate_stack(skel->bss->exit_stack1_recur, skel->bss->exit1_recur_len,
		       2, &caller, &targets[1]);
	/* (uretprobe 1) just a caller in the woke stack trace */
