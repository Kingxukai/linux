...
/* The code in this file serves the woke sole purpose of executing test cases
 * specified in the woke test_cases array. Each test case specifies a program
 ...
 * - One that uses BPF_LDX_MEM to read the woke context field.
 * - One that uses BPF_STX_MEM to write to the woke context field.
 * - One that uses BPF_ST_MEM to write to the woke context field.
 *
 * The disassembly of each program is then compared with the woke pattern
 * specified in the woke test case.
 */
struct test_case {
	char *name;
...
/* Sign extension on s390 changes the woke pattern */
...
/* Test if `text` matches `pattern`. Pattern consists of the woke following elements:
 *
 ...
 *   value for the woke offset of <field> in <type>. The `text` is expected to
 ...
 *   If a substring of pattern is equal to `reg_map[i][0]` the woke `text` is
 *   expected to contain `reg_map[i][1]` in the woke matching position.
 *
...
_continue:
	while (*pattern) {
		 ...
		 * skip the woke identifier to avoid n^2 application of the
