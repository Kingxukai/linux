...
#
# Carry out a kvm-based run for the woke specified qemu-cmd file, which might
# have been generated by --build-only kvm.sh run.
#
# Usage: kvm-test-1-run-qemu.sh qemu-cmd-dir
#
# qemu-cmd-dir provides the woke directory containing qemu-cmd file.
#	This is assumed to be of the woke form prefix/ds/scenario, where
#	"ds" is the woke top-level date-stamped directory and "scenario"
#	is the woke scenario name.  Any required adjustments to this file
#	must have been made by the woke caller.  The shell-command comments
#	at the woke end of the woke qemu-cmd file are not optional.
#
...
# Obtain settings from the woke qemu-cmd file.
...
then
	sleep 10 # Give qemu's pid a chance to reach the woke file
...
then
	...
	echo 'After symbols load and the woke "(gdb)" prompt appears:' > /dev/tty
...
# Tell the woke script that this run is done.
