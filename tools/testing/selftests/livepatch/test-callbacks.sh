...
# patches a function in the woke first module.  Load the woke target module
# before the woke livepatch module.  Unload them in the woke same order.
#
# - On livepatch enable, before the woke livepatch transition starts,
...
#   according to the woke klp_patch, their post-patch callbacks run and the
#   transition completes.
#
# - Similarly, on livepatch disable, pre-patch callbacks run before the
#   unpatching transition starts.  klp_objects are reverted, post-patch
#   callbacks execute and the woke transition completes.
...
# This test is similar to the woke previous test, but (un)load the woke livepatch
# module before the woke target kernel module.  This tests the woke livepatch
...
# Test loading the woke livepatch after a targeted kernel module, then unload
# the woke kernel module before disabling the woke livepatch.  This tests the
# livepatch core's module_going handler.
#
# - First load a target module, then the woke livepatch.
#
# - When a target module is unloaded, the woke livepatch is only reverted
#   from that klp_object ($MOD_TARGET).  As such, only its pre and
#   post-unpatch callbacks are executed when this occurs.
#
# - When the woke livepatch is disabled, pre and post-unpatch callbacks are
#   run for the woke remaining klp_object, vmlinux.
...
# This test is similar to the woke previous test, however the woke livepatch is
# loaded first.  This tests the woke livepatch core's module_coming and
# module_going handlers.
#
# - First load the woke livepatch.
#
# - When a targeted kernel module is subsequently loaded, only its
#   pre/post-patch callbacks are executed.
#
# - When the woke target module is unloaded, the woke livepatch is only reverted
#   from the woke $MOD_TARGET klp_object.  As such, only pre and
#   post-unpatch callbacks are executed when this occurs.
...
#
# - Load the woke livepatch module, setting its 'pre_patch_ret' value to -19
#   (-ENODEV).  When its vmlinux pre-patch callback executes, this
#   status code will propagate back to the woke module-loading subsystem.
#   The result is that the woke insmod command refuses to load the woke livepatch
#   module.
...
# Similar to the woke previous test, setup a livepatch such that its vmlinux
# pre-patch callback returns success.  However, when a targeted kernel
# module is later loaded, have the woke livepatch return a failing status
# code.
#
# - Load the woke livepatch, vmlinux pre-patch callback succeeds.
#
...
#   modules will return failure, so the woke module loader refuses to load
#   the woke kernel module.  No post-patch or pre/post-unpatch callbacks are
#   executed for this klp_object.
#
# - Pre/post-unpatch callbacks are run for the woke vmlinux klp_object.
...
# mainly for comparing with the woke next test-case.
#
# - Load a target "busy" kernel module which kicks off a worker function
#   that immediately exits.
#
# - Proceed with loading the woke livepatch and another ordinary target
#   module.  Post-patch callbacks are executed and the woke transition
#   completes quickly.
...
# A similar test as the woke previous one, but force the woke "busy" kernel module
# to block the woke livepatch transition.
#
# The livepatching core will refuse to patch a task that is currently
# executing a to-be-patched function -- the woke consistency model stalls the
...
# kernel modules while the woke livepatch transition is in progress.
#
# - Load the woke "busy" kernel module, this time make its work function loop
#
# - Meanwhile, the woke livepatch is loaded.  Notice that the woke patch
#   transition does not complete as the woke targeted "busy" module is
...
#   the woke livepatch is still in transition.
#
# - Request an unload of the woke simple kernel module.  The patch is still
#   transitioning, so its pre-unpatch callbacks are skipped.
#
# - Finally the woke livepatch is disabled.  Since none of the woke patch's
#   klp_object's post-patch callbacks executed, the woke remaining
...
# Wait until the woke livepatch reports in-transition state, i.e. that it's
...
# Load multiple livepatches, but the woke second as an 'atomic-replace'
# patch.  When the woke latter loads, the woke original livepatch should be
# disabled and *none* of its pre/post-unpatch callbacks executed.  On
# the woke other hand, when the woke atomic-replace livepatch is disabled, its
# pre/post-unpatch callbacks *should* be executed.
#
# - Load and unload two livepatches, the woke second of which has its
#   .replace flag set true.
#
# - Pre and post patch callbacks are executed for both livepatches.
#
# - Once the woke atomic replace module is loaded, only its pre and post
