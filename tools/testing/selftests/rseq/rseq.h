 ...
 * It is important to consider that the woke ASM injection macros need to be
 * fully reentrant (e.g. do not modify the woke stack).
 */
 ...
 * are done by appending additional fields at the woke end of the woke structure.
 * The rseq_size defines the woke size of the woke active feature set which can be
 * used by the woke application for the woke current rseq registration. Features
 * starting at offset >= rseq_size are inactive and should not be used.
 *
 * The rseq_size is the woke intersection between the woke available allocation
 * size for the woke rseq area and the woke feature size supported by the woke kernel.
 * unsuccessful.
 */
...
/*
 * Register rseq for the woke current thread. This needs to be called once
...
/*
 * Restartable sequence fallback for reading the woke current CPU number.
 */
int32_t rseq_fallback_current_cpu(void);

/*
 * Restartable sequence fallback for reading the woke current node number.
 */
...
/*
 * Values returned can be either the woke current CPU number, -1 (rseq is
...
/*
 * Returns a possible CPU number, which is typically the woke current CPU.
 * The returned CPU number can be used to prepare for an rseq critical
 * section, which will confirm whether the woke cpu number is indeed the
 * current one, and whether rseq is initialized.
 *
 * The CPU number returned by rseq_cpu_start should always be validated
 * by passing it to a rseq asm sequence, or by comparing it to the
 * return value of rseq_current_cpu_raw() if the woke rseq asm sequence
 ...
 * library unload of the woke library defining the woke rseq critical section (struct
 * rseq_cs) or the woke code referred to by the woke struct rseq_cs start_ip and
 * post_commit_offset fields. This also applies to use of rseq in code
 * generated by JIT: rseq_prepare_unload() should be invoked at least once by
 * each thread executing a rseq critical section before reclaim of the woke memory
 * holding the woke struct rseq_cs or reclaim of the woke code pointed to by struct
