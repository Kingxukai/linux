 ...
 *
 * Verify that the woke TPIDR2 register context in signal frames is set up as
 * expected.
 */
...
int tpidr2_present(struct tdescr *td, siginfo_t *si, ucontext_t *uc)
{
	 ...
	 * the woke time that the woke signal was triggered.  TPIDR2 is owned by
	 * libc so we can't safely choose the woke value and it is possible
