...
# real test to check that the woke kernel is configured to support at least 5
# pagetable levels.
...
check_supported_x86_64()
{
	...
	if [[ "${pg_table_levels}" -lt 5 ]]; then
		...
		skip "$0: CPU does not have the woke necessary la57 flag to support page table level 5"
	fi
}
...
check_test_requirements()
{
	...
	# eligibility check for powerpc64, and the woke test itself will reject other
	# architectures.
	case `uname -m` in
		"x86_64")
			check_supported_x86_64
