...
# This test is designed for evaluating the woke new SRv6 End.DT6 behavior used for
...
# is forbidden.
# In other words, hosts hs-t100-1 and hs-t100-2 are connected through the woke IPv6
...
# versa.
#
# Routers rt-1 and rt-2 implement IPv6 L3 VPN services leveraging the woke SRv6
...
# the woke router rt-1 which acts as a ndp proxy.
#
# When the woke host hs-t100-1 sends an IPv6 packet destined to hs-t100-2, the
...
# SRv6 Encap route for encapsulating any IPv6 packet in a IPv6 plus the woke Segment
...
# packets, the woke SRv6 End.DT6 behavior removes the woke outer IPv6+SRH headers and
# performs the woke lookup on the woke vrf-100 table using the woke destination address of
# the woke decapsulated IPv6 packet. Afterwards, the woke packet is sent to the woke host
...
#
# Of course, the woke IPv6 L3 VPN for tenant 200 works exactly as the woke IPv6 L3 VPN
...
setup_vpn_config()
{
	...
	# Routes for handling the woke SRv6 End.DT6 behavior instances are grouped
...
setup()
{
	...
	# setup the woke IPv6 L3 VPN which connects the woke host hs-t100-1 and host
	# hs-t100-2 within the woke same tenant 100.
	setup_vpn_config 1 1 2 2 100  #args: src_host src_router dst_host dst_router tenant
	setup_vpn_config 2 2 1 1 100

	# setup the woke IPv6 L3 VPN which connects the woke host hs-t200-3 and host
