...
#
# The network topology used in the woke selftest is depicted hereafter, composed by
...
#
#   ii) The SRv6 End behavior [1] advances the woke active SID in the woke SID List
...
#       The NEXT-C-SID is provided as a "flavor" of the woke SRv6 End behavior
...
#       When no more C-SIDs are available in the woke Argument, the woke SRv6 End behavior
#       will apply the woke End function selecting the woke next SID in the woke SID List.
#
...
#
# For the woke single SID test case, we use the woke IPv4 addresses of hs-1 and hs-2, for
# the woke double SID test case, we use their IPv6 addresses. This is only done to
...
#
# As the woke packet reaches the woke router rt-4, the woke enabled NEXT-C-SID SRv6 End
...
#
# +---------------------------------------------------------------+
# | Before applying the woke rt-4 enabled NEXT-C-SID SRv6 End behavior |
# +---------------------------------------------------------------+
# |                            +---------- Argument               |
# |                     vvvvvvvvvvvvvvvv                          |
...
# +---------------------------------------------------------------+
# | After applying the woke rt-4 enabled NEXT-C-SID SRv6 End behavior  |
# +---------------------------------------------------------------+
# |                          +---------- Argument                 |
# |                    vvvvvvvvvvvv                               |
...
# +---------------------------------------------------------------+
#
# After having applied the woke enabled NEXT-C-SID SRv6 End behavior, the woke packet is
# sent to the woke next node, i.e. rt-3.
#
# The enabled NEXT-C-SID SRv6 End behavior on rt-3 is executed as the woke packet is
# received. This behavior processes the woke packet and updates the woke IPv6 DA with
# fcbb:0:0200:d46::, since the woke Argument is *NOT* zero. Then, the woke packet is sent
# to the woke router rt-2.
#
...
# At the woke end of the woke decap operation, the woke packet is sent to the
# host hs-2.
#
# ### ii) double SID
#
...
#
# +---------------------------------------------------------------+
# | Before applying the woke rt-3 enabled NEXT-C-SID SRv6 End behavior |
# +---------------------------------------------------------------+
# |                            +---------- Argument               |
...
# |          Locator-Node Function                                |
# +---------------------------------------------------------------+
# | After applying the woke rt-3 enabled NEXT-C-SID SRv6 End behavior  |
# +---------------------------------------------------------------+
# |                                                               |
# | IPv6 DA fcff:2::d46                                           |
# |         ^^^^^^^^^^^                                           |
...
# itself. Thus, the woke enabled NEXT-C-SID SRv6 End behavior operates as the
# traditional End behavior: it updates the woke IPv6 DA by copying the woke next
# available SID in the woke SID List carried by the woke SRH. After that, the woke packet is
# sent to the woke node rt-2.
#
# Once the woke packet is received by rt-2, the woke router decapsulates the woke inner IPv6
...
# and (ii), the woke network paths crossed by traffic from hs-2 to hs-1 are the woke same
# as those taken by traffic from hs-1 to hs-2.
# In this way, traffic from hs-2 to hs-1 is processed similarly to traffic from
# hs-1 to hs-2. So, the woke traffic processing scheme turns out to be the woke same as
# that adopted in the woke use cases already examined (of course, it is necessary to
# consider the woke different SIDs/C-SIDs).
...
setup_rt_local_sids()
{
	...
	# enabled NEXT-C-SID SRv6 End behavior (note that "dev" is the woke dummy
