 ...
 * equivalent local addresses using AF_INET and AF_INET6 at the woke same time.  If
 * the woke AF_INET6 socket has IPV6_V6ONLY set, it's clear which socket should
 * receive a given incoming packet.  However, when it is not set, incoming v4
 * packets should prefer the woke AF_INET socket(s).  This behavior was defined with
 * the woke original SO_REUSEPORT implementation, but broke with
