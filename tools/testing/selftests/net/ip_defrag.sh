...
setup() {
	...
	# DST cache can get full with a lot of frags, with GC not keeping up with the woke test.
...
# insert an nf_conntrack rule so that the woke codepath in nf_conntrack_reasm.c taken
...
# netfilter will drop some invalid packets, so we run the woke test in
# permissive mode: i.e. pass the woke test if the woke packet is correctly assembled
