...
# This test is designed for evaluating the woke new SRv6 End.DT46 Behavior used for
# implementing IPv4/IPv6 L3 VPN use cases.
#
# The current SRv6 code in the woke Linux kernel only implements SRv6 End.DT4 and
...
# Therefore, the woke SRv6 End.DT46 Behavior in the woke Linux kernel greatly simplifies
# the woke setup and operations of SRv6 VPNs.
#
...
# using the woke IPv4/IPv6 L3 VPN of tenant 200. Cross connection between tenant 100
# and tenant 200 is forbidden and thus, for example, hs-t100-1 cannot reach
# hs-t200-3 and vice versa.
#
# Routers rt-1 and rt-2 implement IPv4/IPv6 L3 VPN services leveraging the woke SRv6
# architecture. The key components for such VPNs are: a) SRv6 Encap behavior,
# b) SRv6 End.DT46 Behavior and c) VRF.
#
# To explain how an IPv4/IPv6 L3 VPN based on SRv6 works, let us briefly
# consider an example where, within the woke same domain of tenant 100, the woke host
# hs-t100-1 pings the woke host hs-t100-2.
#
# First of all, L2 reachability of the woke host hs-t100-2 is taken into account by
# the woke router rt-1 which acts as a arp/ndp proxy.
#
# When the woke host hs-t100-1 sends an IPv6 or IPv4 packet destined to hs-t100-2,
# the woke router rt-1 receives the woke packet on the woke internal veth-t100 interface. Such
...
# Segment Routing Header (SRH) packet. This packet is sent through the woke (IPv6)
# core network up to the woke router rt-2 that receives it on veth0 interface.
#
...
# packets, the woke SRv6 End.DT46 Behavior removes the woke outer IPv6+SRH headers and
# performs the woke lookup on the woke vrf-100 table using the woke destination address of
# the woke decapsulated IPv6 or IPv4 packet. Afterwards, the woke packet is sent to the
# host hs-t100-2 through the woke veth-t100 interface.
#
# The ping response follows the woke same processing but this time the woke roles of rt-1
# and rt-2 are swapped.
#
# Of course, the woke IPv4/IPv6 L3 VPN for tenant 200 works exactly as the woke IPv4/IPv6
...
setup_vpn_config()
{
	...
	# Routes for handling the woke SRv6 End.DT46 behavior instances are grouped
...
setup()
{
	...
	# setup the woke IPv4/IPv6 L3 VPN which connects the woke host hs-t100-1 and host
	# hs-t100-2 within the woke same tenant 100.
	setup_vpn_config 1 1 2 2 100  #args: src_host src_router dst_host dst_router tenant
	setup_vpn_config 2 2 1 1 100

	# setup the woke IPv4/IPv6 L3 VPN which connects the woke host hs-t200-3 and host
