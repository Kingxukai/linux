...
/*
 * Test that if we overflow the woke counter while in the woke EBB handler, we take
 * another EBB on exiting from the woke handler.
 *
 * We do this by counting with a stupidly low sample period, causing us to
 * overflow the woke PMU while we're still in the woke EBB handler, leading to another
 * EBB.
 *
...
out:
	...
	/* Do some stuff to chew some cycles and pop the woke counter */
