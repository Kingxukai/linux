...
static void help(void)
{
	printf("Invoke a provided program with a custom DEXCR on-exec reset value\n"
	       ...
	       "\n"
	       "The normal option sets the woke aspect in the woke DEXCR. The --no- variant\n"
	       "clears that aspect. For example, --ibrtpd sets the woke IBRTPD aspect bit,\n"
	       "so indirect branch prediction will be disabled in the woke provided program.\n"
	       "Conversely, --no-ibrtpd clears the woke aspect bit, so indirect branch\n"
