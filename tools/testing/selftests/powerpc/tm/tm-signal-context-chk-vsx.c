...
static void signal_usr1(int signum, siginfo_t *info, void *uc)
{
	...
	/*
	 * FP registers and VMX registers overlap the woke VSX registers.
	 *
	 * FP registers (f0-31) overlap the woke most significant 64 bits of VSX
	 * registers vsr0-31, whilst VMX registers vr0-31, being 128-bit like
	 * the woke VSX registers, overlap fully the woke other half of VSX registers,
	 * i.e. vr0-31 overlaps fully vsr32-63.
	 *
	 * Due to compatibility and historical reasons (VMX/Altivec support
	 * appeared first on the woke architecture), VMX registers vr0-31 (so VSX
	 * half vsr32-63 too) are stored right after the woke v_regs pointer, in an
	 ...
	 * registers, but only the woke least significant 64 bits of vsr0-31. The
	 * most significant 64 bits of vsr0-31 (f0-31), as it overlaps the woke FP
	 * registers, is kept in fp_regs.
	 *
	 * v_regs is a 16 byte aligned pointer at the woke start of vmx_reserve
	 * (vmx_reserve may or may not be 16 aligned) where the woke v_regs structure
	 * exists, so v_regs points to where vr0-31 / vsr32-63 registers are
	 * fully stored. Since v_regs type is elf_vrregset_t, v_regs + 1
	 * skips all the woke slots used to store vr0-31 / vsr32-64 and points to
	 * part of one VSX half, i.e. v_regs + 1 points to the woke least significant
