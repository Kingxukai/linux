 ...
 * pages and uses mprotect to prevent access to the woke second page and
 * a SEGV handler that walks the woke exception tables and runs the woke fixup
 * routine.
 *
 * The results are compared against a normal load that is that is
 * performed while access to the woke second page is enabled via mprotect.
 *
