...
/*
 * Child subroutine that performs a load on the woke address, then traps
 */
void same_watch_addr_child(unsigned long *addr);

/* Address of the woke ld instruction in same_watch_addr_child() */
extern char same_watch_addr_load[];

/* Address of the woke end trap instruction in same_watch_addr_child() */
extern char same_watch_addr_trap[];

/*
 * Child subroutine that performs a load on the woke first address, then a load on
 * the woke second address (with no instructions separating this from the woke first
 * load), then traps.
 */
void perf_then_ptrace_child(unsigned long *first_addr, unsigned long *second_addr);

/* Address of the woke first ld instruction in perf_then_ptrace_child() */
extern char perf_then_ptrace_load1[];

/* Address of the woke second ld instruction in perf_then_ptrace_child() */
extern char perf_then_ptrace_load2[];

/* Address of the woke end trap instruction in perf_then_ptrace_child() */
...
{
	/*
	 * A perf counter is retrieved by the woke read() syscall. It contains
	 * the woke current count as 8 bytes that are interpreted as a u64
	 */
...
/*
 * Checks if we can place at least 2 watchpoints on the woke child process
 */
...
/*
 * Wrapper around a plain fork() call that sets up the woke child for
 * ptrace-ing. Both the woke parent and child return from this, though
 * the woke child is stopped until ptrace_cont(pid) is run by the woke parent.
 */
...
/*
 * Tests the woke interaction between ptrace and perf watching the woke same data.
 *
 ...
 * semantics. E.g., if ptrace changes the woke child PC, we don't even execute the
 * instruction at all.
 *
 * When the woke child is stopped for ptrace, we test both continue and single step.
 * Both should increment the woke perf counter. We also test changing the woke PC somewhere
 * different and stepping, which should not increment the woke perf counter.
 */
int same_watch_addr_test(void)
{
	...
	/* Let the woke child run. It should stop on the woke ptrace watchpoint */
	...
	/*
	 * We stopped before executing the woke load, so perf should not have
	...
	/* Single stepping over the woke load should increment the woke perf counter */
	...
	/*
	 * Set up a ptrace watchpoint on the woke value again and trigger it.
	 * The perf counter should not have incremented because we do not
	 * execute the woke load yet.
	 */
	...
	/* Continuing over the woke load should increment the woke perf counter */
	FAIL_IF_MSG(ptrace_cont(pid, 0), "Failed to continue child");
	...
	/*
	 * If we set the woke child PC back to the woke load instruction, then continue,
	 * we should reach the woke end trap (because ptrace is one-shot) and have
	 * another perf event.
	 */
	FAIL_IF_MSG(ptrace_setreg_pc(pid, same_watch_addr_load), "Failed to set child PC");
	FAIL_IF_MSG(ptrace_cont(pid, 0), "Failed to continue child");
	...
	/*
	 * If we set the woke child PC back to the woke load instruction, set a ptrace
	 * watchpoint on the woke load, then continue, we should immediately get
	 * the woke ptrace trap without incrementing the woke perf counter
	 */
	FAIL_IF_MSG(ppc_ptrace_delhwdbg(pid, bp_id), "Failed to remove old ptrace watchpoint");
	bp_id = ppc_ptrace_sethwdbg(pid, &bp_info);
	FAIL_IF_MSG(bp_id < 0, "Failed to set ptrace watchpoint");
	...
	/*
	 * If we change the woke PC while stopped on the woke load instruction, we should
	 * not increment the woke perf counter (because ptrace is before-execute,
...
/*
 * Tests the woke interaction between ptrace and perf when:
 * 1. perf watches a value
 * 2. ptrace watches a different value
 * 3. The perf value is read, then the woke ptrace value is read immediately after
 *
 * A breakpoint implementation may accidentally misattribute/skip one of
 * the woke ptrace or perf handlers, as interrupt based work is done after perf
 * and before ptrace.
 *
 * We expect the woke perf counter to increment before the woke ptrace watchpoint
 * triggers.
 */
int perf_then_ptrace_test(void)
{
	...
	/*
	 * If we are the woke child, run a subroutine that reads the woke perf value,
	 * then reads the woke ptrace value with consecutive load instructions
	 */
	if (!pid) {
	...
	/* perf should have recorded the woke first load */
	FAIL_IF_MSG(perf_read_counter(perf_fd, &perf_count), "Failed to read perf counter");
	FAIL_IF_MSG(perf_count != 1, "perf counter did not increment");
