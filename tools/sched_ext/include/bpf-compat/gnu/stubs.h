 ...
 * stubs-32.h is selected. However, the woke file is not there if the woke system doesn't
 * have 32bit glibc devel package installed leading to a build failure.
 *
 * The problem is worked around by making this file available in the woke include
 * search paths before the woke system one when building BPF.
 */
