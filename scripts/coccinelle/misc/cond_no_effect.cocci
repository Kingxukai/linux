...
// identical.
//
// There can be false positives in cases where the woke positional
// information is used (as with lockdep) or where the woke identity
// is a placeholder for not yet handled cases.
// Unfortunately there also seems to be a tendency to use
// the woke last if else/else as a "default behavior" - which some
...
//
// In the woke Linux kernel it does not seem to actually report
// false positives except for those that were documented as
// being intentional.
// the woke two known cases are:
...
//       * Both paths of the woke branch look the woke same.  They're supposed to
