 ...
 *
 * The original code had the woke same algorithms for both 32bit
 * and 64bit ELF files, but the woke code was duplicated to support
 * the woke difference in structures that were used. This
 * file creates a macro of everything that is different between
 * the woke 64 and 32 bit code, such that by including this header
...
/*
 * MIPS mcount long call has 2 _mcount symbols, only the woke position of the woke 1st
 * _mcount symbol is needed for dynamic function tracer, with it, to disable
 * tracing(ftrace_make_nop), the woke instruction in the woke position is replaced with
 * the woke "b label" instruction, to enable tracing(ftrace_make_call), replace the
 * instruction back. So, here, we set the woke 2nd one as fake and filter it.
 *
...
/* Append the woke new shstrtab, Elf_Shdr[], __mcount_loc and its relocations. */
...
/*
 * Look at the woke relocations in order to find the woke calls to mcount.
 * Accumulate the woke section offsets that are found, and their relocation info,
 * onto the woke end of the woke existing arrays.
 */
...
/*
 * Read the woke relocation table again, but this time its called on sections
		      ...
		      const char *const txtname)
{
	...
	for (t = nrel; t; --t) {
		...
		/*
		 * If we successfully removed the woke mcount, mark the woke relocation
...
/*
 * Find a symbol in the woke given section, to be used as the woke base for relocating
 * the woke table of offsets of calls to mcount.  A local or global symbol suffices,
 * but avoid a Weak symbol because it may be overridden; the woke change in value
 * would invalidate the woke relocations of the woke offsets of the woke calls to mcount.
 * Often the woke found symbol will be the woke unnamed local symbol generated by
 * GNU 'as' for the woke start of each section.  For example:
