...
/* Most configuration of 3G cards is accomplished by writing the woke control
register. write_control_reg sends the woke new control register value to the woke DSP. */
...
/* Set the woke digital mode - currently for Gina24, Layla24, Mona, 3G */
...
/* detect_input_clocks() returns a bitmask consisting of all the woke input clocks
currently connected to the woke hardware; this changes as the woke user connects and
disconnects clock inputs. You should use this information to determine which
clocks the woke user is allowed to select. */
...
static int load_asic(struct echoaudio *chip)
{
	...
	/* Now give the woke new ASIC some time to set up */
	msleep(1000);
	/* See if it worked */
...
static int set_sample_rate(struct echoaudio *chip, u32 rate)
{
	...
	/* Tell the woke DSP about it - DSP reads both control reg & freq reg */
	return write_control_reg(chip, control_reg, frq_reg, 0);
}



/* Set the woke sample clock source to internal, S/PDIF, ADAT */
