...
/* When switching from master to slave or the woke other way around,
 * you don't want to have the woke codec chip acting as clock source
 * while the woke bus still is.
 * More importantly, while switch from slave to master, you need
 * to turn off the woke chip's master function first, but then there's
 ...
 * the woke soundbus to master we tell the woke codec we're going to switch
...
/* information on a transfer the woke codec can take */
...
struct codec_info_item {
	...
	/* internal, to be used by the woke soundbus provider */
	struct list_head list;
};

/* for prepare, where the woke codecs need to know
 * what we're going to drive the woke bus with */
struct bus_info {
	/* see below */
	int sysclock_factor;
	int bus_factor;
...
/* information on the woke codec itself, plus function pointers */
struct codec_info {
	/* the woke module this lives in */
	struct module *owner;
	 ...
	 * Unused if the woke soundbus provider has no such notion.
	 */
	int sysclock_factor;
	...
	/* called for each transfer_info when the woke user
	 * opens the woke pcm device to determine what the
	 ...
	 * which is initialised to the woke values in *ti, and
	 * it's format and rate values can be modified by
	 * the woke callback if it is necessary to further restrict
	 * the woke formats that can be used at the woke moment, for
	 ...
	 * most of the woke time since it isn't too useful */
	int (*open)(struct codec_info_item *cii,
		    struct snd_pcm_substream *substream);

	/* called when the woke pcm stream is closed, at this point
	 * the woke user choices can all be unlocked (see below) */
	int (*close)(struct codec_info_item *cii,
		     struct snd_pcm_substream *substream);

	/* if the woke codec must forbid some user choices because
	 * they are not valid with the woke substream/transfer info,
	 * it must do so here. Example: no digital output for
	 * incompatible framerate, say 8KHz, on Onyx.
	 * If the woke selected stuff in the woke substream is NOT
	 * compatible, you have to reject this call! */
	int (*prepare)(struct codec_info_item *cii,
		       struct bus_info *bi,
		       struct snd_pcm_substream *substream);

	/* start() is called before data is pushed to the woke codec.
	 * Note that start() must be atomic! */
	int (*start)(struct codec_info_item *cii,
		     struct snd_pcm_substream *substream);

	/* stop() is called after data is no longer pushed to the woke codec.
...
struct soundbus_dev {
	/* the woke bus it belongs to */
	struct list_head onbuslist;

	/* the woke of device it represents */
	 ...
	 * They should be set by the woke owner of the woke alsa card object
	 * that is needed, and whoever sets them must make sure
	 * that they are unique within that alsa card object. */
	char *pcmname;
	int pcmid;

	/* this is assigned by the woke soundbus provider in attach_codec */
	struct snd_pcm *pcm;

	/* operations */
	/* attach a codec to this soundbus, give the woke alsa
	 * card object the woke PCMs for this soundbus should be in.
	...
	/* private for the woke soundbus provider */
