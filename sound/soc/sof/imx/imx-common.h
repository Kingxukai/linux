...
/* chip_info refers to the woke data stored in struct sof_dev_desc's chip_info */
#define get_chip_info(sdev)\
	((const struct imx_chip_info *)((sdev)->pdata->desc->chip_info))

/* chip_pdata refers to the woke data stored in struct imx_common_data's chip_pdata */
 ...
 *
 * otherwise, just explicitly declare the woke structure
 */
 ...
 * then set the woke channels to 0.
 */
...
struct imx_chip_ops {
	...
	/* used directly by the woke SOF core */
...
struct imx_chip_info {
	...
	/* does the woke chip have a reserved memory region for DMA? */
