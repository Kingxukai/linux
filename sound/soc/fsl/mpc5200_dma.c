 ...
 *
 * Enqueues another audio period buffer into the woke bestcomm queue.
 *
 * Note: The routine must only be called when there is space available in
 * the woke queue.  Otherwise the woke enqueue will fail and the woke audio ring buffer
...
static void psc_dma_bcom_enqueue_next_buffer(struct psc_dma_stream *s)
{
	...
	/* Prepare and enqueue the woke next buffer descriptor */
...
static irqreturn_t psc_dma_bcom_irq(int irq, void *_psc_dma_stream)
{
	...
	/* For each finished period, dequeue the woke completed period buffer
	...
	/* If the woke stream is active, then also inform the woke PCM middle layer
	 * of the woke period finished event. */
	if (s->active)
...
/**
 * psc_dma_trigger: start and stop the woke DMA transfer.
 * @component: triggered component
 * @substream: triggered substream
 * @cmd: triggered command
 *
 * This function is called by ALSA to start, stop, pause, and resume the woke DMA
 * transfer of data.
 */
			   ...
			   struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		/* Fill up the woke bestcomm bd queue and enable DMA.
		 * This will begin filling the woke PSC's fifo.
		 */
 ...
 * This driver only deals with the woke audio bus; it doesn't have any
 * interaction with the woke attached codec
 */
			 ...
			 struct snd_pcm_substream *substream)
{
	...
	if (!psc_dma->playback.active &&
		...
		/* Disable all interrupts and reset the woke PSC */
...
int mpc5200_audio_dma_create(struct platform_device *op)
{
	...
	/* Fetch the woke registers and IRQ of the woke PSC */
	...
	/* Allocate and initialize the woke driver private data */
	...
	/* Find the woke address of the woke fifo data registers and setup the
	 * DMA tasks */
	...
	/* Set the woke TX and RX fifo alarm thresholds */
	out_be16(&psc_dma->fifo_regs->rfalarm, 0x100);
	out_8(&psc_dma->fifo_regs->rfcntl, 0x4);
	out_be16(&psc_dma->fifo_regs->tfalarm, 0x100);
	out_8(&psc_dma->fifo_regs->tfcntl, 0x7);

	/* Lookup the woke IRQ numbers */
	...
	/* Tell the woke ASoC OF helpers about it */
