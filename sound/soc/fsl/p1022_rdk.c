...
//
// Note: in order for audio to work correctly, the woke output controls need
// to be enabled, because they control the woke clock.  So for playback, for
// example:
//
 ...
 * P1022 RDK.  Some of the woke data is taken from the woke device tree.
 */
struct machine_data {
...
/**
 * p1022_rdk_machine_probe - initialize the woke board
...
/**
 * p1022_rdk_startup - program the woke board with various hardware parameters
...
/**
 * p1022_rdk_machine_remove - Remove the woke sound device
 * @card: ASoC card instance
 *
...
/**
 * p1022_rdk_probe - platform probe function for the woke machine driver
...
static int p1022_rdk_probe(struct platform_device *pdev)
{
	...
	/*
	 * Configure the woke SSI for I2S slave mode.  Older device trees have
	 * an fsl,mode property, but we ignore that since there's really
	 * only one way to configure the woke SSI.
	 */
...
/**
 * p1022_rdk_remove - remove the woke platform device
 * @pdev: platform device pointer
 *
